...
describe(ctx.label, () => {
    ...
    test('view-side-custom-and-date-time-columns-project-as-their-types', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, version as version, released_on as releasedOn, signed_off_at as signedOffAt, project_name as projectName, upper(version) as versionUpper from release_overview where id = ?"`)
    ...
    test('view-side-custom-comparable-column-supports-ordering-operators', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, version as version from release_overview where version < ? order by version desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1.3.0",
          ]
    ...
    test('view-side-adapter-bearing-column-reads-through-the-adapter', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, version_bracketed as versionBracketed from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-side-virtual-column-with-adapter-reads-through-the-adapter', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(version) as versionTagged from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('view-side-custom-local-time-column-projects-as-date', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cutoff_clock as cutoffClock from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-side-optional-virtual-column-with-adapter-reads-through-the-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(version) as versionUpperTagged from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('view-side-plain-columns-read-as-their-types', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select is_signed as isSigned, download_count as downloadCount, avg_rating as avgRating, bin_to_uuid(signing_uuid) as signingUuid, release_day_plain as releaseDayPlain, cutoff_plain as cutoffPlain, release_ordinal as releaseOrdinal from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
