...
describe(ctx.label, () => {
    ...
    test('asBigint-on-double-keeps-bigint-arithmetic-exact', async () => {
        // Same name as the lossless cells (audit symmetry), opposite outcome. The deprecated
        // sqlite3 driver has no exact-integer option at all, so the sum comes back rounded
        // (here to 9007199254740994; the bigint param is itself rounded to 9007199254740992 on
        // bind). The marshaller refuses that rounded number rather than return a clean-but-wrong
        // bigint: PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE. The mock is seeded with the same
        // rounded number the real driver returns, so mock and real agree.
        ctx.mockNext([{ id: 1, big: 9007199254740994 }])
        let err: unknown
        try {
