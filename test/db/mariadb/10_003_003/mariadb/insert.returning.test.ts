    ...
    */
    ...
    test('insert-on-conflict-do-nothing-returning-object-execute-insert-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into organization (name, plan) values (?, ?), (?, ?) returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Conflict many A",
                "free",
    ...
    */
    ...
    test('insert-on-conflict-do-nothing-returning-one-column-execute-insert-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into organization (name, plan) values (?, ?), (?, ?) returning name as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    */
    ...
    test('insert-on-conflict-do-nothing-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, name, slug) values (?, ?, ?) returning id as id, archived_at as \`meta.archivedAt\`"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
...
void tIssue; void tProject; void tProjectRelease; void tProjectReview
