...
export class SqlServerSqlBuilder extends AbstractSqlBuilder {
        ...
        for (const property in orderBy) {
            ...
            if (!order) {
                ...
                case 'desc nulls last':
                    ...
                    break
                case 'asc nulls last':
                    orderByColumns += 'iif(' + this._appendColumnAlias(property, params) + ' is null, 1, 0), ' + this._appendColumnAlias(property, params) + ' asc'
                    break
                case 'desc nulls first':
                    orderByColumns += 'iif(' + this._appendColumnAlias(property, params) + ' is not null, 1, 0), ' + this._appendColumnAlias(property, params) + ' desc'
                    break
                case 'insensitive':
                ...
                case 'asc nulls last insensitive':
                    orderByColumns += 'iif(' + this._appendColumnAlias(property, params) + ' is null, 1, 0), ' + this.__appendColumnAliasInsensitive(property, column, params) + ' asc'
                    break
                case 'desc nulls first insensitive':
                    orderByColumns += 'iif(' + this._appendColumnAlias(property, params) + ' is not null, 1, 0), ' + this.__appendColumnAliasInsensitive(property, column, params) + ' desc'
                    break
                default:
    ...
    _substrToEnd(params: any[], valueSource: ToSql, value: any, columnType: string, typeAdapter: TypeAdapter | undefined): string {
        return 'substring(' + this._appendSql(valueSource, params) + ', ' + this._appendValue(value, params, columnType, typeAdapter) + ', len(' + this._appendSql(valueSource, params) +  ') - ' + this._appendValue(value, params, columnType, typeAdapter) +  ')'
    }
    ...
    _substr(params: any[], valueSource: ToSql, value: any, value2: any, columnType: string, typeAdapter: TypeAdapter | undefined): string {
        return 'substring(' + this._appendSql(valueSource, params) + ', ' + this._appendValue(value, params, columnType, typeAdapter) + ', ' + this._appendValue(value2, params, columnType, typeAdapter) + ')'
    }
