...
describe(ctx.label, () => {
    ...
    test('empty-set-scalar-aggregate-is-undefined', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority) as [s], avg(cast(priority as float)) as [a], min(priority) as lo, max(priority) as hi from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('optional-non-empty-array-over-empty-aggregate-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select id as id, name as name from project where organization_id = organization.id and name = @0 order by id offset 0 rows for json path) as projects from organization where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "__no-such-project__",
            1,
    ...
    test('aggregate-as-array-left-join-miss-inner-object-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"marker": ', '"' + string_escape(convert(nvarchar(max), project.slug), 'json') + '"', ', "issue.id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "issue.title": ', isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), '}'), ','), ']') as issues from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('aggregate-as-array-null-projector-single-leaf-inner-object-is-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"marker": ', '"' + string_escape(convert(nvarchar(max), project.slug), 'json') + '"', ', "issue.id": ', isnull(convert(nvarchar, issue.id), 'null'), '}'), ','), ']') as issues from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
