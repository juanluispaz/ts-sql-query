...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"select openIssues.id as id from (select id as id from issue where \`status\` = ?) as openIssues intersect select id as id from issue where id <= ?"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             "open",
//             2,
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  select allIssueIds.id as id from (select id as id from issue) as allIssueIds except select id as id from issue where priority = ? order by id  /* tail */"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             1,
//           ]
...
//     })
    ...
    test('customize-compound-materialised-as-cte-via-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select combined.id as id, combined.label as label from (/* compound-head */  select id as id, \`name\` as label from project where id = ? union select id as id, \`name\` as label from project where id = ?  /* compound-tail */) as combined order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (/* head */  select id as id, \`name\` as label from project where id in (?, ?) union select id as id, \`name\` as label from project where id = ?  /* tail */) as result_for_count"`)
