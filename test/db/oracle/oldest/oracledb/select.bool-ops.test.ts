...
describe(ctx.label, () => {
    ...
    test('and', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where status = :0 and priority = :1"`)
    ...
    test('or', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where priority = :0 or status = :1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "closed",
    ...
    test('negate-not', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where not (status = :0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('and-or-complex', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where (priority <= :0 and status = :1) or status = :2 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "open",
    ...
    test('plain-boolean-column-negated', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where not (billable = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number }>>>()
    ...
    test('plain-boolean-column-as-and-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (billable = 1) and id > :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('plain-boolean-column-as-or-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (billable = 1) or id > :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('plain-boolean-column-kept-by-only-when', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (billable = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(result).toEqual(expected)
    })
