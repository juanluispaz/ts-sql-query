...
describe(ctx.label, () => {
    ...
    test('customLocalDateTime-required-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(published_at) as \`y\`, month(published_at) - 1 as mo, dayofmonth(published_at) as \`d\`, dayofweek(published_at) - 1 as dow, hour(published_at) as \`h\`, minute(published_at) as \`m\`, second(published_at) as \`s\`, round(microsecond(published_at) / 1000) as ms, round(unix_timestamp(published_at) * 1000) as \`t\` from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-required-localDateTime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(published_stamp_plain) as \`y\`, month(published_stamp_plain) - 1 as mo, dayofmonth(published_stamp_plain) as \`d\`, dayofweek(published_stamp_plain) - 1 as dow, hour(published_stamp_plain) as \`h\`, minute(published_stamp_plain) as \`m\`, second(published_stamp_plain) as \`s\`, round(microsecond(published_stamp_plain) / 1000) as ms, round(unix_timestamp(published_stamp_plain) * 1000) as \`t\` from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-required-customLocalDateTime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(published_stamp) as \`y\`, month(published_stamp) - 1 as mo, dayofmonth(published_stamp) as \`d\`, dayofweek(published_stamp) - 1 as dow, hour(published_stamp) as \`h\`, minute(published_stamp) as \`m\`, second(published_stamp) as \`s\`, round(microsecond(published_stamp) / 1000) as ms, round(unix_timestamp(published_stamp) * 1000) as \`t\` from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-source-asOptional-getter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(released_on) as \`y\` from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-localDate-asOptional-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select month(released_on) - 1 as mo, dayofmonth(released_on) as \`d\`, dayofweek(released_on) - 1 as dow from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-localTime-asOptional-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select hour(cutoff_time) as \`h\`, minute(cutoff_time) as \`m\`, second(cutoff_time) as \`s\`, round(microsecond(cutoff_time) / 1000) as ms from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-plain-localDate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(release_day_plain) as \`y\`, month(release_day_plain) - 1 as mo, dayofmonth(release_day_plain) as \`d\`, dayofweek(release_day_plain) - 1 as dow from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-plain-localTime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select hour(cutoff_plain) as \`h\`, minute(cutoff_plain) as \`m\`, second(cutoff_plain) as \`s\`, round(microsecond(cutoff_plain) / 1000) as ms from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
