...
describe(ctx.label, () => {
    ...
    test('from-model/emits-the-same-sql-as-the-descriptor-form', async () => {
        ...
        expect(a.sql).toMatchInlineSnapshot(`"select id as id from issue where (lower(title) like concat(lower(?), '%') or priority > ?) and view_count >= ? and lower(external_ref) like concat('%', lower(?), '%') order by id"`)
