...
describe(ctx.label, () => {
    ...
    test('like-insensitive-and-not-like-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) like lower(@0) and lower(email) not like lower(@1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%ACME%",
            "%GRACE%",
    ...
    test('starts-with-insensitive-and-ends-with-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) like lower(@0 + '%') and lower(email) like lower('%' + @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ADA",
            "TEST",
    ...
    test('not-starts-with-insensitive-and-not-ends-with-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) not like lower(@0 + '%') and lower(email) not like lower('%' + @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ADA",
            "GLOBEX.TEST",
