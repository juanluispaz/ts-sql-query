...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-use-empty-array-for-no-value-explicit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(issue.title) as titles from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        ...
        // json_group_array order isn't guaranteed; sort titles before comparing.
        rows[0]!.titles.sort()
    ...
    test('aggregate-as-array-disallow-when-false-emits-transparently', async () => {
        ...
        const rows = sync(query.executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(issue.title) as titles from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
    ...
    test('aggregate-as-array-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(json_object('id', issue.id, 'body', issue.body)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        // json_group_array order isn't guaranteed; sort issues by id. Issue 1
    ...
    test('aggregate-as-array-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(issue.title) as "meta.titles" from project left join issue on issue.project_id = project.id where project.organization_id = ? group by project.id order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('aggregate-as-array-element-level-rule-1-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as oid, json_group_array(json_object('ref', issue.title, 'assigneeId', issue.assignee_id)) as issues from project left join issue on issue.project_id = project.id where project.organization_id = ? group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-element-level-rule-4-all-optional-not-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, json_group_array(json_object('body', body, 'assigneeId', assignee_id)) as opts from issue where project_id = ? group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-projecting-optional-values-as-nullable-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(json_object('id', issue.id, 'body', issue.body)) as "meta.issues" from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregated-array-projecting-optional-values-as-nullable-then-use-empty-array-for-no-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_group_array(json_object('id', id, 'body', body)) from issue where project_id = project.id) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregated-array-projecting-optional-values-as-nullable-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_group_array(json_object('id', id, 'body', body)) from issue where project_id = project.id) as "meta.issues" from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-query-value-one-column-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_group_array(json_object('id', id, 'body', body)) as result from issue where project_id = project.id) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-query-value-one-column-projecting-optional-values-as-nullable-then-allow-when', async () => {
        ...
        expect(isQueryAllowed(query)).toBe(true)
        const row = sync(query.executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_group_array(json_object('id', id, 'body', body)) as result from issue where project_id = project.id) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
