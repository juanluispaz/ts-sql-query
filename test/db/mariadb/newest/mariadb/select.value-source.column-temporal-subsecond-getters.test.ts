...
describe(ctx.label, () => {
    ...
    test('column-microstamp-gettime-rounds-up', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tTemporalPrecision)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select round(timestampdiff(microsecond, '1970-01-01 00:00:00', micro_stamp) / 1000) as \`t\` from temporal_precision where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('column-shifthour-adapter-datetime-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(shifted_stamp) as \`y\`, month(shifted_stamp) - 1 as mo, dayofmonth(shifted_stamp) as \`d\`, dayofweek(shifted_stamp) - 1 as dow, hour(shifted_stamp) as \`h\`, minute(shifted_stamp) as \`m\`, second(shifted_stamp) as \`s\`, floor(microsecond(shifted_stamp) / 1000) as ms, round(timestampdiff(microsecond, '1970-01-01 00:00:00', shifted_stamp) / 1000) as \`t\` from release_draft where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('column-shifthour-adapter-date-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select year(shifted_release_day) as \`y\`, month(shifted_release_day) - 1 as mo, dayofmonth(shifted_release_day) as \`d\`, dayofweek(shifted_release_day) - 1 as dow from release_draft where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('column-shifthour-adapter-time-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select hour(shifted_cutoff) as \`h\`, minute(shifted_cutoff) as \`m\`, second(shifted_cutoff) as \`s\`, floor(microsecond(shifted_cutoff) / 1000) as ms from release_draft where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
