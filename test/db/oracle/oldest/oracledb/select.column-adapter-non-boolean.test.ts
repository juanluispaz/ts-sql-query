...
describe(ctx.label, () => {
    ...
    test('non-boolean-column-adapters-transform-read-values', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", reviewer_code as "code", score as "score" from project_review where id = :0"`)
    ...
    test('non-boolean-int-column-adapter-scales-the-bound-param-on-write', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project_review where score = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            850,
          ]
