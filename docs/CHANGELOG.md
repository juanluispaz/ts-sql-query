...
**Breaking changes** (runtime behavior a v1 application may notice — each keeps an opt-out that restores the v1 behavior):

- **PostgreSQL, SQL Server: `.minValue(...)` / `.maxValue(...)` now propagate `NULL`.** When an operand is `NULL`, the result is now `NULL` on these two databases — `estimatedHours.minValue(5)` on a `NULL` `estimatedHours` is `NULL`, where **v1 ignored the `NULL` and returned the present value (`5`)**. This aligns them with MySQL / MariaDB / Oracle / SQLite and with the value's declared (optional) type. **To keep the v1 behavior, set `ignoreNullInMinAndMaxValue = true`** on the connection. (The detailed entry under *Fixes* below also describes the `minValueFunction` / `maxValueFunction` opt-in; see [PostgreSQL → `minValue` / `maxValue` and NULL](configuration/supported-databases/postgresql.md#minvalue--maxvalue-and-null) and [SQL Server → `minValue` / `maxValue` and NULL](configuration/supported-databases/sqlserver.md#minvalue--maxvalue-and-null).)
- **SQL Server: `.length()` now counts trailing blanks.** `'Draft  '.length()` is now `7`, matching JavaScript's `String.length`, where **v1 emitted the native `LEN()` and answered `5`** (T-SQL's `LEN` excludes trailing blanks). **To keep the v1 behavior, set `excludeTrailingBlanksInLength = true`** on the connection — it emits the bare native `len(x)` again. See [SQL Server → `.length()` and trailing blanks](configuration/supported-databases/sqlserver.md#length-and-trailing-blanks).
- **Oracle: `.concat(...)` and the affix predicates now propagate `NULL`.** Oracle reads `NULL` as the empty string when concatenating (`'x' || null` is `'x'`), so a `concat` with a `NULL` operand returned a present string where its declared type says the result is optional — and, the worse half, an affix predicate (`startsWith` / `endsWith` / `contains` and their `Insensitive` variants) built on a `NULL` search term collapsed to `like '%'` and **matched every row instead of none**. Concatenation now wraps in a `CASE` that yields `NULL` when an operand is `NULL`, matching the declared (optional) type and every other database, where **v1 emitted the bare `||` and read the `NULL` as empty**. Only operands that can be `NULL` at build time are checked, so a concatenation of required operands is still the bare `||`, and a chain shares a single `CASE`. `concat` and the affix predicates always move together. **To keep the v1 behavior, set `ignoreNullInConcat = true`** on the connection (the bare native `||` again); alternatively `concatFunction` routes concatenation through a null-propagating function you created, propagating `NULL` without the operand-repeating `CASE`. See [Oracle → Concatenation and NULL](configuration/supported-databases/oracle.md#concatenation-and-null) for the opt-outs and the package to create (it must be a package: Oracle cannot overload standalone functions, and the overloads are what keep a `CLOB` from being silently truncated).
- **SQL Server, Oracle: `.replaceAll(...)` is now code-point (case-sensitive) by default.** On these two databases `REPLACE` honours the configured collation, so on a case-insensitive database `'ABCabc'.replaceAll('abc', 'X')` matched **both** cases and returned `'XX'`, silently corrupting the value — **v1 emitted the bare `replace(...)`** and inherited that. The library now forces a binary/code-point collation on the match operands and resets the result collation so it does not leak into a chained comparison — `replace(<src> collate Latin1_General_BIN2, <from> collate Latin1_General_BIN2, <to>) collate DATABASE_DEFAULT` on SQL Server, `collate BINARY` / `collate USING_NLS_COMP` on Oracle — so `replaceAll` is case-sensitive whatever the database collation. **To keep the v1 behavior, set `replaceCollation = ''`** on the connection (the bare native `replace(...)` again — case-insensitive on a standard SQL Server; case-sensitive on a default Oracle). Only SQL Server and Oracle changed; the other four databases ignore collation in `REPLACE`, so they were byte-wise case-sensitive already and are unchanged. See [Collations → `replaceCollation`](configuration/collations.md#replacecollation-sql-server-and-oracle).

**New features**:

- **New dedicated [Collations & case sensitivity](configuration/collations.md) documentation page.** Explains the design (the plain string ops follow your schema's configured collation, the `*Insensitive` ops force insensitive), the per-database defaults, the language-dependent case-folding traps, and every collation lever — with a per-engine reach table for each.
- **New `.collate('<name>')` on string value sources.** Emits `(<expr> collate <name>)` to force a specific collation on a single value — case-sensitive/binary, case-insensitive, or a language-specific one — usable on either operand of a comparison, on a projected / grouped / ordered column, or as a `replaceAll` argument. It is surgical (the index cost is paid only where applied), symmetric to the connection-wide `insensitiveCollation`. Two documented caveats: on PostgreSQL, per-value case-insensitive *equality* needs a non-deterministic collation object; on SQLite, `.collate()` does not reach `LIKE`. See [Collations → `.collate()`](configuration/collations.md#collate-force-a-collation-per-value).
- **New `.replaceAllInsensitive(...)` / `.replaceAllInsensitiveIfValue(...)`.** The case-insensitive twin of `replaceAll`, completing the sensitive/insensitive pair the rest of the string API has (`contains` / `containsInsensitive`, …). The mechanism is per database: collation-driven on SQL Server and Oracle (honours `insensitiveCollation`, accent-capable; Oracle forces the configurable `replaceInsensitiveCollation`, default `BINARY_CI`, when `insensitiveCollation` names none), regex-driven on MySQL / MariaDB (honours the operand collation) and PostgreSQL (case-only — it cannot honour a language collation), and a configurable UDF on SQLite (else a documented case-sensitive `replace` fallback, never an error). The search term is regex-escaped where the engine treats it as a pattern. See [Collations → `replaceAllInsensitive`](configuration/collations.md#replaceallinsensitive-the-insensitive-twin).
- **New `replaceCollation` connection option (SQL Server, Oracle).** Names the collation forced on `replaceAll`'s match operands — default `Latin1_General_BIN2` (SQL Server) / `BINARY` (Oracle); set it to another collation name to force a different one, or to `''` to opt out to the native `replace`. See the breaking-change note above and [Collations → `replaceCollation`](configuration/collations.md#replacecollation-sql-server-and-oracle).
- **New `replaceInsensitiveCollation` connection option (Oracle).** Names the case-insensitive collation Oracle forces on `.replaceAllInsensitive(...)`'s match operands when `insensitiveCollation` names none — default `BINARY_CI` (Oracle's default collation is case-sensitive, so a bare replace would not fold). Set it to another name (e.g. `BINARY_AI` to also fold accents), or to `''` to opt out to the native `replace`. See [Collations → `replaceAllInsensitive`](configuration/collations.md#replaceallinsensitive-the-insensitive-twin).
- **New `replaceAllInsensitiveFunction` connection option (SQLite).** Names a user-registered scalar function to call for `.replaceAllInsensitive(...)`; left unset, `replaceAllInsensitive` falls back to a case-sensitive `replace`. See [Collations → SQLite](configuration/collations.md#sqlite-register-a-udf-or-accept-the-case-sensitive-fallback) for the JS implementation to register per connector.
...
- **SQL Server: new `excludeTrailingBlanksInLength` option for `.length()`.** `.length()` mirrors JavaScript's `String.length`, which counts trailing blanks; to match it on SQL Server — whose native `LEN()` excludes them — the library emits `len(<x> + '.') - 1`. That sentinel is character-correct for every string except one exact extreme: a value whose length already equals its column's declared maximum, where T-SQL's string `+` on two non-`max` character types caps the concatenation at the type's maximum and drops the appended sentinel, so `.length()` under-reports by one there. Setting `excludeTrailingBlanksInLength` to `true` opts into the bare native `len(<x>)` instead: the max-length edge disappears (and the query is lighter) at the cost of excluding trailing blanks again — T-SQL's native semantics, diverging from JavaScript. It is a genuine trade, not a strict improvement — the right choice for an application whose SQL Server columns hold max-length values and whose data does not depend on trailing blanks. See [SQL Server → `.length()` and trailing blanks](configuration/supported-databases/sqlserver.md#length-and-trailing-blanks).

- **New `PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE` error reason.** Distinguishes an integer value that cannot be represented in the requested type without losing precision — a driver-rounded `number`, or an exact value beyond the target type's safe integer range — from a genuine type mismatch (`INVALID_VALUE_RECEIVED_FROM_DATABASE`), so you can catch and remediate the two differently. It carries `value`, `typeName`, and optional `rowIndex` / `columnPath`. See [Error management → Library-raised errors](api/error-management/library-raised-errors.md).

- **`connection.transaction(...)` now supports fully-synchronous execution and a callback that returns a plain value.** When the connection's query runner is configured with a synchronous promise implementation (e.g. `SynchronousPromise`, for the blocking `sync()` style described in [Synchronous query runners](advanced/synchronous-query-runners.md)), a transaction whose body throws now correctly rolls back and **rejects** — previously the rejection could be swallowed under such a provider and the transaction reported as a silent success. The callback may also return a plain, non-`Promise` value — the signature is now `transaction<T>(fn: () => T | Promise<T>)` — so a synchronous body like `sync(connection.transaction(() => { const n = sync(query.executeSelectOne()); return n.total + 1 }))` type-checks and runs. Applications using the default native `Promise` see no behavior change (a body returning a `Promise` still works exactly as before).
...
- **SQL Server: `.length()` / `.toUpperCase()` / `.toLowerCase()` / `.reverse()` on a `uuid` value now emit valid SQL.** Unlike the `concat` / `trim` / `substr` / `stringConcat` operations above — which were already wrapped in a `convert(nvarchar(36), …)` — these string operations on a uuid (e.g. `t.externalRef.asString().length()` / `.toUpperCase()` / `.toLowerCase()`) emitted the bare `uniqueidentifier` column with no conversion, which SQL Server rejects the moment it flows into a string operator (`The data types uniqueidentifier and varchar are incompatible in the add operator`). They now convert the uuid to `nvarchar(36)` first, like the rest of the string API. Other databases were unaffected.
- **SQL Server: `aggregateAsArray` on older `compatibilityVersion` settings no longer overflows on uuid columns, truncates long string columns, or loses `double` precision.** When the configured compatibility version predates native `FOR JSON` support, ts-sql-query assembles the JSON array by hand with `string_agg` + `convert(nvarchar, …)`. Those converts had no length (so `nvarchar(30)`): a uuid column raised an arithmetic-overflow error, and a string value longer than 30 characters was silently truncated in the aggregated JSON. Uuid columns now convert with `nvarchar(36)` and string columns with `nvarchar(max)`. A `double` column had a further problem: a styleless `convert` of a `float` keeps only 6 significant figures, so `123.456789` came back as `123.457` — it now converts with style 3, the lossless round-trip form. Newer compatibility versions use native `json_arrayagg` and were unaffected.
- **`aggregateAsArray` / `aggregateAsArrayOfOneColumn` now return `bigint` values beyond 2^53 exactly.** The aggregated rows travel back as one JSON document, and `JSON.parse` decodes every number as a floating-point double — so a `bigint` larger than `Number.MAX_SAFE_INTEGER` came back **silently rounded** (`9007199254740993n` → `9007199254740992n`). Unlike a plain `bigint` *column* (which the driver can be told to return as a string), the aggregate arrives as a single JSON column, so **no driver setting could intervene** — the rounding was in the parse. ts-sql-query now reads the aggregated JSON as raw text and parses it **without** rounding, so `bigint` (and `int` / `double` / `customInt` / `customDouble`) leaves round-trip exactly — including a `double` a database renders in **scientific notation** in the JSON (e.g. PostgreSQL emits `1.5e-08` for a small value, SQL Server's lossless `convert(..., 3)` is exponential too), which the value parser accepts. On PostgreSQL, MySQL and MariaDB — whose drivers decode a `json` column into a JavaScript value before the library sees it — the aggregate is cast to text in the emitted SQL (`(json_agg(...))::text` / `cast(json_arrayagg(...) as char)`) so the raw string reaches the library; Oracle, SQL Server and SQLite already return the aggregate as text, so their SQL is unchanged. (One residual limit: the deprecated `sqlite3` driver cannot bind a `BigInt` parameter, so writing a value past 2^53 through it still rounds on the way in — a driver limitation, not the library's.)
- **Reading an integer beyond JavaScript's safe range now fails with one clear error instead of silently corrupting the value or raising a misleading one.** When a driver hands back an integer larger than `Number.MAX_SAFE_INTEGER` (`9_007_199_254_740_991`) as an already-rounded JavaScript `number` — which mysql2 (without `supportBigNumbers`), oracledb (without a `fetchTypeHandler`) and the deprecated `sqlite3` do by default — the value has lost precision before the library sees it. Previously this was handled inconsistently by the target type: an `int` value **silently returned the rounded number**, a `stringInt` did the same, while a `bigint` raised `INVALID_VALUE_RECEIVED_FROM_DATABASE` with a message that read as though the *database* had sent an invalid value. All three now raise the new, dedicated `PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE` reason — whichever route the value arrives by (a `number`, a numeric `string`, or a `bigint`) and whether it comes from a scalar column or a JSON aggregate — so no integer read path silently corrupts a value, and the error names the real problem and its remedy. The same reason is now uniform across **every** connector: better-sqlite3 and bun:sqlite (which likewise round when their `safeIntegers` mode is off) raise it, and `node:sqlite` — whose driver instead *refuses* to return an out-of-range integer with a `RangeError`, previously surfaced as `SQL_INVALID_VALUE` — has that driver error **normalized** to `PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE` as well, so an application can catch one reason regardless of which database or driver it runs on. To read such values exactly, enable your driver's big-integer / fetch-as-string mode (see each driver's page under [Query runners](configuration/query-runners/)); for a value that genuinely exceeds `int`, declare the column `bigint`. `double` values, which are approximate by definition, are never affected.
- **SQL Server, Oracle: chaining `.replaceAll(...)` no longer emits invalid SQL.** With the default `replaceCollation`, `replaceAll` resets its result collation, so a second replace over the first — `title.replaceAll('a', 'b').replaceAll('c', 'd')`, and equally `replaceAll(...).collate(...)` or `replaceAll(...).replaceAllInsensitive(...)` — placed two adjacent `collate` clauses (`… collate DATABASE_DEFAULT collate Latin1_General_BIN2`), which the engine rejected (`Msg 156` on SQL Server, `ORA-00907` on Oracle). The inner result is now parenthesised, so the chain is accepted. (Introduced with the new `replaceCollation` behavior in this beta; the other four databases were never affected.)
- **PostgreSQL, MySQL, MariaDB: `.replaceAllInsensitive(search, replacement)` now treats the replacement literally.** These engines implement the case-insensitive replace with `regexp_replace`, whose *replacement* string interprets substitution metacharacters — `\1` / `\&` as backreferences (PostgreSQL, MariaDB) and `$0` (MySQL's ICU engine) — so a replacement containing `\` or `$`, such as path normalization `x.replaceAllInsensitive('/', '\\')`, silently produced a wrong value or an error, diverging from `replaceAll` and from the same call on the other databases (where the replacement is already literal). The replacement is now escaped per engine so it is inserted verbatim.
- **Oracle: a concatenation chain that reuses a column inside a nested `valueWhenNull` / `coalesce` no longer drops the nested NULL emulation.** In the default concat NULL-emulation mode, an independent nested concatenation that reused a column also used by the outer chain — e.g. `body.concat(title).concat(body.concat(x).valueWhenNull('Z'))` with a nullable `body` — rendered the nested value **without** its NULL-propagating `CASE`, returning a wrong value (the column a second time instead of the `'Z'` default). The re-entrancy guard is now keyed on the concatenation node itself rather than the shared column, so the nested concatenation keeps its own emulation.
- **Oracle: `aggregateAsArray` / `aggregateAsArrayOfOneColumn` of a `localDate` column no longer throws.** Oracle's `DATE` serialises inside a JSON aggregate with a midnight time (`"2024-01-15T00:00:00"`), unlike the bare `"2024-01-15"` the other databases emit; the `localDate` marshaller rejected that form as an invalid date and raised `INVALID_VALUE_RECEIVED_FROM_DATABASE`. It now tolerates a time component on a `localDate` string, so the query returns the dates.
...
- **Oracle: reading a `localDate` / `localTime` / `localDateTime` column back through `RETURNING` no longer fails to marshal.** A temporal column returned by an `INSERT` / `UPDATE` / `DELETE … RETURNING` (for example `insertInto(t).values({ … }).returning({ archivedAt: t.archivedAt }).executeInsertOne()`) came back from the `oracledb` driver as an NLS-formatted string (e.g. `15-JUN-24 08.30.00.000000 AM`) instead of a `Date`, so reading the value raised `INVALID_VALUE_RECEIVED_FROM_DATABASE: Invalid localDateTime value received from the db`. The cause was that an Oracle OUT bind with no declared type defaults to a string — a plain `SELECT` of the same column was fetched with its real type and marshalled fine, but the `RETURNING … INTO` path did not. The RETURNING OUT bind now carries the column's type (DATE / TIMESTAMP), so the driver returns a `Date` and the value marshals the same as on `SELECT`. Other databases were unaffected.
...
- **PostgreSQL, SQL Server: `.minValue(...)` / `.maxValue(...)` now propagate a NULL operand to a NULL result, matching the declared type and the other databases (behavior change).** `col.minValue(x)` / `col.maxValue(x)` are typed optional whenever either operand is optional — the same rule `add` / `subtract` / `concat` use — meaning a NULL operand should make the result NULL. MySQL, MariaDB, Oracle and SQLite already do this; PostgreSQL and SQL Server did not, because their native `least` / `greatest` (and SQL Server's `iif` emulation below compatibility version 2022) **ignore** a NULL operand and return the present value. So `estimatedHours.minValue(5)` returned `5` for a NULL `estimatedHours` on those two databases and `NULL` on every other — the same typed call, two different values. They now wrap the operation in a `CASE` that yields NULL when an operand is NULL, so the value matches the declared type everywhere. **This is a runtime value change on PostgreSQL and SQL Server.** Two opt-outs are provided: `ignoreNullInMinAndMaxValue` keeps the native ignore-NULL behavior, and `minValueFunction` / `maxValueFunction` name a user-provided null-propagating function emitted as `func(a, b)` (avoiding the operand-repeating `CASE`). Only the leanest check the build-time optionality needs is emitted — a call with two required operands is still the bare `least` / `greatest`. The row aggregate `min(col)` / `max(col)` over rows — a different function that ignores NULL on every engine by standard SQL — is not affected. On SQL Server the `CASE` also unifies the NULL behavior across the `compatibilityVersion` split (native `least` / `greatest` at 2022+, the `iif` emulation below) and fixes an operand-order asymmetry in that emulation. See [PostgreSQL → `minValue` / `maxValue` and NULL](configuration/supported-databases/postgresql.md#minvalue--maxvalue-and-null) and [SQL Server → `minValue` / `maxValue` and NULL](configuration/supported-databases/sqlserver.md#minvalue--maxvalue-and-null) for the opt-outs and the function to create.
- **SQL Server: `.power(...)` on an integer receiver no longer truncates the fractional result to an integer.** `intColumn.power(0.5)` — a square root, normal usage — computed and returned in `int` because T-SQL's `POWER` takes the data type of its first argument, so `power(2, 0.5)` was `1` instead of `1.4142…`; an integer exponent could even overflow the `int` type where every other database widens (`power(50000, 2)`). The receiver is now cast to `float` first (the same handling `cbrt` already used), so the declared `double` result is honored. A `double` / `customDouble` receiver was already correct and is unchanged.
- **SQL Server: `.valueWhenNull(...)` no longer truncates a wider or longer fallback to the receiver's type.** SQL Server emitted `isnull(a, b)`, which types the result as its **first** argument and coerces the fallback to it — so `.valueWhenNull('abcdef')` on a NULL `varchar(3)` column returned `'abc'`, and a wider numeric fallback was narrowed too — while every other database (`coalesce` / `nvl` / `ifnull`) keeps the widest operand. SQL Server now emits `coalesce`, which types the result as the widest operand, matching the declared type and the other databases.
- **`.valueWhenNull(...)` with a `double` fallback on an integer receiver no longer throws on a fractional value (all databases).** `intColumn.valueWhenNull(doubleSource)` declared its result `int` (the receiver's type) but emitted `coalesce(<int>, <double>)`, which resolves to the wider `double`; when the int was NULL and the double fractional (e.g. `9.99`), the engine returned the fraction and the library rejected its own valid result with `INVALID_VALUE_RECEIVED_FROM_DATABASE: Invalid int value received from the db: 9.99`. `.valueWhenNull(...)` now promotes its declared type to `double` when the fallback is a `double` source, the way `minValue` / `maxValue` / `add` already do.
- **SQLite: `returningLastInsertedId()` after `onConflictDoNothing()` now resolves to `null` when the row is suppressed, on older `compatibilityVersion` settings.** When the configured compatibility version predates `RETURNING` support (SQLite < 3.35), the inserted id is read with `last_insert_rowid()` instead of a `RETURNING` clause. On an `insertInto(t).values(row).onConflictDoNothing().returningLastInsertedId()` whose row collides — so nothing is inserted — `last_insert_rowid()` returns the id of the *previous* successful insert on the connection, so the call returned that stale id instead of the `null` its `number | null` type promises (and that the `RETURNING` path on 3.35+ already returns). Each SQLite driver now checks the statement's affected-row count and resolves to `null` when zero rows were inserted, matching the `RETURNING` path and every other database. Newer compatibility versions (which use `RETURNING`) were unaffected.
- **SQLite: ordering a compound query (`UNION` / `INTERSECT` / `EXCEPT`) with `nulls last` / `nulls first` now emits valid SQL on older `compatibilityVersion` settings.** When the configured compatibility version predates native `NULLS FIRST` / `NULLS LAST` support (SQLite < 3.30), `compound.orderBy('col', 'asc nulls last')` / `'desc nulls first'` (and their `insensitive` variants) is emulated with a `col is null` expression — illegal as a bare term inside a compound `ORDER BY`, so SQLite rejected the statement (`1st ORDER BY term does not match any column in the result set`). The compound is now wrapped in `select * from (<compound>) …` and the emulation applied on the wrapper — the same wrapping already used for the SQL Server NULLS emulation and for value-source ordering above — so it emits valid SQL. SQLite on a newer compatibility version (native `NULLS FIRST` / `NULLS LAST`) and every other database were unaffected.
...
- **The base `SqlBuilder`'s empty-array `in (...)` / `not in (...)` guard now lives in the base.** An empty array can never match, so the library short-circuits `col.in([])` to the false constant (and `col.notIn([])` to true) instead of emitting `col in ()`, which is invalid SQL on every engine. That guard was duplicated identically in all five real dialects while the base kept emitting the invalid form — unreachable, but a hazard for any future dialect that forgot to override. It now lives in the base and all five redundant per-dialect overrides are removed, so every dialect reaches the single base guard; the generated SQL is unchanged for every database. This only affects you if you reach the builder through the `__UNSUPPORTED__/*` escape hatch.
