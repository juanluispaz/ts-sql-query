...
describe(ctx.label, () => {
    ...
    test('compounded-interface-intersect-all-after-union', async () => {
        ...
        assertType<Exact<typeof result, Array<{ status: string }>>>()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as "status" from issue where id <= :0 union select status as "status" from issue where id = :1 intersect all select status as "status" from issue where id <= :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            4,
