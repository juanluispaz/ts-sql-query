...
- SQLite: when `compatibilityVersion >= 3_038`, the generated SQL uses `unixepoch(...)` (added in SQLite 3.38) instead of `cast(strftime('%s', ...) as integer)` to obtain Unix-seconds values. When `compatibilityVersion >= 3_042`, the `'subsec'` modifier (added in SQLite 3.42) is used with `unixepoch()` to obtain Unix-milliseconds values directly, replacing the previous `julianday()`-based arithmetic. Default `Number.POSITIVE_INFINITY` opts into both — the output is functionally identical but shorter and clearer; consumers pinning a lower version continue to emit the older form.
- MariaDB: when `compatibilityVersion >= 10_003`, the generated SQL emits `VALUE(col)` (added in MariaDB 10.3 via [MDEV-12172](https://jira.mariadb.org/browse/MDEV-12172) as a rename of `VALUES(col)`) inside `ON DUPLICATE KEY UPDATE` to reference values being inserted. Default `Number.POSITIVE_INFINITY` opts into this. Consumers pinning a lower version continue to emit the legacy `VALUES(col)` name, which is still accepted inside `ON DUPLICATE KEY UPDATE` on modern MariaDB.
- MySQL: when `compatibilityVersion >= 8_000`, the generated SQL emits the row alias syntax `INSERT ... AS _new_ ON DUPLICATE KEY UPDATE col = _new_.col` instead of the legacy `VALUES(col)` function reference. The row alias was added in MySQL 8.0.19, and `VALUES(col)` was deprecated in 8.0.20 (the deprecation warning is emitted on every execution). The alias name `_new_` follows the same convention the library already uses for the new-row alias in `UPDATE` statements. Default `Number.POSITIVE_INFINITY` opts into this; pinning a lower version (or any value below `8_000`) keeps the legacy `VALUES(col)` form, which is the right choice for MySQL 5 and for the MySQL 8.0.0–8.0.18 stable patches that predate the row alias.
