...
describe(ctx.label, () => {
    ...
    // MySQL's `ON DUPLICATE KEY UPDATE` doesn't accept a column list
    ...
    // MySqlConnection. Kept commented for symmetry.
    // NOT-APPLICABLE: MySQL uses the bare onConflictDoUpdateSet form
    /*
    ...
    test('on-conflict-do-update-with-inserted-row-ref', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.insertInto(tProject)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) on duplicate key update \`name\` = concat(\`name\`, ?, values(\`name\`))"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    */
    ...
    // NOT-APPLICABLE: MySQL has no RETURNING, so returningOneColumn after an upsert is unavailable.
    /*
    ...
    test('on-conflict-on-id-do-update-optional-custom-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tReleaseDraft)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title, stage, \`channel\`, min_version) values (?, ?, ?, ?, ?) on duplicate key update stage = ?, \`channel\` = ?, min_version = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Alpha cut (upsert)",
