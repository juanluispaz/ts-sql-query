...
describe(ctx.label, () => {
    ...
    test('customize-delete-before-query-emits-leading-comment', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* op=purge */  delete from project where id = $1"`)
    ...
    test('customize-delete-hook-fragment-with-column-reference', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where id = $1  /* keyed-by project_id */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    test('customize-delete-all-hooks-combined', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /*+ hint */ from project where id = $1  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                9999,
              ]
    ...
    test('customize-delete-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete /*+ hint */ from issue where id = $1 returning id as id, title as title"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('customize-delete-returning-one-column-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete /*+ hint */ from issue where id = $1 returning title as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('customize-delete-using-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /*+ hint */ from issue using project where issue.project_id = project.id and project.id = $1  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
