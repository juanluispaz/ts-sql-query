...
describe(ctx.label, () => {
    ...
    test('where-after-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where project_id = @0 order by id offset 0 rows fetch next @1 rows only"`)
    ...
    test('where-after-offset', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where project_id = @0 order by id offset @1 rows fetch next @2 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
