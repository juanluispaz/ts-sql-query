...
describe(ctx.label, () => {
    ...
    */
    ...
    test('useEmptyArrayForNoValue-on-one-column-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "id", json_arrayagg(issue.title) as "titles" from project left join issue on issue.project_id = project.id and issue.priority = :0 where project.id = :1 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            1,
    ...
    test('useEmptyArrayForNoValue-on-object-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "id", json_arrayagg(json_object('id' value issue.id, 'title' value issue.title)) as "issues" from project left join issue on issue.project_id = project.id and issue.priority = :0 where project.id = :1 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            1,
    ...
    */
    ...
    // NOT-APPLICABLE: Oracle's JSON_ARRAYAGG rejects the DISTINCT quantifier, so the connection does not expose `aggregateAsArrayDistinct` (typed never here).
    /*
