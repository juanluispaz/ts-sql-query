...
describe(ctx.label, () => {
    ...
    test('values-base-type-required-virtual-column-projects-required-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqBatch as (select * from (values (@0), (@1)) as reqBatch(id)) select id as id, 7 as seq, 'BATCH' as tag from reqBatch order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
