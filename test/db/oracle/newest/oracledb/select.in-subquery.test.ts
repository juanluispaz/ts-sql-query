...
describe(ctx.label, () => {
    ...
    test('in-subquery', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where project_id in (select id as "result" from project where case when published = 't' then 1 else 0 end = :0) order by "id""`)
    ...
    test('not-in-subquery', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where project_id not in (select id as "result" from project where case when published = 't' then 1 else 0 end = :0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
