...
describe(ctx.label, () => {
    ...
    test('onlyWhenOrNull-true-emits-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", priority as "priority" from issue where id = :0"`)
    ...
    test('onlyWhenOrNull-false-emits-null-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", null as "priority" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('onlyWhenOrNull-on-nullable-column-keeps-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", "body" as "body" from issue where id in (:0, :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('onlyWhenOrNull-composes-with-arithmetic', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", null as "bumped" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('onlyWhenOrNull-true-with-arithmetic-emits-full-expression', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", priority + :0 as "bumped" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('onlyWhenOrNull-true-on-custom-boolean-carries-read-remap', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when published = 't' then 1 else 0 end as "pub" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
