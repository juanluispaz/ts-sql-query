...
describe(ctx.label, () => {
    ...
    test('cte-chained-second-selects-from-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId, priority as priority from issue where status = @0), top_priority_open as (select id as id, projectId as projectId from open_issues where priority >= @1) select project.name as projectName, top_priority_open.id as issueId from project inner join top_priority_open on top_priority_open.projectId = project.id"`)
    ...
    test('cte-introduced-after-the-first-build-is-declared-in-the-rebuilt-statement', async () => {
        ...
        expect(query.query()).toMatchInlineSnapshot(`"select id as id, name as name from project where organization_id = @0 order by id"`)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with archived_projects as (select id as id from project where archived_at is not null) select id as id, name as name from project where organization_id = @0 and id not in (select id as [result] from archived_projects) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
