...
describe(ctx.label, () => {
    ...
    test('insert-from-select-arithmetic-string-and-column-ops', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) select id as projectId, id + @0 as number, name + @1 as title, @2 as status, id + organization_id as priority from project where id = @3"`)
    ...
    test('insert-from-select-multiply-substring-and-least', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) select id as projectId, (id * @0) + @1 as number, substring(name, @2, @3) as title, @4 as status, iif(id < @5, id, @6) as priority from project where id = @7"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                100,
                2,
