...
describe(ctx.label, () => {
    ...
    test('returning-in-with-old-value-single-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when inserted.name in (deleted.name) then 1 else 0 end as bit) as nameUnchanged where id = @1"`)
    ...
    test('returning-in-n-with-old-value-later-in-array-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when inserted.name in (@1, deleted.slug) then 1 else 0 end as bit) as nameMatchesPriorIdentity where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "mktg-site",
                "Marketing site",
    ...
    test('returning-is-not-with-old-value-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when inserted.name <> deleted.name then 1 else 0 end as bit) as renamed where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v3",
                1,
    ...
    test('returning-value-when-null-with-old-value-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set body = @0 output inserted.id as id, coalesce(inserted.body, deleted.title) as summary where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                null,
                1,
    ...
    test('returning-between-with-old-value-lower-bound-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = @0 output inserted.id as id, cast(case when inserted.priority between @1 and deleted.priority then 1 else 0 end as bit) as notEscalated where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                1,
    ...
    test('returning-replace-all-with-old-value-find-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, replace(inserted.name collate Latin1_General_BIN2, deleted.name collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT as shortName where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site (EMEA)",
                "Mktg",
    ...
    test('returning-replace-all-if-value-with-old-value-replacement-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, replace(inserted.name collate Latin1_General_BIN2, @1 collate Latin1_General_BIN2, deleted.slug) collate DATABASE_DEFAULT as slugLabel where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site (EMEA)",
                "Marketing site",
    ...
    test('returning-value-when-no-value-with-old-value-fallback-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when deleted.name = @1 then 1 else 0 end as bit) as wasMarketingSite where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v5",
                "Marketing site",
    ...
    test('returning-in-n-on-old-value-receiver-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when deleted.name in (@1, @2) then 1 else 0 end as bit) as wasKnownName where id = @3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v6",
                "Marketing site",
    ...
    test('returning-equals-if-value-with-old-value-receiver-and-a-present-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, cast(case when deleted.slug = @1 then 1 else 0 end as bit) as wasMktgSite where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v2",
                "mktg-site",
    ...
    test('returning-concat-if-value-with-old-value-receiver', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, deleted.name + @1 as [label] where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v3",
                " (was)",
