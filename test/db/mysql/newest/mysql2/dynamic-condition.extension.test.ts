...
describe(ctx.label, () => {
    ...
    test('top-level-extension-rule-invokes-callback', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where exists(select id as result from project where id = issue.project_id and lower(\`name\`) like concat('%', lower(?), '%')) order by id"`)
    ...
    test('extension-rule-composes-with-regular-column-filters', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where \`status\` = ? and exists(select id as result from project where id = issue.project_id and lower(\`name\`) like concat('%', lower(?), '%')) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "mktg",
    ...
    test('extension-rule-under-not-is-negated', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (exists(select id as result from project where id = issue.project_id and lower(\`name\`) like concat('%', lower(?), '%'))) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "legacy",
          ]
