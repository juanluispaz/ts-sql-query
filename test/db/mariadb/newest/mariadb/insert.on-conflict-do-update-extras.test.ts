...
describe(ctx.label, () => {
    ...
    // Not applicable on MariaDB: `onConflictOn(...)` is not typed on MariaDBConnection (the `ON DUPLICATE KEY UPDATE` grammar takes no column list); see `test/db/mariadb/types.negative/insert.test.ts` for the compile-time negative.
    /*
