...
describe(ctx.label, () => {
    ...
    test('avg-int-column-fractional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg(cast(priority as double)) as avgPriority from issue where project_id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('average-and-average-distinct-over-bigint', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg(cast(duration_ms as double)) as \`avg\`, avg(distinct cast(duration_ms as double)) as avgD from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customint-sum-distinct-and-average-return-branches', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct cost_cents) as sd, avg(cast(cost_cents as double)) as av from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('sum-distinct-average-distinct-over-int', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct priority) as sd, avg(distinct cast(priority as double)) as avd from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
