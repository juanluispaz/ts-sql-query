...
describe(ctx.label, () => {
    ...
    */
    ...
    test('recursive-union-all-fn-projecting-optionals-as-nullable-inline', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1(id, title, parentId) as (select id as id, title as title, parent_id as parentId from issue where id = :0 union all select issue.id as id, issue.title as title, issue.parent_id as parentId from issue join recursive_select_1 on recursive_select_1.parentId = issue.id) select id as "id", (select json_arrayagg(json_object('id' value id, 'title' value title, 'parentId' value parentId)) from recursive_select_1) as "tree" from project where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
