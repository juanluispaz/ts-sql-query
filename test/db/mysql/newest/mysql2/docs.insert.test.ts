...
describe(ctx.label, () => {
    ...
    test('docs:insert/insert-many-values', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (\`name\`, organization_id, slug) values (?, ?, ?), (?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                1,
    ...
    */
    ...
    test('docs:insert/insert-on-conflict-do-update', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into \`organization\` (\`name\`, plan) values (?, ?) as _new_ on duplicate key update plan = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                "pro",
    ...
    */
    ...
    test('docs-extra:insert/values-for-insert-in-update', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into \`organization\` (\`name\`, plan) values (?, ?) as _new_ on duplicate key update \`name\` = concat(\`name\`, ?, _new_.\`name\`)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                "pro",
