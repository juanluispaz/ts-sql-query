...
describe(ctx.label, () => {
    ...
    */
    ...
    test('multi-level-cte-bubble-up-with-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issues_in_active.id as issueId, \`organization\`.plan as plan from (select id as id, project_id as projectId from issue where project_id in (select active_projects.id as result from (select id as id, organization_id as organizationId from project where archived_at is null) as active_projects)) as issues_in_active inner join (select id as id, organization_id as organizationId from project where archived_at is null) as active_projects on active_projects.id = issues_in_active.projectId inner join \`organization\` on \`organization\`.id = active_projects.organizationId order by issueId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('compound-as-derived-table-via-for-use-in-query-as', async () => {
        ...
        const rows = await ctx.conn.selectFrom(statuses)
            ...
            .orderBy('s')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select statuses.\`s\` as \`s\` from (select \`status\` as \`s\` from issue where \`status\` = ? union select \`status\` as \`s\` from issue where \`status\` = ?) as statuses order by \`s\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    test('compound-order-by-as-derived-table-via-for-use-in-query-as', async () => {
        ...
        const rows = await ctx.conn.selectFrom(statuses)
            ...
            .orderBy('s')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select statuses.\`s\` as \`s\` from (select \`status\` as \`s\` from issue where \`status\` = ? union select \`status\` as \`s\` from issue where \`status\` = ? order by \`s\` desc) as statuses order by \`s\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
