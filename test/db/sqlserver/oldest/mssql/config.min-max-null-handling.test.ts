...
describe(ctx.label, () => {
    ...
    test('ignoreNullInMinAndMaxValue: keeps bare native least/greatest (ignores NULL)', async () => {
        ...
        // of NULL — the ignore-NULL semantics. Below the 16_000_000 breakpoint (this cell
        // targets SQL Server 2019) the builder emits the `iif` emulation, which yields the
        // same values; at 2022+ it emits native least()/greatest().
        ...
        const rows = await conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(assignee_id > @0, assignee_id, @1) as floored, iif(assignee_id < @2, assignee_id, @3) as capped from issue order by id"`)
    ...
    test('minValueFunction/maxValueFunction: emits func(a, b) and propagates NULL', async () => {
        ...
        const conn = ctx.withMinMaxFunctions('dbo.greatest_strict', 'dbo.least_strict')
        const expected = [
            { id: 1, floored: 5,         capped: 1 },
            { id: 2, floored: 5,         capped: 2 },
