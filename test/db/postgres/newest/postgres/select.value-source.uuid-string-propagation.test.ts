...
describe(ctx.label, () => {
    ...
    test('uuid-string-propagation/survives-allow-when', async () => {
        ...
        const ref = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select external_ref::text || $1 as result from issue where id = $2"`)
    ...
    test('uuid-string-propagation/plain-string-through-wrapper-no-convert', async () => {
        ...
        const value = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title || $1 as result from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "-x",
            1,
