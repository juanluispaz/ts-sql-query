...
import { tIssue, tOrganization, tProject, tProjectReview, tReleaseDraft } from '../../domain/connection.js'
...
describe(ctx.label, () => {
    ...
    test('on-conflict-do-update', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = sync(ctx.conn.insertInto(tProject)
            ...
            if (ctx.realDbEnabled) {
                ...
                const project = sync(ctx.conn.selectFrom(tProject)
    ...
    */
    ...
    test('on-conflict-do-nothing-returning-last-inserted-id-is-nullable', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, "plan") values (?, ?) on conflict do nothing returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Conflict demo",
                "free",
            ...
            ctx.mockNext(null)
            const suppressedId = sync(ctx.conn.insertInto(tOrganization)
                .values({ name: 'Conflict demo', plan: 'free' })
                .onConflictDoNothing()
                .returningLastInsertedId()
                .executeInsert())
    ...
    test('on-conflict-on-columns-do-update-set-value-source-with-values-for-insert', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.insertInto(tProject)
                ...
                .executeInsert())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = project.name || excluded.name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    */
    ...
    test('on-conflict-do-nothing-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            const name = sync(ctx.conn.insertInto(tOrganization)
                .values({ name: 'Conflict demo', plan: 'free' })
                .onConflictDoNothing()
    ...
    test('on-conflict-on-columns-do-update-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            const name = sync(ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'Updated mktg' })
                .onConflictOn(tProject.organizationId, tProject.slug)
                .doUpdateSet({ name: 'Updated mktg' })
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? returning name as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('on-conflict-on-columns-do-update-where-returning-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? where project.name <> ? returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('multi-row-on-conflict-on-columns-do-update-returning-object', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const rows = sync(ctx.conn.insertInto(tProject)
                .values([
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?), (?, ?, ?) on conflict (organization_id, slug) do update set name = excluded.name returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('multi-row-on-conflict-on-columns-do-update-returning-one-column', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const names = sync(ctx.conn.insertInto(tProject)
                .values([
                    { organizationId: 1, slug: 'mktg-site', name: 'Upd A' },
                    { organizationId: 1, slug: 'tools', name: 'Upd B' },
                ])
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?), (?, ?, ?) on conflict (organization_id, slug) do update set name = excluded.name returning name as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('multi-row-on-conflict-on-columns-do-update-returning-object-with-optional-column', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const rows = sync(ctx.conn.insertInto(tIssue)
                .values([
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) values (?, ?, ?, ?, ?), (?, ?, ?, ?, ?) on conflict (project_id, number) do update set title = excluded.title returning id as id, assignee_id as assigneeId"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                1,
    ...
    test('on-conflict-on-columns-do-update-scaled-adapter-set-and-where', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.insertInto(tProjectReview)
                .values({
                    projectId:    1,
                    reviewerCode: 'R-1',
                    score:        3,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_review (project_id, reviewer_code, score, review_time) values (?, ?, ?, ?) on conflict (id) do update set score = ? where project_review.score > ? returning score as score"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "R-1",
    ...
    test('on-conflict-on-id-do-update-optional-custom-columns', async () => {
        ...
        ctx.mockNext(1)
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.insertInto(tReleaseDraft)
                ...
                .executeInsert())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title, stage, channel, min_version) values (?, ?, ?, ?, ?) on conflict (id) do update set stage = ?, channel = ?, min_version = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Alpha cut (upsert)",
    ...
    */
    ...
    test('on-conflict-do-update-with-nested-object-returning', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? returning id as id, name as "audit.name", slug as "audit.slug""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
