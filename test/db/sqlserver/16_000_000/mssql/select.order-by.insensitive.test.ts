...
describe(ctx.label, () => {
    ...
    test('collation-set: order-by-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate Latin1_General_CI_AS"`)
    ...
    test('collation-set: order-by-asc-nulls-first-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate Latin1_General_CI_AS asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('collation-set: order-by-desc-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate Latin1_General_CI_AS desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('collation-set: compound-order-by-insensitive', async () => {
        ...
        const result = await collated.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as [label] from project union select title as [label] from issue) as o_1_ order by [label] collate Latin1_General_CI_AS asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
