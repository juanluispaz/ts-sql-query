...
describe(ctx.label, () => {
    ...
    test('having-over-sum-average-min-aggregates', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, count(id) as total from issue group by project_id having sum(priority) >= ? and avg((priority * 1.0e0)) > ? and min(priority) >= ? order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1.5,
