...
- PostgreSQL: `.round()` now breaks ties **away from zero** on every operand type, matching every other dialect ts-sql-query supports (MariaDB, MySQL, Oracle, SQL Server, SQLite) — so `value.round()` for a `.5` value evaluates to `1` like JavaScript's `Math.round`, not `0`. Previously the same call could give either result depending on the expression it was applied to: a column or arithmetic that stayed on `numeric` rounded away from zero, but anything that went through `.divide(...)` or `.asDouble()` rounded to nearest even (because PostgreSQL's `round(double precision)` is platform-dependent and most systems round half-to-even). Behavior is now uniform regardless of the chain. A new property `usePlatformDependentRound: boolean` on `PostgreSqlConnection` (default `false`) opts into PostgreSQL's native `round(double precision)` rounding when you prefer round-to-even tie-breaking or have queries that depend on the previous result — assign `true` to it inside the connection subclass's constructor. Consumers whose `.round()` calls never produced a value ending in exactly `.5` see no change.
