...
describe(ctx.label, () => {
    ...
    test('customize-update-hook-fragment-with-column-reference', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const affected = sync(connection.update(tIssue)
                .set({ status: 'closed' })
    ...
    test('customize-update-returning-object-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const row = sync(connection.update(tIssue)
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update /*+ hint */ issue set title = ? where id = ? returning id as id, title as title"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched hero copy",
                1,
    ...
    test('customize-update-returning-one-column-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const status = sync(connection.update(tIssue)
                .set({ status: 'reviewed' })
                .where(tIssue.id.equals(1))
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update /*+ hint */ issue set status = ? where id = ? returning status as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "reviewed",
                1,
    ...
    test('customize-update-from-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /*+ hint */ project set name = organization.name from organization where project.organization_id = organization.id and organization.id = ?  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
