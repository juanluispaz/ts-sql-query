...
describe(ctx.label, () => {
    ...
    test('delete-using-join-all-gates-open-walks-to-end-and-runs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* gated issue.id */  delete /* gated issue.project_id */ from issue from project inner join organization on organization.id = project.organization_id where issue.project_id = project.id and issue.id = @0  /* gated project.slug */"`)
