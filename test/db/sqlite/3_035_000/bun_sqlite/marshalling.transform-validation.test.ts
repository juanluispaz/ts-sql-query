...
//     possible through `mockNext`, so these tests are mock-only BY
//     CONSTRUCTION — they ask for a mock-backed connection per
//     test/DESIGN.md §18 with `ctx.mockOnlyConnection()`, which keeps
//     the body running in every mode. They pin the defensive re-typing
...
describe(ctx.label, () => {
    ...
    test('marshalling/from-db-validation/int-from-numeric-string', async () => {
        // MOCK-ONLY: no real driver of this connector hands this representation back, so mockNext is
        // the only way to reach the transformValueFromDB arm under test.
    ...
    test('marshalling/from-db-validation/mandatory-error-carries-row-index-and-column-path', async () => {
        ...
        // and the reason carries rowIndex=1, columnPath='title'. `columnPath` is
