...
describe(ctx.label, () => {
    ...
    test('docs:delete/delete-using', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project where issue.project_id = project.id and lower(project.name) like lower('%' + @0 + '%')"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Legacy",
              ]
    ...
    test('docs-extra:delete/returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.title as [result] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
