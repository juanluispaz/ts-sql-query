...
describe(ctx.label, () => {
    ...
    test('rawfragment-hook-embeds-nested-subqueries-deeply', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(id) as "result" from issue where priority >= :0 order by (select count(id) as "result" from issue where status = :1)) as "wrapped",  id as "id" from project where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "closed",
