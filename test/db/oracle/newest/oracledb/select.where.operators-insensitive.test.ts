...
describe(ctx.label, () => {
    ...
    test('affix-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when lower(email) like lower(replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]') || '%') escape '\\' then 1 else 0 end as "sw", case when lower(email) not like lower(replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]') || '%') escape '\\' then 1 else 0 end as "nsw", case when lower(email) like lower('%' || replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]')) escape '\\' then 1 else 0 end as "ew", case when lower(email) not like lower('%' || replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]')) escape '\\' then 1 else 0 end as "new", case when lower(email) like lower('%' || replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]') || '%') escape '\\' then 1 else 0 end as "ct", case when lower(email) not like lower('%' || replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[]') || '%') escape '\\' then 1 else 0 end as "nct" from app_user where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
