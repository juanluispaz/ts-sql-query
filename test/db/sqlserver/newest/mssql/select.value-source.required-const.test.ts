...
describe(ctx.label, () => {
    ...
    test('required-const/int-double-bigint-boolean', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select @0 as [i], @1 as [d], @2 as bi, @3 as [b]"`)
    ...
    test('required-const/uuid-enum-custom-comparable', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select @0 as [u], @1 as [e], @2 as [c], @3 as cc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('required-const/custom-numeric-and-uuid', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select @0 as ci, @1 as cd, @2 as cu"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            1.5,
    ...
    test('required-const/temporal-time-and-datetime', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select @0 as lt, @1 as ldt, @2 as clt, @3 as cldt"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
