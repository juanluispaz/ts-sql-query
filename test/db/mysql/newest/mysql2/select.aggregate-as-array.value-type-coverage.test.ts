...
describe(ctx.label, () => {
    ...
    test('aggregate-of-boolean-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, json_arrayagg((project.published = 't')) as published from issue left join project on project.id = issue.project_id where issue.id = ? group by issue.id"`)
    ...
    test('aggregate-of-local-date-time-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(created_at) as createdAts from issue where project_id = ? group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-optional-local-date-time-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, json_arrayagg(project.archived_at) as archivedAts from issue left join project on project.id = issue.project_id where issue.id = ? group by issue.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-bigint-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(view_count) as views from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-optional-double-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(estimated_hours) as hours from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-optional-uuid-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(bin_to_uuid(external_ref)) as refs from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-object-with-bigint-uuid-and-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(json_object('views', view_count, 'externalRef', bin_to_uuid(external_ref), 'estimatedHours', estimated_hours)) as issues from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-object-with-boolean-and-local-date-time', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, json_arrayagg(json_object('published', (project.published = 't'), 'createdAt', project.created_at)) as projects from issue left join project on project.id = issue.project_id where issue.id = ? group by issue.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-object-with-adapter-transformed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(json_object('score', score, 'reviewer', reviewer_code)) as reviews from project_review where project_id = ? group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-one-column-optional-branded-adapter-strips-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(optional_release_ordinal) as ords from release_overview where project_id = ? group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
