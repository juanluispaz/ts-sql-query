...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = @0 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) order by parentId asc, id offset 0 rows) select id as id from tree"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             2,
//           ]
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = @0 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) order by 1 offset 0 rows fetch next @1 rows only) select id as id from tree"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             2,
//             10,
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = @0 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) order by 1 offset @1 rows fetch next @2 rows only) select id as id from tree"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             2,
//             5,
...
//
//         expect(ctx.history.length).toBe(2)
//         expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select /* hint */ /* cols */  id as id, name as name from project where id <= @0 window w1 as (partition by organization_id) order by project.organization_id asc, id, project.name desc offset 0 rows fetch next @1 rows only"`)
//         expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
//           [
//             3,
//             2,
...
//     })
    ...
    test('customize-select-distinct-page-hooks-ride-count-wrap', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  select distinct id as id from project where id <= @0 order by id offset 0 rows fetch next @1 rows only  /* tail */"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            3,
            2,
    ...
    test('customize-select-group-by-page-hooks-ride-count-wrap', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  select status as status, count(id) as total from issue group by status order by status offset 0 rows fetch next @0 rows only  /* tail */"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('customize-recursive-select-hooks-render-in-inline-scalar-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as /* warmup */ (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) /* end-of-with */ select id as id, (/* head */  select /* hint */ /* cols */  [result] as [result] from recursive_select_1  /* tail */) as root from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-recursive-select-hooks-render-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as /* warmup */ (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) /* end-of-with */ select id as id, (/* head */  select /* hint */ /* cols */  concat('[', string_agg(convert(nvarchar, [result]), ','), ']') from recursive_select_1  /* tail */) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) select id as id, (select concat('[', string_agg(convert(nvarchar, a_2_.[result]), ','), ']') from (select /* hint */ /* cols */  [result] as [result] from recursive_select_1 window w1 as (partition by result) order by [result] offset 0 rows) as a_2_) as tree from project where id = @1"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             1,
//             1,
...
//     })
    ...
    test('customize-recursive-one-column-ordering-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) select id as id, (select concat('[', string_agg(convert(nvarchar, a_2_.[result]), ','), ']') from (select [result] as [result] from recursive_select_1 order by [result] offset 0 rows) as a_2_) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-recursive-execute-select-page-count-strips-order-by-hooks', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, parent_id as parentId from issue where project_id = @0 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId) select id as id, parentId as parentId from recursive_select_1 order by id asc, parentId asc"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
          ]
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, parent_id as parentId from issue where project_id = @0 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId), result_for_count as (select id as id, parentId as parentId from recursive_select_1) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customize-select-plain-inline-value-renders-query-hooks-but-drops-with-query-hooks', async () => {
        ...
        const scalarResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (/* head */  select count(*) as [result] from issue where project_id = project.id  /* tail */) as [n] from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const arrResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (/* head */  select id as iid from issue where project_id = project.id for json path  /* tail */) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
