...
describe(ctx.label, () => {
    ...
    test('rawfragment-hook-embeds-nested-subqueries-deeply', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(id) as result from issue where priority >= ? order by (select count(id) as result from issue where status = ?)) as "wrapped",  id as id from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "closed",
