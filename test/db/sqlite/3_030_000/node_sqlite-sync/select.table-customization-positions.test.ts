...
describe(ctx.label, () => {
    ...
    test('table-customization: wrapper re-aliased as a select source keeps the hint', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tOrgCustom)
    ...
    test('table-customization: wrapper as a source inside a compound arm', async () => {
        ...
        const rows = sync(orgNames
            .union(projectNames)
            .orderBy('label')
            .executeSelectMany())

        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from /*+ hint */ organization  union select name as label from project where organization_id = ? order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: wrapper as an update target', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = sync(ctx.conn.update(tOrgCustom)
                .set({ name: 'Acme Renamed' })
                .where(tOrgCustom.id.equals(1))
                .executeUpdate())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update /*+ hint */ organization  set name = ? where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Renamed",
                1,
    ...
    test('table-customization: wrapper as a delete target', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = sync(ctx.conn.deleteFrom(tOrgCustom)
                .where(tOrgCustom.name.equals('Throwaway org'))
                .executeDelete())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from /*+ hint */ organization  where name = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Throwaway org",
              ]
