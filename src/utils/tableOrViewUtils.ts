...
export type ColumnsKeyOf<T> = ({ [K in keyof T]-?: T[K] extends AnyValueSource & Column ? K : never })[keyof T]
type ValueSourcesKeyOf<T> = ({ [K in keyof T]-?: T[K] extends AnyValueSource ? K : never })[keyof T]
type RemapValueSourceTypeInWithColumnInfo<T extends ITableOrView<any>, SOURCE extends NSource> = { [K in ValueSourcesKeyOf<T>]: RemapValueSourceType<SOURCE, T[K]> & ExtractColumnInfo<T[K]> }
...
export type FromRef<T extends ITableOrView<any>, REF extends HasSource<any>> = { [K in ValueSourcesKeyOf<T>]: RemapValueSourceType<REF[typeof source], T[K]> & ExtractColumnInfo<T[K]> } & REF
 ...
 */
type WithViewColumns<SOURCE extends NSource, COLUMNS> = { [K in ValueSourcesKeyOf<COLUMNS>]: RemapValueSourceTypeWithOptionalType<SOURCE, COLUMNS[K], WithOptionalTypeOf<COLUMNS[K]>> & Column}
...
export type AutogeneratedPrimaryKeyColumnsTypesOf<T> = ({ [K in keyof T]-?: 
    T[K] extends AnyValueSource & Column 
    ? (
...
export type ColumnsForSetOf<TYPE> = ({ [K in keyof TYPE]-?: 
    TYPE[K] extends AnyValueSource & Column
...
export type RequiredColumnsForSetOf<T> = ({ [K in keyof T]-?: 
    T[K] extends AnyValueSource & Column
    ? (
        T[K] extends ComputedColumn
        ? never
...
export type OptionalColumnsForSetOf<T> = ({ [K in keyof T]-?: 
    T[K] extends AnyValueSource & Column 
    ? (
        T[K] extends ComputedColumn
        ? never
