...
describe(ctx.label, () => {
    ...
    test('customize-insert-from-select-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ NO_LOG */ into project (organization_id, name, slug) select organization_id as organizationId, concat(name, ?) as name, concat(slug, ?) as slug from project where name like concat(?, '%')  /* batch-clone */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                " (clone)",
                "-clone",
    ...
    test('customize-insert-on-conflict-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ ignore into project (name, slug, organization_id) values (?, ?, ?) returning organization_id as organizationId, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    // NOT-APPLICABLE: MariaDB cannot combine INSERT … ON DUPLICATE KEY UPDATE with RETURNING (RETURNING is only supported on plain/IGNORE/REPLACE inserts), so the do-update returning executor is typed `never` on the mariadb dialect. The do-NOTHING returning arm (INSERT IGNORE … RETURNING) IS supported and runs live above.
    ...
    // NOT-APPLICABLE: MariaDB cannot combine INSERT … ON DUPLICATE KEY UPDATE with RETURNING (RETURNING is only supported on plain/IGNORE/REPLACE inserts), so the do-update returning executor is typed `never` on the mariadb dialect.
    ...
    test('customize-insert-default-values-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const affected = await connection.insertInto(tLedgerEntry)
                .defaultValues()
                .customizeQuery({
                    beforeQuery:        connection.rawFragment`/* head */ `,
                    afterInsertKeyword: connection.rawFragment`/*+ hint */`,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /*+ hint */ into ledger_entry () values ()  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
