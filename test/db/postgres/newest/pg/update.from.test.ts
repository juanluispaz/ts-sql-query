...
describe(ctx.label, () => {
    ...
    test('update-from-minmax-poison-case-qualifies-operands', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set assignee_id = case when issue.assignee_id is null or issue.parent_id is null then null else least(issue.assignee_id, issue.parent_id) end from project where issue.project_id = project.id and issue.id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
