...
describe(ctx.label, () => {
    ...
    test('having-over-string-concat-aggregate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, count(id) as total from issue group by project_id having string_agg(status, ',') like ('%' + @0 + '%') order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
