    ...
    */
    ...
    test('compound-order-by-raw-fragment-embedding-multiple-value-sources', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as [label] from project union select title as [label] from issue) as o_1_ order by [label], @0 + @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
