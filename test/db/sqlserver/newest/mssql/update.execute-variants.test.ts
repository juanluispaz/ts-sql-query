...
describe(ctx.label, () => {
    ...
    test('execute-update-many-with-min-max-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0 output inserted.id as id, inserted.status as status where project_id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "reviewed",
                1,
    ...
    test('execute-update-one-column-many-non-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0 output inserted.status as [result] where project_id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "reviewed",
                1,
