...
describe(ctx.label, () => {
    ...
    test('limit-offset-without-order-by-pk-in-projection', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .limit(1).offset(1)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as "status", id as "id" from issue offset :0 rows fetch next :1 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('limit-offset-without-order-by-no-pk-in-projection', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .limit(1).offset(1)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as "status" from issue offset :0 rows fetch next :1 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('offset-if-value-elides-offset-when-undefined', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue order by "id" fetch next :0 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
