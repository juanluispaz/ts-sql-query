...
describe(ctx.label, () => {
    ...
    test('set-if-set-if-value-requires-both-gates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = @0 where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
            1,
    ...
    test('set-if-not-set-if-value-skips-empty-and-already-set', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0, title = @1 where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "New title",
    ...
    test('set-if-has-value-if-value-requires-both-current-and-incoming', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = @0, body = @1, priority = @2 where id = @3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Triaged",
            null,
    ...
    test('ignore-if-has-value-drops-only-populated-sets', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set body = @0 where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            1,
    ...
    test('set-when-false-is-noop-and-when-true-delegates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = @0, priority = @1, body = @2 where id = @3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Base",
            7,
    ...
    test('dynamic-set-one-arg-overload-matches-the-plain-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(dynamicSql).toMatchInlineSnapshot(`"update issue set title = @0, priority = @1 where id = @2"`)
