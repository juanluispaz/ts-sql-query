...
describe(ctx.label, () => {
    ...
    test('union-all-execute-select-page-emits-data-then-count', async () => {
        ...
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count("label") as (select title as "label" from issue union all select name as "label" from project) select count(*) from result_for_count"`)
    ...
    test('union-execute-select-page-deduplicates-in-the-count', async () => {
        ...
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`[]`)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count("label") as (select status as "label" from issue union select status as "label" from issue) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
            data:  Array<{ label: string }>
            count: number
    ...
    test('union-all-execute-select-page-over-a-nested-object-wraps-dot-columns-in-count-cte', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select id as "iid", title as "detail.title", "number" as "detail.num" from issue where id = :0 union all select id as "iid", title as "detail.title", "number" as "detail.num" from issue where id = :1 order by 1 fetch next :2 rows only"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
        ...
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count(iid, "detail.title", "detail.num") as (select id as iid, title as "detail.title", "number" as "detail.num" from issue where id = :0 union all select id as iid, title as "detail.title", "number" as "detail.num" from issue where id = :1) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-execute-select-page-with-extras-data-skips-data-query', async () => {
        ...
        expect(ctx.history.length).toBe(1)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with result_for_count("label") as (select title as "label" from issue union all select name as "label" from project) select count(*) from result_for_count"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
            data:  Array<{ label: string }>
            count: number
