...
describe(ctx.label, () => {
    ...
    test('equals-if-value-in-update-where-fires-old-values-and-values-for-insert-propagation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 where id = @1 and name = @2"`)
    ...
    test('in-if-value-in-update-where-fires-array-iteration-propagation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0 where project_id = @1 and priority in (@2, @3, @4)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('between-in-update-where-fires-two-arg-propagation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0 where project_id = @1 and priority between @2 and @3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('concat-if-value-in-update-set-fires-or-ignore-propagation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = name + @0 where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                " (was)",
                1,
    ...
    test('replace-all-if-value-in-update-set-fires-two-arg-or-ignore-propagation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = replace(name collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "site",
                "gateway",
