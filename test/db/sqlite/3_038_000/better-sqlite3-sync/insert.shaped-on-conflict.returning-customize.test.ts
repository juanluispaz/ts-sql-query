...
describe(ctx.label, () => {
    ...
    test('shaped-on-conflict-do-update-returning-object-with-customize-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (organization_id, name, slug) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? returning id as id, name as name, slug as slug"`)
    ...
    test('shaped-set-if-value-null-skip-composed-with-on-conflict-returning', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.insertInto(tIssue)
