...
describe(ctx.label, () => {
    ...
    test('insert-from-select-without-returning', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = sync(ctx.conn.insertInto(tIssue)
                .from(source)
                .executeInsert())
    ...
    test('insert-from-select-on-conflict-returning-last-id-and-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext([])
            const oneCol = sync(ctx.conn.insertInto(tProject)
                .from(source())
                .onConflictDoNothing()
    ...
    test('insert-from-select-on-conflict-returning-object-none-or-one-and-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(null)
            const noneOrOne = sync(ctx.conn.insertInto(tProject)
                .from(source())
                .onConflictDoNothing()
                .returning({ id: tProject.id, name: tProject.name })
                .executeInsertNoneOrOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) select organization_id as organizationId, slug as slug, name as name from project where organization_id = ? on conflict do nothing returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
            ...
            ctx.mockNext([])
            const many = sync(ctx.conn.insertInto(tProject)
                .from(source())
                .onConflictDoNothing()
                .returning({ id: tProject.id, name: tProject.name })
                .executeInsertMany())
    ...
    test('insert-from-select-bare-on-conflict-do-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) select organization_id as organizationId, slug as slug, name as name from project where id = ? on conflict do update set name = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Marketing site v2",
