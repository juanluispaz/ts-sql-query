...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-of-compound-union-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast((select json_arrayagg(json_object('title', a_1_.title, 'body', a_1_.body)) from (select title as title, body as body from issue where project_id = project.id and id = ? union select title as title, body as body from issue where project_id = project.id and id = ?) as a_1_) as char) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
