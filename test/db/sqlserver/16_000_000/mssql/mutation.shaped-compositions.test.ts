    ...
    test('shaped-insert-customize-query', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await connection.insertInto(tProject)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* shaped-insert */  insert into project (organization_id, name, slug) values (@0, @1, @2)"`)
    ...
    test('shaped-update-customize-query', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await connection.update(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* shaped-update */  update project set name = @0 where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-update-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = await ctx.conn.update(tProject)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, inserted.name as name where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-update-returning-nested-object', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = await ctx.conn.update(tProject)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, inserted.name as [audit.name], inserted.slug as [audit.slug] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-update-allowing-no-where-extend-shape-then-no-arg-dynamic-set-opener', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.updateAllowingNoWhere(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0, organization_id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "No-where widened",
                1,
