...
describe(ctx.label, () => {
    ...
    test('add-column-to-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + id as [sum] from issue where id = @0"`)
    ...
    test('multiply-column-by-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority * id as [p] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('concat-column-to-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, email + full_name as slug from app_user where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('between-with-column-bounds', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority between id and (id + @0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('valueWhenNull-with-column-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce(body, title) as [b] from issue where id in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('modulo-column-by-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority % id as [m] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('merge-optional-from-value-source-argument-projected', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority + assignee_id as [x], cast(case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as bit) as [y], assignee_id + priority as [z] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        `)
        assertType<Exact<typeof result, Array<{ x?: number; y?: boolean; z?: number }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('numeric-operators-column-rhs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority - id as sub, cast(priority as float) / id as div, power(cast(priority as float), id) as pw, log(priority, id) as [ln], round(priority, id) as rnd, atn2(priority, id) as at2, iif(priority < number, priority, number) as mx from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('nullIfValue-with-column-argument', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(body, title) as [b] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inN-mixing-literal-and-value-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority in (@0, id) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('greaterOrEqual-column-vs-column-optional-propagation', async () => {
        ...
        `)
        assertType<Exact<typeof result, Array<{ x?: boolean }>>>()
        expect(result).toEqual(expected)
    })
    test('is-and-isnot-over-two-optional-columns-project-required-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(case when case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1 then 1 when not (case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1) then 0 else null end as bit) as eqIs, cast(case when not (case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1) then 1 when not (not (case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1)) then 0 else null end as bit) as neqIs from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
