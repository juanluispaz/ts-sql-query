...
describe(ctx.label, () => {
    ...
    test('floor', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, floor((priority * 1.0e0) / ?) as \`f\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('ceil', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceil((priority * 1.0e0) / ?) as \`c\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('round', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(cast((priority * 1.0e0) / ? as decimal(65, 30))) as \`r\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('asDouble', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (priority * 1.0e0) as \`d\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('double-required-receiver-unary-f1-fan-out', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs((priority * 1.0e0)) as ab, ceil((priority * 1.0e0)) as ce, floor((priority * 1.0e0)) as fl, sign((priority * 1.0e0)) as sg, sign((priority * 1.0e0)) * power(abs((priority * 1.0e0)), 1.0e0 / 3.0e0) as cb, exp((priority * 1.0e0)) as ex, ln((priority * 1.0e0)) as ln, log10((priority * 1.0e0)) as l10, power((priority * 1.0e0), ?) as pw, log(?, (priority * 1.0e0)) as lgn, round(cast((priority * 1.0e0) as decimal(65, 30)), ?) as rn, atan2((priority * 1.0e0), ?) as \`at\`, greatest((priority * 1.0e0), ?) as mn, least((priority * 1.0e0), ?) as mx from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('asInt-chained-into-numeric-op', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) % ? as \`m\`, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) + ? as \`a\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
