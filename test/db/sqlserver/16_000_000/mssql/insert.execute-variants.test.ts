...
describe(ctx.label, () => {
    ...
    test('execute-insert-multi-row-returning-last-inserted-id-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id values (@0, @1), (@2, @3)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Stark Industries",
                "pro",
    ...
    test('execute-insert-none-or-one-with-returning-one-column-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id as [result] select name as name, [plan] as [plan] from organization where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                -1,
              ]
    ...
    test('execute-insert-none-or-one-with-returning-row-shape-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id as id select name as name, [plan] as [plan] from organization where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                -1,
              ]
    ...
    test('execute-insert-one-throws-no-result-when-row-missing', async () => {
        ...
        // returns no row.
        // MOCK-ONLY: a real INSERT always returns the inserted row, so only the mock can produce the
        // missing row that trips NO_RESULT.
    ...
    test('execute-insert-many-with-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id as [result] values (@0, @1), (@2, @3)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Initrode",
                "free",
    ...
    test('values-accepts-a-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const count = await ctx.conn.insertInto(tIssue)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) values (@0, @1, upper(@2), @3, @4)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                999,
    ...
    test('insert-provided-primary-key-carries-the-pk-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const count = await ctx.conn.insertInto(tCountry)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into country (code, name, region) values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "DE",
                "Germany",
    ...
    test('execute-insert-many-with-returning-row-shape-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id as id, inserted.name as name, inserted.[plan] as [plan] values (@0, @1), (@2, @3)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Initech",
                "free",
    ...
    test('execute-insert-none-or-one-with-returning-row-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) output inserted.id as id, inserted.name as name, inserted.[plan] as [plan] values (@0, @1)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Wonka",
                "pro",
    ...
    test('insert-returning-one-column-throws-invalid-value-on-wrong-typed-value', async () => {
        ...
        // row it actually inserted, so only the mock reaches this gate.
        ctx.mockOnlyConnection()
        ctx.mockNext(1.5)
        let caught: unknown
        try {
    ...
    test('insert-returning-one-column-throws-mandatory-value-on-present-null', async () => {
        ...
        // it actually inserted, so only the mock reaches this gate.
        ctx.mockOnlyConnection()
        ctx.mockNext(null)
        let caught: unknown
        try {
