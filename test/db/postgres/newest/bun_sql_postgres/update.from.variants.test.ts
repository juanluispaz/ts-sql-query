...
describe(ctx.label, () => {
    ...
    test('update-from-returning-scaled-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set score = $1 from project where project_review.project_id = project.id and project_review.id = $2 returning project_review.score as score"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                900,
                1,
    ...
    test('update-from-values-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with orgNames(id, name) as (values ($1::int4, $2::text)) update project set name = orgNames.name from orgNames where project.organization_id = orgNames.id and project.id = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Renamed via values",
