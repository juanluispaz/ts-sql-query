...
describe(ctx.label, () => {
    ...
    test('optional-double-receiver-math', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 16 }).where(tIssue.id.equals(1)).executeUpdate())
    ...
    test('double-column-receiver-overloaded-arithmetic-threads-optional-and-emits-uncast', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, estimated_hours + ? as "a", estimated_hours - ? as "s", estimated_hours * ? as mu, estimated_hours % ? as mo, max(estimated_hours, ?) as mn, min(estimated_hours, ?) as mx from issue where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1.5,
                1.5,
    ...
    test('optional-double-receiver-unary-f1-fan-out', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 8 }).where(tIssue.id.equals(1)).executeUpdate())
