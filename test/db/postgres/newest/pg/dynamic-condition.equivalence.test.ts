...
describe(ctx.label, () => {
    ...
    test('equivalence/boolean-custom-adapter-notequals-and-in-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project where (published = 't') <> $1 and (published = 't') in ($2, $3) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            true,
    ...
    test('equivalence/boolean-plain-column-is-and-is-null-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable is not distinct from $1 and billable is null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('equivalence/boolean-descriptor-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable <> $1 and billable in ($2, $3) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            true,
    ...
    test('equivalence/bigint-descriptor-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where view_count > $1 and view_count <= $2 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            99n,
    ...
    test('equivalence/uuid-not-equals-insensitive-rewrite', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where lower(external_ref::text) <> lower($1) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0A8F9C1E-1111-4222-8333-444455556666",
          ]
    ...
    // Comparable `*IfValue` operators per leaf type through the dynamic dispatcher.
