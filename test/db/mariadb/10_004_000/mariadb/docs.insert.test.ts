...
describe(ctx.label, () => {
    ...
    */
    ...
    test('docs:insert/insert-set-if-not-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority, \`body\`) values (?, ?, ?, ?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                99,
    ...
    */
    ...
    test('docs:insert/insert-with-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (name, slug, organization_id) values (?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Operations",
                "ops",
    ...
    */
    ...
    test('docs:insert/insert-on-conflict-do-nothing', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into organization (name, plan) values (?, ?) returning id as id, name as name, plan as plan"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                "pro",
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB uses the bare onConflictDoUpdateSet form; the targeted .onConflictOn(col).doUpdateSet({...}) is not typed on MariaDBConnection.
    /*
    ...
    */
    ...
    test('docs-extra:insert/insert-on-conflict-do-update-bare', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan) values (?, ?) on duplicate key update plan = ? returning id as id, name as name, plan as plan"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                "pro",
