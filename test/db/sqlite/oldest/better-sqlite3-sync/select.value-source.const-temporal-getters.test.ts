...
describe(ctx.label, () => {
    ...
    test('const-localdate-getters', async () => {
        ...
        const rows = sync(ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', ?) as integer) as "y", cast(strftime('%m', ?) as integer) - 1 as mo, cast(strftime('%d', ?) as integer) as "d", cast(strftime('%w',?) as integer) as dow"`)
    ...
    test('const-localtime-getters', async () => {
        ...
        const rows = sync(ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%H', ?) as integer) as "h", cast(strftime('%M', ?) as integer) as "m", cast(strftime('%S', ?) as integer) as "s""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "12:34:56",
            "12:34:56",
    ...
    test('const-localdatetime-getters', async () => {
        ...
        const rows = sync(ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', ?) as integer) as "y", cast(strftime('%m', ?) as integer) - 1 as mo, cast(strftime('%H', ?) as integer) as "h", round((julianday(?) - 2440587.5) * 86400000.0) as "t""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-15 12:34:56",
            "2024-01-15 12:34:56",
    ...
    test('const-localdatetime-pre-epoch-gettime', async () => {
        ...
        const rows = sync(ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select round((julianday(?) - 2440587.5) * 86400000.0) as pre, round((julianday(?) - 2440587.5) * 86400000.0) as eve, round((julianday(?) - 2440587.5) * 86400000.0) as sub, round((julianday(?) - 2440587.5) * 86400000.0) as far"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1969-12-30 12:00:00",
            "1969-12-31 12:00:00",
    ...
    test('chained-valuewhennull-localdate-getmonth', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%m', ifnull(work_date, ?)) as integer) - 1 as mo from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2000-01-01",
            1,
    ...
    test('chained-nullifvalue-localdate-getmonth', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%m', nullif(work_date, ?)) as integer) - 1 as mo from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2000-01-01",
            1,
    ...
    test('chained-asrequiredinoptionalobject-localdate-getmonth', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%m', review_date) as integer) - 1 as mo from project_review where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('chained-onlywhenornull-localtime-gethours', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%H', started_at) as integer) as "h" from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('chained-modifiers-customlocaldatetime-getfullyear', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', ifnull(signed_off_at, ?)) as integer) as vwn, cast(strftime('%Y', nullif(signed_off_at, ?)) as integer) as nif, cast(strftime('%Y', signed_off_at) as integer) as rio, cast(strftime('%Y', signed_off_at) as integer) as own, cast(strftime('%Y', signed_off_at) as integer) as ign from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2000-01-01 00:00:00",
            "1999-01-01 00:00:00",
