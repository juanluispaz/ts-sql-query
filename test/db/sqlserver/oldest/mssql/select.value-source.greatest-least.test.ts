...
describe(ctx.label, () => {
    ...
    test('minValue-floors-column-with-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(priority > @0, priority, @1) as floored from issue order by id"`)
    ...
    test('maxValue-caps-column-with-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(priority < @0, priority, @1) as capped from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('minValue-column-vs-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(priority > number, priority, number) as atLeastNumber from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('maxValue-on-nullable-column-keeps-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when assignee_id is null then null else iif(assignee_id < @0, assignee_id, @1) end as capped from issue where assignee_id is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            99,
    ...
    test('minValue-maxValue-null-operand-poisons-to-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when assignee_id is null then null else iif(assignee_id > @0, assignee_id, @1) end as floored, case when assignee_id is null then null else iif(assignee_id < @2, assignee_id, @3) end as capped from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
            5,
    ...
    test('minValue-maxValue-both-optional-int-poisons-with-or', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when assignee_id is null or parent_id is null then null else iif(assignee_id > parent_id, assignee_id, parent_id) end as mn, case when assignee_id is null or parent_id is null then null else iif(assignee_id < parent_id, assignee_id, parent_id) end as mx from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('minValue-maxValue-both-optional-bigint-poisons-with-or', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when duration_ms is null or duration_ms is null then null else iif(duration_ms > duration_ms, duration_ms, duration_ms) end as mn, case when duration_ms is null or duration_ms is null then null else iif(duration_ms < duration_ms, duration_ms, duration_ms) end as mx from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('minValue-maxValue-both-optional-double-poisons-with-or', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when avg_rating is null or avg_rating is null then null else iif(avg_rating > avg_rating, avg_rating, avg_rating) end as mn, case when avg_rating is null or avg_rating is null then null else iif(avg_rating < avg_rating, avg_rating, avg_rating) end as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: number; mx?: number }>>>()
        expect(result).toEqual(expected)
    })
    test('minValue-maxValue-operand-optional-int-poisons-single-guard', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when assignee_id is null then null else iif(priority > assignee_id, priority, assignee_id) end as mn, case when assignee_id is null then null else iif(priority < assignee_id, priority, assignee_id) end as mx from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: number; mx?: number }>>>()
        expect(result).toEqual(expected)
    })
