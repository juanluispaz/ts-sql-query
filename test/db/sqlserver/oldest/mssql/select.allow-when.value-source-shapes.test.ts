...
describe(ctx.label, () => {
    ...
    test('in-n-with-allowed-column-in-the-value-list-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where id in (number, @0) order by id"`)
    ...
    test('is-with-allowed-column-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('concat-if-value-on-allowed-string-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title + @0 as [label] from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            " [issue]",
            1,
    ...
    test('replace-all-if-value-with-allowed-second-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(title collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, status) collate DATABASE_DEFAULT as [label] from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "hero",
            1,
