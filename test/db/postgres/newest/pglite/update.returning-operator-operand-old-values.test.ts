...
describe(ctx.label, () => {
    ...
    test('returning-equals-if-value-with-old-value-receiver-and-a-present-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2 returning id as id, old.slug = $3 as "wasMktgSite""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v2",
                1,
    ...
    test('returning-concat-if-value-with-old-value-receiver', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2 returning id as id, old.name || $3 as label"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Marketing site v3",
                1,
