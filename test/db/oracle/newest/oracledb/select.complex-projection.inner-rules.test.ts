...
describe(ctx.label, () => {
    ...
    test('merge-optional-requiredInOptionalObject-is-preserved-through-an-operator', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", case when priority = id then 1 when not (priority = id) then 0 else null end as "meta.flag", assignee_id as "meta.assigneeId" from issue where project_id = :0 order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-1-nested-object-under-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", case when priority = id then 1 when not (priority = id) then 0 else null end as "meta.flag", assignee_id as "meta.assigneeId" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-3-required-inner-object-with-optional-leaf-under-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", title as "detail.title", "body" as "detail.body" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('plain-select-rule-2-same-left-join-object-with-optional-leaf-under-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", project.id as "proj.id", project.name as "proj.name", project.archived_at as "proj.archivedAt" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('plain-select-rule-2-object-dropped-on-left-join-miss-default-as-undefined', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "pid", issue.id as "iss.id", issue.title as "iss.title" from project left join issue on issue.project_id = project.id where project.id in (:0, :1) order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('rule-1-mixing-required-in-optional-object-with-own-required-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", id as "meta.ownId", status as "meta.gate" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-1-optional-object-containing-a-nested-required-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "meta.gate", "number" as "meta.inner.num", priority as "meta.inner.pri" from issue where project_id = :0 order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-union-preserves-a-nested-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "number" as "header.num", title as "header.title" from issue where id = :0 union select id as "iid", "number" as "header.num", title as "header.title" from issue where id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('nested-object-cte-used-via-for-use-in-left-join-becomes-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with proj_cte as (select id as pid, name as "info.name", slug as "info.slug" from project) select issue.id as "iid", proj_cte."info.name" as "proj.name", proj_cte."info.slug" as "proj.slug" from issue left join proj_cte on proj_cte.pid = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('left-join-object-of-only-optional-columns-applies-rule-4-not-rule-2', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "pid", issue."body" as "opt.body", issue.assignee_id as "opt.assigneeId" from project left join issue on issue.project_id = project.id where project.id in (:0, :1) order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('rule-3-required-outer-object-containing-a-rule-1-required-in-optional-inner-object-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", title as "detail.title", status as "detail.meta.gate", assignee_id as "detail.meta.assigneeId" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-1-outer-rule-1-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", status as "meta.gate", "body" as "meta.inner.innerGate", assignee_id as "meta.inner.extra" from issue where project_id = :0 order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-1-outer-rule-2-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", issue.status as "meta.gate", project.name as "meta.proj.name", project.archived_at as "meta.proj.arch" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-1-outer-rule-4-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", status as "meta.gate", "body" as "meta.inner.body", assignee_id as "meta.inner.assigneeId" from issue where id in (:0, :1) order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('matrix-rule-2-outer-rule-1-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", project.id as "proj.id", project.slug as "proj.inner.gate", project.archived_at as "proj.inner.arch" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-2-outer-rule-3-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", project.name as "proj.name", issue.title as "proj.inner.title", issue."body" as "proj.inner.body" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-2-outer-rule-4-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", project.name as "proj.name", project.archived_at as "proj.inner.arch" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-3-outer-rule-2-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", issue.title as "detail.title", project.name as "detail.proj.name", project.archived_at as "detail.proj.arch" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-4-outer-rule-1-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "outer.own", status as "outer.inner.gate", assignee_id as "outer.inner.extra" from issue where project_id = :0 order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('matrix-rule-4-outer-rule-2-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "iid", issue."body" as "outer.own", project.name as "outer.proj.name", project.archived_at as "outer.proj.arch" from issue left join project on project.id = issue.project_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('matrix-rule-4-outer-rule-4-inner-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "outer.own", estimated_hours as "outer.inner.est", parent_id as "outer.inner.parent" from issue where id in (:0, :1) order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('sole-optional-inner-own-table-rule-4-collapse-drops-wrapper-and-type-is-optional-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "wrapper.inner.body", assignee_id as "wrapper.inner.assigneeId" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('sole-optional-inner-left-join-rule-2-collapse-drops-wrapper-and-type-is-optional-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "pid", issue.id as "wrapper.inner.iid", issue."number" as "wrapper.inner.num" from project left join issue on issue.project_id = project.id where project.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('requiredInOptionalObject-leaf-demoted-through-with-view', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with meta_cte as (select id as iid, status as "meta.gate", assignee_id as "meta.extra" from issue where id in (:0, :1)) select iid as "iid", "meta.gate" as "meta.gate", "meta.extra" as "meta.extra" from meta_cte order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('merge-optional-requiredInOptionalObject-demoted-to-optional-through-operator', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as "meta.flag", "number" as "meta.ownReq" from issue where id in (:0, :1) order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('merge-optional-requiredInOptionalObject-demoted-to-optional-through-operator-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as "meta.flag", "number" as "meta.ownReq" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('rule-1-wrapper-of-sole-rule-1-inner-with-own-required-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "wrapper.inner.gate", "number" as "wrapper.inner.ownReq" from issue where project_id = :0 order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-2-wrapper-of-sole-rule-2-inner-with-const-required-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "pid", issue.id as "wrapper.inner.iid", :0 as "wrapper.inner.constReq" from project left join issue on issue.project_id = project.id where project.id in (:1, :2) order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('nested-object-cte-used-via-for-use-in-left-join-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with proj_cte as (select id as pid, name as "info.name", slug as "info.slug" from project where id = :0) select issue.id as "iid", proj_cte."info.name" as "proj.name", proj_cte."info.slug" as "proj.slug" from issue left join proj_cte on proj_cte.pid = issue.project_id where issue.id in (:1, :2) order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('cte-re-projected-rule-2-object-as-nullable-surfaces-null-on-miss', async () => {
        ...
        const rows = await connection.selectFrom(projectIssueCte)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with project_issue_cte as (select project.id as pid, issue.id as "iss.id", issue.title as "iss.title" from project left join issue on issue.project_id = project.id where project.id in (:0, :1)) select pid as "pid", "iss.id" as "iss.id", "iss.title" as "iss.title" from project_issue_cte order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('cte-re-projected-rule-3-object-with-optional-leaf-as-nullable', async () => {
        ...
        const rows = await connection.selectFrom(issueCte)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issue_detail_cte as (select id as iid, title as "detail.title", "body" as "detail.body" from issue where project_id = :0) select iid as "iid", "detail.title" as "detail.title", "detail.body" as "detail.body" from issue_detail_cte order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('cte-with-nested-object-aggregate-projecting-then-as-required-in-optional-object-carries-flag', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with org_projects_cte as (select "organization".id as orgId, json_arrayagg(json_object('id' value project.id, 'name' value project.name, 'archivedAt' value project.archived_at)) as "meta.projects" from "organization" left join project on project.organization_id = "organization".id group by "organization".id) select orgId as "orgId", "meta.projects" as "meta.projects" from org_projects_cte order by "orgId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            orgId: number
    ...
    test('cte-with-nested-object-carrying-a-custom-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with wl_cte as (select id as wid, case approved when 'A' then 1 when 'R' then 0 else null end as "status.approved" from issue_worklog where id = :0) select wid as "wid", "status.approved" as "status.approved" from wl_cte"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
