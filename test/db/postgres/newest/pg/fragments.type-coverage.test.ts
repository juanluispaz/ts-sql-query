...
import { tIssue, tIssueWorklog, tOrganization, tProjectRelease, type ReleaseChannel, type WorklogActivity } from '../../domain/connection.js'
...
describe(ctx.label, () => {
    ...
    test('fragment-with-type-date-time-and-enum-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select work_date as wd, started_at as st, activity as act from issue_worklog where id = $1"`)
    ...
    test('fragment-with-type-double-custom-and-custom-local-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select 1.5 as avg, channel as ch, signing_key as key, released_on as "rOn", cutoff_time as cut, signed_off_at as soff from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-widen-the-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select version as "v", 1.5 as avg, signing_key as key, channel as ch from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-enum-and-custom-arms', async () => {
        ...
        const actRow = await c.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(activity) as "maxActivity" from issue_worklog where issue_id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-custom-double-and-local-date-time-arms', async () => {
        ...
        const amtRow = await c.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select billed_amount as amt from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const soffRow = await c.selectFrom(tProjectRelease)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select signed_off_at as soff from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select upper(activity) as tagged from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) > 0 as "any" from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('aggregate-fragment-with-type-boolean-optional-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority) > 5 as "any" from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
