...
describe(ctx.label, () => {
    ...
    */
    ...
    test('inline-aggregate-of-object-with-having', async () => {
        ...
        expect(row).toEqual(expected)
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, cast((select json_arrayagg(json_object('id', a_1_.id, 'count', a_1_.count)) from (select project.id as id, count(issue.id) as count from project inner join issue on issue.project_id = project.id where project.organization_id = \`organization\`.id group by project.id having count(issue.id) > ?) as a_1_) as char) as busyProjects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    */
    ...
    test('null-inline-aggregate-then-use-empty-array-for-no-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, cast(null as char) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast((select json_arrayagg(json_object('id', a_1_.id, 'title', a_1_.title)) from (select id as id, title as title from issue where project_id = project.id order by id limit 2147483647) as a_1_) as char) as \`meta.issues\` from project where organization_id = ? order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    */
    ...
    test('null-inline-aggregate-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast(null as char) as \`meta.issues\` from project where organization_id = ? order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-aggregate-order-by-asc-nulls-last', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result is null, result asc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-order-by-desc-nulls-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result is not null, result desc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-order-by-asc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by lower(result) asc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-order-by-asc-nulls-last-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result is null, lower(result) asc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-of-one-column-keeps-the-branded-element-type', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(\`channel\`) from project_release where project_id = project.id) as char) as channels from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
...
//             projects: Array<{ id: number; name: string }>
//         }>>()
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, cast((select json_arrayagg(json_object('id', a_1_.id, 'name', a_1_.\`name\`)) from (/* agg-head */  select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id and archived_at is null union select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id and archived_at is not null  /* agg-tail */) as a_1_) as char) as projects from \`organization\` where id = ?"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             1,
//           ]
...
//     })
    ...
    test('inline-aggregate-of-distinct-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select distinct \`status\` as result from issue where project_id = project.id) as a_1_) as char) as statuses from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-of-distinct-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(json_object('status', a_1_.\`status\`, 'priority', a_1_.priority)) from (select distinct \`status\` as \`status\`, priority as priority from issue where project_id = project.id) as a_1_) as char) as issueKinds from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-with-group-by-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, cast((select json_arrayagg(json_object('assigneeId', a_1_.assigneeId, 'count', a_1_.count)) from (select issue.assignee_id as assigneeId, count(issue.id) as count from project inner join issue on issue.project_id = project.id where project.organization_id = \`organization\`.id group by issue.assignee_id) as a_1_) as char) as byAssignee from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-with-having-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, cast((select json_arrayagg(json_object('assigneeId', a_1_.assigneeId, 'count', a_1_.count)) from (select issue.assignee_id as assigneeId, count(issue.id) as count from project inner join issue on issue.project_id = project.id where project.organization_id = \`organization\`.id group by issue.assignee_id having count(issue.id) > ?) as a_1_) as char) as byAssignee from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            1,
    ...
    */
    ...
    test('inline-aggregate-of-distinct-object-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(json_object('status', a_1_.\`status\`, 'body', a_1_.body)) from (select distinct \`status\` as \`status\`, body as body from issue where project_id = project.id) as a_1_) as char) as kinds from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-of-compound-union-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast((select json_arrayagg(json_object('title', a_2_.title, 'body', a_2_.body)) from (select title as title, body as body from issue where project_id = project.id and id = ? union select title as title, body as body from issue where project_id = project.id and id = ?) as a_2_) as char) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('inline-aggregate-of-compound-union-rule2-left-join-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast((select json_arrayagg(json_object('name', a_1_.\`name\`)) from (select app_user.full_name as \`name\` from issue left join app_user on app_user.id = issue.assignee_id where issue.id = ? union select app_user.full_name as \`name\` from issue left join app_user on app_user.id = issue.assignee_id where issue.id = ?) as a_1_) as char) as \`names\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('inline-aggregate-of-compound-union-rule1-gate-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast((select json_arrayagg(json_object('ref', a_1_.ref, 'assigneeId', a_1_.assigneeId)) from (select body as ref, assignee_id as assigneeId from issue where id = ? union select body as ref, assignee_id as assigneeId from issue where id = ?) as a_1_) as char) as items"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('inline-aggregate-nested-inside-inline-aggregate-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, cast((select json_arrayagg(json_object('pid', id, 'issues', (select json_arrayagg(json_object('iid', id)) from issue where project_id = project.id))) from project where organization_id = \`organization\`.id) as char) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-wrapping-inline-object-aggregate-projecting-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast(json_arrayagg((select json_arrayagg(json_object('id', id, 'body', body)) from issue where project_id = project.id)) as char) as \`nested\` from project where id = ? group by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result asc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result desc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by lower(result) limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by lower(result) desc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result is not null, lower(result) desc limit 2147483647) as a_1_) as char) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
