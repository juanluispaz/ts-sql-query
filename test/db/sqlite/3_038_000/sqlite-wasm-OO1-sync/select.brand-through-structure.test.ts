...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-of-one-column-preserves-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_group_array(version) as versions, json_group_array(channel) as channels from project_release where project_id = ? group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-preserves-enum-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, json_group_array(activity) as activities from issue_worklog where issue_id = ? group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-union-carries-the-seed-brand-onto-the-result', async () => {
        ...
        const p2 = ctx.conn.selectFrom(tProjectRelease)
            ...
            .select({ channel: tProjectRelease.channel })
        const rows = sync(p1.union(p2).orderBy('channel').executeSelectMany())
    ...
    test('cte-carries-the-brand-through-for-use-in-query-as', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(releaseCte)
    ...
    test('recursive-union-all-preserves-custom-brand', async () => {
        ...
        ctx.mockNext(expected)
        const rows = sync(connection.selectFrom(tProjectRelease)
    ...
    test('recursive-union-all-preserves-enum-brand', async () => {
        ...
        ctx.mockNext(expected)
        const rows = sync(connection.selectFrom(tIssueWorklog)
