...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(issue.title null on null) as [meta.titles] from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.id order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('aggregate-as-array-element-level-rule-1-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as oid, json_arrayagg(json_object('ref':issue.title, 'assigneeId':issue.assignee_id)) as issues from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-element-level-rule-4-all-optional-not-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, json_arrayagg(json_object('body':body, 'assigneeId':assignee_id)) as opts from issue where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-projecting-optional-values-as-nullable-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('id':issue.id, 'body':issue.body)) as [meta.issues] from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-query-value-one-column-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_arrayagg(json_object('id':id, 'body':body)) as [result] from issue where project_id = project.id) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
