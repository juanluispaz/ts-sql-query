    ...
    test('set-minmax-poison-case-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .where(tIssue.id.equals(1))
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set assignee_id = least(assignee_id, parent_id) where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('collation-forks-in-a-mutation-set-and-where', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tAppUser)
                ...
                .where(tAppUser.email.collate('utf8mb4_bin').equals('ada@acme.test'))
                .executeUpdate()

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update app_user set full_name = regexp_replace(full_name, ?, ?) where (email collate utf8mb4_bin) = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Love",
                "X",
    ...
    test('concat-with-optional-operand-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set body = concat(title, body) where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
