...
## Multi-row `insert().values([...]).onConflictDoNothing()` wrongly exposes `executeInsertOne` (its targeted twin and every other `doNothing` path drop it)

**Where**: `src/expressions/insert.ts:125` —
`CustomizableExecutableMultipleInsert.onConflictDoNothing` routes to the
**non-optional** `CustomizableExecutableMultipleInsertOnConflict` (`:109-113`,
whose `returning` / `returningOneColumn` are the non-optional `ReturningFnType` /
`ReturningOneColumnFnType` → **expose `executeInsertOne`**). Every other
`onConflictDoNothing` / `doNothing` in the file routes to its `…OnConflictOptional`
sibling instead (which uses `ReturningOptionalFnType` / `ReturningOneColumnOptionalFnType`
→ **no `executeInsertOne`**): Simple bare `:94` and targeted `:103`, FromSelect
bare `:63` and targeted `:72`, and — decisively — the Multiple builder's OWN
targeted `.onConflictOn(cols).doNothing()` at `:134`
(`CustomizableExecutableMultipleInsertOnConflictOptional`, `:115-119`). Line 125
is the sole outlier. Runtime is identical: `onConflictDoNothing()` and the
targeted `doNothing()` both just set `this.__onConflictDoNothing = true`.

**Reproduction** (compile-repro, PostgreSQL cell — `onConflictDoNothing` is
supported per `:125`; deleted after verifying):
```ts
// COMPILES today, but must NOT: onConflictDoNothing can suppress every row, so
// the batch may return 0 rows and executeInsertOne would throw NO_RESULT.
connection.insertInto(t).values([rowA, rowB]).onConflictDoNothing()
    .returning({ id: t.id }).executeInsertOne()

// Correctly a TYPE ERROR (executeInsertOne absent) — the targeted twin:
connection.insertInto(t).values([rowA, rowB]).onConflictOn(t.slug).doNothing()
    .returning({ id: t.id }).executeInsertOne()

// Also correctly a TYPE ERROR — the Simple bare twin:
connection.insertInto(t).values(rowA).onConflictDoNothing()
    .returning({ id: t.id }).executeInsertOne()
```
`executeInsertOne` demands exactly one returned row (throws `NO_RESULT` on 0,
`MORE_THAN_ONE_ROW` on >1). `onConflictDoNothing` is an instruction that can skip
rows, so the batch can legitimately return 0 rows — offering `executeInsertOne`
on that path leads the caller straight to a runtime error while the type promised
a value. **Fix**: `:125` NEXT arg → `CustomizableExecutableMultipleInsertOnConflictOptional<TABLE, USING>`
(matching `:134`). Only observable change is dropping `executeInsertOne` from the
Multiple bare `doNothing` returning path; `returningLastInsertedId` is unaffected
(both Multiple variants return the same `AutogeneratedPrimaryKeyColumnsTypesOf<TABLE>[]`).

**Current workaround in the suite**: none — the type wrongly permits it, so no
test can exercise the intended contract yet. When fixed, add a `@ts-expect-error`
negative-type lock in `postgres/types.negative/insert.test.ts` (mirroring the
targeted-twin form): `insertInto(t).values([...]).onConflictDoNothing().returning({...}).executeInsertOne()`
must not compile, with a passing `executeInsertNoneOrOne()` control. See
`MISSING_TESTS_AUDIT_50.md` §B-1.
