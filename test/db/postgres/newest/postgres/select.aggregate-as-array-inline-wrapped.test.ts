...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-of-one-column-keeps-the-branded-element-type', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_agg(channel) from project_release where project_id = project.id) as channels from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-wrapping-inline-object-aggregate-projecting-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, json_agg((select json_agg(json_build_object('id', id, 'body', body)) from issue where project_id = project.id)) as nested from project where id = $1 group by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
