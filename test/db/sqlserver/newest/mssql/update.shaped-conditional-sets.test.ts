...
describe(ctx.label, () => {
    ...
    test('shaped-set-if-has-value-if-value-requires-current-and-incoming', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0, archived_at = @1 where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Overwritten name",
                null,
    ...
    test('shaped-ignore-if-has-value-drops-only-populated-renamed-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set archived_at = @0 where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                null,
                1,
    ...
    test('shaped-allowing-no-where-set-when-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = @0"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = @0, published = case when (@1 = 1) then 't' else 'f' end"`)
            expect(trueSql).not.toBe(falseSql)
            assertType<Exact<typeof affected, number>>()
    ...
    test('shaped-extend-shape-on-allowing-no-where-adds-renamed-keys', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0, published = case when (@1 = 1) then 't' else 'f' end"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Extended rename",
                false,
    ...
    test('shaped-allowing-no-where-ignore-if-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = @0, published = case when (@1 = 1) then 't' else 'f' end"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = @0"`)
            expect(trueSql).not.toBe(falseSql)
            expect(affected).toBe(4)
        })
    })
    ...
    test('shaped-allowing-no-where-ignore-if-has-value-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = @0, archived_at = @1"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = @0"`)
            expect(trueSql).not.toBe(falseSql)
            expect(affected).toBe(4)
        })
