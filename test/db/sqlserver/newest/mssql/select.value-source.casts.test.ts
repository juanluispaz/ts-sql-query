...
describe(ctx.label, () => {
    ...
    test('asBigint-on-double-chained-into-bigint-op', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(cast(priority as float), 0) + @0 as [a], round(cast(priority as float), 0) % @1 as [m] from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2n,
            2n,
