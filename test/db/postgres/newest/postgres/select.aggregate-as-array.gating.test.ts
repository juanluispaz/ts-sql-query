...
describe(ctx.label, () => {
    ...
    test('null-inline-aggregated-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (null)::text as direct, (null)::text as kept, (null)::text as dropped, (null)::text as ignored, (null)::text as "keptIgnored" from project where id = $1"`)
    ...
    test('null-aggregate-as-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, (null)::text as kept, (null)::text as dropped, (null)::text as ignored, (null)::text as "keptIgnored" from project left join issue on issue.project_id = project.id where project.id = $1 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
