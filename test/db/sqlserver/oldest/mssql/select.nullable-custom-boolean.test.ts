...
describe(ctx.label, () => {
    ...
    test('insert-nullable-custom-boolean-true-emits-case-when-remap', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue_worklog (issue_id, work_date, activity, approved) values (@0, @1, @2, case @3 when convert(bit, 1) then 'A' when convert(bit, 0) then 'R' else null end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
    ...
    test('is-not-null-on-custom-boolean-string-adapter-matches-non-null-rows', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('is-not-null-on-custom-boolean-numeric-adapter-matches-all-rows', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
