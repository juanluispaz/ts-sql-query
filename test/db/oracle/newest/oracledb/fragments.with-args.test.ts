...
describe(ctx.label, () => {
    ...
    test('build-fragment-with-args-if-value-emits-when-value-present', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "x" from dual where :1 = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('build-fragment-with-args-if-value-short-circuits-noop-on-undefined', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "x" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('build-fragment-with-maybe-optional-args-reads-optional-from-value-source-argument', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 + assignee_id as "r" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('build-fragment-with-maybe-optional-args-reads-optional-from-value-source-after-plain-arg', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, "body") as "r" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('build-fragment-with-maybe-optional-args-keeps-required-with-no-optional-value-source-after-plain-arg', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, title) as "r" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('build-fragment-with-args-arity-1-over-bigint-arg', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select abs(:0) as "r" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -5n,
          ]
    ...
    test('build-fragment-with-args-if-value-arity-1-emits-when-present', async () => {
        ...
        await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "x" from dual where :1 > 0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            5,
    ...
    test('build-fragment-with-maybe-optional-args-arity-3-over-string', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, :2) as "r" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
            null,
    ...
    test('args-arity-5-coalesce', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, :2, :3, :4) as "r" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
            "b",
    ...
    test('args-if-value-arity-3-emits-when-present', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where coalesce(:0, :1) = :2 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "z",
            "y",
    ...
    test('args-if-value-arity-4-emits-when-present', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where coalesce(:0, :1, :2) = :3 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "z",
            "y",
    ...
    test('args-if-value-arity-5-emits-when-present', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where coalesce(:0, :1, :2, :3) = :4 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "z",
            "y",
    ...
    test('maybe-optional-args-arity-0-constant', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select 42 as "r" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ r: number }>>>()
    ...
    test('maybe-optional-args-arity-4-coalesce', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, :2, :3) as "r" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
            null,
    ...
    test('aggregate-args-arity-0-count', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-args-arity-2-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority + id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-args-arity-3-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority + id + "number") as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-args-arity-4-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority + id + "number" + project_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-args-arity-5-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority + id + "number" + project_id + priority) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-maybe-optional-arity-2-max', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-maybe-optional-arity-3-max', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + "number") as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-maybe-optional-arity-4-max', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + "number" + project_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-maybe-optional-arity-5-max', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + "number" + project_id + priority) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-maybe-optional-arity-1-optional-operand-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(assignee_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('aggregate-maybe-optional-arity-2-optional-operand-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + assignee_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('aggregate-maybe-optional-arity-3-optional-operand-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + assignee_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('aggregate-maybe-optional-arity-4-optional-operand-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + "number" + assignee_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('aggregate-maybe-optional-arity-5-optional-operand-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority + id + "number" + project_id + assignee_id) as "result" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('aggregate-if-value-arity-3-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid" from issue group by project_id having sum(priority + id) > :0 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('aggregate-if-value-arity-4-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid" from issue group by project_id having sum(priority + id + "number") > :0 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('aggregate-if-value-arity-5-sum', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid" from issue group by project_id having sum(priority + id + "number" + project_id) > :0 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('maybe-optional-args-arity-4-value-source-after-plain-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, "body", :2) as "r" from issue where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('maybe-optional-args-arity-4-value-source-after-plain-required-control', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, title, :2) as "r" from issue where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('maybe-optional-args-arity-5-value-source-after-plain-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, "body", :2, :3) as "r" from issue where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('maybe-optional-args-arity-5-value-source-after-plain-required-control', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select coalesce(:0, :1, title, :2, :3) as "r" from issue where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "b",
    ...
    test('maybe-optional-args-arity-3-value-source-after-plain-null-propagates-to-undefined', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 + :1 + assignee_id as "r" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
