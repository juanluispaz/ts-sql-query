...
describe(ctx.label, () => {
    ...
    test('intersect', async () => {
        ...
        const right = ctx.conn.selectFrom(tIssue)
            ...
            .select({ status: tIssue.status })
        const result = sync(left.intersect(right).executeSelectMany())
    ...
    test('except', async () => {
        ...
        const small = ctx.conn.selectFrom(tIssue)
            .where(tIssue.id.lessOrEqual(2))
            .select({ status: tIssue.status })
        const result = sync(all.except(small).executeSelectMany())
    ...
    test('compound-order-by-desc', async () => {
        ...
        ctx.mockNext(expected)
        const result = sync(ctx.conn.selectFrom(tProject)
            .select({ label: tProject.name })
    ...
    test('compound-order-by-asc-nulls-last', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label is null, label asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ label: string }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('compound-order-by-desc-nulls-first', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label is not null, label desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ label: string }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('compound-with-optional-seed-column-yields-optional-result', async () => {
        ...
        const other = ctx.conn.selectFrom(tProject)
            .where(tProject.archivedAt.isNull())
            .select({ a: tProject.archivedAt })
        const result = sync(optSeed.union(other).executeSelectMany())
    ...
    test('chained-compound-three-way-union', async () => {
        ...
        const result = sync(a.union(b).union(c).orderBy('label').executeSelectMany())
    ...
    test('chained-compound-union-then-union-all', async () => {
        ...
        const result = sync(a.union(b).unionAll(c).orderBy('label').executeSelectMany())
    ...
    test('compound-order-by-value-source-secondary', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-with-limit-and-offset', async () => {
        ...
        ctx.mockNext(expected)
        const result = sync(ctx.conn.selectFrom(tProject).select({ label: tProject.name })
            .union(ctx.conn.selectFrom(tIssue).select({ label: tIssue.title }))
            .orderBy('label')
            .limit(2)
            .offset(3)
    ...
    test('union-with-remaining-insensitive-order-by-modes', async () => {
        ...
        const r1 = sync(compound().orderBy('label', 'insensitive').executeSelectMany())
        ...
        const r2 = sync(compound().orderBy('label', 'desc insensitive').executeSelectMany())
        ...
        ctx.mockNext(ascOrder)
        const r3 = sync(compound().orderBy('label', 'asc nulls first insensitive').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by lower(label) asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(ascOrder)
        const r4 = sync(compound().orderBy('label', 'asc nulls last insensitive').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label is null, lower(label) asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(r4).toEqual(ascOrder)

        // 'desc nulls first insensitive'
        ctx.mockNext(descOrder)
        const r5 = sync(compound().orderBy('label', 'desc nulls first insensitive').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label is not null, lower(label) desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(r5).toEqual(descOrder)

        // 'desc nulls last insensitive'
        ctx.mockNext(descOrder)
        const r6 = sync(compound().orderBy('label', 'desc nulls last insensitive').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by lower(label) desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('compound-arm-parenthesized-by-its-own-limit', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .orderBy('label')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project where id = ? limit ?) union select title as label from issue where id = ? order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-arm-parenthesized-by-its-own-order-by-and-limit', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .orderBy('label')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from project where id = ? union all select * from (select title as label from issue where project_id = ? order by label limit ?) order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('compound-execute-select-none-or-one', async () => {
        ...
        const found = sync(ctx.conn.selectFrom(tProject)
            .where(tProject.id.equals(1))
            .select({ label: tProject.name })
            .union(
                ctx.conn.selectFrom(tIssue)
        ...
        const none = sync(ctx.conn.selectFrom(tProject)
            .where(tProject.id.equals(-1))
            .select({ label: tProject.name })
            .union(
                ctx.conn.selectFrom(tIssue)
    ...
    test('one-column-compound-executes-directly-to-scalar-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as result from project where id = ? union select slug as result from project where id = ? order by result"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-query-and-params-accessors', async () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"select name as label from project where id = ? union select title as label from issue where id = ? order by label"`)
        ...
        const expected = [{ label: 'Marketing site' }, { label: 'Update hero copy' }]
        ctx.mockNext(expected)
        const rows = sync(built.executeSelectMany())
    ...
    test('distinct-left-arm-survives-into-compound', async () => {
        ...
        const right = ctx.conn.selectFrom(tIssue)
            .where(tIssue.id.equals(1))
            .select({ label: tIssue.status })
        const result = sync(left.unionAll(right).orderBy('label').executeSelectMany())
    ...
    test('compound-order-by-value-source-with-explicit-mode', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, ? desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-insensitive-order-by-wrap-with-limit-offset', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .offset(2)
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by lower(label) asc limit ? offset ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('compound-value-source-order-by-wrap-with-limit', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .limit(2)
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, ? limit ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-wrap-execute-select-page', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by lower(label) asc limit ? offset ?"`)
    ...
    test('compound-arm-parenthesized-by-its-own-customize-before-order-by-items', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .orderBy('label')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project order by project.name asc) union select title as label from issue order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ label: string }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('compound-execute-select-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        try {
            result = sync(ctx.conn.selectFrom(tProject)
                .where(tProject.id.equals(1))
                .select({ label: tProject.name })
                .union(
                    ctx.conn.selectFrom(tProject)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from project where id = ? union select name as label from project where id = ? order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-unionAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        ctx.mockNext(expected)
        const result = sync(ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(1)).select({ iid: tIssue.id })
            .unionAll(ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(2)).select({ iid: tIssue.id }))
            .orderBy('iid')
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as iid from issue where id = ? union all select id as iid from issue where id = ?) as o_1_ order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-intersect-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        ctx.mockNext(expected)
        const result = sync(ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            .intersect(ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(1)).select({ iid: tIssue.id }))
            .orderBy('iid')
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as iid from issue where id in (?, ?) intersect select id as iid from issue where id = ?) as o_1_ order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-except-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as iid from issue where id in (?, ?) except select id as iid from issue where id = ?) as o_1_ order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-order-by-raw-fragment-embedding-multiple-value-sources', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, 0 + ? + ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-order-by-raw-fragment-embedding-value-source-in-expression', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, coalesce(?, 0)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
