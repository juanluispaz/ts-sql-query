    ...
    test('shaped-set-when-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = await ctx.conn.update(tProject)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = :0, slug = :1 where id = :2 returning id, name, slug into :3, :4, :5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-extend-shape-with-a-column-object-value-executes', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.updateAllowingNoWhere(tOrganization)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update "organization" set name = :0, "plan" = :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
