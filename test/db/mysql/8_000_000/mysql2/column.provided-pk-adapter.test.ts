...
describe(ctx.label, () => {
    ...
    // NOT-APPLICABLE: MySQL has no RETURNING, so reading the adapter-scaled PK back via returning() is unavailable.
    /*
    test('returning-reads-the-adapter-scaled-provided-primary-key-back-unscaled', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into invoice (invoice_no, total) values ($1, $2) returning invoice_no as "invoiceNo", total as total"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                400,
                900,
