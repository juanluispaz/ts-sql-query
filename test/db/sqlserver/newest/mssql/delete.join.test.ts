...
describe(ctx.label, () => {
    ...
    // Not applicable on SQL Server: `.innerJoin` / `.leftJoin` on DELETE are typed `never` for sqlserver (the `DELETE … USING` JOIN grammar in `OnExpressionFnType` is gated to `'mariaDB' | 'mySql'`); on SQL Server use `.using(table).where(...)` — see `delete.using.test.ts`.
    /*
