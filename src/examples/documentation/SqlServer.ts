...
async function main() {
    ...
    expectedQuery.push(`select id as id, name as name from company where lower(name) like lower('%' + @0 + '%')`)
    expectedParams.push(`["ACME"]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push([{
        id: 12,
    ...
    expectedQuery.push(`select customer.id as id, customer.first_name as firstName, customer.last_name as lastName, customer.birthday as birthday, company.id as [company.id], company.name as [company.name] from customer inner join company on company.id = customer.company_id where company.name = @0 and (lower(customer.first_name) like lower('%' + @1 + '%') or lower(customer.last_name) like lower('%' + @2 + '%')) order by lower([company.name]) asc, birthday desc`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name, favouriteCoustomer.first_name + @0 + favouriteCoustomer.last_name as [favouriteCustomer.name] from company left outer join customer as favouriteCoustomer on company.parent_id = favouriteCoustomer.id where (exists(select id as [result] from customer where lower(first_name + @1 + last_name) like lower('%' + @2 + '%')) = 1) and company.parent_id = @3`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name from company left outer join customer as favouriteCoustomer on company.parent_id = favouriteCoustomer.id where favouriteCoustomer.id in (select parent_id as [result] from company where lower(name) like lower('%' + @0 + '%')) and company.parent_id = @1`)
