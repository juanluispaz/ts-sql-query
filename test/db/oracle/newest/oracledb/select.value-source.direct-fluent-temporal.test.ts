...
describe(ctx.label, () => {
    ...
    test('view-customLocalTime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from cutoff_clock) as "h", extract(minute from cutoff_clock) as "m", trunc(extract(second from cutoff_clock)) as "s", to_number(to_char(cutoff_clock, 'FF3')) as "ms" from release_overview where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-source-asOptional-getter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from released_on) as "y" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-getter/customLocalDate-value-when-null-get-month-and-null-if-value-get-day', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(month from nvl(released_on, :0)) - 1 as "mo", mod(trunc(nullif(released_on, :1)) - trunc(nullif(released_on, :2), 'IW') + 1, 7) as "dow" from project_release where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('modifier-getter/customLocalTime-value-when-null-get-hours-and-null-if-value-get-seconds', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from nvl(cutoff_time, :0)) as "h", trunc(extract(second from nullif(cutoff_time, :1))) as "s" from project_release where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('modifier-getter/localDateTime-value-when-null-get-full-year-and-null-if-value-get-hours', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from nvl(created_at, :0)) as "y", extract(hour from nullif(created_at, :1)) as "h" from "organization" where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1970-01-01T00:00:00.000Z,
