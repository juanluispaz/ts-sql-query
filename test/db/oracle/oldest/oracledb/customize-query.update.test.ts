...
// tOrganization is used only by the update-from test commented out below.
void tOrganization
...
describe(ctx.label, () => {
    ...
    test('customize-update-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update /*+ hint */ issue set title = :0 where id = :1 returning id, title into :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched hero copy",
                1,
    ...
    //             .executeUpdate()
    //
    //         expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /*+ hint */ project set project.name = "organization".name from "organization" where project.organization_id = "organization".id and "organization".id = :0  /* tail */"`)
    //         expect(ctx.lastParams).toMatchInlineSnapshot(`
    //           [
    //             99999,
    //           ]
