...
describe(ctx.label, () => {
    ...
    test('union-all-of-aggregate-as-array-arms-merges-array-typed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name)))::text as projects from organization left join project on project.organization_id = organization.id where organization.id = $1 group by organization.id union all select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name)))::text as projects from organization left join project on project.organization_id = organization.id where organization.id = $2 group by organization.id order by "orgId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-of-depth-3-nested-object-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, number as "outer.mid.num", body as "outer.mid.body" from issue where id = $1 union select id as iid, number as "outer.mid.num", body as "outer.mid.body" from issue where id = $2 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-left-join-arms-keeps-the-nested-object-optional-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id as "proj.id", project.name as "proj.name", project.archived_at as "proj.archivedAt" from issue left join project on project.id = issue.project_id where issue.id = $1 union all select issue.id as iid, project.id as "proj.id", project.name as "proj.name", project.archived_at as "proj.archivedAt" from issue left join project on project.id = issue.project_id where issue.id = $2 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-rule-1-required-in-optional-object-arms-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, status as "meta.gate", assignee_id as "meta.assigneeId" from issue where id = $1 union all select id as iid, status as "meta.gate", assignee_id as "meta.assigneeId" from issue where id = $2 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-aggregate-as-array-arms-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as projects from organization left join project on project.organization_id = organization.id where organization.id = $1 group by organization.id union all select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as projects from organization left join project on project.organization_id = organization.id where organization.id = $2 group by organization.id order by "orgId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('intersect-of-rule-3-required-object-with-optional-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, title as "detail.title", body as "detail.body" from issue where id in ($1, $2) intersect select id as iid, title as "detail.title", body as "detail.body" from issue where id = $3 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('intersectAll-of-rule-3-required-object-with-optional-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, title as "detail.title", body as "detail.body" from issue where id in ($1, $2) intersect all select id as iid, title as "detail.title", body as "detail.body" from issue where id = $3 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('except-of-rule-3-required-object-with-optional-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, title as "detail.title", body as "detail.body" from issue where id in ($1, $2) except select id as iid, title as "detail.title", body as "detail.body" from issue where id = $3 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('exceptAll-of-rule-3-required-object-with-optional-leaf-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, title as "detail.title", body as "detail.body" from issue where id in ($1, $2) except all select id as iid, title as "detail.title", body as "detail.body" from issue where id = $3 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-nested-object-aggregate-arms-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as "wrap.projects" from organization left join project on project.organization_id = organization.id where organization.id = $1 group by organization.id union all select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as "wrap.projects" from organization left join project on project.organization_id = organization.id where organization.id = $2 group by organization.id order by "orgId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-aggregate-as-array-arms-projecting-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as "meta.projects" from organization left join project on project.organization_id = organization.id where organization.id = $1 group by organization.id union all select organization.id as "orgId", (json_agg(json_build_object('id', project.id, 'name', project.name, 'archivedAt', project.archived_at)))::text as "meta.projects" from organization left join project on project.organization_id = organization.id where organization.id = $2 group by organization.id order by "orgId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
