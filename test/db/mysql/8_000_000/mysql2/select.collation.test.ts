...
describe(ctx.label, () => {
    ...
    // collation (SQL Server, Oracle). MySQL's REPLACE ignores collation, so the
    ...
    */
    ...
    test('replaceAll on a uuid receiver converts before collate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(bin_to_uuid(external_ref), bin_to_uuid(external_ref), ?) as \`v\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "X",
            1,
    ...
    test('replaceAllInsensitive on a uuid receiver converts before collate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, regexp_replace(bin_to_uuid(external_ref), replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '.', '\\\\.'), '*', '\\\\*'), '+', '\\\\+'), '?', '\\\\?'), '^', '\\\\^'), '$', '\\\\$'), '{', '\\\\{'), '}', '\\\\}'), '(', '\\\\('), ')', '\\\\)'), '|', '\\\\|'), '[', '\\\\['), ']', '\\\\]'), ?) as \`v\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "X",
            1,
    ...
    test('replaceAll on a uuid receiver with a uuid replacement leaves the replacement bare', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(bin_to_uuid(external_ref), bin_to_uuid(external_ref), bin_to_uuid(external_ref)) as \`v\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('replaceAllInsensitive with a value-source replacement operand', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select regexp_replace(?, ?, replace(replace(?, '\\\\', '\\\\\\\\'), '$', '\\\\$')) as \`v\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ZabcZabc",
            "abc",
    ...
    test('replaceAllInsensitive with both operands value sources', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select regexp_replace(?, replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(?, '\\\\', '\\\\\\\\'), '.', '\\\\.'), '*', '\\\\*'), '+', '\\\\+'), '?', '\\\\?'), '^', '\\\\^'), '$', '\\\\$'), '{', '\\\\{'), '}', '\\\\}'), '(', '\\\\('), ')', '\\\\)'), '|', '\\\\|'), '[', '\\\\['), ']', '\\\\]'), replace(replace(?, '\\\\', '\\\\\\\\'), '$', '\\\\$')) as \`v\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ZabcZabc",
            "abc",
    ...
    test('replaceAllInsensitiveIfValue present-value arms with a value-source operand', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select regexp_replace(?, replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(?, '\\\\', '\\\\\\\\'), '.', '\\\\.'), '*', '\\\\*'), '+', '\\\\+'), '?', '\\\\?'), '^', '\\\\^'), '$', '\\\\$'), '{', '\\\\{'), '}', '\\\\}'), '(', '\\\\('), ')', '\\\\)'), '|', '\\\\|'), '[', '\\\\['), ']', '\\\\]'), ?) as vfind, regexp_replace(?, ?, replace(replace(?, '\\\\', '\\\\\\\\'), '$', '\\\\$')) as vrepl"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ZabcZabc",
            "abc",
    ...
    test('collate on a uuid receiver converts before collate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(external_ref) collate utf8mb4_bin as \`v\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('insensitive orderBy on a uuid alias converts before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(external_ref) as ref from issue where external_ref is not null order by ref collate utf8mb4_general_ci"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('insensitive comparison with a uuid value operand converts before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select ? = bin_to_uuid(external_ref) collate utf8mb4_general_ci as eq, ? <> bin_to_uuid(external_ref) collate utf8mb4_general_ci as ne, ? like bin_to_uuid(external_ref) collate utf8mb4_general_ci as lk, ? not like bin_to_uuid(external_ref) collate utf8mb4_general_ci as nl from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "x",
    ...
    test('insensitive affix predicates with a uuid value operand convert before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select ? like concat(replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') collate utf8mb4_general_ci as \`starts\`, ? not like concat(replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') collate utf8mb4_general_ci as notStarts, ? like concat('%', replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')) collate utf8mb4_general_ci as \`ends\`, ? not like concat('%', replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')) collate utf8mb4_general_ci as notEnds, ? like concat('%', replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') collate utf8mb4_general_ci as \`contains\`, ? not like concat('%', replace(replace(replace(bin_to_uuid(external_ref), '\\\\', '\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') collate utf8mb4_general_ci as notContains from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "x",
