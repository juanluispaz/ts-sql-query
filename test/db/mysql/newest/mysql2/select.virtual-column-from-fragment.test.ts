...
describe(ctx.label, () => {
    ...
    test('release-custom-uuid-virtual-required', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(signing_key) as \`v\` from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-uuid-virtual-required', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(signing_uuid) as \`v\` from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
