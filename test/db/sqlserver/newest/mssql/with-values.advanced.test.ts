...
describe(ctx.label, () => {
    ...
    test('values-aliased-via-as-keeps-original-with-name', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch as (select * from (values (@0, @1), (@2, @3)) as projectPatch(id, name)) select pp.[] as id, pp.[] as name from projectPatch as pp order by id"`)
    ...
    test('values-for-use-in-left-join-as-emits-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch as (select * from (values (@0, @1)) as projectPatch(id, name)) select project.id as pid, pp.[] as newName from project left join projectPatch as pp on pp.[] = project.id where project.id <= @2 order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
