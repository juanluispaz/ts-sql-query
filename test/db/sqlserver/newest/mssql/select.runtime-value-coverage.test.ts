...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-left-join-miss-inner-object-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('marker':project.slug, 'issue.id':issue.id, 'issue.title':issue.title)) as issues from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('aggregate-as-array-null-projector-single-leaf-inner-object-is-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('marker':project.slug, 'issue.id':issue.id)) as issues from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
