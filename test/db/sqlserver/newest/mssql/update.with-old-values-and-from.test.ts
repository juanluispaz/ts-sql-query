    ...
    */
    ...
    test('returning-old-value-with-from-then-left-join-drops-nested-object-on-join-miss', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, deleted.name as oldName, app_user.full_name as [obj.assignee] from issue left join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('returning-old-value-with-from-then-left-join-projects-nullable-assignee-on-miss', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, deleted.name as oldName, app_user.full_name as assignee from issue left join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
