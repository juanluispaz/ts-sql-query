...
describe(ctx.label, () => {
    ...
    test('no-value-boolean-collapses-inside-an-and-chain', async () => {
        ...
        const rows = sync(connection.selectFrom(tOrganization)
            .where(tOrganization.id.equals(1).and(connection.noValueBoolean()))
            .select({ id: tOrganization.id, name: tOrganization.name })
            .executeSelectMany())

        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from organization where id = ?"`)
    ...
    test('negated-neutral-marks-as-sole-where-emit-no-where-clause', async () => {
        ...
        ctx.mockNext(expectedMock)
        const viaNoValue = sync(connection.selectFrom(tOrganization)
        ...
        ctx.mockNext(expectedMock)
        const viaElidedIfValue = sync(connection.selectFrom(tOrganization)
