...
describe(ctx.label, () => {
    ...
    test('fragment-with-type-date-time-and-enum-arms', async () => {
        ...
        const c = ctx.conn
        const rows = sync(c.selectFrom(tIssueWorklog)
    ...
    test('fragment-with-type-double-custom-and-custom-local-arms', async () => {
        ...
        ctx.mockNext([expected])
        const c = ctx.conn
        const rows = sync(c.selectFrom(tProjectRelease)
    ...
    test('aggregate-fragment-with-type-scalar-arms', async () => {
        ...
        ctx.mockNext(expected)
        const c = ctx.conn
        const result = sync(c.selectFrom(tIssue)
            .select({
    ...
    test('aggregate-fragment-with-type-optional-temporal-arms', async () => {
        ...
        const plain = sync(c.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.id.equals(-1))
            .select({
        ...
        const dt = sync(c.selectFrom(tIssue)
            .where(tIssue.id.equals(-1))
        ...
        const custom = sync(c.selectFrom(tProjectRelease)
            .where(tProjectRelease.id.equals(-1))
            .select({
    ...
    test('aggregate-fragment-with-type-optional-custom-scalar-arms', async () => {
        ...
        const wl = sync(c.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.id.equals(-1))
            .select({
        ...
        const rel = sync(c.selectFrom(tProjectRelease)
            .where(tProjectRelease.id.equals(-1))
            .select({
    ...
    test('aggregate-fragment-with-type-enum-and-custom-arms', async () => {
        ...
        const c = ctx.conn
        const actRow = sync(c.selectFrom(tIssueWorklog)
        ...
        const chRow = sync(c.selectFrom(tProjectRelease)
            .select({
    ...
    test('fragment-with-type-custom-double-and-local-date-time-arms', async () => {
        ...
        const amtRow = sync(c.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.id.equals(1))
            .select({
        ...
        const soffRow = sync(c.selectFrom(tProjectRelease)
            .where(tProjectRelease.id.equals(1))
            .select({
    ...
    test('fragment-with-type-optional-arm-plain-local-date-time', async () => {
        ...
        ctx.mockNext([expected])
        const c = ctx.conn
        const rows = sync(c.selectFrom(tOrganization)
    ...
    test('fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const c = ctx.conn
        const row = sync(c.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.id.equals(1))
            .select({
    ...
    test('aggregate-fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const c = ctx.conn
        const row = sync(c.selectFrom(tIssue)
            .select({
    ...
    test('aggregate-fragment-with-type-boolean-optional-threads-a-trailing-type-adapter', async () => {
        ...
        const row = sync(c.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority) > 5 as any from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
