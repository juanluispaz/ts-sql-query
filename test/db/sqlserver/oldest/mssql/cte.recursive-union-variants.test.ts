...
//     })
    ...
    test('recursive-one-column-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) select id as id, (select concat('[', string_agg(convert(nvarchar, [result]), ','), ']') from recursive_select_1) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('recursive-result-execute-select-page', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as id, title as title from recursive_select_1 order by id offset 0 rows fetch next @3 rows only"`)
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select count(*) from recursive_select_1"`)
    ...
    // TODO[LIMITATION]: see LIMITATIONS.md — SQL Server rejects a bare bind parameter as an ORDER BY term (error 1008: it reads a lone variable/literal there as an ordinal column position). `orderBy(<no-table value source>)` renders the const as a bare `@param`, so `select … from recursive_select_1 order by id, @param` is rejected — the same way a plain `select … order by col, @p` is. Only the bare-parameter ORDER BY term is unsupported here; the recursive routing to the outer select is correct.
    /*
    ...
    */
    ...
    test('recursive-multi-column-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id = @0 union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as id, (select id as id, title as title from recursive_select_1 for json path) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('recursive-select-consumed-via-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with tree as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join tree on issue.parent_id = tree.id) select id as id, title as title from tree order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-customize-order-by-hooks-then-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id), tree as (select id as id, title as title from recursive_select_1 order by title asc, id asc offset 0 rows) select id as id, title as title from tree order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-customize-dual-rehoming-then-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (/* head */  select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id  /* tail */), tree as (select id as id, title as title from recursive_select_1 order by title asc, id offset 0 rows fetch next @3 rows only) select id as id, title as title from tree order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-result-order-by-limit-inline-scalar-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as [result] from issue where id = @0 union all select issue.id as [result] from issue join recursive_select_1 on issue.parent_id = recursive_select_1.[result]) select id as id, (select [result] as [result] from recursive_select_1 order by result offset 0 rows fetch next @1 rows only) as root from project where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('recursive-result-order-by-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id = @0 union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as id, (select id as id, title as title from recursive_select_1 order by id offset 0 rows for json path) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('recursive-result-unioned-with-plain-select-compound', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id = @0 union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as id, title as title from recursive_select_1 union select id as id, title as title from issue where id = @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-result-order-by-alone-then-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id), tree as (select id as id, title as title from recursive_select_1 order by id offset 0 rows) select id as id, title as title from tree order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-result-execute-select-page-with-whole-statement-customize', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as id, title as title from recursive_select_1 order by id offset 0 rows fetch next @3 rows only  /* tail */"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('recursive-result-execute-select-page-with-outer-projection-customize', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title from issue where id in (@0, @1, @2) union all select issue.id as id, issue.title as title from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select /* hint */ /* cols */  id as id, title as title from recursive_select_1 order by id offset 0 rows fetch next @3 rows only"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('recursive-union-all-fn-projecting-optionals-as-nullable-inline', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, title as title, parent_id as parentId from issue where id = @0 union all select issue.id as id, issue.title as title, issue.parent_id as parentId from issue join recursive_select_1 on recursive_select_1.parentId = issue.id) select id as id, (select id as id, title as title, parentId as parentId from recursive_select_1 for json path) as tree from project where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('recursive-union-all-re-projects-value-transforming-adapter-column-round-trip', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1 as (select id as id, score as score from project_review where id = @0 union all select project_review.id as id, project_review.score as score from project_review join recursive_select_1 on project_review.id = (recursive_select_1.id + @1)) select id as id, score as score from recursive_select_1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1000,
