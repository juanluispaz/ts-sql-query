...
describe(ctx.label, () => {
    ...
    test('update-allowing-no-where-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.updateAllowingNoWhere(tIssue)
                .set({ status: 'archived' })
                .executeUpdate())
    ...
    test('update-allowing-no-where-from-with-a-correlated-join', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = project.slug from project where project.id = issue.project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
            assertType<Exact<typeof affected, number>>()
            expect(affected).toBe(4)
        })
    ...
    test('update-allowing-no-where-from-then-left-join-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = ifnull(organization.name, project.slug) from project left join organization on organization.id = project.organization_id where project.id = issue.project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
            assertType<Exact<typeof affected, number>>()
    ...
    test('update-allowing-no-where-returning-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const rows = sync(ctx.conn.updateAllowingNoWhere(tIssue)
                .set({ status: 'archived' })
