...
describe(ctx.label, () => {
    ...
    test('shaped-conditional-set-family-skips-payload-keys-outside-the-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority, \`body\`, assignee_id, parent_id, estimated_hours, external_ref) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"`)
