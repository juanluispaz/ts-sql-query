...
describe(ctx.label, () => {
    ...
    test('multi-row-values-with-uniform-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into "organization" (name, "plan") values (:0, :1); insert into "organization" (name, "plan") values (:2, :3); end;"`)
    ...
    test('multi-row-values-with-some-rows-omitting-optional-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into "organization" (name, "plan") values (:0, :1); insert into "organization" (name, "plan", verified) values (:2, :3, case when (:4 = 1) then 'Y' else 'N' end); insert into "organization" (name, "plan") values (:5, :6); end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mixed-A",
                "free",
    ...
    */
    ...
    // NOT-APPLICABLE: Oracle has no INSERT…ON CONFLICT (uses MERGE); the bare `onConflictDoUpdateSet` is not typed on OracleConnection.
    /*
