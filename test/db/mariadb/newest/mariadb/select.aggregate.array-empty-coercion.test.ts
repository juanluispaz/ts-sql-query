...
describe(ctx.label, () => {
    ...
    test('aggregateAsArrayDistinct-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id, project.name as name, json_arrayagg(distinct json_object('status', issue.status, 'body', issue.\`body\`)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id, project.name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
