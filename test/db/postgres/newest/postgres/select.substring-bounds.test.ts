...
describe(ctx.label, () => {
    ...
    test('substring-bounds/a-negative-start-is-clamped-to-the-first-character', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, 1, id) as sub from issue where id = $1"`)
    ...
    test('substring-bounds/a-positive-start-shifts-the-position-and-shortens-the-length', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, $1, id - $2) as sub from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
