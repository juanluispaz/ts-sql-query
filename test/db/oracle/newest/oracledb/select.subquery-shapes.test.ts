    ...
    */
    ...
    test('compound-as-inline-query-value', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(id) as "result" from issue union select count(id) as "result" from issue) as "total" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('compound-as-derived-table-via-for-use-in-query-as', async () => {
        ...
        const rows = await ctx.conn.selectFrom(statuses)
            ...
            .orderBy('s')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with statuses("s") as (select status as "s" from issue where status = :0 union select status as "s" from issue where status = :1) select "s" as "s" from statuses order by "s""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    test('compound-as-inline-aggregated-array-value', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_arrayagg(a_1_.result) from (select status as result from issue where status = :0 union select status as result from issue where status = :1) a_1_) as "statuses" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
