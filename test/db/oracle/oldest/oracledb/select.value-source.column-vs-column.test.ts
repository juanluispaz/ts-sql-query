...
describe(ctx.label, () => {
    ...
    test('valueWhenNull-with-column-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl("body", title) as "b" from issue where id in (:0, :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('modulo-column-by-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", mod(priority, id) as "m" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('merge-optional-from-value-source-argument-projected', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority + assignee_id as "x", case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as "y", assignee_id + priority as "z" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('numeric-operators-column-rhs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority - id as "sub", priority / id as "div", power(priority, id) as "pw", log(id, priority) as "ln", round(priority, id) as "rnd", atan2(priority, id) as "at2", least(priority, "number") as "mx" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('greaterOrEqual-column-vs-column-optional-propagation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when priority >= assignee_id then 1 when not (priority >= assignee_id) then 0 else null end as "x" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-and-isnot-over-two-optional-columns-project-required-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when decode(assignee_id, parent_id, 1, 0 ) = 1 then 1 when not (decode(assignee_id, parent_id, 1, 0 ) = 1) then 0 else null end as "eqIs", case when decode(assignee_id, parent_id, 1, 0 ) = 0 then 1 when not (decode(assignee_id, parent_id, 1, 0 ) = 0) then 0 else null end as "neqIs" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('lessOrEqual-string-column-rhs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when status <= status then 1 else 0 end as "le" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
