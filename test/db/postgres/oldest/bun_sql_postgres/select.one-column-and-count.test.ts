...
describe(ctx.label, () => {
    ...
    test('select-one-column-execute-one-returns-scalar', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as result from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('select-one-column-optional-present-row-null-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select body as result from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('select-count-all-with-where-narrows-the-count', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as result from issue where status = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('select-one-column-of-expression-still-routes-via-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority + $1 as result from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
          ]
    ...
    test('select-multi-column-execute-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, status as status from issue where project_id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('select-multi-column-execute-none-or-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        // object returns without a throw. The SQL is captured (before the throw
        // fires) in both modes.
        ctx.mockNext({ id: 1, status: 'open' })
        let caught: unknown
    ...
    test('select-one-column-execute-page-returns-scalar-array-and-count', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select priority as result from issue order by result limit $1 offset $2"`)
    ...
    test('select-one-column-optional-execute-many-null-elements', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select body as result from issue order by result asc nulls last"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('select-one-column-query-and-params-accessors', async () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"select status as result from issue where id = $1"`)
