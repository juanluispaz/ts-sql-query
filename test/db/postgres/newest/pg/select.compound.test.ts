...
describe(ctx.label, () => {
    ...
    test('compound-order-by-value-source-collate-secondary', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .union(ctx.conn.selectFrom(tIssue).select({ label: tIssue.title }))
            .orderBy('label')
            .orderBy(ctx.conn.const('a', 'string').collate('C'))
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as label from project union select title as label from issue) as o_1_ order by label, $1 collate "C""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
          ]
    ...
    test('compound-order-by-multiple-aliases-inline', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, id as iid from issue where project_id = $1 union select project_id as pid, id as iid from issue where project_id = $2 order by pid, iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
