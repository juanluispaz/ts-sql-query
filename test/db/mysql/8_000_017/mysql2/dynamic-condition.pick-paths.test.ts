...
describe(ctx.label, () => {
    ...
    test('dynamic-pick-paths-walks-into-nested-availableFields', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority as \`meta.priority\` from issue where project_id = ? order by id"`)
    ...
    test('dynamic-pick-paths-nested-only-drops-top-level-siblings', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`status\` as \`meta.status\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
