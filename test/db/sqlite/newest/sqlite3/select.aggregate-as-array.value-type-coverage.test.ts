...
describe(ctx.label, () => {
    ...
    test('aggregate-of-bigint-column-as-array', async () => {
        // Exercises the `bigint` value type in the JSON aggregate: the library reads
        // the aggregate as a raw JSON string and parses it WITHOUT rounding, so a
        // bigint's digits survive the round-trip. `tIssue.viewCount` is `bigint`.
        //
        // This cell seeds values WITHIN the safe-integer range. The deprecated
        // `sqlite3` driver has no exact-integer API and cannot bind a BigInt param:
        // it coerces it to Number, rounding past 2^53 on the WRITE — so a >2^53 value
        // never reaches the column intact here. That is a driver limitation, not the
        // library's (the lossless read-side parse is the same on every connector).
        //
