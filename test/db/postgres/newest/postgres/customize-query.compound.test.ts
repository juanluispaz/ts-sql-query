...
describe(ctx.label, () => {
    ...
    test('customize-compound-hook-interpolates-bound-param', async () => {
        ...
        // tests-audit-disable-next-line mock-only -- hook param lands inside a /* */ comment; pg strips the comment then rejects the unused $1 (DESIGN §1 #18)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"/* tenant=$1 */  select name as label from project where archived_at is null union select title as label from issue where status = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            42,
            "open",
