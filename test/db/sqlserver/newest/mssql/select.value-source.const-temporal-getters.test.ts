...
describe(ctx.label, () => {
    ...
    test('const-localdatetime-pre-epoch-gettime', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select datediff_big(millisecond, '1970-01-01 00:00:00', @0) as pre, datediff_big(millisecond, '1970-01-01 00:00:00', @1) as eve, datediff_big(millisecond, '1970-01-01 00:00:00', @2) as sub, datediff_big(millisecond, '1970-01-01 00:00:00', @3) as far"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
