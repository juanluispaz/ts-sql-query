...
// projectSlug: 'slug', archived: 'archivedAt' }. MariaDB uses the BARE shaped
// form (`.onConflictDoUpdateDynamicSet(...)` / `.onConflictDoUpdateSet(...)`)
// because `.onConflictOn(...)` is not typed on `MariaDBConnection` (the
// `ON DUPLICATE KEY UPDATE` grammar takes no conflict target). Seed (org 1,
// slug 'mktg-site') already exists, so every conflict fires; an
// `ON DUPLICATE KEY UPDATE` that updates an existing row counts as 2 affected
// rows (1 attempted insert + 1 update) — the documented engine semantics.
// `ctx.withRollback` reverts the row between tests.
...
describe(ctx.label, () => {
    ...
    test('shaped-set-if-set-if-value-keeps-staged-slug-when-new-value-missing-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .shapedAs({ orgId: 'organizationId', projectName: 'name', projectSlug: 'slug', archived: 'archivedAt' })
                .set({ orgId: 1, projectName: 'ignored', projectSlug: 'mktg-site' })
                .onConflictDoUpdateDynamicSet({ projectName: 'Reactivated', projectSlug: 'mktg-site-keep' })
                .setIfSetIfValue({ projectName: 'Renamed', projectSlug: undefined })
                .executeInsert()

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, \`name\`, slug) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?, project.slug = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
    ...
    test('shaped-ignore-if-set-drops-a-staged-renamed-key-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .shapedAs({ orgId: 'organizationId', projectName: 'name', projectSlug: 'slug', archived: 'archivedAt' })
                .set({ orgId: 1, projectName: 'ignored', projectSlug: 'mktg-site' })
                .onConflictDoUpdateDynamicSet({ projectName: 'Reactivated', projectSlug: 'mktg-site-v2' })
    ...
    test('shaped-ignore-if-has-no-value-prunes-a-null-renamed-key-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .shapedAs({ orgId: 'organizationId', projectName: 'name', projectSlug: 'slug', archived: 'archivedAt' })
                .set({ orgId: 1, projectName: 'ignored', projectSlug: 'mktg-site' })
                .onConflictDoUpdateDynamicSet({ projectName: 'Reactivated', archived: null })
    ...
    test('shaped-extend-shape-then-set-adds-a-renamed-key-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, \`name\`, slug) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?, project.archived_at = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
    ...
    // NOT-APPLICABLE: MariaDB's `ON DUPLICATE KEY UPDATE` has no WHERE clause, so `dynamicWhere()` / `where(...)` are not typed on the shaped conflict node off `MariaDBConnection`; the targeted `onConflictOn(...).doUpdateSet(...)` opener is not typed either. See this dialect's `types.negative` suite.
    /*
    ...
    test('shaped-empty-on-conflict-update-set-degrades-to-conflict-noop', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .shapedAs({ orgId: 'organizationId', projectName: 'name', projectSlug: 'slug', archived: 'archivedAt' })
                .set({ orgId: 1, projectName: 'ignored', projectSlug: 'mktg-site' })
                .onConflictDoUpdateDynamicSet({ archived: null })
                .ignoreAnySetWithNoValue()
                .executeInsert()
