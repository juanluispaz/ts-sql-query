...
describe(ctx.label, () => {
    ...
    test('delete-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, deleted.title + @0 as tag where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                " [gone]",
                3,
    ...
    test('delete-returning-nested-object-projection', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, deleted.title as [meta.title], deleted.priority as [meta.priority] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
              ]
    ...
    test('delete-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release output deleted.channel as [result] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-release-draft-returning-one-column-optional-branded', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from release_draft output deleted.stage as [result] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-release-draft-returning-object-optional-branded-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from release_draft output deleted.id as id, deleted.stage as stage where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from release_draft output deleted.id as id, deleted.title as [meta.sibling], deleted.stage as [meta.gate] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.body as [result] where project_id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, deleted.body as [meta.body] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProjectReview)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review output deleted.review_date as reviewDate, deleted.review_time as reviewTime where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProjectRelease)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release output deleted.released_on as releasedOn, deleted.cutoff_time as cutoffTime, deleted.published_at as publishedAt where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-localdatetime-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProject)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project output deleted.archived_at as archivedAt where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('delete-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = await ctx.conn.deleteFrom(tProjectReview)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review output deleted.review_time as [result] where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
