...
describe(ctx.label, () => {
    ...
    test('numeric custom boolean adapter writes as case-when-then-trueValue', async () => {
        ...
        ctx.mockNext(1)
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.update(tIssueWorklog)
    ...
    test('equals-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const fired = [{ id: 1 }]
        ctx.mockNext(fired)
        const firedRows = sync(ctx.conn.selectFrom(tOrganization)
        ...
        ctx.mockNext(elided)
        const elidedRows = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('equals-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const fired = [{ id: 1 }]
        ctx.mockNext(fired)
        const firedRows = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(elided)
        const elidedRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('is-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        ctx.mockNext(fired)
        const firedRows = sync(ctx.conn.selectFrom(tProject)
        ...
        ctx.mockNext(elided)
        const elidedRows = sync(ctx.conn.selectFrom(tProject)
