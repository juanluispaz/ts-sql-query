...
describe(ctx.label, () => {
    ...
    test('query-called-twice-returns-the-cached-sql-without-re-binding-params', () => {
        ...
        expect(first).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)"`)
    ...
    test('params-before-query-builds-on-demand', () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)"`)
    })
    ...
    test('rawfragment-embeds-all-four-insert-forms', () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"select /* audit: insert into issue (project_id, "number", title, status, priority) select project_id as "projectId", "number" as "number", title as "title", status as "status", priority as "priority" from issue where status = :0 | begin insert into issue (project_id, "number", title, status, priority) values (:1, :2, :3, :4, :5); insert into issue (project_id, "number", title, status, priority) values (:6, :7, :8, :9, :10); end; | insert into ledger_entry (entry_no, amount, memo, discount) values (default, default, default, default) | insert into issue (project_id, "number", title, status, priority) values (:11, :12, :13, :14, :15) */ id as "id" from project where id = :16"`)
        expect(built.params()).toMatchInlineSnapshot(`
          [
            "open",
            1,
