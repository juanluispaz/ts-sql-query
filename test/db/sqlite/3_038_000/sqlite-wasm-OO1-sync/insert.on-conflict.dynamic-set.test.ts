    ...
    */
    ...
    test('empty-on-conflict-update-set-degrades-to-conflict-noop-preserving-returning-id', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do nothing returning id as id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('empty-on-conflict-update-set-degrades-to-conflict-noop-preserving-returning-object-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do nothing returning id as id, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
...
// NOT-APPLICABLE: SQLite has no ON CONFLICT ON CONSTRAINT
    ...
    */
    ...
    test('conflict-do-nothing-returning-last-inserted-id-on-colliding-row-resolves-null', async () => {
        ...
        await ctx.withRollback(async () => {
            const id = sync(ctx.conn.insertInto(tProject)
                ...
                .returningLastInsertedId()
                .executeInsert())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict do nothing returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('empty-on-conflict-update-set-degrades-preserving-returning-one-column-many', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const ids = sync(ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'ignored' })
                .onConflictOn(tProject.organizationId, tProject.slug)
                .doUpdateDynamicSet({ archivedAt: null })
                .ignoreAnySetWithNoValue()
