...
describe(ctx.label, () => {
    ...
    test('insert-optional-custom-columns-set-to-values', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title, stage, channel, min_version) values (?, ?, ?, ?, ?)"`)
            ...
            const row = sync(ctx.conn.selectFrom(tReleaseDraft)
                ...
                .executeSelectOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, stage as stage, channel as channel, min_version as minVersion from release_draft where id = ?"`)
    ...
    test('insert-optional-custom-columns-omitted', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title) values (?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                12,
                "Omitted draft",
