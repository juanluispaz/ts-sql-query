...
describe(ctx.label, () => {
    ...
    test('execute-delete-with-min-max-passes-when-count-in-range', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.deleteFrom(tIssue)
    ...
    test('execute-delete-none-or-one-with-returning-one-column', async () => {
        ...
        ctx.mockNext('open')
        await ctx.withRollback(async () => {
            const result = sync(ctx.conn.deleteFrom(tIssue)
    ...
    test('execute-delete-many-with-min-max-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            const rows = sync(ctx.conn.deleteFrom(tIssue)
                .where(tIssue.projectId.equals(1))
                .returning({ id: tIssue.id, status: tIssue.status })
                .executeDeleteMany(1, 5))

            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where project_id = ? returning id as id, status as status"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('execute-delete-one-column-many-non-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where project_id = ? returning status as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('execute-delete-returning-row-shape-execute-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            try {
                result = sync(ctx.conn.deleteFrom(tIssue)
                    .where(tIssue.projectId.equals(1))
                    .returning({ id: tIssue.id, status: tIssue.status })
                    .executeDeleteOne())
            } catch (e) {
    ...
    test('delete-query-called-twice-returns-the-cached-sql-without-re-binding-params', () => {
        ...
        expect(first).toMatchInlineSnapshot(`"delete from issue where id in (?, ?)"`)
    ...
    test('delete-params-before-query-builds-on-demand', () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"delete from issue where id = ?"`)
