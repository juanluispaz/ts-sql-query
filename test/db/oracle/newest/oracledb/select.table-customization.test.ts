...
describe(ctx.label, () => {
    ...
    test('table-customization: parameterized customization threads a runtime param into the raw fragment', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as "id" from (select * from "organization" where :0 >= 0) "o" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('table-customization: parameterized customization as an INNER JOIN target threads its bound param', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "projectId", "o".name as "orgName" from project inner join (select * from "organization" where :0 >= 0) "o" on "o".id = project.organization_id where project.organization_id = :1 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            1,
    ...
    test('table-customization: withSqlHint applied to a VIEW emits the hint on the view name', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vProjCustom)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "v".id as "id", "v".name as "name" from /*+ hint */ project_overview "v" where "v".id = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: two-param customization threads two bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as "id" from (select * from "organization" where :0 >= 0 and :1 >= 0) "o" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('table-customization: three-param customization threads three bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as "id" from (select * from "organization" where :0 >= 0 and :1 >= 0 and :2 >= 0) "o" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('table-customization: four-param customization threads four bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as "id" from (select * from "organization" where :0 >= 0 and :1 >= 0 and :2 >= 0 and :3 >= 0) "o" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('table-customization: five-param customization threads five bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as "id" from (select * from "organization" where :0 >= 0 and :1 >= 0 and :2 >= 0 and :3 >= 0 and :4 >= 0) "o" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
