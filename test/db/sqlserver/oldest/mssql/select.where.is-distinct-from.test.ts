...
describe(ctx.label, () => {
    ...
    test('is-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is null"`)
    ...
    test('is-with-value', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when assignee_id = @0 then 1 else 0 end = 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('is-not-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is not null"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('is-not-with-value', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when assignee_id = @0 then 1 else 0 end = 1)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('is-with-column-comparison', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (assignee_id = parent_id) or (assignee_id is null and parent_id is null) then 1 else 0 end = 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('is-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where @0 is null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "See ADR-014",
          ]
    ...
    test('is-not-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where @0 is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "See ADR-014",
          ]
