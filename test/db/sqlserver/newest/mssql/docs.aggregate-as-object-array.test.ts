...
describe(ctx.label, () => {
    ...
    test('docs:aggregate-as-object-array/inline-aggregated-of-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select json_arrayagg(a_1_.[result] null on null) from (select name as [result] from project where organization_id = organization.id order by [result] offset 0 rows) as a_1_) as projectNames from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
