...
describe(ctx.label, () => {
    ...
    test('values in a compound-union arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch as (select * from (values (@0, @1), (@2, @3)) as projectPatch(id, name)) select id as id, name as name from projectPatch union select id as id, name as name from project where id = @4 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            "from-values-a",
    ...
    test('values-as-intersect-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1), (@2)) as idList(id)) select id as id from idList intersect select id as id from project order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-except-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1), (@2)) as idList(id)) select id as id from idList except select id as id from project order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-union-all-arm-preserves-duplicates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1)) as idList(id)) select id as id from idList union all select id as id from project where id = @2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-non-seed-trailing-union-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1)) as idList(id)) select id as id from project where id in (@2, @3) union select id as id from idList order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            200,
    ...
    test('values-source-with-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1), (@2)) as idList(id)) select id as id from idList order by id offset 0 rows fetch next @3 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-with-limit-and-offset', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1), (@2)) as idList(id)) select id as id from idList order by id offset @3 rows fetch next @4 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-count-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList as (select * from (values (@0), (@1), (@2)) as idList(id)) select count(id) as total from idList"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-group-by', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with groupRows as (select * from (values (@0, @1), (@2, @3), (@4, @5)) as groupRows([n], grp)) select grp as grp, count([n]) as total from groupRows group by grp order by grp"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-sourced-scalar-inline-equals-operand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPick as (select * from (values (@0, @1), (@2, @3)) as projectPick(projectId, tag)) select id as id, name as name from project where id = (select projectId as [result] from projectPick where tag = @4) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "primary",
    ...
    test('values-self-joined-to-its-own-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode as (select * from (values (@0, @1, @2), (@3, @4, @5)) as treeNode(id, parentId, name)) select treeNode.id as id, anc.name as ancName from treeNode inner join treeNode as anc on anc.id = treeNode.parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
