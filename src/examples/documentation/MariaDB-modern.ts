...
async function main() {
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push("select company.id as id, company.name as name, concat(favouriteCoustomer.first_name, ?, favouriteCoustomer.last_name) as `favouriteCustomer.name` from company left outer join customer as favouriteCoustomer on company.parent_id = favouriteCoustomer.id where exists(select id as result from customer where lower(concat(first_name, ?, last_name)) like concat('%', lower(?), '%')) and company.parent_id = ?")
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name from company left outer join customer as favouriteCoustomer on company.parent_id = favouriteCoustomer.id where favouriteCoustomer.id in (select parent_id as result from company where lower(name) like concat('%', lower(?), '%')) and company.parent_id = ?`)
