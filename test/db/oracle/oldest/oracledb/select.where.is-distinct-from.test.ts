...
describe(ctx.label, () => {
    ...
    test('is-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(assignee_id, :0, 1, 0 ) = 1"`)
    ...
    test('is-not-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(assignee_id, :0, 1, 0 ) = 0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-with-column-comparison', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(assignee_id, parent_id, 1, 0 ) = 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('is-with-null-const-receiver-and-column-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(:0, "body", 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-not-with-null-const-receiver-and-column-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(:0, "body", 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(:0, :1, 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            "See ADR-014",
    ...
    test('is-not-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(:0, :1, 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            "See ADR-014",
    ...
    test('is-between-two-columns-sharing-a-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('userId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".id as "organizationId", app_user.id as "userId" from "organization" inner join app_user on decode(app_user.verified, "organization".verified, 1, 0 ) = 1 order by "organizationId", "userId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('is-not-between-two-columns-sharing-a-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('userId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".id as "organizationId", app_user.id as "userId" from "organization" inner join app_user on decode(app_user.verified, "organization".verified, 1, 0 ) = 0 order by "organizationId", "userId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ organizationId: number, userId: number }>>>()
        expect(rows).toEqual(expected)
    })
