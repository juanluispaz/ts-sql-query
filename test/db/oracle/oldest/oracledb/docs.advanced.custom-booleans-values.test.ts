...
describe(ctx.label, () => {
    ...
    test('docs:custom-booleans-values/insert-maps-true-false-to-Y-N', async () => {
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into "organization" (name, "plan", verified) values (:0, :1, case when (:2 = 1) then 'Y' else 'N' end) returning id into :3"`)
    ...
    test('docs:custom-booleans-values/select-maps-Y-N-to-true-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", case when verified = 'Y' then 1 else 0 end as "verified" from "organization" where (verified = 'Y')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
