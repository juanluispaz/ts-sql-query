...
describe(ctx.label, () => {
    ...
    test('docs:dynamic-queries/with-equals-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title, status as status from issue where status = $1 order by id"`)
    ...
    test('docs:dynamic-queries/builder-style', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where status = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('docs-extra:dynamic-queries/ignore-when-as-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title, null::text as body from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
