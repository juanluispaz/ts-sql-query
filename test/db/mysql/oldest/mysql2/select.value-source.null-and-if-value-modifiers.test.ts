...
describe(ctx.label, () => {
    ...
    test('valueWhenNull-int-receiver-widens-to-a-fractional-double-fallback', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ifnull(assignee_id, (priority * 1.0e0) / ?) as \`w\` from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('modifier-trio-on-plain-double-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (priority * 1.0e0) as req, null as own, null as ign from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
