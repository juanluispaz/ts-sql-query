...
describe(ctx.label, () => {
    ...
    test('as-alias-recurses-through-a-required-nested-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with proj_cte as (select id as pid, id as "info.id", name as "info.name" from project where archived_at is null) select pc.pid as "pid", pc."info.id" as "info.id", pc."info.name" as "info.name" from proj_cte pc order by "pid""`)
    ...
    test('for-use-in-left-join-as-recurses-through-the-nested-object-and-makes-it-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with proj_cte as (select id as pid, id as "info.id", name as "info.name" from project) select issue.id as "iid", pc."info.id" as "proj.id", pc."info.name" as "proj.name" from issue left join proj_cte pc on pc.pid = issue.project_id order by "iid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            iid:   number
