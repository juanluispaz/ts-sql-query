...
describe(ctx.label, () => {
    ...
    test('insert-from-select-returning-full-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) output inserted.id as id, inserted.project_id as projectId, inserted.number as number, inserted.title as title, inserted.status as status, inserted.priority as priority select @0 as projectId, number as number, title as title, @1 as status, priority as priority from issue where project_id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                "draft",
    ...
    test('insert-from-select-source-with-cte', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with active_projects as (select id as id from project where archived_at is null) insert into issue (project_id, number, title, status, priority) select issue.project_id as projectId, issue.number + @0 as number, issue.title as title, @1 as status, issue.priority as priority from issue inner join active_projects on active_projects.id = issue.project_id where issue.project_id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1000,
                "draft",
    ...
    */
    ...
    test('insert-from-select-with-on-conflict-on-columns-do-nothing', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) select organization_id as organizationId, slug as slug, name as name from project where organization_id = @0 on conflict (organization_id, slug) do nothing"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('insert-from-select-on-conflict-returning-last-id-and-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ids = await ctx.conn.insertInto(tProject)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) output inserted.id select organization_id as organizationId, slug as slug, name as name from project where organization_id = @0 on conflict do nothing"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
            ...
            const oneCol = await ctx.conn.insertInto(tProject)
                ...
                .executeInsertMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) output inserted.id as [result] select organization_id as organizationId, slug as slug, name as name from project where organization_id = @0 on conflict do nothing"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    */
    ...
    test('insert-from-select-on-conflict-returning-object-none-or-one-and-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) output inserted.id as id, inserted.name as name select organization_id as organizationId, slug as slug, name as name from project where organization_id = @0 on conflict do nothing"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    */
    ...
    // NOT-APPLICABLE: SQL Server has no INSERT … ON CONFLICT (uses MERGE), so onConflictDoUpdateSet is not typed on the from-select insert
    /*
    ...
    */
    ...
    // NOT-APPLICABLE: This dialect does not support the bare (no-target) ON CONFLICT DO UPDATE upsert on a from-select insert, so onConflictDoUpdate* is typed never here.
    /*
