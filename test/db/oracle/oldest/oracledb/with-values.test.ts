...
describe(ctx.label, () => {
    ...
    test('values in select-from', async () => {
        ...
        await ctx.conn.selectFrom(patch)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", name as "name" from projectPatch"`)
    ...
    test('values in update-from', async () => {
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual) update project set project.name = projectPatch.name from projectPatch where project.id = projectPatch.id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "renamed",
    ...
    test('values in a compound-union arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", name as "name" from projectPatch union select id as "id", name as "name" from project where id = :4 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            "from-values-a",
    ...
    test('values-as-intersect-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual union all select :2 from dual) select id as "id" from idList intersect select id as "id" from project order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-except-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual union all select :2 from dual) select id as "id" from idList minus select id as "id" from project order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-union-all-arm-preserves-duplicates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual) select id as "id" from idList union all select id as "id" from project where id = :2 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-non-seed-trailing-union-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual) select id as "id" from project where id in (:2, :3) union select id as "id" from idList order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            200,
    ...
    test('values-source-with-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual union all select :2 from dual) select id as "id" from idList order by "id" fetch next :3 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-with-limit-and-offset', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual union all select :2 from dual) select id as "id" from idList order by "id" offset :3 rows fetch next :4 rows only"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-count-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual union all select :1 from dual union all select :2 from dual) select count(id) as "total" from idList"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-group-by', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with groupRows("n", grp) as (select :0, :1 from dual union all select :2, :3 from dual union all select :4, :5 from dual) select grp as "grp", count("n") as "total" from groupRows group by grp order by "grp""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-sourced-scalar-inline-equals-operand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPick(projectId, tag) as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", name as "name" from project where id = (select projectId as "result" from projectPick where tag = :4) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "primary",
    ...
    test('values-self-joined-to-its-own-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (select :0, :1, :2 from dual union all select :3, :4, :5 from dual) select treeNode.id as "id", anc.name as "ancName" from treeNode inner join treeNode anc on anc.id = treeNode.parentId order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-self-joined-via-left-join-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (select :0, :1, :2 from dual union all select :3, :4, :5 from dual) select treeNode.id as "id", anc.name as "ancName" from treeNode left join treeNode anc on anc.id = treeNode.parentId order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-self-joined-with-two-aliased-clones-neither-is-the-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (select :0, :1, :2 from dual union all select :3, :4, :5 from dual) select "a".id as "id", "b".name as "ancName" from treeNode "a" inner join treeNode "b" on "b".id = "a".parentId order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-in-both-arms-of-a-compound-hoists-two-distinct-withs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual union all select :2, :3 from dual), idList(id) as (select :4 from dual union all select :5 from dual) select id as "id" from projectPatch union all select id as "id" from idList order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-in-a-non-correlated-exists-hoists-the-with', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (select :0 from dual) select id as "id" from project where exists(select id as "result" from idList) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
