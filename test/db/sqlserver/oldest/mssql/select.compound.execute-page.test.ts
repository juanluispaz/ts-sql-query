...
describe(ctx.label, () => {
    ...
    test('union-all-execute-select-page-over-a-nested-object-wraps-dot-columns-in-count-cte', async () => {
        ...
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (select id as iid, title as [detail.title], number as [detail.num] from issue where id = @0 union all select id as iid, title as [detail.title], number as [detail.num] from issue where id = @1) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
