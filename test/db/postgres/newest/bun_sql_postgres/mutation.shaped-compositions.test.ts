    ...
    test('shaped-insert-customize-query', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await connection.insertInto(tProject)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* shaped-insert */  insert into project (organization_id, name, slug) values ($1, $2, $3)"`)
    ...
    test('shaped-update-customize-query', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await connection.update(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* shaped-update */  update project set name = $1 where id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-update-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = await ctx.conn.update(tProject)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2 returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-update-allowing-no-where-extend-shape-then-no-arg-dynamic-set-opener', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.updateAllowingNoWhere(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1, organization_id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "No-where widened",
                1,
