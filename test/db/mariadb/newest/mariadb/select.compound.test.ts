...
describe(ctx.label, () => {
    ...
    test('compound-wrap-execute-select-page', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (select name as label from project union select title as label from issue order by lower(label) asc) select count(*) from result_for_count"`)
