...
describe(ctx.label, () => {
    ...
    test('inner-join-view-keeps-columns-required', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.name as "projectName", release_overview.version as version from project inner join release_overview on release_overview.project_id = project.id where project.id = $1 order by version"`)
    ...
    test('inner-join-values-keeps-columns-required', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectCode(projectId, code) as (values ($1::int4, $2::text)) select project.name as name, projectCode.code as code from project inner join projectCode on projectCode.projectId = project.id order by name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "MKTG",
    ...
    test('values-as-inline-query-value-hoists-with-to-outer', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectCode(projectId, code) as (values ($1::int4, $2::text)) select (select code as result from projectCode where projectId = $3) as "pickedCode""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "MKTG",
    ...
    test('view-as-inline-query-value-hoists-source-to-outer', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select version as result from release_overview where id = $1) as "firstVersion""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
