...
import type { AutogeneratedIdColumnKeys, ColumnKeys, IdColumnKeys, ProvidedIdColumnKeys, WritableColumnKeys } from './types.js'
...
export function extractIdColumnsFrom<O extends object, EXCLUDE extends ColumnKeys<O> = never>(obj: O, exclude?: EXCLUDE[]): { [K in Exclude<IdColumnKeys<O>, EXCLUDE>]: O[K] } {
    ...
    for (let key in obj) {
        ...
        if (isColumn(value)) {
            const columnPrivate = __getColumnPrivate(value)
            if (columnPrivate.__writable && columnPrivate.__isPrimaryKey) {
                result[key] = value
            }
        }
    }
...
export function extractAutogeneratedIdColumnsFrom<O extends object, EXCLUDE extends ColumnKeys<O> = never>(obj: O, exclude?: EXCLUDE[]): { [K in Exclude<AutogeneratedIdColumnKeys<O>, EXCLUDE>]: O[K] } {
    ...
    for (let key in obj) {
        ...
        if (isColumn(value)) {
            const columnPrivate = __getColumnPrivate(value)
            if (columnPrivate.__writable && columnPrivate.__isAutogeneratedPrimaryKey) {
                result[key] = value
            }
        }
    }
...
export function extractProvidedIdColumnsFrom<O extends object, EXCLUDE extends ColumnKeys<O> = never>(obj: O, exclude?: EXCLUDE[]): { [K in Exclude<ProvidedIdColumnKeys<O>, EXCLUDE>]: O[K] } {
    ...
    for (let key in obj) {
        ...
        if (isColumn(value)) {
            const columnPrivate = __getColumnPrivate(value)
            if (columnPrivate.__writable && columnPrivate.__isPrimaryKey && !columnPrivate.__isAutogeneratedPrimaryKey) {
                result[key] = value
            }
        }
    }
...
export function extractProvidedIdColumnNamesFrom<O extends object, EXCLUDE extends ColumnKeys<O> = never>(obj: O, exclude?: EXCLUDE[]): Exclude<ProvidedIdColumnKeys<O>, EXCLUDE>[] {
    if (!obj) {
        return []
    }
    const ignore: any = {}
