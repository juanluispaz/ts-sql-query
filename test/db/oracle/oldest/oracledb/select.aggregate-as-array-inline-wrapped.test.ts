...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-use-empty-array-for-no-value-explicit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", (select json_arrayagg(json_object('id' value a_1_.id, 'name' value a_1_.name)) from (select id as id, name as name from project where organization_id = "organization".id order by id offset 0 rows) a_1_) as "projects" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('id' value a_1_.id, 'title' value a_1_.title)) from (select id as id, title as title from issue where project_id = project.id order by id offset 0 rows) a_1_) as "meta.issues" from project where organization_id = :0 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-aggregate-order-by-asc-nulls-last', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result asc nulls last offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result desc nulls first offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) asc offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc-nulls-last-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) asc nulls last offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-one-column-keeps-the-branded-element-type', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(channel) from project_release where project_id = project.id) as "channels" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-compound-union-with-customize-query-survives-in-derived-table', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", (select json_arrayagg(json_object('id' value a_1_.id, 'name' value a_1_.name)) from (/* agg-head */  select id as id, name as name from project where organization_id = "organization".id and archived_at is null union select id as id, name as name from project where organization_id = "organization".id and archived_at is not null  /* agg-tail */) a_1_) as "projects" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-distinct-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select distinct status as result from issue where project_id = project.id) a_1_) as "statuses" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-distinct-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(json_object('status' value a_1_.status, 'priority' value a_1_.priority)) from (select distinct status as status, priority as priority from issue where project_id = project.id) a_1_) as "issueKinds" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-with-group-by-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", (select json_arrayagg(json_object('assigneeId' value a_1_.assigneeId, 'count' value a_1_.count)) from (select issue.assignee_id as assigneeId, count(issue.id) as count from project inner join issue on issue.project_id = project.id where project.organization_id = "organization".id group by issue.assignee_id) a_1_) as "byAssignee" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-with-having-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", (select json_arrayagg(json_object('assigneeId' value a_1_.assigneeId, 'count' value a_1_.count)) from (select issue.assignee_id as assigneeId, count(issue.id) as count from project inner join issue on issue.project_id = project.id where project.organization_id = "organization".id group by issue.assignee_id having count(issue.id) > :0) a_1_) as "byAssignee" from "organization" where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            1,
    ...
    test('inline-aggregate-of-distinct-object-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(json_object('status' value a_1_.status, 'body' value a_1_."body")) from (select distinct status as status, "body" as "body" from issue where project_id = project.id) a_1_) as "kinds" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-recursive-union-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive_select_1(id, title, "body") as (select id as id, title as title, "body" as "body" from issue where id = :0 union all select issue.id as id, issue.title as title, issue."body" as "body" from issue join recursive_select_1 on issue.parent_id = recursive_select_1.id) select id as "id", (select json_arrayagg(json_object('id' value id, 'title' value title, 'body' value "body")) from recursive_select_1) as "tree" from project where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('inline-aggregate-of-compound-union-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('title' value a_1_.title, 'body' value a_1_."body")) from (select title as title, "body" as "body" from issue where project_id = project.id and id = :0 union select title as title, "body" as "body" from issue where project_id = project.id and id = :1) a_1_) as "issues" from project where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('inline-aggregate-of-compound-union-rule1-gate-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_arrayagg(json_object('ref' value a_1_."ref", 'assigneeId' value a_1_.assigneeId)) from (select "body" as "ref", assignee_id as assigneeId from issue where id = :0 union select "body" as "ref", assignee_id as assigneeId from issue where id = :1) a_1_) as "items" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('inline-aggregate-nested-inside-inline-aggregate-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "orgId", (select json_arrayagg(json_object('pid' value id, 'issues' value (select json_arrayagg(json_object('iid' value id)) from issue where project_id = project.id))) from project where organization_id = "organization".id) as "projects" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-wrapping-inline-object-aggregate-projecting-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", json_arrayagg((select json_arrayagg(json_object('id' value id, 'body' value "body")) from issue where project_id = project.id)) as "nested" from project where id = :0 group by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result asc offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc-nulls-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result asc nulls first offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result desc offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-last', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by result desc nulls last offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) desc offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-last-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) desc nulls last offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (select json_arrayagg(a_1_.result) from (select name as result from project where organization_id = "organization".id order by lower(result) desc nulls first offset 0 rows) a_1_) as "projectNames" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
