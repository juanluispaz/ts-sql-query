...
describe(ctx.label, () => {
    ...
    test('table-left-join-preserves-custom-comparable-and-custom-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, project_release.version as version, project_release.channel as channel from project left join project_release on project_release.project_id = project.id and project_release.channel = @0 where project.id = @1"`)
    ...
    test('table-left-join-preserves-enum-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, issue_worklog.activity as activity from issue left join issue_worklog on issue_worklog.issue_id = issue.id where issue.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('aggregate-as-array-of-one-column-preserves-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg('"' + string_escape(convert(nvarchar(max), version), 'json') + '"', ','), ']') as versions, concat('[', string_agg('"' + string_escape(convert(nvarchar(max), channel), 'json') + '"', ','), ']') as channels from project_release where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-preserves-enum-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, concat('[', string_agg('"' + string_escape(convert(nvarchar(max), activity), 'json') + '"', ','), ']') as activities from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-union-carries-the-seed-brand-onto-the-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select channel as channel from project_release where project_id = @0 union select channel as channel from project_release where project_id = @1 order by channel"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('view-left-join-preserves-branded-column-and-virtual', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, release_overview.version as version, upper(release_overview.version) as versionUpper from project left join release_overview on release_overview.project_id = project.id and release_overview.version = @0 where project.id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1.3.0-beta.1",
            1,
