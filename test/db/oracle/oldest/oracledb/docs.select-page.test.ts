...
describe(ctx.label, () => {
    ...
    test('docs-extra:select-page/plain-filtered-count-retains-where-drops-paging', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select id as "id", title as "title" from issue where priority > :0 order by "id" offset :1 rows fetch next :2 rows only"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
            0,
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from issue where priority > :0"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            1,
          ]
