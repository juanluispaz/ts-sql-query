...
describe(ctx.label, () => {
    ...
    test('count-after-distinct-wraps-query-in-result-for-count-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (select distinct status as status from issue order by status offset 0 rows) select count(*) from result_for_count"`)
    ...
    test('count-after-group-by-wraps-query-in-result-for-count-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (select status as status, count(id) as total from issue group by status order by status offset 0 rows) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
    ...
    test('count-after-distinct-and-group-by-still-wraps-in-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (select distinct status as status, priority as priority from issue group by status, priority order by status offset 0 rows) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
