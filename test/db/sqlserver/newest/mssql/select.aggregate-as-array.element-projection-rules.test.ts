...
describe(ctx.label, () => {
    ...
    test('element-top-rule-3-own-table-optional-leaf-default-drops-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, json_arrayagg(json_object('title':title, 'body':body)) as issues from issue where project_id = @0 group by project_id"`)
    ...
    test('element-containing-a-nested-rule-1-required-in-optional-object-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, json_arrayagg(json_object('title':title, 'meta.gate':status, 'meta.assigneeId':assignee_id)) as issues from issue where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-top-pure-rule-4-all-optional-element-drops-when-all-null-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, json_arrayagg(json_object('body':issue.body, 'assigneeId':issue.assignee_id)) as items from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
