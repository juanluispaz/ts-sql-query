...
describe(ctx.label, () => {
    ...
    test('insert-optional-custom-columns-set-to-values', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title, stage, channel, min_version) values ($1, $2, $3, $4, $5)"`)
            ...
            const row = await ctx.conn.selectFrom(tReleaseDraft)
                ...
                .executeSelectOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, stage as stage, channel as channel, min_version as "minVersion" from release_draft where id = $1"`)
    ...
    test('insert-optional-custom-columns-omitted', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into release_draft (id, title) values ($1, $2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                12,
                "Omitted draft",
