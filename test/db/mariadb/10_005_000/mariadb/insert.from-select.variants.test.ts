    ...
    */
    ...
    test('insert-from-select-on-conflict-returning-object-none-or-one-and-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, slug, name) select organization_id as organizationId, slug as slug, name as name from project where organization_id = ? returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
