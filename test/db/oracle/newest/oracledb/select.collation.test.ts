...
describe(ctx.label, () => {
    ...
    test('collate on a uuid receiver converts before collate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", raw_to_uuid(external_ref) collate BINARY as "v" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('insensitive orderBy on a uuid alias converts before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", raw_to_uuid(external_ref) as "ref" from issue where external_ref is not null order by "ref" collate BINARY_CI"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('insensitive comparison with a uuid value operand converts before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when :0 = raw_to_uuid(external_ref) collate BINARY_CI then 1 when not (:1 = raw_to_uuid(external_ref) collate BINARY_CI) then 0 else null end as "eq", case when :2 <> raw_to_uuid(external_ref) collate BINARY_CI then 1 when not (:3 <> raw_to_uuid(external_ref) collate BINARY_CI) then 0 else null end as "ne", case when :4 like raw_to_uuid(external_ref) collate BINARY_CI escape '\\' then 1 when not :5 like raw_to_uuid(external_ref) collate BINARY_CI escape '\\' then 0 else null end as "lk", case when :6 not like raw_to_uuid(external_ref) collate BINARY_CI escape '\\' then 1 when not :7 not like raw_to_uuid(external_ref) collate BINARY_CI escape '\\' then 0 else null end as "nl" from issue where id = :8"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "x",
    ...
    test('insensitive affix predicates with a uuid value operand convert before collate', async () => {
        ...
        const result = await collated.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when :0 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 1 when not :1 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 0 else null end as "starts", case when :2 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 1 when not :3 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 0 else null end as "notStarts", case when :4 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) collate BINARY_CI escape '\\' then 1 when not :5 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) collate BINARY_CI escape '\\' then 0 else null end as "ends", case when :6 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) collate BINARY_CI escape '\\' then 1 when not :7 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) collate BINARY_CI escape '\\' then 0 else null end as "notEnds", case when :8 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 1 when not :9 like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 0 else null end as "contains", case when :10 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 1 when not :11 not like (case when replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace(raw_to_uuid(external_ref), '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) collate BINARY_CI escape '\\' then 0 else null end as "notContains" from issue where id = :12"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "x",
