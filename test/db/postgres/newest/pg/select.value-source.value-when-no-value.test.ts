...
describe(ctx.label, () => {
    ...
    test('value-when-no-value-false-as-selected-column-elided-emits-false-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, false as "statusMatches" from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-when-no-value-false-as-selected-column-fires-uses-primary-sql', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, status = $1 as "statusMatches" from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            1,
