...
describe(ctx.label, () => {
    ...
    test('marshalling/uuid-insert-select-roundtrip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority, external_ref) values (?, ?, ?, ?, ?, uuid_blob(?)) returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9004,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, uuid_str(external_ref) as ref from issue where number = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                9004,
              ]
