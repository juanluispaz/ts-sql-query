...
describe(ctx.label, () => {
    ...
    // NOT-APPLICABLE: SQLite has no `UPDATE … JOIN` syntax. `.innerJoin` / `.leftJoin` / `.leftOuterJoin` on UPDATE are typed `never` on this dialect (only MariaDB / MySQL narrow `OnExpressionFnType` to enable these); the equivalent pattern is `.update(t).from(j).set(...).where(...)` (see update.from.test.ts).
    /*
