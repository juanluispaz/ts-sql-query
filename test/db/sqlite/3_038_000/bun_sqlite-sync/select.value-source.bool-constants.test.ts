...
describe(ctx.label, () => {
    ...
    test('connection.true-in-projection', async () => {
        ...
        ctx.mockNext(true)
        const conn = ctx.conn
        const row = sync(conn.selectFromNoTable()
    ...
    test('connection.true-in-where', async () => {
        ...
        const conn = ctx.conn
        const rows = sync(conn.selectFrom(tOrganization)
            .where(conn.true())
    ...
    test('negate-of-boolean-constants-in-projection', async () => {
        ...
        const row = sync(conn.selectFromNoTable()
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select 0 as notTrue, 1 as notFalse"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('negate-of-boolean-constants-in-where', async () => {
        ...
        const conn = ctx.conn
        ctx.mockNext([])
        const none = sync(conn.selectFrom(tOrganization)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const all = sync(conn.selectFrom(tOrganization)
    ...
    test('and-with-true-constant-reduces-away-the-neutral-operand', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const right = sync(conn.selectFrom(tOrganization)
        ...
        ctx.mockNext([{ id: 1 }])
        const left = sync(conn.selectFrom(tOrganization)
