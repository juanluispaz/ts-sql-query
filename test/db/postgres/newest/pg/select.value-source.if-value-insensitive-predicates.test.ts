...
describe(ctx.label, () => {
    ...
    test('equals-and-not-equals-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) = lower($1) and lower(email) <> lower($2) order by id"`)
    ...
    test('like-insensitive-and-not-like-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email ilike $1 and email not ilike $2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%ACME%",
            "%GRACE%",
    ...
    test('starts-with-insensitive-and-ends-with-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email ilike ($1 || '%') and email ilike ('%' || $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ADA",
            "TEST",
    ...
    test('not-starts-with-insensitive-and-not-ends-with-insensitive-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email not ilike ($1 || '%') and email not ilike ('%' || $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ADA",
            "GLOBEX.TEST",
