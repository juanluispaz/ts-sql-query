...
describe(ctx.label, () => {
    ...
    test('not-in-if-value-fires-when-array-non-empty', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status not in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    test('is-if-value-and-is-not-if-value-fire-with-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when assignee_id = @0 then 1 else 0 end = 1 and not (case when assignee_id = @1 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            2,
    ...
    test('in-n-and-not-in-n-variadic-spread', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority not in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('in-n-positive-variadic-spread', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
