...
- **PostgreSQL, SQL Server: `.modulo(...)` on a `double` / `customDouble` value source now emits valid SQL.** Applying `modulo` to a floating-point value source — for example `worklog.billedAmount.modulo(3)` on a `customDouble` column, or `table.intColumn.asDouble().modulo(3)` — emitted a bare `%` over a floating-point operand, which PostgreSQL (`operator does not exist: double precision %`) and SQL Server (`The data types float and int are incompatible in the modulo operator`) both reject at runtime. PostgreSQL now emits `mod((…)::numeric, (…)::numeric)` and SQL Server casts both operands to `numeric`, so the operator resolves. The `int` / `bigint` / `customInt` receivers (plain `%`), MySQL / MariaDB / SQLite (which accept float `%`) and Oracle (which already used `MOD(…)`) are unchanged.
