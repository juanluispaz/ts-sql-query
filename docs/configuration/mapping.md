...
!!! warning
    ...
    - This is supported by [PostgreSQL](../configuration/supported-databases/postgresql.md), [SQL Server](../configuration/supported-databases/sqlserver.md), [SQLite](../configuration/supported-databases/sqlite.md), and [Oracle](../configuration/supported-databases/oracle.md). On Oracle, the `Values` view can be used in `select`/join contexts; combining it with `update.from(...)` / `deleteFrom.using(...)` is rejected at compile time because Oracle's SQL does not support those clauses.
