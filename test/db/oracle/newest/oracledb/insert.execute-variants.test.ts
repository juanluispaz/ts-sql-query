...
describe(ctx.label, () => {
    ...
    test('execute-insert-none-or-one-with-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            `)
            // Real DB returns the engine-assigned id (non-deterministic);
            // the mock pins the exact value. Both modes still validate.
            if (!ctx.realDbEnabled) expect(result).toBe(500)
            else expect(typeof result).toBe('number')
        })
    })
    ...
    */
    ...
    // NOT-APPLICABLE: Oracle rejects RETURNING with INSERT...SELECT (ORA-03049), so a 0-row returning insert is not expressible (returning on from-select is `never`).
    /*
