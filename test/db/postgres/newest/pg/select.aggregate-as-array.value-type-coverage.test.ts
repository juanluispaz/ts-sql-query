...
describe(ctx.label, () => {
    ...
    test('aggregate-of-double-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select estimated_hours as result from issue where project_id = project.id order by result) as a_1_))::text as hours from project where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-bigint-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select view_count as result from issue where project_id = project.id order by result) as a_1_))::text as views from project where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-uuid-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select external_ref as result from issue where project_id = project.id order by result) as a_1_))::text as refs from project where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-time-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select created_at as result from issue where project_id = project.id order by result) as a_1_))::text as stamps from project where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-boolean-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select billable as result from issue_worklog where issue_id = issue.id order by result) as a_1_))::text as flags from issue where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select work_date as result from issue_worklog where issue_id = issue.id order by result) as a_1_))::text as dates from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select billed_amount as result from issue_worklog where issue_id = issue.id order by result) as a_1_))::text as amounts from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as "issueId", (json_agg(billed_amount))::text as amounts from issue_worklog where issue_id = $1 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select started_at as result from issue_worklog where issue_id = issue.id order by result) as a_1_))::text as clocks from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-uuid-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select signing_key as result from project_release where project_id = project.id order by result) as a_1_))::text as keys from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select released_on as result from project_release where project_id = project.id order by result) as a_1_))::text as days from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select cutoff_time as result from project_release where project_id = project.id order by result) as a_1_))::text as cutoffs from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select signed_off_at as result from project_release where project_id = project.id order by result) as a_1_))::text as signoffs from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-wrapped-object-with-uuid-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(json_build_object('ref', a_1_.ref, 'cnt', a_1_.cnt)) from (select external_ref as ref, count(id) as cnt from issue where project_id = project.id group by external_ref) as a_1_))::text as refs from project where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
