...
describe(ctx.label, () => {
    ...
    test('customize-select-plain-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (/* head */  select id as id, \`name\` as \`name\` from project where id <= ? order by id  /* tail */) select count(*) from result_for_count"`)
