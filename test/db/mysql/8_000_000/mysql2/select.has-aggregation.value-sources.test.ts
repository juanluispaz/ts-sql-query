...
describe(ctx.label, () => {
    ...
    test('has-aggregation/is-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, max(assignee_id) <=> ? as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('has-aggregation/is-with-an-aggregating-operand-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, project_id <=> min(id) as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ projectId: number; matches: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('has-aggregation/is-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, project_id <=> ? as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/concat-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having concat(max(title), ?) = ? order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
    ...
    test('has-aggregation/concat-if-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having concat(title, ?) = ? order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
