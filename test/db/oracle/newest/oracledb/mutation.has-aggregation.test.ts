...
describe(ctx.label, () => {
    ...
    test('has-aggregation/insert-no-aggregate-in-any-clause-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)  /* tail */"`)
    ...
    test('has-aggregation/insert-from-select-with-a-plain-projection-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority) select :0 as "projectId", :1 as "number", name as "title", :2 as "status", organization_id as "priority" from project where id = :3  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                902,
    ...
    test('has-aggregation/insert-from-select-with-an-aggregating-projection-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority) select :0 as "projectId", :1 as "number", :2 as "title", :3 as "status", count(id) as "priority" from project where organization_id = :4  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                900,
    ...
    test('has-aggregation/insert-set-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority, assignee_id) values (:0, :1, :2, :3, :4, (select count(id) as "result" from app_user))  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                901,
    ...
    test('has-aggregation/insert-multiple-rows-with-plain-values-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin /* head */  insert /* hint */ into issue (project_id, "number", title, status, priority, assignee_id) values (:0, :1, :2, :3, :4, :5)  /* tail */; /* head */  insert /* hint */ into issue (project_id, "number", title, status, priority, assignee_id) values (:6, :7, :8, :9, :10, :11)  /* tail */; end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                913,
    ...
    test('has-aggregation/insert-multiple-rows-with-an-aggregating-subquery-in-one-row-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin /* head */  insert /* hint */ into issue (project_id, "number", title, status, priority, assignee_id) values (:0, :1, :2, :3, :4, :5)  /* tail */; /* head */  insert /* hint */ into issue (project_id, "number", title, status, priority, assignee_id) values (:6, :7, :8, :9, :10, (select count(id) as "result" from app_user))  /* tail */; end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                911,
    ...
    test('has-aggregation/insert-returning-plain-columns-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4) returning title, status into :5, :6  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                921,
    ...
    // NOT-APPLICABLE: Oracle has no INSERT…ON CONFLICT (uses MERGE); `onConflictOn` and the DO UPDATE `where` are narrowed away.
    ...
    test('has-aggregation/insert-before-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head users=(select count(id) as "result" from app_user) */  insert /* hint */ into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                931,
    ...
    test('has-aggregation/insert-after-insert-keyword-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint users=(select count(id) as "result" from app_user) */ into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                932,
    ...
    test('has-aggregation/insert-after-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /* hint */ into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)  /* tail users=(select count(id) as "result" from app_user) */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                933,
    ...
    test('has-aggregation/update-no-aggregate-in-any-clause-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ project set project.name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('has-aggregation/update-set-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ issue set assignee_id = (select count(id) as "result" from app_user) where id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('has-aggregation/update-join-on-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ project set project.name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id and project.organization_id <= (select count(id) as "result" from "organization") where project.id = issue.project_id and issue.id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('has-aggregation/update-where-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ issue set status = :0 where id = :1 and priority <= (select count(id) as "result" from app_user)  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('has-aggregation/update-returning-plain-columns-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ project set project.name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = :0 returning project.id, project.name into :1, :2  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('has-aggregation/update-before-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head users=(select count(id) as "result" from app_user) */  update /* hint */ issue set status = :0 where id = :1  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('has-aggregation/update-after-update-keyword-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint users=(select count(id) as "result" from app_user) */ issue set status = :0 where id = :1  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('has-aggregation/update-after-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  update /* hint */ issue set status = :0 where id = :1  /* tail users=(select count(id) as "result" from app_user) */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                1,
    ...
    test('has-aggregation/delete-no-aggregate-in-any-clause-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue where id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-join-on-with-plain-condition-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue using project inner join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and issue.id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-join-on-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue using project inner join "organization" on "organization".id = project.organization_id and "organization".id < (select count(id) as "result" from app_user) where issue.project_id = project.id and issue.id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-where-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue where id = :0 and priority <= (select count(id) as "result" from app_user)  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-returning-plain-columns-reports-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue where id = :0 returning id, title into :1, :2  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                {
    ...
    test('has-aggregation/delete-before-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head users=(select count(id) as "result" from app_user) */  delete /* hint */ from issue where id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-after-delete-keyword-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint users=(select count(id) as "result" from app_user) */ from issue where id = :0  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('has-aggregation/delete-after-query-fragment-with-an-aggregating-subquery-reports-aggregation', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /* hint */ from issue where id = :0  /* tail users=(select count(id) as "result" from app_user) */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
