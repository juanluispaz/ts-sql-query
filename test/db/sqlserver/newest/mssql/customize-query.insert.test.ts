...
describe(ctx.label, () => {
    ...
    test('customize-insert-all-hooks-combined', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /*+ hint */ into project (name, slug, organization_id) values (@0, @1, @2), (@3, @4, @5)  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Status page",
                "status-page",
    ...
    test('customize-insert-from-select-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ NO_LOG */ into project (organization_id, name, slug) select organization_id as organizationId, name + @0 as name, slug + @1 as slug from project where name like (@2 + '%')  /* batch-clone */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                " (clone)",
                "-clone",
...
//     })
    test('customize-insert-returning-one-column-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) output inserted.slug as [result] values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    test('customize-insert-returning-last-inserted-id-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) output inserted.id values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
