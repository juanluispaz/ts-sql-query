...
describe(ctx.label, () => {
    ...
    */
    ...
    test('custom-numeric/customdouble-operation1-math', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, power(?, ?) as "p", log(?, ?) as ln, round(?, ?) as rn, cast(? as real) / ? as di, atan2(?, ?) as at2 from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/cast-double-as-int-and-bigint', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(?) as integer) as asInt, cast(round(?) as integer) as asBig from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1.7,
            1.7,
    ...
    test('custom-numeric/bigint-arithmetic-optional', async () => {
        ...
        ctx.mockNext(expected)
        const result = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('custom-numeric/optional-custom-receiver-add-keeps-brand-and-optional', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(vReleaseOverview)
    ...
    test('custom-numeric/brand-survival-customint-minmax', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, max(round(?), ?) as mn, min(round(?), ?) as mx from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            10,
    ...
    test('custom-numeric/brand-survival-customdouble-math', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(?) as ab, ceil(?) as ce, floor(?) as fl, round(?) as ro, exp(?) as ex, ln(?) as "l", log10(?) as l10, sign(?) * power(abs(?), 1.0e0 / 3.0e0) as cb from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            4,
    ...
    test('custom-numeric/brand-survival-customdouble-divide-modulo', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(? as real) / ? as di, ? % ? as mo from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    */
    ...
    test('custom-numeric/customdouble-adapter-column-through-multiply', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tReleaseDraft)
    ...
    test('custom-numeric/minmax-optional-customdouble-receiver-poisons', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, max(budget, ?) as mn, min(budget, ?) as mx from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            100,
    ...
    test('custom-numeric/minmax-operand-optional-customint-keeps-brand', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, max(release_ordinal, optional_release_ordinal) as mn, min(release_ordinal, optional_release_ordinal) as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
    test('custom-numeric/minmax-both-optional-customint-keeps-brand', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, max(optional_release_ordinal, optional_release_ordinal) as mn, min(optional_release_ordinal, optional_release_ordinal) as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
    test('custom-numeric/minmax-both-required-customint-value-source-keeps-brand', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, max(release_ordinal, release_ordinal) as mn, min(release_ordinal, release_ordinal) as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn: ReleaseTag; mx: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
