...
describe(ctx.label, () => {
    ...
    test('insert-from-select-multiply-substring-and-least', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) select id as projectId, (id * @0) + @1 as number, substring(name, @2, @3) as title, @4 as status, least(id, @5) as priority from project where id = @6"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                100,
                2,
