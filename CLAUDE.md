...
- **Typecheck only** (closest thing to a lint): `bun run validate` (or `npm run validate`) — runs `tsc --noEmit`. Use this to verify changes compile.
- **Build the publishable artifact**: `npm run build` — wipes `dist/`, runs `tsc -p tsconfig.build.json`. Required before `npm run dist*`. The publish flow stays on npm/Node intentionally — don't switch this to bun.
