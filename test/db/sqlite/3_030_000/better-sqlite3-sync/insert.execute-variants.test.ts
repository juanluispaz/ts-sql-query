...
describe(ctx.label, () => {
    ...
    test('execute-insert-none-or-one-with-returning-one-column', async () => {
        ...
        ctx.mockNext(500)
        await ctx.withRollback(async () => {
            const result = sync(ctx.conn.insertInto(tOrganization)
            ...
            `)
            // Real DB writes the row and returns its engine-assigned id;
            // the mock pre-shapes it to 500.
    ...
    test('execute-insert-none-or-one-with-returning-row-shape-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, "plan") select name as name, "plan" as "plan" from organization where id = ? returning id as id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                -1,
              ]
    ...
    test('execute-insert-one-with-returning-one-column-empty-result-throws-no-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, "plan") select name as name, "plan" as "plan" from organization where id = ? returning id as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                -1,
              ]
    ...
    test('values-accepts-a-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const count = sync(ctx.conn.insertInto(tIssue)
                .values({
                    projectId: 1,
                    number:    999,
    ...
    test('insert-provided-primary-key-carries-the-pk-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const count = sync(ctx.conn.insertInto(tCountry)
    ...
    test('execute-insert-many-with-returning-row-shape-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            const rows = sync(ctx.conn.insertInto(tOrganization)
                .values([
    ...
    test('execute-insert-from-multi-row-select-returning-row-shape-execute-one-throws-more-than-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            try {
                result = sync(ctx.conn.insertInto(tOrganization)
                    .from(source)
                    .returning({ id: tOrganization.id })
                    .executeInsertOne())
            } catch (e) {
