...
describe(ctx.label, () => {
    ...
    test('element-top-rule-3-own-table-optional-leaf-default-drops-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, concat('[', string_agg(concat('{', '"title": ', '"' + string_escape(convert(nvarchar(max), title), 'json') + '"', ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), body), 'json') + '"', 'null'), '}'), ','), ']') as issues from issue where project_id = @0 group by project_id"`)
    ...
    test('element-containing-a-nested-rule-1-required-in-optional-object-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, concat('[', string_agg(concat('{', '"title": ', '"' + string_escape(convert(nvarchar(max), title), 'json') + '"', ', "meta.gate": ', isnull('"' + string_escape(convert(nvarchar(max), status), 'json') + '"', 'null'), ', "meta.assigneeId": ', isnull(convert(nvarchar, assignee_id), 'null'), '}'), ','), ']') as issues from issue where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-a-left-joined-inner-object-rule-2-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.project_id as grp, concat('[', string_agg(concat('{', '"iid": ', convert(nvarchar, issue.id), ', "proj.id": ', isnull(convert(nvarchar, project.id), 'null'), ', "proj.name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), ', "proj.archivedAt": ', isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), '}'), ','), ']') as items from issue left join project on project.id = issue.project_id where issue.project_id = @0 group by issue.project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-rule-1-object-with-originally-required-left-join-sibling-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.project_id as grp, concat('[', string_agg(concat('{', '"iid": ', convert(nvarchar, issue.id), ', "meta.ownId": ', convert(nvarchar, issue.id), ', "meta.gate": ', isnull('"' + string_escape(convert(nvarchar(max), issue.status), 'json') + '"', 'null'), ', "meta.projName": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), '}'), ','), ']') as items from issue left join project on project.id = issue.project_id where issue.project_id = @0 group by issue.project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-left-join-all-optional-object-applies-rule-4-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"pid": ', convert(nvarchar, project.id), ', "opt.body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), ', "opt.assigneeId": ', isnull(convert(nvarchar, issue.assignee_id), 'null'), '}'), ','), ']') as items from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-top-rule-1-required-in-optional-object-gate-null-drops-whole-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"ref": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), ', "assigneeId": ', isnull(convert(nvarchar, issue.assignee_id), 'null'), '}'), ','), ']') as items from project inner join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-sole-optional-inner-object-collapses-on-aggregate-path-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"iid": ', convert(nvarchar, issue.id), ', "wrapper.inner.body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), ', "wrapper.inner.assigneeId": ', isnull(convert(nvarchar, issue.assignee_id), 'null'), '}'), ','), ']') as items from project inner join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-top-pure-rule-4-all-optional-element-drops-when-all-null-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), ', "assigneeId": ', isnull(convert(nvarchar, issue.assignee_id), 'null'), '}'), ','), ']') as items from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-containing-rule-2-left-join-object-realizes-a-miss-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"pid": ', convert(nvarchar, project.id), ', "iss.id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "iss.title": ', isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ', "iss.body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), '}'), ','), ']') as items from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-top-rule-2-all-left-join-element-drops-on-miss-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "title": ', isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), '}'), ','), ']') as items from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-own-table-optional-leaf-default-drops-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select title as title, body as body from issue where project_id = project.id for json path) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-rule-1-gate-leaf-at-top-default-keeps-element-drops-gate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select body as [ref], assignee_id as assigneeId from issue where project_id = project.id for json path) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-rule-2-left-join-leaf-at-top-default-keeps-element-drops-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select app_user.full_name as name from issue left join app_user on app_user.id = issue.assignee_id where issue.project_id = project.id for json path) as assignees from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-top-rule-2-multi-left-join-leaves-as-nullable-surface-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select issue.id as id, issue.title as title, issue.body as body from project left join issue on issue.project_id = project.id where project.organization_id = organization.id for json path) as items from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-top-rule-4-all-optional-default-keeps-empty-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select issue.body as body, issue.assignee_id as assigneeId from project left join issue on issue.project_id = project.id where project.organization_id = organization.id for json path) as items from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-nested-rule-1-gate-object-default-drops-nested-on-null-gate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select id as iid, body as [meta.gate], assignee_id as [meta.assigneeId] from issue where project_id = project.id for json path) as items from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-nested-rule-2-left-join-object-default-drops-nested-on-miss', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select project.id as pid, issue.id as [iss.id], issue.title as [iss.title], issue.body as [iss.body] from project left join issue on issue.project_id = project.id where project.organization_id = organization.id for json path) as items from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-sole-optional-inner-object-collapses-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select issue.id as iid, issue.body as [wrapper.inner.body], issue.assignee_id as [wrapper.inner.assigneeId] from project inner join issue on issue.project_id = project.id where project.organization_id = organization.id for json path) as items from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-own-table-multi-optional-leaf-as-nullable-surfaces-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select title as title, body as body, assignee_id as assigneeId from issue where project_id = project.id for json path) as items from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
