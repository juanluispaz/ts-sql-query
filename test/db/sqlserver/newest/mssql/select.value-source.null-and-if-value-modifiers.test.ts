...
describe(ctx.label, () => {
    ...
    test('boolean/if-value-source-and-or-with-concrete-boolean-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when status = @0 and priority > @1 then 1 else 0 end as bit) as andFlag, cast(case when status = @2 or priority > @3 then 1 else 0 end as bit) as orFlag from issue where id in (@4, @5, @6) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            1,
    ...
    test('is-null-is-not-null-on-optional-custom-double', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when budget is null then 1 else 0 end as bit) as [n], cast(case when budget is not null then 1 else 0 end as bit) as nn from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('is-null-is-not-null-on-optional-custom-local-date', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when target_day is null then 1 else 0 end as bit) as [n], cast(case when target_day is not null then 1 else 0 end as bit) as nn from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; n: boolean; nn: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('is-null-is-not-null-on-optional-custom-local-time', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when cutoff is null then 1 else 0 end as bit) as [n], cast(case when cutoff is not null then 1 else 0 end as bit) as nn from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; n: boolean; nn: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('modifier-trio-on-custom-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when published = 't' then 1 else 0 end as bit) as req, null as own, null as ign from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/local-date-is-null-is-not-null', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when review_date is null then 1 else 0 end as bit) as [n], cast(case when review_date is not null then 1 else 0 end as bit) as nn from project_review where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-verified-required-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when verified = 'Y' then 1 else 0 end as bit) as req, null as own, null as ign from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-approved-nullable-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) as req, null as own, null as ign from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-invoiced-required-numeric-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when invoiced = 1 then 1 else 0 end as bit) as req, null as own, null as ign from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
