...
describe(ctx.label, () => {
    ...
    test('where: custom boolean column equals column with same adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('userId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as "orgId", app_user.id as "userId" from organization inner join app_user on app_user.verified = organization.verified where organization.id = $1 order by "userId""`)
    ...
    test('where: custom boolean column equals a boolean value source', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where (verified = 'Y') = (id > $1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('update set: custom boolean column from a boolean expression', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.update(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set published = case when id > $1 then 't' else 'f' end where id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                0,
                1,
    ...
    test('insert set: custom boolean column from a boolean expression', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await ctx.conn.insertInto(tProject)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, published) values ($1, $2, $3, case when $4 > $5 then 't' else 'f' end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Remap insert",
    ...
    test('insert custom boolean then read it back round-trips to true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const inserted = await ctx.conn.insertInto(tOrganization)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan, verified) values ($1, $2, case when $3::bool then 'Y' else 'N' end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Initech",
                "pro",
            ...
            const row = await ctx.conn.selectFrom(tOrganization)
                ...
                .executeSelectOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select (verified = 'Y') as verified from organization where name = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Initech",
              ]
    ...
    test('numeric custom boolean adapter writes as case-when-then-trueValue', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.update(tIssueWorklog)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set invoiced = case when id > $1 then 1 else 0 end where id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                0,
                1,
    ...
    test('combinator: verified custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where verified = 'Y' and $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('combinator: verified custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where verified = 'Y' or $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('combinator: published custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where published = 't' and $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('combinator: published custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where published = 't' or $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('combinator: approved nullable custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where approved = 'A' and $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('combinator: approved nullable custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where approved = 'A' or $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('combinator: invoiced numeric custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where invoiced = 1 and $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('combinator: invoiced numeric custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where invoiced = 1 or $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('equals-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where (verified = 'Y') = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('equals-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (approved = 'A') = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where (published = 't') is not distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where (verified = 'Y') is not distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (approved = 'A') is not distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-not-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where (published = 't') is distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-not-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where (verified = 'Y') is distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-not-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (approved = 'A') is distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-not-if-value-on-invoiced-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (invoiced = 1) is distinct from $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('paren-and-with-or-right-operand-remaps-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where id > $1 and (verified = 'Y' or plan = $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            "pro",
    ...
    test('negate-and-with-remapping-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where not (verified = 'Y' and plan = $1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "pro",
          ]
    ...
    test('not-in-on-invoiced-custom-boolean-emits-numeric-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (invoiced = 1) not in ($1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('not-in-on-verified-custom-boolean-emits-string-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where (verified = 'Y') not in ($1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (invoiced = 1) in ($1, $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('not-in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where (invoiced = 1) not in ($1, $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('or-of-and-from-published-custom-boolean-receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where published = 't' or (id > $1 and archived_at is not null) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('and-of-or-from-published-custom-boolean-receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where published = 't' and (id < $1 or archived_at is not null) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('equals-if-value-negate-on-invoiced-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not ((invoiced = 1) = $1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
