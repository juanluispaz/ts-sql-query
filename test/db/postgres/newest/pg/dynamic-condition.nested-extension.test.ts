...
describe(ctx.label, () => {
    ...
    test('depth-3-nested-projection-recurses-with-non-empty-prefix', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where app_user.full_name ilike ('%' || $1 || '%') order by id"`)
    ...
    test('extension-scoped-under-nested-projection-key-is-invoked-on-recurse', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id from issue left join project on project.id = issue.project_id where exists(select id as result from project where id = issue.project_id and name ilike ('%' || $1 || '%')) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mktg",
          ]
    ...
    test('extension-callback-returning-non-value-source-throws-typed-error', async () => {
        ...
        const extension = {
            broken: (_v: string): BooleanResult => 'not-a-value-source' as any,
        }
        type Filter = DynamicCondition<{ id: 'int' }, { broken: (v: string) => BooleanResult }>
    ...
    test('extension-callback-returning-non-boolean-value-source-throws-with-type-name', async () => {
        ...
        const selectFields = { id: tIssue.id }
        const extension = {
            stringified: (_v: string): BooleanResult => tIssue.title as any,
        }
        type Filter = DynamicCondition<{ id: 'int' }, { stringified: (v: string) => BooleanResult }>
    ...
    test('column-level-object-valued-extension-rule-dispatches-nested', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where id > $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
          ]
