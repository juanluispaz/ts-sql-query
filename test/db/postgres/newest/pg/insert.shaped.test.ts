...
describe(ctx.label, () => {
    ...
    test('shaped-values-single-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3)"`)
    ...
    test('shaped-dynamic-values-multi-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3), ($4, $5, $6)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped DV A",
    ...
    test('shaped-values-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3) returning id as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped returning",
    ...
    test('shaped-multi-row-set-for-all', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, published) values ($1, $2, $3, case when $4::bool then 't' else 'f' end), ($5, $6, $7, case when $8::bool then 't' else 'f' end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped FA A",
    ...
    test('shaped-set-if-set-if-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, archived_at) values ($1, $2, $3, $4)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Overwritten",
    ...
    // value to every row in the batch, gated per row by that row's staged state.
    // The nullable `archived` (archived_at) is the gate column throughout. The
    // non-`When` variants assert the batch params; the `When` variants pair a
    // `false` (no-op) arm against a `true` (dispatch) arm so the param delta
    // proves the dispatch fired.
    ...
    test('shaped-multi-row-set-for-all-if-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, archived_at) values ($1, $2, $3, $4), ($5, $6, $7, $8)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "FA IfSet A",
    ...
    test('shaped-values-returning-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3) returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
    ...
    test('shaped-values-returning-last-inserted-id', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3) returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped last id",
    ...
    test('shaped-on-conflict-do-update-set-where-then-and-or', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values ($1, $2, $3) on conflict (organization_id, slug) do update set name = $4 where (project.name <> $5 and project.archived_at is null) or (project.published = 't') = $6"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
    ...
    test('shaped-set-if-value-null-skips-enum-defaulted-shaped-key', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await ctx.conn.insertInto(tWebhookEvent)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into webhook_event (issue_id) values ($1)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('shaped-set-if-value-null-skips-boolean-adapter-defaulted-shaped-key', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await ctx.conn.insertInto(tOrganization)
                ...
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan) values ($1, $2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Skip Verified Inc",
                "free",
