...
describe(ctx.label, () => {
    ...
    // MySQL has no UPDATE ... RETURNING, so `.returning({...})` is typed `never`
    // on the update builder and the returning-many execute-shape is off the typed
    // surface. Kept here commented for symmetry with the other dialects.
