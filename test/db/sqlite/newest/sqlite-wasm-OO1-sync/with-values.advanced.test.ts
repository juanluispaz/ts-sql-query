...
describe(ctx.label, () => {
    ...
    test('values-aliased-via-as-keeps-original-with-name', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(pp)
    ...
    test('values-optional-column-allows-undefined-per-row', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(patch)
    ...
    test('values-with-custom-typed-columns-emits-customint-customdouble-casts', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(billing)
            .select({
    ...
    test('values-base-type-optional-virtual-column-projects-optional-and-is-not-a-tuple-member', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(batch)
    ...
    test('values-branded-column-through-for-use-in-left-join-as-keeps-the-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values (?, ?), (?, ?)), issueBillingJoin(issueId, amount) as (values (?, ?)) select issueBilling.issueId as base, ibj.issueId as joined from issueBilling left join issueBillingJoin as ibj on ibj.issueId = issueBilling.issueId order by base"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-base-type-required-virtual-column-projects-required-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqBatch(id) as (values (?), (?)) select id as id, 7 as seq, 'BATCH' as tag from reqBatch order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-virtual-column-referencing-a-sibling-column-inlines-the-reference', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(nums)
            .select({ id: nums.id, doubled: nums.doubled })
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vdouble(id) as (values (?), (?)) select id as id, id * 2 as doubled from vdouble order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
