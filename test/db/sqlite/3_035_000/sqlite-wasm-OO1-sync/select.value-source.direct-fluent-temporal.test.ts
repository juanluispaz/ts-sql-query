...
describe(ctx.label, () => {
    ...
    test('localDate-between-not-between', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-03-04",
            "2024-03-05",
        ...
        const notRows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date not between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-03-04",
            "2024-03-05",
    ...
    test('localTime-between-not-between', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "09:00:00",
            "11:00:00",
        ...
        const notRows = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at not between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "09:00:00",
            "11:00:00",
    ...
    test('customLocalTime-in-array-in-n', async () => {
        ...
        const inRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
            "16:00:00",
        ...
        const inNRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customLocalTime-between-not-between', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "16:30:00",
            "18:00:00",
        ...
        const notRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time not between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "16:30:00",
            "18:00:00",
    ...
    test('customLocalDateTime-between-not-between', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-01 00:00:00",
            "2024-02-01 00:00:00",
        ...
        const notRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at not between ? and ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-01 00:00:00",
            "2024-02-01 00:00:00",
    ...
    test('view-customLocalTime-getters', async () => {
        ...
        ctx.mockNext(expected)
        const rows = sync(ctx.conn.selectFrom(vReleaseOverview)
    ...
    test('custom-source-asOptional-getter', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', released_on) as integer) as "y" from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-plain-localDateTime-getters', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expected)
            const rows = sync(ctx.conn.selectFrom(vProjectOverview)
    ...
    test('view-custom-localDate-asOptional-getters', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', released_on) as integer) as "y", cast(strftime('%m', released_on) as integer) - 1 as mo, cast(strftime('%d', released_on) as integer) as "d", cast(strftime('%w',released_on) as integer) as dow from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-plain-localDate-getters', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', release_day_plain) as integer) as "y", cast(strftime('%m', release_day_plain) as integer) - 1 as mo, cast(strftime('%d', release_day_plain) as integer) as "d", cast(strftime('%w',release_day_plain) as integer) as dow from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customLocalDate-in-array-in-n', async () => {
        ...
        const inRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-15",
            "2024-03-01",
    ...
    test('customLocalDate-not-in-array-not-in-n', async () => {
        ...
        const notInRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-15",
          ]
        ...
        ctx.mockNext(expected)
        const notInNRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customLocalDate-if-value-family-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const eq = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.equalsIfValue(d1)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-15",
        ...
        const ne = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.notEqualsIfValue(d1)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on <> ? order by id"`)
        expect(ne).toEqual([{ id: 2 }, { id: 3 }])

        ctx.mockNext([{ id: 1 }])
        const lt = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.lessThanIfValue(mid)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on < ? order by id"`)
        expect(lt).toEqual([{ id: 1 }])

        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const ge = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.greaterOrEqualIfValue(mid)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on >= ? order by id"`)
        expect(ge).toEqual([{ id: 2 }, { id: 3 }])

        ctx.mockNext([{ id: 3 }])
        const gt = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.greaterThanIfValue(mid)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on > ? order by id"`)
        expect(gt).toEqual([{ id: 3 }])

        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const le = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.lessOrEqualIfValue(mid)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on <= ? order by id"`)
        expect(le).toEqual([{ id: 1 }, { id: 2 }])

        ctx.mockNext([{ id: 1 }])
        const inif = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.inIfValue([d1])).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on in (?) order by id"`)
        expect(inif).toEqual([{ id: 1 }])

        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const ninif = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.notInIfValue([d1])).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on not in (?) order by id"`)
        expect(ninif).toEqual([{ id: 2 }, { id: 3 }])

        // Elides: undefined value / array drop the sole WHERE.
        ctx.mockNext(all)
        const eqElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.equalsIfValue(noDate)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(eqElide).toEqual(all)

        ctx.mockNext(all)
        const ltElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.lessThanIfValue(noDate)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ltElide).toEqual(all)

        ctx.mockNext(all)
        const inElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.inIfValue(noDates)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
    ...
    test('customLocalTime-not-in-array-not-in-n', async () => {
        ...
        const notInRows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
          ]
    ...
    test('customLocalTime-if-value-family-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const eq = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.equalsIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const ne = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.notEqualsIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time <> ? order by id"`)
        expect(ne).toEqual([{ id: 2 }, { id: 3 }])

        ctx.mockNext([{ id: 3 }])
        const lt = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.lessThanIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time < ? order by id"`)
        expect(lt).toEqual([{ id: 3 }])

        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const ge = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.greaterOrEqualIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time >= ? order by id"`)
        expect(ge).toEqual([{ id: 1 }, { id: 2 }])

        ctx.mockNext([{ id: 2 }])
        const gt = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.greaterThanIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time > ? order by id"`)
        expect(gt).toEqual([{ id: 2 }])

        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const le = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.lessOrEqualIfValue(t)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time <= ? order by id"`)
        expect(le).toEqual([{ id: 1 }, { id: 3 }])

        ctx.mockNext([{ id: 1 }])
        const inif = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.inIfValue([t])).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time in (?) order by id"`)
        expect(inif).toEqual([{ id: 1 }])

        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const ninif = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.notInIfValue([t])).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time not in (?) order by id"`)
        expect(ninif).toEqual([{ id: 2 }, { id: 3 }])

        // Elides.
        ctx.mockNext(all)
        const eqElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.equalsIfValue(noTime)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(eqElide).toEqual(all)

        ctx.mockNext(all)
        const ltElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.lessThanIfValue(noTime)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ltElide).toEqual(all)

        ctx.mockNext(all)
        const inElide = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.inIfValue(noTimes)).select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(inElide).toEqual(all)
    })
    ...
    test('modifier-getter/customLocalDate-value-when-null-get-month-and-null-if-value-get-day', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%m', ifnull(released_on, ?)) as integer) - 1 as mo, cast(strftime('%w',nullif(released_on, ?)) as integer) as dow from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2000-01-01",
            "1999-06-06",
    ...
    test('modifier-getter/customLocalTime-value-when-null-get-hours-and-null-if-value-get-seconds', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%H', ifnull(cutoff_time, ?)) as integer) as "h", cast(strftime('%S', nullif(cutoff_time, ?)) as integer) as "s" from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "00:00:00",
            "12:00:00",
    ...
    test('modifier-getter/localDateTime-value-when-null-get-full-year-and-null-if-value-get-hours', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%Y', ifnull(created_at, ?)) as integer) as "y", cast(strftime('%H', nullif(created_at, ?)) as integer) as "h" from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1970-01-01 00:00:00",
            "1999-01-01 00:00:00",
