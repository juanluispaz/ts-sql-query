...
describe(ctx.label, () => {
    ...
    test('customize-delete-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete /*+ hint */ from issue where id = :0 returning id, title into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('customize-delete-returning-one-column-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete /*+ hint */ from issue where id = :0 returning title into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    //             .executeDelete()
    //
    //         expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  delete /*+ hint */ from issue using project where issue.project_id = project.id and project.id = :0  /* tail */"`)
    //         expect(ctx.lastParams).toMatchInlineSnapshot(`
    //           [
    //             99999,
    //           ]
