...
describe(ctx.label, () => {
    ...
    test('starts-with-if-value-and-ends-with-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title like ($1 || '%') and title like ('%' || $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Migrate",
            "ESM",
    ...
    test('not-starts-with-if-value-and-not-ends-with-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not like ($1 || '%') and title not like ('%' || $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Update",
            "users",
    ...
    test('not-contains-if-value-fires-and-elides', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not like ('%' || $1 || '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "nav",
          ]
    ...
    test('replace-all-if-value-fires-with-both-args', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(status, $1, $2) as masked from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "o",
            "0",
