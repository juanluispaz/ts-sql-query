...
describe(ctx.label, () => {
    ...
    test('order-by-value-source-column-asc-nulls-last', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy(tIssue.id, 'asc')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", assignee_id as "assigneeId" from issue order by issue.assignee_id asc nulls last, issue.id asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('order-by-value-source-column-asc-nulls-first', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy(tIssue.id, 'asc')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", assignee_id as "assigneeId" from issue order by issue.assignee_id asc nulls first, issue.id asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; assigneeId?: number }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('order-by-value-source-column-desc-nulls-first', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy(tIssue.id, 'asc')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", assignee_id as "assigneeId" from issue order by issue.assignee_id desc nulls first, issue.id asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; assigneeId?: number }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('order-by-value-source-column-desc-nulls-last', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy(tIssue.id, 'asc')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", assignee_id as "assigneeId" from issue order by issue.assignee_id desc nulls last, issue.id asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; assigneeId?: number }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('order-by-value-source-column-asc-nulls-first-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", title as "title" from issue order by lower(issue.title) asc nulls first"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-value-source-column-asc-nulls-last-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", title as "title" from issue order by lower(issue.title) asc nulls last"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-value-source-column-desc-nulls-first-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", title as "title" from issue order by lower(issue.title) desc nulls first"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-value-source-column-desc-nulls-last-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", title as "title" from issue order by lower(issue.title) desc nulls last"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
})
