...
describe(ctx.label, () => {
    ...
    test('force-type-cast-adapter-in-insert-values-position', async () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"insert into project (id, name) values (:0, :1)"`)
    ...
    test('force-type-cast-adapter-in-update-set-position', async () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"update project set name = :0 where id = :1"`)
        expect(built.params()).toMatchInlineSnapshot(`
          [
            "Renamed",
            1,
