...
async function main() {
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id, first_name || $1 || last_name as name, birthday as birthday from customer where first_name like ('%' || $2 || '%') order by lower(first_name || $3 || last_name), birthday asc nulls last`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`with recursive recursive_select_1 as (select id as id, name as name, parent_id as parentId from company where id = $1 union all select company.id as id, company.name as name, company.parent_id as parentId from company join recursive_select_1 on recursive_select_1.parentId = company.id) select id as id, name as name, parentId as "parentId" from recursive_select_1`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectManyRows`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as "result.id", first_name as "result.firstName", last_name as "result.lastName", birthday as "result.birthday", company_id as "result.companyId", first_name || $1 || last_name as "result.name", calculateAge(birthday) as "result.age" from customer where id = $2`)
    expectedParams.push(`[" ",9]`)
    expectedType.push(`selectOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select customer.first_name as "firstName", customer.last_name as "lastName", customer.birthday as birthday from customer join company on company.id = customer.company_id where company.name = $1`)
    expectedParams.push(`["My company name"]`)
    expectedType.push(`selectManyRows`)
    ...
    const companyTreeWithFavouriteCustomer = await connection.selectFrom(tCompany)
        ...
        .recursiveUnionAll((child) => {
            child
            const a =  connection.selectFrom(tCompany)
                .optionalLeftJoin(tCustomerLeftJoin).on(tCompany.favouriteCustomerId.equals(tCustomerLeftJoin.id))
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name, favouriteCustomer.first_name || $1 || favouriteCustomer.last_name as "favouriteCustomer.name" from company left outer join customer as favouriteCustomer on company.parent_id = favouriteCustomer.id where exists(select id as result from customer where (first_name || $2 || last_name) ilike ('%' || $3 || '%')) and company.parent_id = $4`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name from company left outer join customer as favouriteCustomer on company.parent_id = favouriteCustomer.id where favouriteCustomer.id in (select parent_id as result from company where name ilike ('%' || $1 || '%')) and company.parent_id = $2`)
    ...
    result = undefined
    expectedResult.push(result)
    expectedQuery.push(`call myOwnprocedure($1)`)
    expectedParams.push(`[10]`)
    ...
    result = 20
    expectedResult.push(result)
    expectedQuery.push(`select myOwnFunction($1)`)
    expectedParams.push(`[10]`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id from customer where (first_name collate "C") = $1`)
    ...
    const caseSensitiveMatch = await connection.selectFrom(tCustomer)
        .where(tCustomer.firstName.collate('C').equals('ADA'))
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, regexp_replace(first_name, $1, $2, 'gi') as cleaned from customer where id = $3`)
