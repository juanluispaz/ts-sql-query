    ...
    */
    ...
    test('delete-using-table-then-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
