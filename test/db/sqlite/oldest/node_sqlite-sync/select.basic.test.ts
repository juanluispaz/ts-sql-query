...
describe(ctx.label, () => {
    ...
    test('list-organizations-ordered-by-name', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tOrganization)
            .select({
                id:   tOrganization.id,
    ...
    test('executable-select-query-and-params-accessors', async () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"select id as id, name as name from project where id = ? order by id"`)
    ...
    test('select-params-before-query-builds-on-demand-and-query-cache-holds', () => {
        ...
        expect(first).toMatchInlineSnapshot(`"select id as id, name as name from project where id = ? order by id"`)
