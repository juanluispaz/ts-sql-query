...
describe(ctx.label, () => {
    ...
    test('custom-numeric/double-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (priority * 1.0e0) + ? as sum, (priority * 1.0e0) - ? as diff, (priority * 1.0e0) * ? as prod from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/customdouble-operation1-math', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, power(?, ?) as \`p\`, log(?, ?) as ln, round(cast(? as decimal(65, 30)), ?) as rn, (? * 1.0e0) / ? as di, atan2(?, ?) as at2 from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/customint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cost_cents + ? as \`a\`, cost_cents - ? as \`s\`, cost_cents * ? as mu, cost_cents % cost_cents as mo, greatest(cost_cents, ?) as mn, least(cost_cents, ?) as mx, sign(cost_cents) as sg from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/customdouble-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, billed_amount + billed_amount as \`a\`, billed_amount - ? as \`s\`, billed_amount * ? as mu, greatest(billed_amount, ?) as mn, least(billed_amount, ?) as mx, abs(billed_amount) as ab, sign(billed_amount) as sg from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('custom-numeric/brand-survival-customint-rounding', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceil(?) as \`c\`, floor(?) as \`f\`, round(?) as \`r\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            6,
    ...
    test('custom-numeric/brand-survival-customint-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(?) + ? as \`a\`, round(?) - ? as \`s\`, round(?) * ? as mu, round(?) % ? as mo from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            4,
    ...
    test('custom-numeric/brand-survival-customint-nullhandling', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ifnull(?, ?) as vwn, nullif(?, ?) as niv from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            0,
    ...
    test('custom-numeric/brand-survival-customdouble-power-logn-atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, power(?, ?) as \`p\`, log(?, ?) as lg, atan2(?, ?) as at2 from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-divide-modulo', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (? * 1.0e0) / ? as di, ? % ? as mo from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/adapter-column-through-non-add-arithmetic', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, optional_release_ordinal - ? as optSub, release_ordinal * ? as reqMul from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('custom-numeric/optional-customdouble-receiver-through-multiply', async () => {
        ...
        const result = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, budget * ? as \`v\` from release_draft where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/minmax-optional-customdouble-receiver-poisons', async () => {
        ...
        const result = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, greatest(budget, ?) as mn, least(budget, ?) as mx from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            100,
