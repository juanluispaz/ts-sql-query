...
describe(ctx.label, () => {
    ...
    test('null-inline-aggregated-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, null as direct, null as kept, null as dropped, null as ignored, null as keptIgnored from project where id = ?"`)
    ...
    test('null-aggregate-as-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, null as kept, null as dropped, null as ignored, null as keptIgnored from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
