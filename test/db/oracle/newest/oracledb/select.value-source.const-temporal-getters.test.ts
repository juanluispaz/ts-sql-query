...
describe(ctx.label, () => {
    ...
    test('const-localdate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from :0) as "y", extract(month from :1) - 1 as "mo", extract(day from :2) as "d", mod(trunc(:3) - trunc(:4, 'IW') + 1, 7) as "dow" from dual"`)
    ...
    test('const-localtime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from :0) as "h", extract(minute from :1) as "m", trunc(extract(second from :2)) as "s" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('const-localtime-milliseconds', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select to_number(to_char(:0, 'FF3')) as "ms" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1970-01-01T12:34:56.000Z,
          ]
    ...
    test('const-localdatetime-more-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(day from :0) as "d", mod(trunc(:1) - trunc(:2, 'IW') + 1, 7) as "dow", extract(minute from :3) as "m", trunc(extract(second from :4)) as "s", to_number(to_char(:5, 'FF3')) as "ms" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2024-01-15T12:34:56.000Z,
            2024-01-15T12:34:56.000Z,
    ...
    test('chained-valuewhennull-localdate-getmonth', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(month from nvl(work_date, :0)) - 1 as "mo" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('chained-nullifvalue-localdate-getmonth', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(month from nullif(work_date, :0)) - 1 as "mo" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2000-01-01T00:00:00.000Z,
            1,
    ...
    test('chained-asrequiredinoptionalobject-localdate-getmonth', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(month from review_date) - 1 as "mo" from project_review where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('chained-onlywhenornull-localtime-gethours', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from started_at) as "h" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('chained-modifiers-customlocaldatetime-getfullyear', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from nvl(signed_off_at, :0)) as "vwn", extract(year from nullif(signed_off_at, :1)) as "nif", extract(year from signed_off_at) as "rio", extract(year from signed_off_at) as "own", extract(year from signed_off_at) as "ign" from project_release where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2000-01-01T00:00:00.000Z,
    ...
    test('optional-const-localtime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from :0) as "h", extract(minute from :1) as "m", trunc(extract(second from :2)) as "s", to_number(to_char(:3, 'FF3')) as "ms" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1970-01-01T12:34:56.000Z,
            1970-01-01T12:34:56.000Z,
