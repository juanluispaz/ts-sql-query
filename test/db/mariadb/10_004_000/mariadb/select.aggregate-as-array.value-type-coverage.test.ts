...
describe(ctx.label, () => {
    ...
    */
    ...
    test('aggregate-of-optional-uuid-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(json_arrayagg(external_ref) as char) as refs from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    */
    ...
    test('aggregate-of-object-with-bigint-uuid-and-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(json_arrayagg(json_object('views', view_count, 'externalRef', external_ref, 'estimatedHours', estimated_hours)) as char) as issues from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB types `forUseAsInlineAggregatedArrayValue` as `never` for an inline aggregate carrying `group by` (also `having`/`compound`/`distinct`) — MariaDB allows no outer references in an inner FROM (no LATERAL), so this correlated wrapped-object shape is forbidden by design. A MariaDB user can never build it; casting past the guard only emits SQL MariaDB rejects at execution (ER_BAD_FIELD_ERROR: Unknown column 'project.id'). Body kept (canonical shape) for cross-cell diff parity per the symmetry rule.
    /*
...
void assertType; void expect; void tIssue; void tIssueWorklog; void tProject; void tProjectReview; void test; void vReleaseOverview
