...
describe(ctx.label, () => {
    ...
    test('shaped-set-if-set-if-value-keeps-staged-slug-when-new-value-missing-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (?, ?, ?) on duplicate key update name = ?, slug = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
    ...
    test('shaped-extend-shape-then-set-adds-a-renamed-key-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (?, ?, ?) on duplicate key update name = ?, archived_at = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
