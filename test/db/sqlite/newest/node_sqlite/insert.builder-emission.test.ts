...
describe(ctx.label, () => {
    ...
    test('rawfragment-embeds-all-four-insert-forms', () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"select /* audit: insert into issue (project_id, number, title, status, priority) select project_id as projectId, number as number, title as title, status as status, priority as priority from issue where status = ? | insert into issue (project_id, number, title, status, priority) values (?, ?, ?, ?, ?), (?, ?, ?, ?, ?) | insert into ledger_entry default values | insert into issue (project_id, number, title, status, priority) values (?, ?, ?, ?, ?) */ id as id from project where id = ?"`)
        expect(built.params()).toMatchInlineSnapshot(`
          [
            "open",
            1,
