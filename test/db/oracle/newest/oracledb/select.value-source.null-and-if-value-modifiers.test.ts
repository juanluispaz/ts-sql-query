...
describe(ctx.label, () => {
    ...
    test('valueWhenNull-numeric-with-literal-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(assignee_id, :0) as "owner" from issue order by "id""`)
    ...
    test('valueWhenNull-int-receiver-widens-to-a-fractional-double-fallback', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(assignee_id, priority / :0) as "w" from issue order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('nullIfValue-string-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(status, :0) as "statusOrNull" from issue order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "closed",
          ]
    ...
    test('temporal/local-date-null-if-value-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(work_date, :0) as "d" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/local-time-value-when-null-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(started_at, :0) as "t" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/local-date-time-null-if-value-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(created_at, created_at) as "d" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/custom-local-date-null-if-value-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(released_on, :0) as "d" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/custom-local-date-time-value-when-null-literal-on-null-row', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(signed_off_at, :0) as "x" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/custom-local-time-value-when-null-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(cutoff_time, cutoff_time) as "x" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/local-date-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(work_date, :0) as "d", nvl(work_date, work_date) as "d2" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/local-date-time-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(created_at, :0) as "d", nvl(created_at, created_at) as "d2" from "organization" where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            1,
    ...
    test('temporal/custom-local-date-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(released_on, :0) as "d", nvl(released_on, released_on) as "d2" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            2,
    ...
    test('temporal/local-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(started_at, :0) as "t", nullif(started_at, started_at) as "t2" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1970-01-01T08:00:00.000Z,
            1,
    ...
    test('temporal/custom-local-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(cutoff_time, :0) as "t", nullif(cutoff_time, cutoff_time) as "t2" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/custom-local-date-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(signed_off_at, :0) as "x", nullif(signed_off_at, signed_off_at) as "x2" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            1,
    ...
    test('boolean/value-when-null-and-null-if-value-on-plain-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(billable, :0) as "bwn", nullif(billable, :1) as "bni" from issue_worklog order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            0,
    ...
    test('boolean/value-when-null-and-null-if-value-on-custom-boolean-remaps-in-coalesce', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(case approved when 'A' then 1 when 'R' then 0 else null end, :0) as "awn", nullif(case approved when 'A' then 1 when 'R' then 0 else null end, :1) as "ani" from issue_worklog order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            0,
    ...
    test('boolean/value-when-null-and-null-if-value-with-value-source-double-remap', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(case approved when 'A' then 1 when 'R' then 0 else null end, case when invoiced = 1 then 1 else 0 end) as "awn", nullif(case approved when 'A' then 1 when 'R' then 0 else null end, case when invoiced = 1 then 1 else 0 end) as "ani" from issue_worklog order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; awn: boolean; ani?: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('boolean/if-value-source-and-or-with-concrete-boolean-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when status = :0 and priority > :1 then 1 else 0 end as "andFlag", case when status = :2 or priority > :3 then 1 else 0 end as "orFlag" from issue where id in (:4, :5, :6) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            1,
    ...
    test('null-modifiers-on-custom-comparable-version-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(version, :0) as "v", nullif(version, :1) as "vni", version as "vopt" from project_release where version is not null order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0.0.0",
            "0.9.0",
    ...
    test('null-modifiers-on-custom-channel-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(channel, :0) as "c", nullif(channel, :1) as "cni", channel as "copt" from project_release where channel is not null order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "stable",
            "canary",
    ...
    test('null-modifiers-on-enum-activity-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(activity, :0) as "a", nullif(activity, :1) as "ani", activity as "aopt" from issue_worklog where activity is not null order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "coding",
            "meeting",
    ...
    test('null-modifiers-on-optional-enum-hits-real-null-branch', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(stage, :0) as "coalesced", nullif(stage, :1) as "nulled", stage as "opt" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "draft",
            "final",
    ...
    test('null-modifiers-on-optional-custom-hits-real-null-branch', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(channel, :0) as "coalesced", nullif(channel, :1) as "nulled", channel as "opt" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "stable",
            "canary",
    ...
    test('null-modifiers-on-optional-custom-comparable-hits-real-null-branch', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(min_version, :0) as "coalesced", nullif(min_version, :1) as "nulled", min_version as "opt" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0.0.0",
            "9.9.9",
    ...
    test('is-null-is-not-null-on-optional-custom-double', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when budget is null then 1 else 0 end as "n", case when budget is not null then 1 else 0 end as "nn" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('is-null-is-not-null-on-optional-custom-local-date', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when target_day is null then 1 else 0 end as "n", case when target_day is not null then 1 else 0 end as "nn" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; n: boolean; nn: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('is-null-is-not-null-on-optional-custom-local-time', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when cutoff is null then 1 else 0 end as "n", case when cutoff is not null then 1 else 0 end as "nn" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; n: boolean; nn: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('value-source-null-modifiers-on-optional-enum-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(stage, stage) as "cvs", nullif(stage, stage) as "nvs" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('value-source-null-modifiers-on-optional-custom-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(channel, channel) as "cvs", nullif(channel, channel) as "nvs" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('value-source-null-modifiers-on-optional-custom-comparable-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(min_version, min_version) as "cvs", nullif(min_version, min_version) as "nvs" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('modifier-trio-on-enum-activity-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", activity as "req", null as "own", null as "ign" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-channel-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", channel as "req", null as "own", null as "ign" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-comparable-version-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", version as "req", null as "own", null as "ign" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when published = 't' then 1 else 0 end as "req", null as "own", null as "ign" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-plain-temporal-leaves', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", work_date as "dReq", null as "dOwn", null as "dIgn", started_at as "tReq", null as "tOwn", null as "tIgn" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('modifier-trio-on-custom-temporal-leaves', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", released_on as "dReq", null as "dOwn", null as "dIgn", cutoff_time as "tReq", null as "tOwn", null as "tIgn", signed_off_at as "dtReq", null as "dtOwn", null as "dtIgn" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-plain-local-date-time-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", created_at as "req", null as "own", null as "ign" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-int-and-double-leaves', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cost_cents as "cReq", null as "cOwn", null as "cIgn", billed_amount as "bReq", null as "bOwn", null as "bIgn" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/local-date-null-if-value-source', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(work_date, work_date) as "d" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/local-time-value-when-null-source', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(started_at, started_at) as "t" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/custom-local-date-null-if-value-source', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(released_on, released_on) as "d" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal/custom-local-time-value-when-null-literal', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(cutoff_time, :0) as "x" from project_release where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1970-01-01T08:00:00.000Z,
            1,
    ...
    test('temporal/local-date-time-null-if-value-literal', async () => {
        ...
        const row = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nullif(created_at, :0) as "d" from "organization" where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/local-date-is-null-is-not-null', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when review_date is null then 1 else 0 end as "n", case when review_date is not null then 1 else 0 end as "nn" from project_review where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('asOptional-on-boolean-and-plain-temporal-leaves', async () => {
        ...
        const row1 = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", billable as "b", work_date as "d", started_at as "t" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
        ...
        const row2 = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", created_at as "c" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('null-modifiers-on-verified-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(case when verified = 'Y' then 1 else 0 end, :0) as "wn", nullif(case when verified = 'Y' then 1 else 0 end, :1) as "ni" from "organization" order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('null-modifiers-on-published-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(case when published = 't' then 1 else 0 end, :0) as "wn", nullif(case when published = 't' then 1 else 0 end, :1) as "ni" from project order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('null-modifiers-on-invoiced-required-numeric-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(case when invoiced = 1 then 1 else 0 end, :0) as "wn", nullif(case when invoiced = 1 then 1 else 0 end, :1) as "ni" from issue_worklog order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('modifier-trio-on-verified-required-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when verified = 'Y' then 1 else 0 end as "req", null as "own", null as "ign" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-approved-nullable-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case approved when 'A' then 1 when 'R' then 0 else null end as "req", null as "own", null as "ign" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-invoiced-required-numeric-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when invoiced = 1 then 1 else 0 end as "req", null as "own", null as "ign" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-plain-double-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(priority as float) as "req", null as "own", null as "ign" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
