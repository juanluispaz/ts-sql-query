...
describe(ctx.label, () => {
    ...
    test('like-insensitive-and-not-like-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where lower(title) like lower(:0) escape '\\' and lower(title) not like lower(:1) escape '\\' order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%TRIAGE%",
            "%DRAFT%",
    ...
    test('starts-with-ends-with-contains', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where title like (:0 || '%') escape '\\' and title like ('%' || :1) escape '\\' and title like ('%' || :2 || '%') escape '\\' order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Re",
            "me",
    ...
    test('not-starts-with-not-ends-with-not-contains', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where title not like (:0 || '%') escape '\\' and title not like ('%' || :1) escape '\\' and title not like ('%' || :2 || '%') escape '\\' order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Re",
            "me",
    ...
    test('nested-and-or-not', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where status = :0 and (priority > :1 or not (title like ('%' || :2 || '%') escape '\\')) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            5,
