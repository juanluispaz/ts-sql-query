...
describe(ctx.label, () => {
    ...
    test('select-distinct-from-table', async () => {
        ...
        const rows = sync(ctx.conn.selectDistinctFrom(tIssue)
    ...
    test('select-distinct-with-join-and-where', async () => {
        ...
        const rows = sync(ctx.conn.selectDistinctFrom(tProject)
    ...
    test('distinct-source-consumed-as-user-cte', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(distinctStatuses)
