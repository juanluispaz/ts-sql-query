...
describe(ctx.label, () => {
    ...
    test('has-aggregation/value-when-no-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when count(id) > @0 then 1 else 0 end as bit) as busy from issue group by project_id order by projectId"`)
    ...
    test('has-aggregation/value-when-no-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id > @0 then 1 else 0 end as bit) as busy from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('has-aggregation/in-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when count(id) in (@0, @1) then 1 else 0 end as bit) as busy from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/in-n-with-an-aggregating-list-element-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id in (@0, count(id)) then 1 else 0 end as bit) as busy from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/in-n-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id in (@0, @1) then 1 else 0 end as bit) as busy from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('has-aggregation/in-with-a-non-aggregating-subquery-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where project_id in (select id as [result] from project where slug = @0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mktg-site",
          ]
    ...
    test('has-aggregation/value-when-null-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, coalesce(max(assignee_id), @0) as lead from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('has-aggregation/value-when-null-with-an-aggregating-replacement-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where coalesce(assignee_id, (select count(id) as [result] from project)) = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('has-aggregation/value-when-null-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where coalesce(assignee_id, (select id as [result] from project where slug = @0)) = @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "legacy",
            4,
    ...
    test('has-aggregation/is-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when case when max(assignee_id) = @0 then 1 else 0 end = 1 then 1 when not (case when max(assignee_id) = @1 then 1 else 0 end = 1) then 0 else null end as bit) as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            3,
    ...
    test('has-aggregation/is-with-an-aggregating-operand-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when case when project_id = min(id) then 1 else 0 end = 1 then 1 when not (case when project_id = min(id) then 1 else 0 end = 1) then 0 else null end as bit) as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ projectId: number; matches: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('has-aggregation/is-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id = @0 then 1 else 0 end as bit) as matches from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/in-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId from issue group by project_id having count(id) in (@0, @1) order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/if-value-comparisons-with-neither-receiver-aggregating-report-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId from issue group by project_id having project_id > @0 and project_id in (@1, @2, @3) order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('has-aggregation/concat-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (max(title) + @0) = @1 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
    ...
    test('has-aggregation/concat-if-value-with-a-missing-value-and-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (max(title)) = @0 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Redesign navbar",
          ]
    ...
    test('has-aggregation/concat-if-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (title + @0) = @1 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
    ...
    test('has-aggregation/between-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when count(id) between @0 and @1 then 1 else 0 end as bit) as inRange from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/between-with-an-aggregating-lower-bound-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id between count(id) and @0 then 1 else 0 end as bit) as inRange from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
          ]
    ...
    test('has-aggregation/between-with-an-aggregating-upper-bound-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id between @0 and count(id) then 1 else 0 end as bit) as inRange from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/between-with-no-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, cast(case when project_id between @0 and @1 then 1 else 0 end as bit) as inRange from issue group by project_id order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            5,
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (replace(max(title) collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT) = @2 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
            "header",
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-replacement-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (replace(title collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, max(status)) collate DATABASE_DEFAULT) = @1 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-searched-text-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as name from project group by name having (replace(name collate Latin1_General_BIN2, max(slug) collate Latin1_General_BIN2, @0) collate DATABASE_DEFAULT) = @1 order by name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "squad",
            "Internal squad",
    ...
    test('has-aggregation/replace-all-if-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (replace(title collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT) = @2 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
            "header",
