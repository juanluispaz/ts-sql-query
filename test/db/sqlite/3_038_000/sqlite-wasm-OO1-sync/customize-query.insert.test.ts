...
describe(ctx.label, () => {
    ...
    test('customize-insert-before-query-emits-leading-comment', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = sync(connection.insertInto(tProject)
    ...
    test('customize-insert-hook-fragment-with-column-reference', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const inserted = sync(connection.insertInto(tIssue)
    ...
    test('customize-insert-returning-object-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const row = sync(connection.insertInto(tProject)
                .values({ name: 'Mobile app', slug: 'mobile-app', organizationId: 1 })
                .returning({
                    organizationId: tProject.organizationId,
                    name:           tProject.name,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) values (?, ?, ?) returning organization_id as organizationId, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    test('customize-insert-on-conflict-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) values (?, ?, ?) on conflict do nothing returning organization_id as organizationId, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    test('customize-insert-returning-one-column-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const slug = sync(connection.insertInto(tProject)
                .values({ name: 'Mobile app', slug: 'mobile-app', organizationId: 1 })
                .returningOneColumn(tProject.slug)
                .customizeQuery({ afterInsertKeyword: connection.rawFragment`/*+ hint */` })
                .executeInsertOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) values (?, ?, ?) returning slug as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    test('customize-insert-returning-last-inserted-id-with-hooks', async () => {
        ...
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const id = sync(connection.insertInto(tProject)
                .values({ name: 'Mobile app', slug: 'mobile-app', organizationId: 1 })
                .returningLastInsertedId()
                .customizeQuery({ afterInsertKeyword: connection.rawFragment`/*+ hint */` })
                .executeInsert())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (name, slug, organization_id) values (?, ?, ?) returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mobile app",
                "mobile-app",
    ...
    test('customize-insert-on-conflict-do-update-returning-object-with-hooks', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? returning id as id, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('customize-insert-on-conflict-do-update-returning-one-column-none-or-one', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? returning slug as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('customize-insert-on-conflict-do-update-from-select-returning', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const rows = sync(connection.insertInto(tProject)
                .from(source)
                .onConflictOn(tProject.organizationId, tProject.slug)
                .doUpdateSet({ name: 'Reactivated' })
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ hint */ into project (organization_id, slug, name) select organization_id as organizationId, slug as slug, name as name from project where id = ? on conflict (organization_id, slug) do update set name = ? returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Reactivated",
    ...
    test('customize-insert-default-values-with-hooks', async () => {
        // The three RawFragment hooks on an INSERT … DEFAULT VALUES: the fragments
        // land around the insert keyword and after the statement, alongside the
        // DEFAULT VALUES clause. `tLedgerEntry`'s columns all carry DB defaults /
        // are autogenerated, so `defaultValues()` is a valid insert.
        ctx.mockNext(1)
        const connection = ctx.conn
        await ctx.withRollback(async () => {
            const affected = sync(connection.insertInto(tLedgerEntry)
                .defaultValues()
                .customizeQuery({
                    beforeQuery:        connection.rawFragment`/* head */ `,
                    afterInsertKeyword: connection.rawFragment`/*+ hint */`,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  insert /*+ hint */ into ledger_entry default values  /* tail */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
