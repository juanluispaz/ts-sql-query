...
describe(ctx.label, () => {
    ...
    test('equivalence/is-and-is-not', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where priority = @0 and status <> @1 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2,
            "closed",
    ...
    test('equivalence/like-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where title like @0 and title not like @1 and lower(title) like lower(@2) and lower(title) not like lower(@3) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "%a%",
            "%b%",
    ...
    test('equivalence/affix-sensitive', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where title like (@0 + '%') and title not like (@1 + '%') and title like ('%' + @2) and title not like ('%' + @3) and title like ('%' + @4 + '%') and title not like ('%' + @5 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    test('equivalence/affix-insensitive', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) like lower(@0 + '%') and lower(title) not like lower(@1 + '%') and lower(title) like lower('%' + @2) and lower(title) not like lower('%' + @3) and lower(title) like lower('%' + @4 + '%') and lower(title) not like lower('%' + @5 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    test('equivalence/boolean-column-custom-adapter-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project where cast(case when published = 't' then 1 else 0 end as bit) = @0 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('equivalence/uuid-as-string-operator-path', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref like ('%' + @0 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "abc",
          ]
    ...
    test('equivalence/if-value-equalable-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where priority = @0 and priority <> @1 and priority = @2 and priority <> @3 and priority in (@4, @5) and priority not in (@6) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('equivalence/if-value-string-equality-and-like-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) = lower(@0) and lower(title) <> lower(@1) and title like @2 and title not like @3 and lower(title) like lower(@4) and lower(title) not like lower(@5) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "Foo",
            "Bar",
    ...
    test('equivalence/custom-uuid-descriptor-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key = @0 and signing_key like ('%' + @1 + '%') and signing_key like (@2 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
    ...
    test('equivalence/is-and-is-not-with-explicit-null', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where body is null and estimated_hours is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('equivalence/boolean-custom-adapter-notequals-and-in-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project where cast(case when published = 't' then 1 else 0 end as bit) <> @0 and cast(case when published = 't' then 1 else 0 end as bit) in (@1, @2) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            true,
    ...
    test('equivalence/boolean-plain-column-is-and-is-null-dispatch', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when billable = @0 then 1 else 0 end = 1 and billable is null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('dyn/bigint-descriptor-equalable-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms = @0 and duration_ms <> @1 and case when duration_ms = @2 then 1 else 0 end = 1 and not (case when duration_ms = @3 then 1 else 0 end = 1) and duration_ms in (@4, @5) and duration_ms not in (@6) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            20n,
    ...
    test('dyn/bigint-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms = @0 and duration_ms <> @1 and case when duration_ms = @2 then 1 else 0 end = 1 and not (case when duration_ms = @3 then 1 else 0 end = 1) and duration_ms in (@4, @5) and duration_ms not in (@6) and duration_ms < @7 and duration_ms >= @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            20n,
    ...
    test('dyn/double-descriptor-equalable-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours = @0 and estimated_hours <> @1 and case when estimated_hours = @2 then 1 else 0 end = 1 and estimated_hours in (@3, @4) and estimated_hours not in (@5) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('dyn/double-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours = @0 and estimated_hours <> @1 and case when estimated_hours = @2 then 1 else 0 end = 1 and estimated_hours in (@3, @4) and estimated_hours not in (@5) and estimated_hours < @6 and estimated_hours <= @7 and estimated_hours >= @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('dyn/boolean-descriptor-isnot-notin-isnotnull', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when billable = @0 then 1 else 0 end = 1) and billable not in (@1, @2) and billable is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            true,
    ...
    test('dyn/boolean-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable = @0 and billable <> @1 and case when billable = @2 then 1 else 0 end = 1 and not (case when billable = @3 then 1 else 0 end = 1) and billable in (@4, @5) and billable not in (@6) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('dyn/enum-descriptor-is-isnot-notin-nullable', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where activity = @0 and activity <> @1 and activity not in (@2) and activity is null and activity is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "coding",
            "review",
    ...
    test('dyn/enum-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where activity = @0 and activity <> @1 and activity = @2 and activity <> @3 and activity in (@4, @5) and activity not in (@6) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "coding",
            "review",
    ...
    test('dyn/localdate-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date = @0 and work_date <> @1 and work_date = @2 and work_date <> @3 and work_date in (@4, @5) and work_date not in (@6) and work_date <= @7 and work_date > @8 and work_date is null and work_date is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
    ...
    test('dyn/localdate-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date = @0 and work_date <> @1 and work_date = @2 and work_date <> @3 and work_date in (@4, @5) and work_date not in (@6) and work_date <= @7 and work_date > @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
            2024-04-01T00:00:00.000Z,
    ...
    test('dyn/localtime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at = @0 and started_at <> @1 and case when started_at = @2 then 1 else 0 end = 1 and not (case when started_at = @3 then 1 else 0 end = 1) and started_at in (@4, @5) and started_at not in (@6) and started_at <= @7 and started_at > @8 and started_at is null and started_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
    ...
    test('dyn/localtime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at = @0 and started_at <> @1 and case when started_at = @2 then 1 else 0 end = 1 and not (case when started_at = @3 then 1 else 0 end = 1) and started_at in (@4, @5) and started_at not in (@6) and started_at <= @7 and started_at > @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            1970-01-01T08:00:00.000Z,
            1970-01-01T12:00:00.000Z,
    ...
    test('dyn/localdatetime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where created_at = @0 and created_at <> @1 and created_at = @2 and created_at <> @3 and created_at in (@4, @5) and created_at not in (@6) and created_at < @7 and created_at <= @8 and created_at > @9 and created_at is null and created_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2020-01-01T00:00:00.000Z,
    ...
    test('dyn/localdatetime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where created_at = @0 and created_at <> @1 and created_at = @2 and created_at <> @3 and created_at in (@4, @5) and created_at not in (@6) and created_at < @7 and created_at <= @8 and created_at > @9 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2020-01-01T00:00:00.000Z,
            2021-06-15T12:00:00.000Z,
    ...
    test('dyn/customint-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents = @0 and cost_cents <> @1 and cost_cents = @2 and cost_cents <> @3 and cost_cents in (@4, @5) and cost_cents not in (@6) and cost_cents <= @7 and cost_cents >= @8 and cost_cents is null and cost_cents is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10,
            20,
    ...
    test('dyn/customint-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents = @0 and cost_cents <> @1 and cost_cents = @2 and cost_cents <> @3 and cost_cents in (@4, @5) and cost_cents not in (@6) and cost_cents <= @7 and cost_cents >= @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10,
            20,
    ...
    test('dyn/customdouble-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount = @0 and billed_amount <> @1 and billed_amount = @2 and billed_amount <> @3 and billed_amount in (@4, @5) and billed_amount not in (@6) and billed_amount <= @7 and billed_amount >= @8 and billed_amount is null and billed_amount is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10.5,
            20.5,
    ...
    test('dyn/customdouble-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount = @0 and billed_amount <> @1 and billed_amount = @2 and billed_amount <> @3 and billed_amount in (@4, @5) and billed_amount not in (@6) and billed_amount <= @7 and billed_amount >= @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10.5,
            20.5,
    ...
    test('dyn/customcomparable-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where version <> @0 and version = @1 and version <> @2 and version in (@3, @4) and version not in (@5) and version <= @6 and version >= @7 and version is null and version is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "2.0.0",
            "1.5.0",
    ...
    test('dyn/customcomparable-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where version = @0 and version <> @1 and version = @2 and version <> @3 and version not in (@4) and version < @5 and version > @6 and version >= @7 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "1.2.0",
            "2.0.0",
    ...
    test('dyn/custom-descriptor-equalable-nullable', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where channel = @0 and channel = @1 and channel <> @2 and channel not in (@3) and channel is null and channel is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "stable",
            "beta",
    ...
    test('dyn/custom-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where channel = @0 and channel = @1 and channel <> @2 and channel not in (@3) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "stable",
            "beta",
    ...
    test('dyn/customlocaldate-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on = @0 and released_on <> @1 and released_on = @2 and released_on <> @3 and released_on in (@4, @5) and released_on not in (@6) and released_on <= @7 and released_on > @8 and released_on is null and released_on is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T10:00:00.000Z,
    ...
    test('dyn/customlocaldate-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on = @0 and released_on <> @1 and released_on = @2 and released_on <> @3 and released_on in (@4, @5) and released_on not in (@6) and released_on <= @7 and released_on > @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T10:00:00.000Z,
            2024-06-01T10:00:00.000Z,
    ...
    test('dyn/customlocaltime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time = @0 and cutoff_time <> @1 and cutoff_time = @2 and cutoff_time <> @3 and cutoff_time in (@4, @5) and cutoff_time not in (@6) and cutoff_time <= @7 and cutoff_time > @8 and cutoff_time is null and cutoff_time is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            1970-01-01T08:00:00.000Z,
            1970-01-01T12:00:00.000Z,
    ...
    test('dyn/customlocaltime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time = @0 and cutoff_time <> @1 and cutoff_time = @2 and cutoff_time <> @3 and cutoff_time in (@4, @5) and cutoff_time not in (@6) and cutoff_time <= @7 and cutoff_time > @8 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            1970-01-01T08:00:00.000Z,
            1970-01-01T12:00:00.000Z,
    ...
    test('dyn/customlocaldatetime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at = @0 and signed_off_at <> @1 and case when signed_off_at = @2 then 1 else 0 end = 1 and not (case when signed_off_at = @3 then 1 else 0 end = 1) and signed_off_at in (@4, @5) and signed_off_at not in (@6) and signed_off_at <= @7 and signed_off_at > @8 and signed_off_at is null and signed_off_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
    ...
    test('dyn/uuid-descriptor-core-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = @0 and external_ref <> @1 and case when external_ref = @2 then 1 else 0 end = 1 and not (case when external_ref = @3 then 1 else 0 end = 1) and external_ref in (@4, @5) and external_ref not in (@6) and external_ref < @7 and external_ref <= @8 and external_ref > @9 and external_ref >= @10 and external_ref is null and external_ref is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
    ...
    test('dyn/uuid-descriptor-string-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where lower(external_ref) = lower(@0) and lower(external_ref) <> lower(@1) and external_ref like @2 and external_ref not like @3 and lower(external_ref) like lower(@4) and lower(external_ref) not like lower(@5) and external_ref like (@6 + '%') and external_ref not like (@7 + '%') and external_ref like ('%' + @8) and external_ref not like ('%' + @9) and external_ref like ('%' + @10 + '%') and external_ref not like ('%' + @11 + '%') and external_ref like (@12 + '%') and external_ref not like (@13 + '%') and external_ref like ('%' + @14) and external_ref not like ('%' + @15) and external_ref like ('%' + @16 + '%') and external_ref not like ('%' + @17 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0A8F",
            "FFFF",
    ...
    test('dyn/uuid-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = @0 and external_ref <> @1 and case when external_ref = @2 then 1 else 0 end = 1 and not (case when external_ref = @3 then 1 else 0 end = 1) and external_ref in (@4, @5) and external_ref not in (@6) and external_ref < @7 and external_ref <= @8 and external_ref > @9 and external_ref >= @10 and external_ref like @11 and external_ref like ('%' + @12 + '%') and external_ref like (@13 + '%') and lower(external_ref) = lower(@14) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556661",
            "0a8f9c1e-1111-4222-8333-444455556662",
    ...
    test('dyn/customuuid-descriptor-core-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key <> @0 and case when signing_key = @1 then 1 else 0 end = 1 and not (case when signing_key = @2 then 1 else 0 end = 1) and signing_key in (@3, @4) and signing_key not in (@5) and signing_key < @6 and signing_key > @7 and signing_key <= @8 and signing_key >= @9 and signing_key is null and signing_key is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556662",
            "0a8f9c1e-1111-4222-8333-444455556663",
    ...
    test('dyn/customuuid-descriptor-string-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key like @0 and signing_key not like @1 and lower(signing_key) like lower(@2) and lower(signing_key) not like lower(@3) and signing_key not like (@4 + '%') and signing_key like ('%' + @5) and signing_key not like ('%' + @6) and signing_key not like ('%' + @7 + '%') and signing_key like (@8 + '%') and signing_key not like (@9 + '%') and signing_key like ('%' + @10) and signing_key not like ('%' + @11) and signing_key like ('%' + @12 + '%') and signing_key not like ('%' + @13 + '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "%1111%",
            "%9999%",
    ...
    test('dyn/customuuid-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key = @0 and signing_key <> @1 and case when signing_key = @2 then 1 else 0 end = 1 and not (case when signing_key = @3 then 1 else 0 end = 1) and signing_key in (@4, @5) and signing_key not in (@6) and signing_key < @7 and signing_key <= @8 and signing_key > @9 and signing_key >= @10 and signing_key like @11 and signing_key like ('%' + @12 + '%') and signing_key not like (@13 + '%') and lower(signing_key) <> lower(@14) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556661",
            "0a8f9c1e-1111-4222-8333-444455556662",
    ...
    test('dyn-t4/if-value-comparable-worklog-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms > @0 and duration_ms <= @1 and cost_cents < @2 and cost_cents > @3 and billed_amount < @4 and billed_amount > @5 and work_date < @6 and work_date >= @7 and started_at < @8 and started_at >= @9 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            99n,
    ...
    test('dyn-t4/if-value-issue-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours > @0 and not (case when estimated_hours = @1 then 1 else 0 end = 1) and created_at >= @2 and title = @3 and title <> @4 and title in (@5, @6) and title not in (@7) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('dyn-t4/if-value-comparable-custom-temporal-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on < @0 and released_on >= @1 and cutoff_time < @2 and cutoff_time >= @3 and signed_off_at < @4 and signed_off_at >= @5 order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-12-31T10:00:00.000Z,
            2024-01-01T10:00:00.000Z,
    ...
    test('dyn-t4/base-op-int-and-enum-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where priority <> @0 and status = @1 and status <> @2 and status in (@3, @4) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            4,
            "open",
