...
describe(ctx.label, () => {
    ...
    test('dyn/bigint-descriptor-equalable-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms = ? and duration_ms <> ? and duration_ms <=> ? and not (duration_ms <=> ?) and duration_ms in (?, ?) and duration_ms not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            20n,
    ...
    test('dyn/bigint-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms = ? and duration_ms <> ? and duration_ms <=> ? and not (duration_ms <=> ?) and duration_ms in (?, ?) and duration_ms not in (?) and duration_ms < ? and duration_ms >= ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10n,
            20n,
    ...
    test('dyn/double-descriptor-equalable-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours = ? and estimated_hours <> ? and estimated_hours <=> ? and estimated_hours in (?, ?) and estimated_hours not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('dyn/double-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours = ? and estimated_hours <> ? and estimated_hours <=> ? and estimated_hours in (?, ?) and estimated_hours not in (?) and estimated_hours < ? and estimated_hours <= ? and estimated_hours >= ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('dyn/boolean-descriptor-isnot-notin-isnotnull', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billable <=> ?) and billable not in (?, ?) and billable is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            true,
    ...
    test('dyn/boolean-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable = ? and billable <> ? and billable <=> ? and not (billable <=> ?) and billable in (?, ?) and billable not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('dyn/enum-descriptor-is-isnot-notin-nullable', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where activity <=> ? and not (activity <=> ?) and activity not in (?) and activity is null and activity is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "coding",
            "review",
    ...
    test('dyn/enum-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where activity = ? and activity <> ? and activity <=> ? and not (activity <=> ?) and activity in (?, ?) and activity not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "coding",
            "review",
    ...
    test('dyn/localdate-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date = ? and work_date <> ? and work_date <=> ? and not (work_date <=> ?) and work_date in (?, ?) and work_date not in (?) and work_date <= ? and work_date > ? and work_date is null and work_date is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
    ...
    test('dyn/localdate-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date = ? and work_date <> ? and work_date <=> ? and not (work_date <=> ?) and work_date in (?, ?) and work_date not in (?) and work_date <= ? and work_date > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
            2024-04-01T00:00:00.000Z,
    ...
    test('dyn/localtime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at = ? and started_at <> ? and started_at <=> ? and not (started_at <=> ?) and started_at in (?, ?) and started_at not in (?) and started_at <= ? and started_at > ? and started_at is null and started_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "08:00:00",
            "12:00:00",
    ...
    test('dyn/localtime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at = ? and started_at <> ? and started_at <=> ? and not (started_at <=> ?) and started_at in (?, ?) and started_at not in (?) and started_at <= ? and started_at > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "08:00:00",
            "12:00:00",
    ...
    test('dyn/localdatetime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where created_at = ? and created_at <> ? and created_at <=> ? and not (created_at <=> ?) and created_at in (?, ?) and created_at not in (?) and created_at < ? and created_at <= ? and created_at > ? and created_at is null and created_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2020-01-01T00:00:00.000Z,
    ...
    test('dyn/localdatetime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where created_at = ? and created_at <> ? and created_at <=> ? and not (created_at <=> ?) and created_at in (?, ?) and created_at not in (?) and created_at < ? and created_at <= ? and created_at > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2020-01-01T00:00:00.000Z,
            2021-06-15T12:00:00.000Z,
    ...
    test('dyn/customint-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents = ? and cost_cents <> ? and cost_cents <=> ? and not (cost_cents <=> ?) and cost_cents in (?, ?) and cost_cents not in (?) and cost_cents <= ? and cost_cents >= ? and cost_cents is null and cost_cents is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10,
            20,
    ...
    test('dyn/customint-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents = ? and cost_cents <> ? and cost_cents <=> ? and not (cost_cents <=> ?) and cost_cents in (?, ?) and cost_cents not in (?) and cost_cents <= ? and cost_cents >= ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10,
            20,
    ...
    test('dyn/customdouble-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount = ? and billed_amount <> ? and billed_amount <=> ? and not (billed_amount <=> ?) and billed_amount in (?, ?) and billed_amount not in (?) and billed_amount <= ? and billed_amount >= ? and billed_amount is null and billed_amount is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10.5,
            20.5,
    ...
    test('dyn/customdouble-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount = ? and billed_amount <> ? and billed_amount <=> ? and not (billed_amount <=> ?) and billed_amount in (?, ?) and billed_amount not in (?) and billed_amount <= ? and billed_amount >= ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            10.5,
            20.5,
    ...
    test('dyn/customcomparable-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where version <> ? and version <=> ? and not (version <=> ?) and version in (?, ?) and version not in (?) and version <= ? and version >= ? and version is null and version is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "2.0.0",
            "1.5.0",
    ...
    test('dyn/customcomparable-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where version = ? and version <> ? and version <=> ? and not (version <=> ?) and version not in (?) and version < ? and version > ? and version >= ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "1.2.0",
            "2.0.0",
    ...
    test('dyn/custom-descriptor-equalable-nullable', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where \`channel\` = ? and \`channel\` <=> ? and not (\`channel\` <=> ?) and \`channel\` not in (?) and \`channel\` is null and \`channel\` is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "stable",
            "beta",
    ...
    test('dyn/custom-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where \`channel\` = ? and \`channel\` <=> ? and not (\`channel\` <=> ?) and \`channel\` not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "stable",
            "beta",
    ...
    test('dyn/customlocaldate-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on = ? and released_on <> ? and released_on <=> ? and not (released_on <=> ?) and released_on in (?, ?) and released_on not in (?) and released_on <= ? and released_on > ? and released_on is null and released_on is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T10:00:00.000Z,
    ...
    test('dyn/customlocaldate-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on = ? and released_on <> ? and released_on <=> ? and not (released_on <=> ?) and released_on in (?, ?) and released_on not in (?) and released_on <= ? and released_on > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T10:00:00.000Z,
            2024-06-01T10:00:00.000Z,
    ...
    test('dyn/customlocaltime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time = ? and cutoff_time <> ? and cutoff_time <=> ? and not (cutoff_time <=> ?) and cutoff_time in (?, ?) and cutoff_time not in (?) and cutoff_time <= ? and cutoff_time > ? and cutoff_time is null and cutoff_time is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "08:00:00",
            "12:00:00",
    ...
    test('dyn/customlocaltime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time = ? and cutoff_time <> ? and cutoff_time <=> ? and not (cutoff_time <=> ?) and cutoff_time in (?, ?) and cutoff_time not in (?) and cutoff_time <= ? and cutoff_time > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "08:00:00",
            "12:00:00",
    ...
    test('dyn/customlocaldatetime-descriptor-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at = ? and signed_off_at <> ? and signed_off_at <=> ? and not (signed_off_at <=> ?) and signed_off_at in (?, ?) and signed_off_at not in (?) and signed_off_at <= ? and signed_off_at > ? and signed_off_at is null and signed_off_at is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
    ...
    test('dyn/customlocaldatetime-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at = ? and signed_off_at <> ? and signed_off_at <=> ? and not (signed_off_at <=> ?) and signed_off_at in (?, ?) and signed_off_at not in (?) and signed_off_at <= ? and signed_off_at > ? order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2024-01-01T00:00:00.000Z,
            2024-06-15T12:00:00.000Z,
    ...
    test('dyn/uuid-descriptor-core-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = uuid_to_bin(?) and external_ref <> uuid_to_bin(?) and external_ref <=> uuid_to_bin(?) and not (external_ref <=> uuid_to_bin(?)) and external_ref in (uuid_to_bin(?), uuid_to_bin(?)) and external_ref not in (uuid_to_bin(?)) and external_ref < uuid_to_bin(?) and external_ref <= uuid_to_bin(?) and external_ref > uuid_to_bin(?) and external_ref >= uuid_to_bin(?) and external_ref is null and external_ref is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
    ...
    test('dyn/uuid-descriptor-string-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where lower(bin_to_uuid(external_ref)) = lower(?) and lower(bin_to_uuid(external_ref)) <> lower(?) and bin_to_uuid(external_ref) like ? and bin_to_uuid(external_ref) not like ? and lower(bin_to_uuid(external_ref)) like lower(?) and lower(bin_to_uuid(external_ref)) not like lower(?) and bin_to_uuid(external_ref) like concat(?, '%') and bin_to_uuid(external_ref) not like concat(?, '%') and bin_to_uuid(external_ref) like concat('%', ?) and bin_to_uuid(external_ref) not like concat('%', ?) and bin_to_uuid(external_ref) like concat('%', ?, '%') and bin_to_uuid(external_ref) not like concat('%', ?, '%') and lower(bin_to_uuid(external_ref)) like concat(lower(?), '%') and lower(bin_to_uuid(external_ref)) not like concat(lower(?), '%') and lower(bin_to_uuid(external_ref)) like concat('%', lower(?)) and lower(bin_to_uuid(external_ref)) not like concat('%', lower(?)) and lower(bin_to_uuid(external_ref)) like concat('%', lower(?), '%') and lower(bin_to_uuid(external_ref)) not like concat('%', lower(?), '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0A8F",
            "FFFF",
    ...
    test('dyn/uuid-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = uuid_to_bin(?) and external_ref <> uuid_to_bin(?) and external_ref <=> uuid_to_bin(?) and not (external_ref <=> uuid_to_bin(?)) and external_ref in (uuid_to_bin(?), uuid_to_bin(?)) and external_ref not in (uuid_to_bin(?)) and external_ref < uuid_to_bin(?) and external_ref <= uuid_to_bin(?) and external_ref > uuid_to_bin(?) and external_ref >= uuid_to_bin(?) and bin_to_uuid(external_ref) like ? and bin_to_uuid(external_ref) like concat('%', ?, '%') and bin_to_uuid(external_ref) like concat(?, '%') and lower(bin_to_uuid(external_ref)) = lower(?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556661",
            "0a8f9c1e-1111-4222-8333-444455556662",
    ...
    test('dyn/customuuid-descriptor-core-ops', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key <> uuid_to_bin(?) and signing_key <=> uuid_to_bin(?) and not (signing_key <=> uuid_to_bin(?)) and signing_key in (uuid_to_bin(?), uuid_to_bin(?)) and signing_key not in (uuid_to_bin(?)) and signing_key < uuid_to_bin(?) and signing_key > uuid_to_bin(?) and signing_key <= uuid_to_bin(?) and signing_key >= uuid_to_bin(?) and signing_key is null and signing_key is not null order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556662",
            "0a8f9c1e-1111-4222-8333-444455556663",
    ...
    test('dyn/customuuid-descriptor-string-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where bin_to_uuid(signing_key) like ? and bin_to_uuid(signing_key) not like ? and lower(bin_to_uuid(signing_key)) like lower(?) and lower(bin_to_uuid(signing_key)) not like lower(?) and bin_to_uuid(signing_key) not like concat(?, '%') and bin_to_uuid(signing_key) like concat('%', ?) and bin_to_uuid(signing_key) not like concat('%', ?) and bin_to_uuid(signing_key) not like concat('%', ?, '%') and lower(bin_to_uuid(signing_key)) like concat(lower(?), '%') and lower(bin_to_uuid(signing_key)) not like concat(lower(?), '%') and lower(bin_to_uuid(signing_key)) like concat('%', lower(?)) and lower(bin_to_uuid(signing_key)) not like concat('%', lower(?)) and lower(bin_to_uuid(signing_key)) like concat('%', lower(?), '%') and lower(bin_to_uuid(signing_key)) not like concat('%', lower(?), '%') order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "%1111%",
            "%9999%",
    ...
    test('dyn/customuuid-descriptor-ifvalue-fire', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key = uuid_to_bin(?) and signing_key <> uuid_to_bin(?) and signing_key <=> uuid_to_bin(?) and not (signing_key <=> uuid_to_bin(?)) and signing_key in (uuid_to_bin(?), uuid_to_bin(?)) and signing_key not in (uuid_to_bin(?)) and signing_key < uuid_to_bin(?) and signing_key <= uuid_to_bin(?) and signing_key > uuid_to_bin(?) and signing_key >= uuid_to_bin(?) and bin_to_uuid(signing_key) like ? and lower(bin_to_uuid(signing_key)) like concat('%', lower(?), '%') and bin_to_uuid(signing_key) not like concat(?, '%') and lower(bin_to_uuid(signing_key)) <> lower(?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556661",
            "0a8f9c1e-1111-4222-8333-444455556662",
    ...
    test('equivalence/if-value-issue-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours > ? and not (estimated_hours <=> ?) and created_at >= ? and title <=> ? and not (title <=> ?) and title in (?, ?) and title not in (?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('equivalence/base-op-int-and-enum-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as id from issue where not (priority <=> ?) and \`status\` = ? and \`status\` <> ? and \`status\` in (?, ?) order by id"`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            4,
            "open",
