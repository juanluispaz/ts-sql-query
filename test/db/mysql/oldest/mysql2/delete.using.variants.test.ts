...
describe(ctx.label, () => {
    ...
    test('delete-using-cte-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using issue, (select id as id from project where archived_at is null) as active_projects where issue.project_id = active_projects.id and issue.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    */
    ...
    // NOT-APPLICABLE: This dialect does not support DELETE ... USING/JOIN ... RETURNING, so the using-join returning() form is not available here.
    /*
    ...
    */
    ...
    test('delete-using-values-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectIds(id) as (values row(?)) delete from issue using issue, projectIds where issue.project_id = projectIds.id and issue.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                99999,
