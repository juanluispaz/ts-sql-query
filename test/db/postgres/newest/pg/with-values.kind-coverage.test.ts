...
describe(ctx.label, () => {
    ...
    test('values-tuple-cast-per-numeric-and-boolean-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with kindSampler("n", big, dbl, flag) as (values ($1::int4, $2::int8, $3::float8, $4::bool)) select "n" as "n", big as big, dbl as dbl, flag as flag from kindSampler"`)
    ...
    test('values-tuple-cast-per-branded-text-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with brandedSampler(ver, chan, act) as (values ($1, $2, $3)) select ver as ver, chan as chan, act as act from brandedSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1.2.0",
            "stable",
    ...
    test('values-tuple-boolean-column-with-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ flag: v.flag })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with boolAdapterSampler(flag) as (values (case when $1::bool then 'Y' else 'N' end)) select (flag = 'Y') as flag from boolAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('values-tuple-custom-kind-column-with-type-adapter-shifts-write-and-read', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with releaseTagSampler(ordinal) as (values ($1::int4)) select ordinal as ordinal from releaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2005,
          ]
    ...
    test('values-tuple-optional-custom-kind-column-with-type-adapter-shifts-and-passes-null', async () => {
        ...
        const present = await ctx.conn.selectFrom(v1)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optionalReleaseTagSampler(ordinal) as (values ($1::int4)) select ordinal as ordinal from optionalReleaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2005,
          ]
        ...
        const none = await ctx.conn.selectFrom(v2)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optionalReleaseTagSampler(ordinal) as (values ($1)) select ordinal as ordinal from optionalReleaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('values-tuple-cast-per-temporal-kind-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with temporalSampler("d", "t", ts) as (values ($1::date, $2::time, $3::timestamp)) select "d" as "d", "t" as "t", ts as ts from temporalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            null,
    ...
    test('values-tuple-cast-per-uuid-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with uuidSampler(ref, signing) as (values ($1::uuid, $2)) select ref as ref, signing as signing from uuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('values-tuple-cast-per-custom-temporal-kind-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ d: v.d, t: v.t, ts: v.ts })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with customTemporalSampler("d", "t", ts) as (values ($1, $2, $3)) select "d" as "d", "t" as "t", ts as ts from customTemporalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            null,
    ...
    test('values-tuple-virtual-column-from-fragment-per-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with virtualSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vbig, $3 as vdbl, $4 as vopt from virtualSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            100n,
    ...
    test('values-tuple-optional-column-with-type-adapter-scales-and-passes-null', async () => {
        ...
        const present = await ctx.conn.selectFrom(v1)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with scaledOptionalSampler(score) as (values ($1::int4)) select score as score from scaledOptionalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
          ]
    ...
    test('values-tuple-required-column-per-remaining-kind-no-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqSampler(str, cents, money) as (values ($1::text, $2::int4, $3::float8)) select str as str, cents as cents, money as money from reqSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abc",
            12,
    ...
    test('values-tuple-required-column-per-kind-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqAdapterSampler(big, dbl, str, money, ver, chan, act) as (values ($1::int8, $2::float8, $3::text, $4::float8, $5, $6, $7)) select big as big, dbl as dbl, str as str, money as money, ver as ver, chan as chan, act as act from reqAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100n,
            2.5,
    ...
    test('values-tuple-required-uuid-and-custom-uuid-column-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ ref: v.ref, signing: v.signing })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqAdapterUuidSampler(ref, signing) as (values ($1::uuid, $2)) select ref as ref, signing as signing from reqAdapterUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            "11111111-2222-4333-8444-555566667777",
    ...
    test('values-tuple-optional-column-per-kind-no-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optSampler(flag, "n", big, dbl, str, cents, money, ver, chan, act) as (values ($1::bool, $2::int4, $3::int8, $4::float8, $5::text, $6::int4, $7::float8, $8, $9, $10)) select flag as flag, "n" as "n", big as big, dbl as dbl, str as str, cents as cents, money as money, ver as ver, chan as chan, act as act from optSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            7,
    ...
    test('values-tuple-optional-uuid-and-custom-uuid-column-no-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ ref: v.ref, signing: v.signing })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optUuidSampler(ref, signing) as (values ($1::uuid, $2)) select ref as ref, signing as signing from optUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            "11111111-2222-4333-8444-555566667777",
    ...
    test('values-tuple-optional-column-per-kind-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optAdapterSampler(flag, big, dbl, str, money, ver, chan, act) as (values ($1::bool, $2::int8, $3::float8, $4::text, $5::float8, $6, $7, $8)) select flag as flag, big as big, dbl as dbl, str as str, money as money, ver as ver, chan as chan, act as act from optAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            100n,
    ...
    test('values-tuple-optional-uuid-and-custom-uuid-column-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ ref: v.ref, signing: v.signing })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optAdapterUuidSampler(ref, signing) as (values ($1::uuid, $2)) select ref as ref, signing as signing from optAdapterUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            "11111111-2222-4333-8444-555566667777",
    ...
    test('values-tuple-required-virtual-column-from-fragment-per-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvReqSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vint, $3 as vcents, $4 as vmoney, $5 as vver, $6 as vchan, $7 as vact from vvReqSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            42,
    ...
    test('values-tuple-required-virtual-uuid-and-custom-uuid-column-from-fragment', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvReqUuidSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vref, $3 as vsigning from vvReqUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            "0a8f9c1e-1111-4222-8333-444455556666",
    ...
    test('values-tuple-optional-virtual-column-from-fragment-per-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvOptSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vint, $3 as vbig, $4 as vcents, $5 as vmoney, $6 as vver, $7 as vchan, $8 as vact from vvOptSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            42,
    ...
    test('values-tuple-optional-virtual-uuid-and-custom-uuid-column-from-fragment', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vref: v.vref, vsigning: v.vsigning })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvOptUuidSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vref, $3 as vsigning from vvOptUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            "0a8f9c1e-1111-4222-8333-444455556666",
    ...
    test('values-tuple-optional-virtual-temporal-column-from-fragment-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvTemporalSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vd, $3 as vt, $4 as vts from vvTemporalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            null,
    ...
    test('values-tuple-required-virtual-column-from-fragment-per-kind-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vint: v.vint, vcents: v.vcents, vmoney: v.vmoney, vver: v.vver, vchan: v.vchan, vact: v.vact })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvReqAdapterSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vint, $3 as vcents, $4 as vmoney, $5 as vver, $6 as vchan, $7 as vact from vvReqAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            42,
    ...
    test('values-tuple-required-virtual-uuid-and-custom-uuid-column-from-fragment-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vref: v.vref, vsigning: v.vsigning })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvReqAdapterUuidSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vref, $3 as vsigning from vvReqAdapterUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            "0a8f9c1e-1111-4222-8333-444455556666",
    ...
    test('values-tuple-optional-virtual-column-from-fragment-per-kind-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vint: v.vint, vbig: v.vbig, vcents: v.vcents, vmoney: v.vmoney, vver: v.vver, vchan: v.vchan, vact: v.vact })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvOptAdapterSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vint, $3 as vbig, $4 as vcents, $5 as vmoney, $6 as vver, $7 as vchan, $8 as vact from vvOptAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            42,
    ...
    test('values-tuple-optional-virtual-uuid-and-custom-uuid-column-from-fragment-with-passthrough-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vref: v.vref, vsigning: v.vsigning })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvOptAdapterUuidSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vref, $3 as vsigning from vvOptAdapterUuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            "0a8f9c1e-1111-4222-8333-444455556666",
    ...
    test('values-tuple-optional-virtual-temporal-column-from-fragment-with-passthrough-adapter-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ n: v.n, vd: v.vd, vt: v.vt, vts: v.vts })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vvTemporalAdapterSampler("n") as (values ($1::int4)) select "n" as "n", $2 as vd, $3 as vt, $4 as vts from vvTemporalAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            null,
    ...
    test('values-tuple-required-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with exprSampler(id) as (values ($1 + (2))) select id as id from exprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
    ...
    test('values-tuple-optional-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optExprSampler(id) as (values ($1 + (2))) select id as id from optExprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
