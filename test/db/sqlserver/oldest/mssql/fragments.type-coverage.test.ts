...
describe(ctx.label, () => {
    ...
    test('fragment-with-type-double-custom-and-custom-local-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select 1.5 as [avg], channel as ch, signing_key as [key], released_on as rOn, cutoff_time as cut, signed_off_at as soff from project_release where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-widen-the-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select version as [v], 1.5 as [avg], signing_key as [key], channel as ch from project_release where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-scalar-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(title) as maxTitle, avg(priority) as avgPrio, cast(case when count(*) > 0 then 1 else 0 end as bit) as [any], max(view_count) as maxView from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('aggregate-fragment-with-type-optional-scalar-arms', async () => {
        ...
        const result = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority) as mi, avg(priority) as md, max(title) as ms, cast(case when max(priority) > 5 then 1 when not (max(priority) > 5) then 0 else null end as bit) as mb from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('fragment-with-type-optional-arms-over-worklog', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(case when (billable = 1) then 1 when not ((billable = 1)) then 0 else null end as bit) as bill, work_date as wd, started_at as st, cost_cents as cost, billed_amount as amt, activity as act from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-over-release', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select signing_key as [key], released_on as rOn, cutoff_time as cut, signed_off_at as soff, version as [v] from project_release where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(case when (billable = 1) then 1 else 0 end as bit) as req, cast(case when (billable = 1) then 1 when not ((billable = 1)) then 0 else null end as bit) as opt from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
