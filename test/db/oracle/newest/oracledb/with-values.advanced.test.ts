...
describe(ctx.label, () => {
    ...
    test('values-base-type-optional-virtual-column-projects-optional-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with batchRow(id, "label") as (values (:0, :1), (:2, :3)) select id as "id", "label" as "label", null as "rank", null as "note" from batchRow order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-base-type-required-virtual-column-projects-required-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqBatch(id) as (values (:0), (:1)) select id as "id", 7 as "seq", 'BATCH' as "tag" from reqBatch order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-virtual-column-referencing-a-sibling-column-inlines-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(nums)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vdouble(id) as (values (:0), (:1)) select id as "id", id * 2 as "doubled" from vdouble order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
