...
describe(ctx.label, () => {
    ...
    test('rawfragment-hook-embeds-nested-subqueries-deeply', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select /* closed=(select count(id) as "result" from issue where status = :0) */ count(id) as "result" from issue where priority >= :1) as "wrapped",  id as "id" from project where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "closed",
            2,
