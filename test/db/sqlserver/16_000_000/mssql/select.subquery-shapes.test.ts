    ...
    */
    ...
    test('compound-as-derived-table-via-for-use-in-query-as', async () => {
        ...
        const rows = await ctx.conn.selectFrom(statuses)
            ...
            .orderBy('s')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with statuses as (select status as [s] from issue where status = @0 union select status as [s] from issue where status = @1) select [s] as [s] from statuses order by [s]"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    */
    ...
    test('compound-as-inline-aggregated-array-value', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select concat('[', string_agg('"' + string_escape(convert(nvarchar(max), a_1_.[result]), 'json') + '"', ','), ']') from (select status as [result] from issue where status = @0 union select status as [result] from issue where status = @1) as a_1_) as statuses"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
