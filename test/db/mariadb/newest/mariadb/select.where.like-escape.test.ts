...
describe(ctx.label, () => {
    ...
    test('contains-column-argument', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where \`body\` like concat('%', replace(replace(replace(title, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') order by id"`)
    ...
    test('like-column-argument', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where \`body\` like title order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('not-contains-column-argument', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where \`body\` not like concat('%', replace(replace(replace(title, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'), '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
