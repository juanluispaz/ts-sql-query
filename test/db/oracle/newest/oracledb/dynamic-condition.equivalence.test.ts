...
describe(ctx.label, () => {
    ...
    test('equivalence/if-value-issue-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as "id" from issue where estimated_hours > :0 and decode(estimated_hours, :1, 1, 0 ) = 0 and created_at >= :2 and decode(title, :3, 1, 0 ) = 1 and decode(title, :4, 1, 0 ) = 0 and title in (:5, :6) and title not in (:7) order by "id""`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            2.5,
            3.5,
    ...
    test('equivalence/base-op-int-and-enum-family', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(priority, :0, 1, 0 ) = 0 and status = :1 and status <> :2 and status in (:3, :4) order by "id""`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            4,
            "open",
