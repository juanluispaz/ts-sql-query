...
describe(ctx.label, () => {
    ...
    test('optional-non-empty-array-over-empty-aggregate-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select json_arrayagg(json_object('id', id, 'name', name) order by id) from project where organization_id = organization.id and name = ?) as projects from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "__no-such-project__",
            1,
