...
describe(ctx.label, () => {
    ...
    test('inline-value-keeps-custom-comparable-and-uuid-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when version = (select version as "result" from project_release where id = :0) then 1 when not (version = (select version as "result" from project_release where id = :1)) then 0 else null end as "sameSemver", case when signing_key = (select signing_key as "result" from project_release where id = :2) then 1 when not (signing_key = (select signing_key as "result" from project_release where id = :3)) then 0 else null end as "sameKey" from project_release where id = :4"`)
    ...
    test('inline-value-keeps-custom-numeric-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when billed_amount = (select billed_amount as "result" from issue_worklog where id = :0) then 1 when not (billed_amount = (select billed_amount as "result" from issue_worklog where id = :1)) then 0 else null end as "sameMoney", case when cost_cents = (select cost_cents as "result" from issue_worklog where id = :2) then 1 when not (cost_cents = (select cost_cents as "result" from issue_worklog where id = :3)) then 0 else null end as "sameCents" from issue_worklog where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
