...
describe(ctx.label, () => {
    ...
    test('depth-3-nested-projection-recurses-with-non-empty-prefix', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id" from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where lower(app_user.full_name) like lower('%' || :0 || '%') escape '\\' order by "id""`)
    ...
    test('extension-scoped-under-nested-projection-key-is-invoked-on-recurse', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id" from issue left join project on project.id = issue.project_id where exists(select id as "result" from project where id = issue.project_id and lower(name) like lower('%' || :0 || '%') escape '\\') order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mktg",
          ]
