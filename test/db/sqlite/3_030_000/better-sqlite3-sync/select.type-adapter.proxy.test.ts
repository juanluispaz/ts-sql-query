...
describe(ctx.label, () => {
    ...
    test('proxy-adapter/custom-boolean-projected-through-with-clause', async () => {
        ...
        const rows = sync(connection.selectFrom(verifiedOrgs)
            .select({
                id:       verifiedOrgs.id,
    ...
    test('proxy-adapter/filtering-the-proxied-boolean-skips-the-remap', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with verifiedOrgs as (select id as id, name as name, (verified = 'Y') as verified from organization) select id as id, name as name from verifiedOrgs where verified = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
