    ...
    test('set-minmax-poison-case-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .where(tIssue.id.equals(1))
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set assignee_id = case when assignee_id is null or parent_id is null then null else least(assignee_id, parent_id) end where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('collation-forks-in-a-mutation-set-and-where', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tAppUser)
                ...
                .where(tAppUser.email.collate('C').equals('ada@acme.test'))
                .executeUpdate()

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update app_user set full_name = regexp_replace(full_name, $1, $2, 'gi') where (email collate "C") = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Love",
                "X",
    ...
    test('concat-with-optional-operand-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set body = title || body where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
