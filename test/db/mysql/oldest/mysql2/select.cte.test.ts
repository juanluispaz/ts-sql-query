...
describe(ctx.label, () => {
    ...
    test('with-clause/forUseInQueryAs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select projectCountPerOrg.orgName as orgName, projectCountPerOrg.projects as projects from (select \`organization\`.id as orgId, \`organization\`.\`name\` as orgName, count(project.id) as projects from project inner join \`organization\` on \`organization\`.id = project.organization_id group by \`organization\`.id, \`organization\`.\`name\`) as projectCountPerOrg where projectCountPerOrg.projects > ? order by orgName"`)
    ...
    test('with-clause/re-projects-value-transforming-adapter-columns-round-trip', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select reviewScores.pid as pid, reviewScores.score as score, reviewScores.reviewerCode as reviewerCode from (select project_id as pid, score as score, reviewer_code as reviewerCode from project_review) as reviewScores where reviewScores.pid = ? order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
