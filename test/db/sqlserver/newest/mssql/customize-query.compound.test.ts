...
describe(ctx.label, () => {
    ...
    test('customize-compound-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (/* head */  select id as id, name as [label] from project where id in (@0, @1) union select id as id, name as [label] from project where id = @2 order by id offset 0 rows  /* tail */) select count(*) from result_for_count"`)
    ...
    test('customize-compound-inline-aggregated-array-via-for-use-as-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_arrayagg(a_1_.[result] null on null) from (/* compound-head */  select name as [result] from project where id = @0 union select name as [result] from project where id = @1  /* compound-tail */) as a_1_) as [names]"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
