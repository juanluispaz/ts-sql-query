...
-- Issues 1 and 2 carry a uuid `external_ref`; issues 3 and 4 leave it NULL
-- (not every issue has an external reference). The two uuids are distinct in
-- every substring the insensitive-`like` tests probe and neither contains
-- the `abc` token the dynamic-condition tests filter on.
...
INSERT INTO issue_worklog (id, issue_id, work_date, started_at, minutes, duration_ms, billable, activity, approved, billed_amount, invoiced, cost_cents) VALUES
    (1, 1, '2024-03-04', '09:15:00', 90,   5400000, TRUE,  'coding', 'A', 200, 1, 100),
    (2, 2, '2024-03-05', '14:00:00', NULL, NULL,    FALSE, 'review', 'R', 50, 0, 100),
...
INSERT INTO project_release (id, project_id, version, channel, signing_key, released_on, cutoff_time, signed_off_at, published_at, is_signed, download_count, avg_rating) VALUES
    (1, 1, '1.2.0',        'stable', '0a8f9c1e-1111-4222-8333-444455556666', '2024-01-15', '17:00:00', '2024-01-14 12:30:00', '2024-01-16 09:00:00', TRUE,  4200000042, 4.5),
    (2, 1, '1.3.0-beta.1', 'beta',   NULL,                                   '2024-02-20', '18:30:00', NULL, '2024-02-21 10:00:00', NULL,  NULL,       NULL),
    (3, 2, '0.9.0',        'canary', '7b3e9d20-2222-4c55-9b66-dddd00009999', '2024-03-01', '16:00:00', '2024-02-28 09:00:00', '2024-03-02 11:00:00', FALSE, 100,        3.0);

-- review 1: reviewer_code 'R-7A2' (read bracketed -> '[R-7A2]'), score 850
-- (read ÷10 -> 85), review_date 2024-05-20 (a Monday), review_time 14:30:45.
...
SELECT setval(pg_get_serial_sequence('project_review',  'id'), (SELECT MAX(id) FROM project_review));
