...
describe(ctx.label, () => {
    ...
    test('custom-numeric/customdouble-math', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, sqrt(@0) as sq, sign(@1) * power(cast(abs(@2) as float), 1.0e0 / 3.0e0) as cb, exp(@3) as [e], log(@4) as [l], log10(@5) as l10 from issue where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            4,
    ...
    test('custom-numeric/customdouble-operation1-math', async () => {
        ...
        expect(result).toEqual(expected)
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, power(@0, @1) as [p], log(@2, @3) as [ln], round(@4, @5) as rn, cast(@6 as float) / @7 as di, atn2(@8, @9) as at2 from issue where id = @10"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/cast-double-as-int-and-bigint', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(@0, 0) as bigint) as asInt, cast(round(@1, 0) as bigint) as asBig from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1.7,
            1.7,
    ...
    test('custom-numeric/bigint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, view_count + @0 as [a], view_count - @1 as [s], view_count % @2 as [m], iif(view_count > @3, view_count, @4) as mn, iif(view_count < @5, view_count, @6) as mx, abs(view_count) as ab, sign(view_count) as sg from issue where id = @7"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1n,
            1n,
    ...
    test('custom-numeric/customint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cost_cents + @0 as [a], cost_cents - @1 as [s], cost_cents * @2 as mu, cost_cents % cost_cents as mo, iif(cost_cents > @3, cost_cents, @4) as mn, iif(cost_cents < @5, cost_cents, @6) as mx, sign(cost_cents) as sg from issue_worklog where id = @7"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/customdouble-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, billed_amount + billed_amount as [a], billed_amount - @0 as [s], billed_amount * @1 as mu, iif(billed_amount > @2, billed_amount, @3) as mn, iif(billed_amount < @4, billed_amount, @5) as mx, abs(billed_amount) as ab, sign(billed_amount) as sg from issue_worklog where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('custom-numeric/custom-arithmetic-both-operand-optional', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cost_cents + cost_cents as ci, billed_amount + billed_amount as cd from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-numeric/bigint-arithmetic-value-source-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, view_count + view_count as [a], view_count - view_count as [s], view_count + view_count as [o] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-numeric/customdouble-roundn-with-customdouble-value-source-precision', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when round(@0, @1) > @2 then 1 else 0 end as bit) as big from issue where id = @3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/customdouble-modulo-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(billed_amount as numeric(38, 16)) % cast(billed_amount as numeric(38, 16)) as [m] from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-numeric/customint-modulo-const', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cost_cents % @0 as [m] from issue_worklog where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/branded-newtype-keeps-or-erases-the-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(@0) as keepInt, sign(@1) as eraseInt, sqrt(@2) as keepDbl, sign(@3) as eraseDbl from issue where id = @4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
            5,
    ...
    test('custom-numeric/optional-custom-receiver-add-keeps-brand-and-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, optional_release_ordinal + @0 as ord from release_overview where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            1,
    ...
    test('custom-numeric/brand-survival-customint-rounding', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceiling(@0) as [c], floor(@1) as [f], round(@2, 0) as [r] from issue where id = @3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            6,
    ...
    test('custom-numeric/brand-survival-customint-minmax', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(round(@0, 0) > @1, round(@2, 0), @3) as mn, iif(round(@4, 0) < @5, round(@6, 0), @7) as mx from issue where id = @8"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            10,
    ...
    test('custom-numeric/brand-survival-customint-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(@0, 0) + @1 as [a], round(@2, 0) - @3 as [s], round(@4, 0) * @5 as mu, round(@6, 0) % @7 as mo from issue where id = @8"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            4,
    ...
    test('custom-numeric/brand-survival-customdouble-math', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(@0) as ab, ceiling(@1) as ce, floor(@2) as fl, round(@3, 0) as ro, exp(@4) as ex, log(@5) as [l], log10(@6) as l10, sign(@7) * power(cast(abs(@8) as float), 1.0e0 / 3.0e0) as cb from issue where id = @9"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            4,
    ...
    test('custom-numeric/brand-survival-customdouble-power-logn-atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, power(@0, @1) as [p], log(@2, @3) as lg, atn2(@4, @5) as at2 from issue where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-roundn', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(@0, @1) as rl, round(@2, @3) as rv from issue where id = @4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(@0, 0) + @1 as [a], round(@2, 0) - @3 as [s], round(@4, 0) * @5 as mu from issue where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-divide-modulo', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(@0 as float) / @1 as di, cast(@2 as numeric(38, 16)) % cast(@3 as numeric(38, 16)) as mo from issue where id = @4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/adapter-column-through-non-add-arithmetic', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, optional_release_ordinal - @0 as optSub, release_ordinal * @1 as reqMul from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('custom-numeric/minmax-optional-customdouble-receiver-poisons', async () => {
        ...
        const result = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when budget is null then null else iif(budget > @0, budget, @1) end as mn, case when budget is null then null else iif(budget < @2, budget, @3) end as mx from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            100,
    ...
    test('custom-numeric/minmax-operand-optional-customint-keeps-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when optional_release_ordinal is null then null else iif(release_ordinal > optional_release_ordinal, release_ordinal, optional_release_ordinal) end as mn, case when optional_release_ordinal is null then null else iif(release_ordinal < optional_release_ordinal, release_ordinal, optional_release_ordinal) end as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
    test('custom-numeric/minmax-both-optional-customint-keeps-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when optional_release_ordinal is null or optional_release_ordinal is null then null else iif(optional_release_ordinal > optional_release_ordinal, optional_release_ordinal, optional_release_ordinal) end as mn, case when optional_release_ordinal is null or optional_release_ordinal is null then null else iif(optional_release_ordinal < optional_release_ordinal, optional_release_ordinal, optional_release_ordinal) end as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
    test('custom-numeric/minmax-both-required-customint-value-source-keeps-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, iif(release_ordinal > release_ordinal, release_ordinal, release_ordinal) as mn, iif(release_ordinal < release_ordinal, release_ordinal, release_ordinal) as mx from release_overview order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn: ReleaseTag; mx: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
