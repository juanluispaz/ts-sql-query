...
describe(ctx.label, () => {
    ...
    test('having-and-then-or-chains-after-initial-having', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as status, count(id) as total from issue group by status having (count(id) > $1 and count(id) < $2) or max(priority) = $3 order by status"`)
    ...
    test('group-by-having-then-select-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(id) as result from issue group by status having count(id) > $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('group-by-having-then-select-count-all', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as result from issue group by status having count(id) > $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('having-with-aggregate-arithmetic-expression', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as total from issue group by project_id having (count(id) * $1) > $2 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            6,
    ...
    test('having-over-sum-average-min-aggregates', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as total from issue group by project_id having sum(priority) >= $1 and avg(priority::float) > $2 and min(priority) >= $3 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1.5,
    ...
    test('having-over-string-concat-aggregate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as total from issue group by project_id having string_agg(status, ',') like ('%' || $1 || '%') order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
