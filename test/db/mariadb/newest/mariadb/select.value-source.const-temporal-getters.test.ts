...
describe(ctx.label, () => {
    ...
    test('const-localtime-milliseconds', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select floor(microsecond(?) / 1000) as ms"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "12:34:56",
          ]
    ...
    test('const-localdatetime-more-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select dayofmonth(?) as \`d\`, dayofweek(?) - 1 as dow, minute(?) as \`m\`, second(?) as \`s\`, floor(microsecond(?) / 1000) as ms"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2024-01-15T12:34:56.000Z,
            2024-01-15T12:34:56.000Z,
    ...
    test('optional-const-localtime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select hour(?) as \`h\`, minute(?) as \`m\`, second(?) as \`s\`, floor(microsecond(?) / 1000) as ms"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "12:34:56",
            "12:34:56",
    ...
    test('const-localdatetime-subsecond-getters-truncate', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select second(?) as s1, floor(microsecond(?) / 1000) as ms1, second(?) as s2, floor(microsecond(?) / 1000) as ms2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
