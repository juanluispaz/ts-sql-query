...
describe(ctx.label, () => {
    ...
    test('table-customization: wrapper as left-join target widens columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.name as projectName, organization.name as orgName from project left join /*+ hint */ organization  on organization.id = project.organization_id where project.organization_id = @0 order by projectName"`)
    ...
    test('table-customization: wrapper re-aliased as a select source keeps the hint', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select [o].name as orgName from /*+ hint */ organization as [o] where [o].id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: wrapper as a source inside a compound arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as [label] from /*+ hint */ organization  union select name as [label] from project where organization_id = @0 order by [label]"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: wrapper as an update target', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update /*+ hint */ organization  set name = @0 where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Renamed",
                1,
    ...
    test('table-customization: wrapper as a delete target', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from /*+ hint */ organization  where name = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Throwaway org",
              ]
