...
describe(ctx.label, () => {
    ...
    test('count-after-distinct-wraps-query-in-result-for-count-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (select distinct \`status\` as \`status\` from issue) as result_for_count"`)
    ...
    test('count-after-group-by-wraps-query-in-result-for-count-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (select \`status\` as \`status\`, count(id) as total from issue group by \`status\`) as result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
    ...
    test('count-after-distinct-and-group-by-still-wraps-in-cte', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (select distinct \`status\` as \`status\`, priority as priority from issue group by \`status\`, priority) as result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
    ...
    test('count-over-inner-join-uses-unwrapped-fast-path', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select project.id as id, issue.title as title from project inner join issue on issue.project_id = project.id where project.id > ? order by title limit ? offset ?"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            0,
            10,
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from project inner join issue on issue.project_id = project.id where project.id > ?"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('count-over-left-join-counts-unmatched-rows', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select project.id as id, issue.title as title from project left join issue on issue.project_id = project.id where project.id > ? order by title limit ? offset ?"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            0,
            10,
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from project left join issue on issue.project_id = project.id where project.id > ?"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            0,
          ]
