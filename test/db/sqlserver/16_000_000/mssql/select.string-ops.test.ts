...
describe(ctx.label, () => {
    ...
    test('length', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, len(full_name + '.') - 1 as len from app_user where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('length-non-ascii', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select len(@0 + '.') - 1 as accented, len(@1 + '.') - 1 as cjk"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "café",
            "日本",
    ...
    test('length-and-substr-keep-trailing-spaces', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select len(@0 + '.') - 1 as len, substring(@1, @2, 2147483647) as whole, substring(@3, @4, 2147483647) as tail"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Draft  ",
            "Draft  ",
    ...
    test('substring-to-end', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, @0, 2147483647) as sub from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            1,
    ...
    test('negative-index-follows-javascript', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select right(@0, @1) as lastChar, right(@2, @3) as fromEnd, right(@4, @5) as fromEndFar, left(right(@6, @7), @8) as fromEndCount, left(right(@9, @10), @11) as fromEndFarCount, substring(@12, @13, 2147483647) as clamped, substring(@14, @15, @16) as clampedEnd, substring(@17, @18, @19) as swapped"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            1,
    ...
    test('negative-count-clamps-to-empty-string', async () => {
        ...
        const result = await conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select substring(@0, @1, @2) as fromStart, left(right(@3, @4), @5) as fromEnd"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            3,
    ...
    test('substring-numeric-start-value-source-end', async () => {
        ...
        assertType<Exact<typeof result, Array<{ id: number; sub: string }>>>()
        expect(result).toEqual(expected)
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, 1, priority) as sub from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('substr-to-end-with-value-source-start', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, priority + 1, 2147483647) as sub from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('string-predicate-on-optional-receiver-projects-optional-boolean', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when body like ('%' + @0 + '%') then 1 when not body like ('%' + @1 + '%') then 0 else null end as bit) as hasNeedle from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "token",
            "token",
    ...
    test('string-transforms-on-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select upper(body) as up, lower(body) as lo, len(body + '.') - 1 as len, ltrim(body) as tl, rtrim(body) as tr from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('string-reverse-on-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select reverse(body) as rev from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('substr-family-with-optional-value-source-argument', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, assignee_id + 1, 2147483647) as se, substring(title, assignee_id + 1, 2147483647) as sse, substring(title, assignee_id + 1, @0) as s2, substring(title, assignee_id + 1, @1 - assignee_id) as sg from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
    ...
    test('substr-to-end-on-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(version_bracketed, @0, 2147483647) as sub from release_overview where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('substr-to-end-on-optional-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(channel_bracketed, @0, 2147483647) as sub from release_overview where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('trim-substr-substr-to-end-replace-all-on-optional-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, trim(body) as tr, substring(body, @0, @1) as s2, substring(body, @2, 2147483647) as se, replace(body collate Latin1_General_BIN2, @3 collate Latin1_General_BIN2, @4) collate DATABASE_DEFAULT as ra from issue where id = @5"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('affix-predicates-with-a-value-source-term', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when title like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 when not title like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 0 else null end as bit) as ct, cast(case when title like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]')) then 1 when not title like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]')) then 0 else null end as bit) as ew, cast(case when title not like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 when not title not like ('%' + replace(replace(replace(body, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 0 else null end as bit) as nc from issue where project_id = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
