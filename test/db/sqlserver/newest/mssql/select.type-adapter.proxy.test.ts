...
describe(ctx.label, () => {
    ...
    test('proxy-adapter/custom-boolean-projected-through-with-clause', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with verifiedOrgs as (select id as id, name as name, cast(case when verified = 'Y' then 1 else 0 end as bit) as verified from organization) select id as id, name as name, verified as verified from verifiedOrgs order by id"`)
    ...
    test('proxy-adapter/filtering-the-proxied-boolean-skips-the-remap', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with verifiedOrgs as (select id as id, name as name, cast(case when verified = 'Y' then 1 else 0 end as bit) as verified from organization) select id as id, name as name from verifiedOrgs where verified = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
