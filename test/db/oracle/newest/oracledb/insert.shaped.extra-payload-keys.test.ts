...
describe(ctx.label, () => {
    ...
    test('shaped-conditional-set-family-skips-payload-keys-outside-the-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority, "body", assignee_id, parent_id, estimated_hours, external_ref) values (:0, :1, :2, :3, :4, :5, :6, :7, :8, uuid_to_raw(:9))"`)
    ...
    test('shaped-multi-row-set-for-all-family-skips-payload-keys-outside-the-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into issue (project_id, "number", title, status, priority, "body", assignee_id, parent_id, estimated_hours, external_ref) values (:0, :1, :2, :3, :4, :5, :6, :7, :8, uuid_to_raw(:9)); insert into issue (project_id, "number", title, status, priority, "body", assignee_id, parent_id, estimated_hours, external_ref) values (:10, :11, :12, :13, :14, :15, :16, :17, :18, uuid_to_raw(:19)); end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                8810,
