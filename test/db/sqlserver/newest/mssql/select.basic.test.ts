...
describe(ctx.label, () => {
    ...
    test('active-projects-of-organization', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, slug as slug from project where organization_id = @0 and archived_at is null order by name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('issues-joined-with-project', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as issueId, issue.title as title, project.name as projectName from issue inner join project on project.id = issue.project_id where issue.status = @0 order by issueId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
