...
describe(ctx.label, () => {
    ...
    test('group-by-having-then-select-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(id) as "result" from issue group by status having count(id) > :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('group-by-having-then-select-count-all', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as "result" from issue group by status having count(id) > :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('having-with-aggregate-arithmetic-expression', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as "total" from issue group by project_id having (count(id) * :0) > :1 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            6,
    ...
    test('having-over-sum-average-min-aggregates', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as "total" from issue group by project_id having sum(priority) >= :0 and avg(cast(priority as float)) > :1 and min(priority) >= :2 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1.5,
    ...
    test('having-over-string-concat-aggregate', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) as "total" from issue group by project_id having listagg(status, ',') within group (order by status) like ('%' || :0 || '%') escape '\\' order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
