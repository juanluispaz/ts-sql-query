...
describe(ctx.label, () => {
    ...
    test('custom-boolean-remap/a-column-with-the-same-literals-emits-the-raw-column-name', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update organization set verified = app_user.verified from app_user where organization.id = $1 and app_user.id = $2"`)
    ...
    test('custom-boolean-remap/a-column-with-different-literals-to-an-optional-target-emits-case-source-when-mapped-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case invoiced when 1 then 'A' when 0 then 'R' else null end where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
              ]
    ...
    test('custom-boolean-remap/a-plain-boolean-column-to-a-required-target-emits-case-when-value-then-true-else-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update organization set verified = case when col_matrix.m_bool then 'Y' else 'N' end from col_matrix where organization.id = $1 and col_matrix.id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                1,
    ...
    test('custom-boolean-remap/a-plain-boolean-column-to-an-optional-target-emits-case-value-when-true-value-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case col_matrix.m_bool when true then 'A' when false then 'R' else null end from col_matrix where issue_worklog.id in ($1, $2) and col_matrix.id = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                3,
    ...
    test('custom-boolean-remap/a-parenthesised-boolean-expression-to-an-optional-target-emits-case-when-expression-when-not-expression-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case when minutes > $1 then 'A' when not minutes > $2 then 'R' else null end where id = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                60,
                60,
    ...
    test('custom-boolean-remap/an-unparenthesised-boolean-expression-to-an-optional-target-repeats-the-expression-bare', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case when duration_ms is null then 'A' when not duration_ms is null then 'R' else null end where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('custom-boolean-remap/a-boolean-literal-to-a-required-target-emits-case-when-param-then-true-else-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update organization set verified = case when $1::bool then 'Y' else 'N' end where id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                true,
                2,
    ...
    test('custom-boolean-remap/a-boolean-literal-to-an-optional-target-emits-case-param-when-true-value-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case $1::bool when true then 'A' when false then 'R' else null end where id = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                false,
                1,
