...
describe(ctx.label, () => {
    ...
    test('length-non-ascii', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select length($1) as accented, length($2) as cjk"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "café",
            "日本",
    ...
    test('length-and-substr-keep-trailing-spaces', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select length($1) as len, substr($2, $3) as whole, substr($4, $5) as tail"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Draft  ",
            "Draft  ",
    ...
    test('negative-index-follows-javascript', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select right($1, $2) as "lastChar", right($3, $4) as "fromEnd", right($5, $6) as "fromEndFar", left(right($7, $8), $9) as "fromEndCount", left(right($10, $11), $12) as "fromEndFarCount", substr($13, $14) as clamped, substr($15, $16, $17) as "clampedEnd", substr($18, $19, $20) as swapped"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            1,
    ...
    test('negative-count-clamps-to-empty-string', async () => {
        ...
        const result = await conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select substr($1, $2, $3) as "fromStart", left(right($4, $5), $6) as "fromEnd""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            3,
    ...
    test('substring-numeric-start-value-source-end', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, 1, priority) as sub from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('substr-to-end-with-value-source-start', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, priority + 1) as sub from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('substr-two-arg-with-value-source-boundaries', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, priority + 1, id) as sub from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('substr-family-with-optional-value-source-argument', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, assignee_id + 1) as se, substr(title, assignee_id + 1) as sse, substr(title, assignee_id + 1, $1) as s2, substr(title, assignee_id + 1, $2 - assignee_id) as sg from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
    ...
    test('concat-with-optional-value-source-argument-and-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title || body as "j", body || $1 as "r" from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            2,
    ...
    test('replaceAll-single-value-source-overloads', async () => {
        ...
        const row = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(email, $1, full_name) as cvs, replace(email, full_name, $2) as vsc from app_user where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "@",
            "-",
    ...
    test('replaceAllIfValue-single-value-source-overloads', async () => {
        ...
        const row = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(email, $1, full_name) as sv, replace(email, full_name, $2) as vs from app_user where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "@",
            "-",
    ...
    test('substr-numeric-start-value-source-count-and-substring-both-value-sources', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(title, $1, priority) as sc, substr(title, id + 1, priority - id) as ss from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('substr-to-end-on-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(version_bracketed, $1) as sub from release_overview where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('substr-to-end-on-optional-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(channel_bracketed, $1) as sub from release_overview where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('trim-substr-substr-to-end-replace-all-on-optional-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, trim(body) as tr, substr(body, $1, $2) as s2, substr(body, $3) as se, replace(body, $4, $5) as ra from issue where id = $6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('to-upper-case-on-optional-receiver-null-row-absent', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(body) as up from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('to-upper-case-on-optional-adapter-view-column-null-row-absent', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(channel_bracketed) as up from release_overview where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('to-upper-case-on-virtual-bracket-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(upper(version)) as "t" from release_overview where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('to-upper-case-on-virtual-bracket-table-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(upper(activity)) as "t" from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('affix-predicates-with-a-value-source-term', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title like ('%' || replace(replace(replace(body, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') as ct, title like ('%' || replace(replace(replace(body, '\\', '\\\\'), '%', '\\%'), '_', '\\_')) as ew, title not like ('%' || replace(replace(replace(body, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') as nc from issue where project_id = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-as-string-string-methods', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, length(external_ref::text) as len, upper(external_ref::text) as up, lower(external_ref::text) as lo from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
