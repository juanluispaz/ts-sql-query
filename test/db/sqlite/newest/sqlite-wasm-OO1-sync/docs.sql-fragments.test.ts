...
describe(ctx.label, () => {
    ...
    test('docs:sql-fragments/fragment-with-type', async () => {
        ...
        // doc-start
        const rows = sync(connection.selectFrom(tAppUser)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(id + id) as idDoubled from app_user where email = ?"`)
    ...
    test('docs:sql-fragments/customize-query-select', async () => {
        ...
        // doc-start
        const row = sync(connection.selectFrom(tAppUser)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select /*+ some hints */ id as id, email as email, full_name as fullName from app_user where id = ? /* trailing comment */"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:sql-fragments/customize-query-insert', async () => {
        ...
        await ctx.withRollback(async () => {
            // doc-start
            const id = sync(connection.insertInto(tAppUser)
                .set({
                    email:    'new@acme.test',
                    fullName: 'New User',
                })
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert /*+ some hints */ into app_user (email, full_name) values (?, ?) returning id /* trailing comment */"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "new@acme.test",
                "New User",
    ...
    test('docs:sql-fragments/customize-query-update', async () => {
        ...
        await ctx.withRollback(async () => {
            // doc-start
            const affected = sync(connection.update(tProject)
    ...
    test('docs:sql-fragments/customize-query-delete', async () => {
        ...
        await ctx.withRollback(async () => {
            // doc-start
            const affected = sync(connection.deleteFrom(tProject)
    ...
    test('docs-extra:sql-fragments/raw-fragment-as-where', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, email as email, full_name as fullName from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
