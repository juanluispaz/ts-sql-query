...
describe(ctx.label, () => {
    ...
    test('optional-plus-optional-stays-optional-and-realizes-both-inhabitants', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, minutes + minutes as sum from issue_worklog where id in (?, ?) order by id"`)
    ...
    test('required-plus-optional-realizes-the-null-inhabitant', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, issue_id + minutes as total from issue_worklog where id in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
