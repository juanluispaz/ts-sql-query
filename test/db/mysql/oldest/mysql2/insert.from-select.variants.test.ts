...
describe(ctx.label, () => {
    ...
    // canonical body would not compile.
    ...
    */
    ...
    test('insert-from-select-source-with-cte', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, \`number\`, title, \`status\`, priority) select issue.project_id as projectId, issue.\`number\` + ? as \`number\`, issue.title as title, ? as \`status\`, issue.priority as priority from issue inner join (select id as id from project where archived_at is null) as active_projects on active_projects.id = issue.project_id where issue.project_id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1000,
                "draft",
    ...
    // NOT-APPLICABLE: targeted on-conflict (onConflictOn) is not typed on MySqlConnection (only the bare upsert form).
    /*
