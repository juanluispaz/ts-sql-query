...
describe(ctx.label, () => {
    ...
    test('custom-boolean-remap/a-column-with-the-same-literals-emits-the-raw-column-name', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update \`organization\`, app_user set \`organization\`.verified = app_user.verified where \`organization\`.id = ? and app_user.id = ?"`)
    ...
    test('custom-boolean-remap/a-plain-boolean-column-to-a-required-target-emits-case-when-value-then-true-else-false', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update \`organization\`, col_matrix set \`organization\`.verified = case when col_matrix.m_bool then 'Y' else 'N' end where \`organization\`.id = ? and col_matrix.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                1,
    ...
    test('custom-boolean-remap/a-plain-boolean-column-to-an-optional-target-emits-case-value-when-true-value-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog, col_matrix set issue_worklog.approved = case col_matrix.m_bool when true then 'A' when false then 'R' else null end where issue_worklog.id in (?, ?) and col_matrix.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                3,
    ...
    test('custom-boolean-remap/a-boolean-literal-to-an-optional-target-emits-case-param-when-true-value-else-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set approved = case ? when true then 'A' when false then 'R' else null end where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                false,
                1,
