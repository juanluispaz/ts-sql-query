...
describe(ctx.label, () => {
    ...
    test('from-model/emits-the-same-sql-as-the-descriptor-form', async () => {
        ...
        expect(modelSql).toMatchInlineSnapshot(`"select id as id from issue where (title ilike ($1 || '%') or priority > $2) and view_count >= $3 and external_ref::text ilike ('%' || $4 || '%') order by id"`)
    ...
    test('from-model/boolean-field-maps-and-rounds', async () => {
        ...
        expect(dynSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable = $1 order by id"`)
    ...
    test('from-model/numeric-literal-union-maps-to-enum', async () => {
        ...
        expect(ctx.lastSql).toBe(dynSql)
        expect(ctx.lastParams).toEqual(dynParams)
        expect(dynSql).toMatchInlineSnapshot(`"select id as id from issue where priority = $1 order by id"`)
        expect(dynParams).toMatchInlineSnapshot(`
          [
            2,
          ]
