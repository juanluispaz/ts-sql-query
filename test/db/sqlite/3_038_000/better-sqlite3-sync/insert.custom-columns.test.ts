...
describe(ctx.label, () => {
    ...
    test('insert-into-project-release-marshals-custom-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(99)
            const inserted = sync(ctx.conn.insertInto(tProjectRelease)
                .values({
                    projectId:   1,
                    version:     '2.0.0',
                    channel:     'stable',
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, signing_key, released_on, cutoff_time, signed_off_at) values (?, ?, ?, ?, ?, ?, ?)"`)
            ...
            if (ctx.realDbEnabled) {
                const ids = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('insert-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('canary')
            const channel = sync(ctx.conn.insertInto(tProjectRelease)
                .values({
                    projectId:  1,
                    version:    '3.0.0',
                    channel:    'canary',
    ...
    test('insert-into-project-review-marshals-non-boolean-adapters', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(99)
            const inserted = sync(ctx.conn.insertInto(tProjectReview)
                .values({
                    projectId:    1,
                    reviewerCode: 'R-9',
                    score:        8.5,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_review (project_id, reviewer_code, score, review_date, review_time) values (?, ?, ?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "R-9",
            ...
            expect(typeof inserted).toBe('number')
            if (ctx.realDbEnabled) {
                const ids = sync(ctx.conn.selectFrom(tProjectReview)
    ...
    test('insert-worklog-with-branded-custom-and-numeric-boolean-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue_worklog (issue_id, work_date, activity, billed_amount, cost_cents, invoiced) values (?, ?, ?, ?, ?, case when ? then 1 else 0 end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "2024-03-12",
