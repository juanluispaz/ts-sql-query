...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-use-empty-array-for-no-value-explicit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ','), ']') as titles from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
    ...
    test('aggregate-as-array-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), '}'), ','), ']') as issues from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ','), ']') as [meta.titles] from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.id order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('aggregate-as-array-element-level-rule-1-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as oid, concat('[', string_agg(concat('{', '"ref": ', isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ', "assigneeId": ', isnull(convert(nvarchar, issue.assignee_id), 'null'), '}'), ','), ']') as issues from project left join issue on issue.project_id = project.id where project.organization_id = @0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-element-level-rule-4-all-optional-not-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, concat('[', string_agg(concat('{', '"body": ', isnull('"' + string_escape(convert(nvarchar(max), body), 'json') + '"', 'null'), ', "assigneeId": ', isnull(convert(nvarchar, assignee_id), 'null'), '}'), ','), ']') as opts from issue where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-projecting-optional-values-as-nullable-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), '}'), ','), ']') as [meta.issues] from project left join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregated-array-projecting-optional-values-as-nullable-then-use-empty-array-for-no-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select id as id, body as body from issue where project_id = project.id for json path) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregated-array-projecting-optional-values-as-nullable-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select id as id, body as body from issue where project_id = project.id for json path) as [meta.issues] from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-query-value-one-column-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select concat('[', string_agg(concat('{', '"id": ', convert(nvarchar, id), ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), body), 'json') + '"', 'null'), '}'), ','), ']') as [result] from issue where project_id = project.id) as issues from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
