...
describe(ctx.label, () => {
    ...
    //   release 1 -> 0a8f9c1e-…, 2 -> NULL, 3 -> 7b3e9d20-….
    // customComparable — tProjectRelease.version ('Semver', required):
    //   release 1 -> 1.2.0, 2 -> 1.3.0-beta.1, 3 -> 0.9.0.
    // custom — tProjectRelease.channel ('ReleaseChannel', required):
    //   release 1 -> stable, 2 -> beta, 3 -> canary.
    ...
    // ==================================================================
    // The next layer on the direct-fluent Comparable surface: ordered
    // comparison (lessThan / greaterThan / lessOrEqual / greaterOrEqual)
    // and the three mixed `between` overloads (between(TYPE,VAL) /
    // between(VAL,TYPE) / between(VAL,VAL)) against a VALUE-SOURCE operand,
    // plus customInt / customDouble `*IfValue`. The const-operand surface
    // is saturated above; these close the value-source-operand + custom
    // numeric `*IfValue` gaps. Representatives per leaf-family suffice — the
    // ordered-comparison / between emission is identical across leaves of a
    // family, only the operand column and its marshalling differ.
    // ==================================================================
