...
describe(ctx.label, () => {
    ...
    test('rule-1-nested-object-under-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, cast(case when priority = id then 1 when not (priority = id) then 0 else null end as bit) as [meta.flag], assignee_id as [meta.assigneeId] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-union-preserves-a-nested-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, number as [header.num], title as [header.title] from issue where id = @0 union select id as iid, number as [header.num], title as [header.title] from issue where id = @1 order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('merge-optional-requiredInOptionalObject-demoted-to-optional-through-operator', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, cast(case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as bit) as [meta.flag], number as [meta.ownReq] from issue where id in (@0, @1) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('merge-optional-requiredInOptionalObject-demoted-to-optional-through-operator-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid, cast(case when priority = assignee_id then 1 when not (priority = assignee_id) then 0 else null end as bit) as [meta.flag], number as [meta.ownReq] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('cte-with-top-level-aggregate-as-array-nullable-leaf-carries-projecting-flag', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with org_projects_cte as (select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), ', "archivedAt": ', isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), '}'), ','), ']') as projects from organization left join project on project.organization_id = organization.id group by organization.id) select orgId as orgId, projects as projects from org_projects_cte order by orgId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            orgId:    number
    ...
    test('cte-with-nested-object-aggregate-projecting-then-as-required-in-optional-object-carries-flag', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with org_projects_cte as (select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), ', "archivedAt": ', isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), '}'), ','), ']') as [meta.projects] from organization left join project on project.organization_id = organization.id group by organization.id) select orgId as orgId, [meta.projects] as [meta.projects] from org_projects_cte order by orgId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            orgId: number
    ...
    test('cte-with-nested-object-carrying-a-custom-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with wl_cte as (select id as wid, cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) as [status.approved] from issue_worklog where id = @0) select wid as wid, [status.approved] as [status.approved] from wl_cte"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
