...
describe(ctx.label, () => {
    ...
    test('custom-numeric/customint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueScored)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + @0 as [a], priority - @1 as [s], priority * @2 as mu, priority % priority as mo, greatest(priority, @3) as mn, least(priority, @4) as mx, sign(priority) as sg from issue where id = @5"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
