...
describe(ctx.label, () => {
    ...
    */
    ...
    test('useEmptyArrayForNoValue-on-one-column-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id, concat('[', string_agg(isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), ','), ']') as titles from project left join issue on issue.project_id = project.id and issue.priority = @0 where project.id = @1 group by project.id"`)
    ...
    test('useEmptyArrayForNoValue-on-object-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, issue.id), 'null'), ', "title": ', isnull('"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', 'null'), '}'), ','), ']') as issues from project left join issue on issue.project_id = project.id and issue.priority = @0 where project.id = @1 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            1,
    ...
    */
    ...
    // NOT-APPLICABLE: SQL Server's `for json path` aggregation has no DISTINCT object-array form, so the connection does not expose `aggregateAsArrayDistinct` (typed never here).
    /*
