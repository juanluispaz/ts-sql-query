...
describe(ctx.label, () => {
    ...
    test('aggregate-of-local-date-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, json_arrayagg(work_date null on null) as dates from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-object-with-adapter-transformed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(json_object('score':score, 'reviewer':reviewer_code)) as reviews from project_review where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-one-column-optional-branded-adapter-strips-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(optional_release_ordinal null on null) as ords from release_overview where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-double-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select estimated_hours as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as hours from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-bigint-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select view_count as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as views from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-uuid-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select external_ref as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as refs from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-time-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select created_at as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as stamps from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-boolean-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select billable as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as flags from issue where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select work_date as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as dates from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select billed_amount as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as amounts from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, json_arrayagg(billed_amount null on null) as amounts from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select started_at as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as clocks from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-uuid-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select signing_key as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as keys from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select released_on as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as days from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select cutoff_time as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as cutoffs from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select signed_off_at as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as signoffs from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
