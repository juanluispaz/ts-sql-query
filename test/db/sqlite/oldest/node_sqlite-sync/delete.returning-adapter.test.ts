...
describe(ctx.label, () => {
    ...
    test('delete-project-review-returning-bracket-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('R-7A2')
            const code = sync(ctx.conn.deleteFrom(tProjectReview)
                .where(tProjectReview.id.equals(1))
                .returningOneColumn(tProjectReview.reviewerCode)
                .executeDeleteOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = ? returning reviewer_code as result"`)
    ...
    test('delete-project-review-returning-scaled-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(850)
            const score = sync(ctx.conn.deleteFrom(tProjectReview)
                .where(tProjectReview.id.equals(1))
                .returningOneColumn(tProjectReview.score)
                .executeDeleteOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = ? returning score as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-project-release-returning-virtual-fragment-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('STABLE')
            const tag = sync(ctx.conn.deleteFrom(tProjectRelease)
                .where(tProjectRelease.id.equals(1))
                .returningOneColumn(tProjectRelease.versionTag)
                .executeDeleteOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release where id = ? returning upper(channel) as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-col-matrix-virtual-adapter-returning-scaled-int', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(42)
            const cents = sync(ctx.conn.deleteFrom(tColMatrixVirtualAdapter)
