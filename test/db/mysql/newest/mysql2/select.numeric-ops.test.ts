...
describe(ctx.label, () => {
    ...
    test('add', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + ? as p2 from issue where id = ?"`)
    ...
    test('subtract', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority - ? as p_minus_1 from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('multiply', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority * ? as doubled from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('floor', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, floor(cast(priority as double) / ?) as \`f\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('ceil', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceil(cast(priority as double) / ?) as \`c\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('round', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(cast(cast(priority as double) / ? as decimal(65, 30))) as \`r\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, atan2(priority, ?) as \`a\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('asDouble', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(priority as double) as \`d\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('nullIfValue', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(priority, ?) as \`p\` from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('abs-and-arithmetic-negation', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority * ? as neg, abs(priority * ?) as absNeg from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
            -1,
    ...
    test('optional-double-receiver-math', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, estimated_hours / ? as \`d\`, power(estimated_hours, ?) as \`p\`, sqrt(estimated_hours) as \`s\`, round(cast(estimated_hours as decimal(65, 30))) as \`r\` from issue where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                2,
    ...
    test('double-column-receiver-overloaded-arithmetic-threads-optional-and-emits-uncast', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, estimated_hours + ? as \`a\`, estimated_hours - ? as \`s\`, estimated_hours * ? as mu, estimated_hours % ? as mo, greatest(estimated_hours, ?) as mn, least(estimated_hours, ?) as mx from issue where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1.5,
                1.5,
    ...
    test('optional-bigint-receiver-unary-math', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(duration_ms) as \`a\`, sign(duration_ms) as \`s\` from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-custom-int-receiver-unary-math', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(optional_release_ordinal) as \`a\`, sign(optional_release_ordinal) as \`s\` from release_overview where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-custom-double-receiver-unary-math', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(billed_amount) as \`a\`, sign(billed_amount) as \`s\` from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-double-receiver-unary-f1-fan-out', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            if (ctx.realDbEnabled) {
                ...
                expect(Number(row.cb)).toBeCloseTo(2, 10)
    ...
    test('double-required-receiver-unary-f1-fan-out', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, abs(cast(priority as double)) as ab, ceil(cast(priority as double)) as ce, floor(cast(priority as double)) as fl, sign(cast(priority as double)) as sg, sign(cast(priority as double)) * power(abs(cast(priority as double)), 1.0e0 / 3.0e0) as cb, exp(cast(priority as double)) as ex, ln(cast(priority as double)) as ln, log10(cast(priority as double)) as l10, power(cast(priority as double), ?) as pw, log(?, cast(priority as double)) as lgn, round(cast(cast(priority as double) as decimal(65, 30)), ?) as rn, atan2(cast(priority as double), ?) as \`at\`, greatest(cast(priority as double), ?) as mn, least(cast(priority as double), ?) as mx from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('optional-bigint-receiver-rounding-arithmetic-fan-out', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceil(duration_ms) as ce, floor(duration_ms) as fl, round(duration_ms) as ro, duration_ms - ? as \`s\`, duration_ms % ? as mo, greatest(duration_ms, ?) as mn, least(duration_ms, ?) as mx from issue_worklog where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            400000n,
            1000000n,
    ...
    test('asInt-chained-into-numeric-op', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) % ? as \`m\`, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) + ? as \`a\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('asBigint-chained-into-bigint-op', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + ? as \`a\`, priority % ? as \`m\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2n,
            2n,
