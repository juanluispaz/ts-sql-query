...
describe(ctx.label, () => {
    ...
    test('delete-using-table-then-inner-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project inner join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
    ...
    test('delete-using-table-then-inner-join-returning-nested', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, deleted.title as [meta.title], deleted.priority as [meta.priority] from project inner join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project left join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-outer-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project left outer join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-join-returning-nullable-joined-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, organization.name as org from project left join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-join-returning-left-join-optional-column-as-nullable-surfaces-present-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, project_release.version as version from project left join project_release on project_release.project_id = project.id where issue.project_id = project.id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('delete-using-table-then-inner-join-on-and-compound-join-condition', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project inner join organization on organization.id = project.organization_id and organization.name = @0 where issue.project_id = project.id and project.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                2,
    ...
    test('delete-using-table-then-inner-join-dynamic-on-or', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project inner join organization on organization.id = project.organization_id or organization.name = @0 where issue.project_id = project.id and project.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "never-matches",
                2,
    ...
    // NOT-APPLICABLE: `.innerJoin` / `.leftJoin` on DELETE are typed `never` on SQL Server (its grammar has no JOIN-on-DELETE form); use `.using(table).where(...)` instead.
    /*
    ...
    */
    ...
    test('delete-allowing-no-where-using-table-then-inner-join-removes-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release from project inner join organization on organization.id = project.organization_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
            assertType<Exact<typeof affected, number>>()
            if (ctx.realDbEnabled) expect(typeof affected).toBe('number')
    ...
    test('delete-using-table-then-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project join organization on organization.id = project.organization_id where issue.project_id = project.id and project.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-two-inner-joins-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue from project inner join organization on organization.id = project.organization_id inner join project_release on project_release.project_id = project.id where issue.project_id = project.id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
