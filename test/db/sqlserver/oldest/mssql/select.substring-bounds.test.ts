...
describe(ctx.label, () => {
    ...
    test('substring-bounds/a-negative-start-is-clamped-to-the-first-character', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, 1, id) as sub from issue where id = @0"`)
    ...
    test('substring-bounds/a-positive-start-shifts-the-position-and-shortens-the-length', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, @0, id - @1) as sub from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
