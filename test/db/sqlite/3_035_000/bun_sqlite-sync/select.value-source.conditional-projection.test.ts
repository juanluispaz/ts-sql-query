...
describe(ctx.label, () => {
    ...
    test('boolean-only-when-gates-where', async () => {
        ...
        ctx.mockNext(kept)
        const keptRows = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(all)
        const allRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('onlyWhenOrNull-true-on-custom-boolean-carries-read-remap', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (published = 't') as pub from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('onlyWhenOrNull-false-on-custom-boolean-emits-null-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, null as pub from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
