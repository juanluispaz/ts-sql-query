...
describe(ctx.label, () => {
    ...
    test('view-side-plain-columns-read-as-their-types', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select is_signed as isSigned, download_count as downloadCount, avg_rating as avgRating, signing_uuid as signingUuid, release_day_plain as releaseDayPlain, cutoff_plain as cutoffPlain, release_ordinal as releaseOrdinal from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
