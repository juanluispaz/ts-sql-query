...
describe(ctx.label, () => {
    ...
    test('acos', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, acos((priority * 1.0e0) / ?) as \`v\` from issue where id = ?"`)
    ...
    test('asin', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, asin((priority * 1.0e0) / ?) as \`v\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            1,
    ...
    test('trig-domain-safe/double-required-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, acos((priority * 1.0e0) / ?) as ac, asin((priority * 1.0e0) / ?) as \`as\`, atan((priority * 1.0e0) / ?) as \`at\`, cos((priority * 1.0e0) / ?) as co, sin((priority * 1.0e0) / ?) as si, tan((priority * 1.0e0) / ?) as ta from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            10,
    ...
    test('trig-cot/double-required-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cot((priority * 1.0e0) / ?) as ct from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            1,
    ...
    test('trig-domain-safe/optional-int-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, acos((assignee_id * 1.0e0) / ?) as ac, asin((assignee_id * 1.0e0) / ?) as \`as\`, atan((assignee_id * 1.0e0) / ?) as \`at\`, cos((assignee_id * 1.0e0) / ?) as co, sin((assignee_id * 1.0e0) / ?) as si, tan((assignee_id * 1.0e0) / ?) as ta from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('trig-cot/optional-int-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cot((assignee_id * 1.0e0) / ?) as ct from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
