...
describe(ctx.label, () => {
    ...
    test('value-when-no-value-false-as-selected-column-elided-emits-false-literal', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when (0=1) then 1 else 0 end as bit) as statusMatches from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-when-no-value-false-as-selected-column-fires-uses-primary-sql', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when status = @0 then 1 else 0 end as bit) as statusMatches from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            1,
    ...
    test('value-when-no-value-boolean-value-source-as-selected-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority > @0 then 1 else 0 end as bit) as statusMatches from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
