    ...
    */
    ...
    test('custom-numeric/customint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueScored)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + ? as "a", priority - ? as "s", priority * ? as mu, priority % priority as mo, max(priority, ?) as mn, min(priority, ?) as mx, sign(priority) as sg from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/customdouble-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueScored)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority + priority as "a", priority - ? as "s", priority * ? as mu, max(priority, ?) as mn, min(priority, ?) as mx, abs(priority) as ab, sign(priority) as sg from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
