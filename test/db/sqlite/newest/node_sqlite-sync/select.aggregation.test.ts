...
describe(ctx.label, () => {
    ...
    test('avg-int-column-fractional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg(cast(priority as real)) as avgPriority from issue where project_id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('average-and-average-distinct-over-bigint', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg(cast(duration_ms as real)) as avg, avg(distinct cast(duration_ms as real)) as avgD from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customint-sum-distinct-and-average-return-branches', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct cost_cents) as sd, avg(cast(cost_cents as real)) as av from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('sum-distinct-average-distinct-over-int', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct priority) as sd, avg(distinct cast(priority as real)) as avd from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('aggregate-arithmetic-expression-as-select-column', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority) + ? as "x" from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-arithmetic-expression-in-order-by', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, sum(priority) as total from issue group by project_id order by sum(issue.priority) + ?, projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
