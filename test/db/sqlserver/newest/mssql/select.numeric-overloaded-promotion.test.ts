...
describe(ctx.label, () => {
    ...
    test('int-receiver-maxValue-double-column-promotes-result-to-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when estimated_hours is null then null else least(priority, estimated_hours) end as hi from issue where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('int-receiver-minValue-double-column-promotes-result-to-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when estimated_hours is null then null else greatest(priority, estimated_hours) end as lo from issue where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
