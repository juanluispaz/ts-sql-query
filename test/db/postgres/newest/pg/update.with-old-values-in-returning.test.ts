...
describe(ctx.label, () => {
    ...
    test('returning-computed-expression-over-old-value-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2 returning old.name || $3 as "oldTag", name as "newName""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mktg v4",
                1,
    ...
    test('returning-old-and-new-adapter-column-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set score = $1 where id = $2 returning old.score as "oldScore", score as "newScore""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                900,
                1,
    ...
    test('returning-old-optional-column-as-nullable-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2 returning old.archived_at as "oldArchivedAt", name as "newName""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Internal tools v2",
                2,
    ...
    test('allowing-no-where-returning-old-and-new-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1 returning id as id, old.name as "oldName", name as "newName""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Renamed all",
              ]
