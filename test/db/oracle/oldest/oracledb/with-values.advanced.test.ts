...
describe(ctx.label, () => {
    ...
    test('values-aliased-via-as-keeps-original-with-name', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual union all select :2, :3 from dual) select pp.id as "id", pp.name as "name" from projectPatch pp order by "id""`)
    ...
    test('values-for-use-in-left-join-as-emits-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual) select project.id as "pid", pp.name as "newName" from project left join projectPatch pp on pp.id = project.id where project.id <= :2 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
    ...
    test('values-optional-column-allows-undefined-per-row', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchOpt(id, newName) as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", newName as "newName" from projectPatchOpt order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
    ...
    test('values-with-custom-typed-columns-emits-customint-customdouble-casts', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (select :0, :1 from dual union all select :2, cast(:3 as number) from dual) select issueId as "issueId", amount as "amount" from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-virtual-column-from-fragment-with-custom-type-emits-inline-fragment', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (select :0, :1 from dual union all select :2, cast(:3 as number) from dual) select issueId as "issueId", 'open' as "state" from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-optional-virtual-column-from-fragment-with-custom-type-emits-inline-fragment', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (select :0, :1 from dual union all select :2, cast(:3 as number) from dual) select issueId as "issueId", raw_to_uuid(null) as "billingRef" from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-base-type-optional-virtual-column-projects-optional-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with batchRow(id, "label") as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", "label" as "label", null as "rank", null as "note" from batchRow order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-branded-column-through-for-use-in-left-join-as-keeps-the-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (select :0, :1 from dual union all select :2, cast(:3 as number) from dual), issueBillingJoin(issueId, amount) as (select :4, :5 from dual) select issueBilling.issueId as "base", ibj.issueId as "joined" from issueBilling left join issueBillingJoin ibj on ibj.issueId = issueBilling.issueId order by "base""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-base-type-required-virtual-column-projects-required-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqBatch(id) as (select :0 from dual union all select :1 from dual) select id as "id", 7 as "seq", 'BATCH' as "tag" from reqBatch order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-virtual-column-referencing-a-sibling-column-inlines-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(nums)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vdouble(id) as (select :0 from dual union all select :1 from dual) select id as "id", id * 2 as "doubled" from vdouble order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
