...
describe(ctx.label, () => {
    ...
    test('aggregate-of-optional-uuid-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(external_ref) as refs from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-object-with-bigint-uuid-and-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(json_object('views', view_count, 'externalRef', external_ref, 'estimatedHours', estimated_hours)) as issues from issue where project_id = ? group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
