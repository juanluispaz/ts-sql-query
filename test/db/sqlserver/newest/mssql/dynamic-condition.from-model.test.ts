...
describe(ctx.label, () => {
    ...
    test('from-model/emits-the-same-sql-as-the-descriptor-form', async () => {
        ...
        expect(modelSql).toMatchInlineSnapshot(`"select id as id from issue where (lower(title) like lower(@0 + '%') or priority > @1) and view_count >= @2 and external_ref like ('%' + @3 + '%') order by id"`)
    ...
    test('from-model/nested-filter-emits-expected-sql', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id from project left outer join organization as organization on organization.id = project.organization_id where project.name like ('%' + @0 + '%') and organization.name = @1 order by id"`)
