...
describe(ctx.label, () => {
    ...
    test('table-customization: wrapper re-aliased as a select source keeps the hint', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".name as "orgName" from /*+ hint */ "organization" "o" where "o".id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: wrapper as a source inside a compound arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as "label" from /*+ hint */ "organization"  union select name as "label" from project where organization_id = :0 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
