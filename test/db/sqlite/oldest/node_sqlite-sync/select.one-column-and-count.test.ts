...
describe(ctx.label, () => {
    ...
    test('select-count-all-with-where-narrows-the-count', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as result from issue where status = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('select-one-column-of-expression-still-routes-via-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority + ? as result from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
          ]
    ...
    test('select-one-column-execute-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        try {
            result = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('select-one-column-optional-execute-many-null-elements', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select body as result from issue order by result is null, result asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('select-one-column-query-and-params-accessors', async () => {
        ...
        ctx.mockNext('open')
        const result = sync(built.executeSelectOne())
