...
describe(ctx.label, () => {
    ...
    test('adapter-columns-projected-through-a-compound-union', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as pid, score as score, reviewer_code as reviewerCode from project_review union select project_id as pid, score as score, reviewer_code as reviewerCode from project_review where id = @0 order by pid"`)
    ...
    test('adapter-column-through-inline-aggregated-array-value-per-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(convert(nvarchar, score), ','), ']') from project_review where project_id = project.id) as scores from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
