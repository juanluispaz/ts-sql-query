...
describe(ctx.label, () => {
    ...
    test('union-all-of-rule-4-all-optional-arms-drops-object-when-every-leaf-is-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", "body" as "opt.body", assignee_id as "opt.assigneeId" from issue where id = :0 union all select id as "iid", "body" as "opt.body", assignee_id as "opt.assigneeId" from issue where id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('union-of-rule-1-required-in-optional-object-arms-stays-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", status as "meta.gate", assignee_id as "meta.assigneeId" from issue where id = :0 union select id as "iid", status as "meta.gate", assignee_id as "meta.assigneeId" from issue where id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-aggregate-as-array-arms-merges-array-typed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name)) as "projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :0 group by "organization".id union all select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name)) as "projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :1 group by "organization".id order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-of-depth-3-nested-objects-recurses-through-columns-for-with-view', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "iid", title as "outer.mid.title", "number" as "outer.mid.num" from issue where id = :0 union select id as "iid", title as "outer.mid.title", "number" as "outer.mid.num" from issue where id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-nested-object-aggregate-arms-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name, 'archivedAt' value project.archived_at)) as "wrap.projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :0 group by "organization".id union all select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name, 'archivedAt' value project.archived_at)) as "wrap.projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :1 group by "organization".id order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-aggregate-as-array-arms-projecting-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name, 'archivedAt' value project.archived_at)) as "meta.projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :0 group by "organization".id union all select "organization".id as "orgId", json_arrayagg(json_object('id' value project.id, 'name' value project.name, 'archivedAt' value project.archived_at)) as "meta.projects" from "organization" left join project on project.organization_id = "organization".id where "organization".id = :1 group by "organization".id order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
