...
void tProjectReview
// Every test in this cell is now commented out (see the blocks below); these
// sentinels satisfy noUnusedLocals while the imports stay for cross-cell symmetry.
void expect; void test; void assertType; void tIssue; void tOrganization; void tProject
export type { Exact }
...
describe(ctx.label, () => {
    ...
    test('delete-using-with-extra-filter-on-using', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project, "organization" where issue.project_id = project.id and project.organization_id = "organization".id and "organization".id = :0"`)
    ...
    */
    ...
    test('delete-using-multiple-source-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project, "organization" where issue.project_id = project.id and project.organization_id = "organization".id and issue.priority = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    */
    ...
    test('delete-using-with-returning-none-or-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = :0 returning issue.id, issue.title into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
                {
    ...
    */
    ...
    test('delete-allowing-no-where-using-without-where-removes-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
            assertType<Exact<typeof affected, number>>()
    ...
    */
    ...
    test('delete-using-with-returning-auxiliary-using-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = :0 returning project.slug into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
                {
    ...
    */
    ...
    test('delete-using-returning-scaled-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review using project where project_review.project_id = project.id and project_review.id = :0 returning project_review.score into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    */
    ...
    test('delete-using-returning-nullable-projected-optional', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = :0 returning issue.id, issue."body" into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
                {
    ...
    */
    ...
    test('delete-using-returning-nested-using-joined-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = :0 returning issue.id, project.slug, project.name into :1, :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
                {
    ...
    */
    ...
    test('delete-using-left-join-returning-rule2-object-miss-drops-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project left join project_review on project_review.project_id = project.id where issue.project_id = project.id and issue.id = :0 returning issue.id, project_review.reviewer_code, :1 into :2, :3, :4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                1,
    ...
    */
    ...
    // NOT-APPLICABLE: Oracle accepts WITH only as a prefix to SELECT, so DELETE … USING <Values> (ORA-00928) is not emittable
    /*
