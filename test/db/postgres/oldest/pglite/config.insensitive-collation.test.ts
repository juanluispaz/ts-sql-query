...
describe(ctx.label, () => {
    ...
    test('collation: equalsInsensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email = $1 collate "C""`)
        ...
        await empty.selectFrom(tAppUser)
            ...
            .select({ id: tAppUser.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email = $1"`)
    })
    ...
    test('collation: likeInsensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .select({ id: tAppUser.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email ilike $1 collate "C""`)
        ...
        await empty.selectFrom(tAppUser)
            ...
            .select({ id: tAppUser.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email ilike $1"`)
    })
    ...
    test('collation: notLikeInsensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .select({ id: tAppUser.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where email not ilike $1 collate "C""`)
    ...
    test('collation: affix operators with a uuid receiver', async () => {
        ...
        await collated.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref::text ilike ($1 || '%') collate "C""`)
        ...
        await empty.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref::text ilike ('%' || $1 || '%')"`)
    })
    ...
    test('collation: negated affix operators with a uuid receiver', async () => {
        ...
        await collated.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref::text not ilike ($1 || '%') collate "C""`)
        ...
        await empty.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref::text not ilike ('%' || $1 || '%')"`)
    })
    ...
    test('collation: aggregate-array-order-by-insensitive', async () => {
        ...
        await collated.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select name as result from project where organization_id = organization.id order by name collate "C" asc) as a_1_))::text as names from organization where id = $1"`)
        ...
        await empty.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((select json_agg(a_1_.result) from (select name as result from project where organization_id = organization.id order by result asc) as a_1_))::text as names from organization where id = $1"`)
    })
})
