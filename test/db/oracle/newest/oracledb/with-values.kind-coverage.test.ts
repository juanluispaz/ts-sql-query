...
describe(ctx.label, () => {
    ...
    test('values-tuple-column-with-type-adapter-scales-write-and-read', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with scaledSampler(score) as (values (:0)) select score as "score" from scaledSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
          ]
    ...
    test('values-tuple-boolean-column-with-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ flag: v.flag })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with boolAdapterSampler(flag) as (values (case when (:0 = 1) then 'Y' else 'N' end)) select case when flag = 'Y' then 1 else 0 end as "flag" from boolAdapterSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('values-tuple-custom-kind-column-with-type-adapter-shifts-write-and-read', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with releaseTagSampler(ordinal) as (values (:0)) select ordinal as "ordinal" from releaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2005,
          ]
    ...
    test('values-tuple-optional-custom-kind-column-with-type-adapter-shifts-and-passes-null', async () => {
        ...
        const present = await ctx.conn.selectFrom(v1)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optionalReleaseTagSampler(ordinal) as (values (:0)) select ordinal as "ordinal" from optionalReleaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2005,
          ]
        ...
        const none = await ctx.conn.selectFrom(v2)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optionalReleaseTagSampler(ordinal) as (values (cast(:0 as number))) select ordinal as "ordinal" from optionalReleaseTagSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('values-tuple-cast-per-temporal-kind-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with temporalSampler("d", "t", ts) as (values (cast(:0 as date), :1, cast(:2 as timestamp))) select "d" as "d", "t" as "t", ts as "ts" from temporalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            null,
    ...
    test('values-tuple-cast-per-uuid-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with uuidSampler("ref", signing) as (values (uuid_to_raw(:0), uuid_to_raw(:1))) select raw_to_uuid("ref") as "ref", raw_to_uuid(signing) as "signing" from uuidSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('values-tuple-cast-per-custom-temporal-kind-via-null-value', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ d: v.d, t: v.t, ts: v.ts })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with customTemporalSampler("d", "t", ts) as (values (cast(:0 as date), :1, cast(:2 as timestamp))) select "d" as "d", "t" as "t", ts as "ts" from customTemporalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            null,
    ...
    test('values-tuple-virtual-column-from-fragment-per-kind', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with virtualSampler("n") as (values (:0)) select "n" as "n", :1 as "vbig", :2 as "vdbl", :3 as "vopt" from virtualSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            100n,
    ...
    test('values-tuple-optional-column-with-type-adapter-scales-and-passes-null', async () => {
        ...
        const present = await ctx.conn.selectFrom(v1)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with scaledOptionalSampler(score) as (values (:0)) select score as "score" from scaledOptionalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
          ]
        ...
        const none = await ctx.conn.selectFrom(v2)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with scaledOptionalSampler(score) as (values (cast(:0 as number))) select score as "score" from scaledOptionalSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('values-tuple-virtual-column-from-fragment-with-type-adapter-brackets-read', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with bracketSampler("n") as (values (:0)) select "n" as "n", upper('led') as "vtag", upper('led') as "vtagO" from bracketSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
          ]
    ...
    test('values-tuple-required-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with exprSampler(id) as (values (:0 + (2))) select id as "id" from exprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
    ...
    test('values-tuple-optional-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = await ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optExprSampler(id) as (values (:0 + (2))) select id as "id" from optExprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
