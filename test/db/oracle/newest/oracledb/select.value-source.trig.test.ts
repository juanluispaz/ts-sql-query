...
describe(ctx.label, () => {
    ...
    test('acos', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", acos(priority / :0) as "v" from issue where id = :1"`)
    ...
    test('asin', async () => {
        ...
        assertType<Exact<typeof result, Array<{ id: number; v: number }>>>()
        expect(result).toEqual(expected)
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", asin(priority / :0) as "v" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            1,
    ...
    test('trig-cot/optional-double-receiver', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const row = await ctx.conn.selectFrom(tIssue)
                ...
                .executeSelectOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", 1 / tan(estimated_hours / :0) as "ct" from issue where id = :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                10,
                1,
    ...
    test('trig-cot/double-required-receiver', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", 1 / tan(cast(priority as float) / :0) as "ct" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            10,
            1,
