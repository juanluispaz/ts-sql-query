...
describe(ctx.label, () => {
    ...
    test('adapter-column-through-inline-aggregated-array-value-per-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(score null on null) from project_review where project_id = project.id) as scores from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
