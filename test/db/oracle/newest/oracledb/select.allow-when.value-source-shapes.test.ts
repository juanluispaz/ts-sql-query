...
describe(ctx.label, () => {
    ...
    test('is-with-allowed-column-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue where decode(assignee_id, parent_id, 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('replace-all-if-value-with-allowed-second-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", replace(title collate BINARY, :0 collate BINARY, status) collate USING_NLS_COMP as "label" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "hero",
            1,
