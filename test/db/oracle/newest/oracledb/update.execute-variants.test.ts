...
describe(ctx.label, () => {
    ...
    test('execute-update-many-with-min-max-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = :0 where project_id = :1 returning id, status into :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "reviewed",
                1,
    ...
    test('execute-update-one-column-many-non-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = :0 where project_id = :1 returning status into :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "reviewed",
                1,
