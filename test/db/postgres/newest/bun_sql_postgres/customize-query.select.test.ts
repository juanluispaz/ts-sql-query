...
describe(ctx.label, () => {
    ...
    test('customize-recursive-select-projection-only-hooks-render-in-wrapping-cte-with-ordering', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = $1 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) order by parentId asc, id) select id as id from tree"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('customize-recursive-select-projection-only-hooks-render-in-wrapping-cte-with-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = $1 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) limit $2) select id as id from tree"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            10,
    ...
    test('customize-recursive-select-projection-only-hooks-render-in-wrapping-cte-with-limit-and-offset', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = $1 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) limit $2 offset $3) select id as id from tree"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            10,
    ...
    test('customize-recursive-select-projection-only-hooks-render-in-wrapping-cte-with-order-by-and-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (/* head */  select id as id, parent_id as parentId from issue where id = $1 union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId  /* tail */), tree as (select /* hint */ /* cols */  id as id, parentId as parentId from recursive_select_1 window w1 as (partition by id) order by parentId asc, id limit $2) select id as id from tree"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            10,
    ...
    test('customize-recursive-select-hooks-render-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as /* warmup */ (select id as result from issue where id = $1 union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) /* end-of-with */ select id as id, ((/* head */  select /* hint */ /* cols */  json_agg(result) from recursive_select_1  /* tail */))::text as tree from project where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-recursive-one-column-custom-window-and-ordering-in-inline-scalar-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as result from issue where id = $1 union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) select id as id, (select /* hint */ /* cols */  result as result from recursive_select_1 window w1 as (partition by result) order by result) as root from project where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-recursive-one-column-custom-window-and-ordering-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as result from issue where id = $1 union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) select id as id, ((select json_agg(a_2_.result) from (select /* hint */ /* cols */  result as result from recursive_select_1 window w1 as (partition by result) order by result) as a_2_))::text as tree from project where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-recursive-one-column-ordering-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as result from issue where id = $1 union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) select id as id, ((select json_agg(a_2_.result) from (select result as result from recursive_select_1 order by result) as a_2_))::text as tree from project where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-select-plain-inline-value-renders-query-hooks-but-drops-with-query-hooks', async () => {
        ...
        const scalarResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (/* head */  select count(*) as result from issue where project_id = project.id  /* tail */) as "n" from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const arrResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ((/* head */  select json_agg(json_build_object('iid', id)) from issue where project_id = project.id  /* tail */))::text as issues from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
