...
describe(ctx.label, () => {
    ...
    test('customize-compound-before-and-after-query-wrap-union', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"/* compound-head */  select name as label from project where archived_at is null union select title as label from issue where status = $1 order by label  /* compound-tail */"`)
    ...
    test('customize-compound-with-query-hooks-wrap-cte', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with openIssues as (select id as id from issue where status = $1) select id as id from openIssues intersect select id as id from issue where id <= $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            2,
    ...
    test('customize-compound-all-hooks-combined-on-except', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  with allIssueIds as (select id as id from issue) select id as id from allIssueIds except select id as id from issue where priority = $1 order by id  /* tail */"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customize-compound-materialised-as-cte-via-for-use-in-query-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with combined as (/* compound-head */  select id as id, name as label from project where id = $1 union select id as id, name as label from project where id = $2  /* compound-tail */) select id as id, label as label from combined order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-inline-scalar-value-via-for-use-as-inline-query-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (/* compound-head */  select name as result from project where id = $1 union select name as result from project where id = $2  /* compound-tail */) as label from project where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-compound-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  select id as id, name as label from project where id in ($1, $2) union select id as id, name as label from project where id = $3 order by id limit $4  /* tail */"`)
    ...
    test('customize-compound-execute-select-page-without-limit-still-wraps-count-with-hooks', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  select id as id, name as label from project where id in ($1, $2) union select id as id, name as label from project where id = $3 order by id  /* tail */"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-as-cte-with-query-hooks-render-around-cte-parens', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with combined as /* with-head */  (/* compound-head */  select id as id, name as label from project where id = $1 union select id as id, name as label from project where id = $2  /* compound-tail */)  /* with-tail */ select id as id, label as label from combined order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-inline-aggregated-array-via-for-use-as-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select ((select json_agg(a_1_.result) from (/* compound-head */  select name as result from project where id = $1 union select name as result from project where id = $2  /* compound-tail */) as a_1_))::text as names"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-arm-custom-window-lands-on-un-parenthesized-first-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from project where id = $1 window w1 as (partition by organization_id) union select name as label from project where id = $2 order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-arm-after-select-keyword-lands-on-first-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select /* hint */ name as label from project where id = $1 union select name as label from project where id = $2 order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-arm-before-columns-lands-on-first-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select /* cols */  name as label from project where id = $1 union select name as label from project where id = $2 order by label"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
