...
describe(ctx.label, () => {
    ...
    test('equivalence/is-and-is-not', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where priority is ? and status is not ? order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            2,
            "closed",
    ...
    test('equivalence/like-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where title like ? escape '\\' and title not like ? escape '\\' and lower(title) like lower(?) escape '\\' and lower(title) not like lower(?) escape '\\' order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "%a%",
            "%b%",
    ...
    test('equivalence/affix-sensitive', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where title like (? || '%') escape '\\' and title not like (? || '%') escape '\\' and title like ('%' || ?) escape '\\' and title not like ('%' || ?) escape '\\' and title like ('%' || ? || '%') escape '\\' and title not like ('%' || ? || '%') escape '\\' order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    test('equivalence/affix-insensitive', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) like lower(? || '%') escape '\\' and lower(title) not like lower(? || '%') escape '\\' and lower(title) like lower('%' || ?) escape '\\' and lower(title) not like lower('%' || ?) escape '\\' and lower(title) like lower('%' || ? || '%') escape '\\' and lower(title) not like lower('%' || ? || '%') escape '\\' order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    // `uuid_str(external_ref)`, and the node-sqlite3 driver exposes no
    // user-function API to register `uuid_str` (see test/FUTURE_CONNECTORS.md).
    /*
    ...
    */
    ...
    test('equivalence/if-value-equalable-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where priority = ? and priority <> ? and priority is ? and priority is not ? and priority in (?, ?) and priority not in (?) order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('equivalence/if-value-string-equality-and-like-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) = lower(?) and lower(title) <> lower(?) and title like ? escape '\\' and title not like ? escape '\\' and lower(title) like lower(?) escape '\\' and lower(title) not like lower(?) escape '\\' order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Foo",
            "Bar",
