...
describe(ctx.label, () => {
    ...
    test('custom-column-under-not-equals-the-direct-negate', async () => {
        ...
        expect(dynSql).toMatchInlineSnapshot(`"select id as id from project_release where not (channel = $1) order by id"`)
    ...
    test('custom-comparable-column-under-or-equals-the-direct-or', async () => {
        ...
        expect(ctx.lastSql).toBe(dynSql)
        expect(ctx.lastParams).toEqual(dynParams)
        expect(dynSql).toMatchInlineSnapshot(`"select id as id from project_release where version = $1 or version = $2 order by id"`)
        expect(dynParams).toMatchInlineSnapshot(`
          [
            "1.2.0",
            "0.9.0",
    ...
    test('model-derived-filter-with-extension-rule-emits-sql', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status = $1 and exists(select id as result from project where id = issue.project_id and name ilike ('%' || $2 || '%')) order by id"`)
    ...
    test('dynamic-condition-in-a-compound-union-matches-the-direct-predicate', async () => {
        ...
        expect(ctx.lastSql).toBe(dynSql)
        expect(ctx.lastParams).toEqual(dynParams)
        expect(dynSql).toMatchInlineSnapshot(`"select id as id from issue where priority > $1 union select id as id from issue where priority = $2"`)
        expect(dynParams).toMatchInlineSnapshot(`
          [
            2,
            1,
