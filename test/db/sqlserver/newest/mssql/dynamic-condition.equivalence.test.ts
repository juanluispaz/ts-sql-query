...
describe(ctx.label, () => {
    ...
    test('equivalence/is-and-is-not', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where priority = @0 and status <> @1 order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            2,
            "closed",
    ...
    test('equivalence/like-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where title like @0 and title not like @1 and lower(title) like lower(@2) and lower(title) not like lower(@3) order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "%a%",
            "%b%",
    ...
    test('equivalence/affix-sensitive', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where title like (@0 + '%') and title not like (@1 + '%') and title like ('%' + @2) and title not like ('%' + @3) and title like ('%' + @4 + '%') and title not like ('%' + @5 + '%') order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    test('equivalence/affix-insensitive', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) like lower(@0 + '%') and lower(title) not like lower(@1 + '%') and lower(title) like lower('%' + @2) and lower(title) not like lower('%' + @3) and lower(title) like lower('%' + @4 + '%') and lower(title) not like lower('%' + @5 + '%') order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Re",
            "Un",
    ...
    test('equivalence/boolean-column-custom-adapter-dispatch', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from project where cast(case when published = 't' then 1 else 0 end as bit) = @0 order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('equivalence/uuid-as-string-operator-path', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where external_ref like ('%' + @0 + '%') order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "abc",
          ]
    ...
    test('equivalence/if-value-equalable-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where priority = @0 and priority <> @1 and priority = @2 and priority <> @3 and priority in (@4, @5) and priority not in (@6) order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('equivalence/if-value-string-equality-and-like-family', async () => {
        ...
        expect(dynamic.sql).toBe(ref.sql)
        expect(dynamic.params).toEqual(ref.params)
        expect(ref.sql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) = lower(@0) and lower(title) <> lower(@1) and title like @2 and title not like @3 and lower(title) like lower(@4) and lower(title) not like lower(@5) order by id"`)
        expect(ref.params).toMatchInlineSnapshot(`
          [
            "Foo",
            "Bar",
