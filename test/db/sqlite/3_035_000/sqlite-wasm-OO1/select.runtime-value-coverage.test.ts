...
describe(ctx.label, () => {
    ...
    test('empty-set-scalar-aggregate-is-undefined', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority) as "s", avg(cast(priority as real)) as "a", min(priority) as lo, max(priority) as hi from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('optional-non-empty-array-over-empty-aggregate-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select json_group_array(json_object('id', a_1_.id, 'name', a_1_.name)) from (select id as id, name as name from project where organization_id = organization.id and name = ? order by id) as a_1_) as projects from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "__no-such-project__",
            1,
    ...
    test('aggregate-as-array-left-join-miss-inner-object-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(json_object('marker', project.slug, 'issue.id', issue.id, 'issue.title', issue.title)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
