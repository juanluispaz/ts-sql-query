...
describe(ctx.label, () => {
    ...
    test('rule-2-left-join-object-mixing-a-const-leaf-stays-optional-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.name as [proj.name], @0 as [proj.tag] from issue left join project on project.id = issue.project_id where issue.id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            1,
    ...
    test('rule-2-left-join-object-mixing-a-const-leaf-dropped-on-join-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as [iss.title], @0 as [iss.tag] from project left join issue on issue.project_id = project.id where project.id in (@1, @2) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('two-different-left-joins-in-one-object-demotes-both-leaves-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.name as [obj.projName], organization.name as [obj.orgName] from issue left join project on project.id = issue.project_id left join organization on organization.id = project.organization_id where issue.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-1-plain-object-with-left-join-required-in-optional-object-leaf-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.name as [proj.name], project.slug as [proj.slug] from issue left join project on project.id = issue.project_id where issue.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('rule-3-required-container-own-and-left-join-leaf-dropped-on-join-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, project.id as [mix.ownId], issue.title as [mix.issTitle] from project left join issue on issue.project_id = project.id where project.id in (@0, @1) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('two-different-left-joins-full-miss-drops-object-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as [obj.issTitle], app_user.full_name as [obj.assigneeName] from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id in (@0, @1) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('two-different-left-joins-partial-miss-keeps-object-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as [obj.issTitle], app_user.full_name as [obj.assigneeName] from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('rule-1-required-in-optional-object-marker-hits-while-demoted-leaf-misses-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as [mix.title], app_user.full_name as [mix.assigneeName] from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('rule-2-const-FIRST-leaf-order-object-dropped-on-join-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, @0 as [iss.tag], issue.title as [iss.title] from project left join issue on issue.project_id = project.id where project.id in (@1, @2) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('all-const-nested-object-is-required-and-never-dropped', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, @0 as [tags.a], @1 as [tags.b] from project where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "x",
            "y",
    ...
    test('two-different-left-joins-plus-const-promotes-to-rule-3-container-survives-full-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as [obj.issTitle], app_user.full_name as [obj.assigneeName], @0 as [obj.tag] from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id in (@1, @2) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('two-different-left-joins-merged-in-one-leaf-plus-const-survives-partial-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id + app_user.id as [obj.combined], @0 as [obj.tag] from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where issue.id in (@1, @2) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('merged-two-left-join-leaf-plus-const-inside-aggregate-element-demotes-leaf-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .groupBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"combined": ', isnull(convert(nvarchar, issue.id + app_user.id), 'null'), ', "tag": ', '"' + string_escape(convert(nvarchar(max), 'rel'), 'json') + '"', '}'), ','), ']') as items from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('merge-optional-required-in-optional-object-fragment-gates-rule-1-drop', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, coalesce(issue.title, issue.status, issue.body) as [grp.frag], project.name as [grp.projName] from project left join issue on issue.project_id = project.id where project.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
