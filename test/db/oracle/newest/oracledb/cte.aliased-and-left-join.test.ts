...
describe(ctx.label, () => {
    ...
    test('cte-aliased-via-as-keeps-original-with-name-in-with-clause', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = :0) select oi.id as "id", oi.projectId as "projectId" from open_issues oi where oi.projectId = :1 order by "id""`)
    ...
    test('cte-forUseInLeftJoin-emits-left-join-and-marks-columns-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = :0) select project.name as "projectName", open_issues.id as "openId" from project left join open_issues on open_issues.projectId = project.id order by "projectName""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('cte-forUseInLeftJoinAs-uses-alias-in-on-and-projection', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = :0) select project.name as "projectName", oi.id as "openId" from project left join open_issues oi on oi.projectId = project.id order by "projectName""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
    ...
    test('cte-aliased-then-joined-against-same-cte-emits-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = :0) select open_issues.projectId as "projectId", open_issues.id as "leftId", oi2.id as "rightId" from open_issues inner join open_issues oi2 on oi2.projectId = open_issues.projectId and oi2.id > open_issues.id order by "leftId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
