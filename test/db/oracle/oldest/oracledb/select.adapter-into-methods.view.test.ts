...
describe(ctx.label, () => {
    ...
    test('version-bracketed-into-substring-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(version_bracketed, :0, :1) as "sg" from release_overview where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('version-bracketed-into-substr-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(version_bracketed, :0, :1) as "sb" from release_overview where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('version-bracketed-into-length-passes-through-number-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", length(version_bracketed) as "len" from release_overview where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('channel-bracketed-into-substring-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(channel_bracketed, :0, :1) as "sg" from release_overview where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('channel-bracketed-into-substr-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(channel_bracketed, :0, :1) as "sb" from release_overview where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('channel-bracketed-into-length-passes-through-number-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", length(channel_bracketed) as "len" from release_overview where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('project-name-replace-all-with-bracketed-find-string-inherits-the-operands-adapter', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", replace(project_name collate BINARY, :0 collate BINARY, version_bracketed) collate USING_NLS_COMP as "rp" from release_overview where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "site",
            1,
