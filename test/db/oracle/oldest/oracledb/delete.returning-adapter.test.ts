...
describe(ctx.label, () => {
    ...
    test('delete-project-review-returning-bracket-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = :0 returning reviewer_code into :1"`)
    ...
    test('delete-project-review-returning-scaled-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = :0 returning score into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-project-release-returning-virtual-fragment-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release where id = :0 returning upper(channel) into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-col-matrix-virtual-adapter-returning-scaled-int', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from col_matrix where id = :0 returning m_int into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
