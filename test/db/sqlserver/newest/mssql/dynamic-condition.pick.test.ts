...
describe(ctx.label, () => {
    ...
    test('pick/aggregate-as-array-field-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('title':issue.title, 'body':issue.body)) as issues from project inner join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
