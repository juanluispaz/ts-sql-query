...
describe(ctx.label, () => {
    ...
    test('valueWhenNull-coalesces', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl("body", :0) as "body" from issue order by "id""`)
    ...
    test('dynamicWhere/single-and-equivalent-to-where', async () => {
        ...
        expect(refSql).toMatchInlineSnapshot(`"select id as "id" from issue where status = :0 order by "id""`)
    ...
    test('dynamicWhere/and-and-chain-equivalent', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as "id" from issue where project_id = :0 and priority > :1 order by "id""`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('dynamicWhere/or-or-chain-equivalent', async () => {
        ...
        expect(ctx.lastSql).toBe(refSql)
        expect(ctx.lastParams).toEqual(refParams)
        expect(refSql).toMatchInlineSnapshot(`"select id as "id" from issue where status = :0 or assignee_id is null order by "id""`)
        expect(refParams).toMatchInlineSnapshot(`
          [
            "closed",
          ]
