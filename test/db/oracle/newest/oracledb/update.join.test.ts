    ...
    */
    ...
    test('update-from-table-then-two-inner-joins-on-from-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set project.name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id inner join project_review on project_review.project_id = issue.project_id where project.id = issue.project_id and issue.id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
