...
describe(ctx.label, () => {
    ...
    test('delete-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const removed = sync(ctx.conn.deleteFrom(tIssue)
    ...
    test('delete-returning-one-column-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            const bumped = sync(ctx.conn.deleteFrom(tIssue)
                .where(tIssue.id.equals(3))
    ...
    test('delete-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.deleteFrom(tIssue)
                .where(tIssue.id.equals(3))
                .returning({
                    id:  tIssue.id,
    ...
    test('delete-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('stable')
            const channel = sync(ctx.conn.deleteFrom(tProjectRelease)
    ...
    test('delete-release-draft-returning-one-column-optional-branded', async () => {
        ...
        await ctx.withRollback(async () => {
            const stage = sync(ctx.conn.deleteFrom(tReleaseDraft)
    ...
    test('delete-release-draft-returning-object-optional-branded-default', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.deleteFrom(tReleaseDraft)
                .where(tReleaseDraft.id.equals(1))
    ...
    test('delete-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const bodies = sync(ctx.conn.deleteFrom(tIssue)
                .where(tIssue.projectId.equals(1))
    ...
    test('delete-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const present = sync(ctx.conn.deleteFrom(tIssue)
            ...
            const absent = sync(ctx.conn.deleteFrom(tIssue)
                .where(tIssue.id.equals(1))
                .returning({
                    id:   tIssue.id,
                    meta: { body: tIssue.body },
    ...
    test('delete-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const before = sync(ctx.conn.selectFrom(tProjectReview)
                .where(tProjectReview.id.equals(1))
                .select({ reviewDate: tProjectReview.reviewDate, reviewTime: tProjectReview.reviewTime })
                .executeSelectOne())

            ctx.mockNext({ reviewDate: new Date(Date.UTC(2024, 4, 20)), reviewTime: new Date(Date.UTC(1970, 0, 1, 14, 30, 45)) })
            const ret = sync(ctx.conn.deleteFrom(tProjectReview)
                .where(tProjectReview.id.equals(1))
                .returning({ reviewDate: tProjectReview.reviewDate, reviewTime: tProjectReview.reviewTime })
                .executeDeleteOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = ? returning review_date as reviewDate, review_time as reviewTime"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const before = sync(ctx.conn.selectFrom(tProjectRelease)
                .where(tProjectRelease.id.equals(1))
            ...
            ctx.mockNext({ releasedOn: new Date(Date.UTC(2024, 0, 15)), cutoffTime: new Date(Date.UTC(1970, 0, 1, 17, 0, 0)), publishedAt: new Date(Date.UTC(2024, 0, 16, 9, 0, 0)) })
            const ret = sync(ctx.conn.deleteFrom(tProjectRelease)
                .where(tProjectRelease.id.equals(1))
                .returning({
                ...
                .executeDeleteOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release where id = ? returning released_on as releasedOn, cutoff_time as cutoffTime, published_at as publishedAt"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-returning-localdatetime-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const before = sync(ctx.conn.selectFrom(tProject)
                .where(tProject.id.equals(4))
                .select({ archivedAt: tProject.archivedAt })
                .executeSelectOne())

            ctx.mockNext({ archivedAt: new Date(Date.UTC(2024, 5, 15, 8, 30, 0)) })
            const ret = sync(ctx.conn.deleteFrom(tProject)
                .where(tProject.id.equals(4))
                .returning({ archivedAt: tProject.archivedAt })
                .executeDeleteOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project where id = ? returning archived_at as archivedAt"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
              ]
    ...
    test('delete-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = sync(ctx.conn.deleteFrom(tProjectReview)
                .where(tProjectReview.id.equals(1))
                .returningOneColumn(tProjectReview.reviewTime)
                .executeDeleteOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = ? returning review_time as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
