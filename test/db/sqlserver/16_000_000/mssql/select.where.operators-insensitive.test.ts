...
describe(ctx.label, () => {
    ...
    test('affix-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(case when lower(email) like lower(replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 else 0 end as bit) as sw, cast(case when lower(email) not like lower(replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 else 0 end as bit) as nsw, cast(case when lower(email) like lower('%' + replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]')) then 1 else 0 end as bit) as ew, cast(case when lower(email) not like lower('%' + replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]')) then 1 else 0 end as bit) as [new], cast(case when lower(email) like lower('%' + replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 else 0 end as bit) as ct, cast(case when lower(email) not like lower('%' + replace(replace(replace(full_name, '[', '[[]'), '%', '[%]'), '_', '[_]') + '%') then 1 else 0 end as bit) as nct from app_user where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
