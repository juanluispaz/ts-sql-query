...
describe(ctx.label, () => {
    ...
    */
    ...
    test('recursive-result-order-by-limit-offset-then-for-use-in-query-as-drops-them', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with tree as (select id as id, title as title from issue where id = @0 union all select issue.id as id, issue.title as title from issue join tree on issue.parent_id = tree.id) select id as id, title as title from tree"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
