...
describe(ctx.label, () => {
    ...
    test('length-and-substr-keep-trailing-spaces', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select len(@0 + '.') - 1 as len, substring(@1, @2) as whole, substring(@3, @4) as tail"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Draft  ",
            "Draft  ",
    ...
    test('substring-to-end', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, @0) as sub from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            1,
    ...
    test('negative-index-follows-javascript', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select right(@0, @1) as lastChar, right(@2, @3) as fromEnd, right(@4, @5) as fromEndFar, left(right(@6, @7), @8) as fromEndCount, left(right(@9, @10), @11) as fromEndFarCount, substring(@12, @13) as clamped, substring(@14, @15, @16) as clampedEnd, substring(@17, @18, @19) as swapped"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            1,
    ...
    test('substr-family-with-optional-value-source-argument', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(title, assignee_id + 1) as se, substring(title, assignee_id + 1) as sse, substring(title, assignee_id + 1, @0) as s2, substring(title, assignee_id + 1, @1 - assignee_id) as sg from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
