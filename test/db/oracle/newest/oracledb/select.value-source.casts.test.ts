...
describe(ctx.label, () => {
    ...
    test('asInt-on-double-uses-round', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(round(priority / :0) as number(38)) as "rounded" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('asDouble-chain-roundtrips-int', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(round(cast(priority as float)) as number(38)) as "p" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('asDouble-on-nullable-column-keeps-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(assignee_id as float) as "a" from issue where id in (:0, :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('asBigint-on-double-chained-into-bigint-op', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(round(cast(priority as float)) as number(38)) + :0 as "a", mod(cast(round(cast(priority as float)) as number(38)), :1) as "m" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2n,
            2n,
    ...
    // TODO[BUG]: see BUGS.md — the emitted SQL computes the sum exactly, but oracledb marshals the NUMBER(38) result through a JavaScript number, so 9007199254740995 arrives as 9007199254740996: an integral value the bigint marshaller accepts, i.e. a clean but wrong bigint.
    /*
