    ...
    */
    ...
    test('update-from-table-then-left-outer-join-on-from-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project, issue left outer join app_user on app_user.id = issue.assignee_id set project.\`name\` = ifnull(app_user.full_name, ?) where project.id = issue.project_id and issue.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Unassigned",
                3,
    ...
    */
    ...
    test('update-from-table-then-left-join-returning-left-join-optional-column-as-nullable-surfaces-present-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project, issue left join app_user on app_user.id = issue.assignee_id set project.\`name\` = issue.title where project.id = issue.project_id and issue.id = ? returning project.id as id, app_user.full_name as assignee"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
              ]
    ...
    */
    ...
    test('update-allowing-no-where-from-table-then-inner-join-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project, issue inner join app_user on app_user.id = issue.assignee_id and issue.id = ? set project.\`name\` = app_user.full_name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-from-table-then-join-on-from-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project, issue join app_user on app_user.id = issue.assignee_id set project.\`name\` = app_user.full_name where project.id = issue.project_id and issue.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
