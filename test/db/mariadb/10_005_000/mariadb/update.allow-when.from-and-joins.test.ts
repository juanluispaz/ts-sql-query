...
describe(ctx.label, () => {
    ...
    test('update-from-join-all-gates-open-walks-to-end-and-runs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* gated issue.id */  update /* gated issue.project_id */ project, issue inner join app_user on app_user.id = issue.assignee_id set project.name = app_user.full_name where project.id = issue.project_id and issue.id = ?  /* gated project.slug */"`)
    ...
    // NOT-SUPPORTED: see ENGINE_SUPPORT.md — MariaDB rejects `RETURNING` on a multi-table `UPDATE a, b SET ...`; single-table `UPDATE ... RETURNING` works from 13.0.1+, but the multi-table form is not accepted on any current MariaDB (mariadb:latest 12.x → ER_PARSE_ERROR). Snapshot pre-baked; uncomment if a MariaDB release adds it.
    /*
    test('update-from-join-all-gates-open-with-allowed-returning-walks-to-end', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project, issue inner join app_user on app_user.id = issue.assignee_id set project.name = app_user.full_name where project.id = issue.project_id and issue.id = ? returning project.id as id, project.name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
