...
describe(ctx.label, () => {
    ...
    test('equals-and-not-equals-and-lt-gt', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where id = @0 and priority > @1 and priority < @2 and status <> @3 order by id"`)
    ...
    test('lessOrEqual-and-greaterOrEqual', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority >= @0 and priority <= @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('in-and-not-in', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status in (@0, @1) and id not in (@2, @3) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "in_progress",
    ...
    test('like-and-not-like', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title like @0 and title not like @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%triage%",
            "%draft%",
    ...
    test('like-insensitive-and-not-like-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where lower(title) like lower(@0) and lower(title) not like lower(@1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%TRIAGE%",
            "%DRAFT%",
    ...
    test('starts-with-ends-with-contains', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title like (@0 + '%') and title like ('%' + @1) and title like ('%' + @2 + '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Re",
            "me",
    ...
    test('not-starts-with-not-ends-with-not-contains', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not like (@0 + '%') and title not like ('%' + @1) and title not like ('%' + @2 + '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Re",
            "me",
    ...
    test('nested-and-or-not', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status = @0 and (priority > @1 or not (title like ('%' + @2 + '%'))) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            5,
