...
describe(ctx.label, () => {
    ...
    test('optional-const/int-double-bigint-boolean', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "i", :1 as "d", :2 as "bi", :3 as "b" from dual"`)
    ...
    test('optional-const/uuid-enum-custom-comparable', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select raw_to_uuid(uuid_to_raw(:0)) as "u", :1 as "e", :2 as "c", :3 as "cc" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('optional-const/custom-numeric-and-uuid', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "ci", :1 as "cd", raw_to_uuid(uuid_to_raw(:2)) as "cu" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            1.5,
    ...
    test('optional-const/temporal-time-and-datetime', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select :0 as "lt", :1 as "ldt", :2 as "clt", :3 as "cldt" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
