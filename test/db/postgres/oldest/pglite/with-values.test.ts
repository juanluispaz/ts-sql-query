...
describe(ctx.label, () => {
    ...
    test('values in select-from', async () => {
        ...
        await ctx.conn.selectFrom(patch)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select id as id, name as name from projectPatch"`)
    ...
    test('values in update-from', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text)) update project set name = projectPatch.name from projectPatch where project.id = projectPatch.id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "renamed",
    ...
    test('values in a compound-union arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select id as id, name as name from projectPatch union select id as id, name as name from project where id = $5 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            "from-values-a",
    ...
    test('values-as-intersect-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4), ($3::int4)) select id as id from idList intersect select id as id from project order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-except-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4), ($3::int4)) select id as id from idList except select id as id from project order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-union-all-arm-preserves-duplicates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4)) select id as id from idList union all select id as id from project where id = $3 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-as-non-seed-trailing-union-arm', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4)) select id as id from project where id in ($3, $4) union select id as id from idList order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            200,
    ...
    test('values-source-with-limit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4), ($3::int4)) select id as id from idList order by id limit $4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-with-limit-and-offset', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4), ($3::int4)) select id as id from idList order by id limit $4 offset $5"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-count-aggregate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4), ($2::int4), ($3::int4)) select count(id) as total from idList"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-source-group-by', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with groupRows("n", grp) as (values ($1::int4, $2::text), ($3::int4, $4::text), ($5::int4, $6::text)) select grp as grp, count("n") as total from groupRows group by grp order by grp"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-sourced-scalar-inline-equals-operand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPick(projectId, tag) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select id as id, name as name from project where id = (select projectId as result from projectPick where tag = $5) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "primary",
    ...
    test('values-self-joined-to-its-own-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (values ($1::int4, $2::int4, $3::text), ($4::int4, $5::int4, $6::text)) select treeNode.id as id, anc.name as "ancName" from treeNode inner join treeNode as anc on anc.id = treeNode.parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-self-joined-via-left-join-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (values ($1::int4, $2::int4, $3::text), ($4::int4, $5::int4, $6::text)) select treeNode.id as id, anc.name as "ancName" from treeNode left join treeNode as anc on anc.id = treeNode.parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-self-joined-with-two-aliased-clones-neither-is-the-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (values ($1::int4, $2::int4, $3::text), ($4::int4, $5::int4, $6::text)) select "a".id as id, "b".name as "ancName" from treeNode as "a" inner join treeNode as "b" on "b".id = "a".parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-in-both-arms-of-a-compound-hoists-two-distinct-withs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text), ($3::int4, $4::text)), idList(id) as (values ($5::int4), ($6::int4)) select id as id from projectPatch union all select id as id from idList order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-in-a-non-correlated-exists-hoists-the-with', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values ($1::int4)) select id as id from project where exists(select id as result from idList) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
