...
describe(ctx.label, () => {
    ...
    test('select-by-adapter-scaled-provided-primary-key-round-trips', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select invoice_no as invoiceNo, total as total from invoice where invoice_no = ?"`)
    ...
    test('insert-scales-the-adapter-provided-primary-key', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const affected = sync(ctx.conn.insertInto(tInvoice)
                .values({ invoiceNo: 30, total: 750 })
                .executeInsert())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into invoice (invoice_no, total) values (?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                300,
                750,
            ...
            if (ctx.realDbEnabled) {
                ...
                const back = sync(ctx.conn.selectFrom(tInvoice)
    ...
    test('returning-reads-the-adapter-scaled-provided-primary-key-back-unscaled', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.insertInto(tInvoice)
                .values({ invoiceNo: 40, total: 900 })
                .returning({ invoiceNo: tInvoice.invoiceNo, total: tInvoice.total })
                .executeInsertOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into invoice (invoice_no, total) values (?, ?) returning invoice_no as invoiceNo, total as total"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                400,
                900,
