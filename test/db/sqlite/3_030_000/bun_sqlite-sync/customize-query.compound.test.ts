...
describe(ctx.label, () => {
    ...
    test('customize-compound-carries-query-execution-name-and-metadata', async () => {
        ...
        ctx.mockNext(expected)
        const connection = ctx.conn
        const result = sync(connection.selectFrom(tProject).where(tProject.id.equals(1)).select({ label: tProject.name })
    ...
    test('customize-compound-materialised-as-cte-via-for-use-in-query-as', async () => {
        ...
        const result = sync(connection.selectFrom(combined)
    ...
    test('customize-compound-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        ctx.mockNext(3)
        const connection = ctx.conn
        const page = sync(connection.selectFrom(tProject).where(tProject.id.in([1, 2])).select({ id: tProject.id, label: tProject.name })
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count as (/* head */  select id as id, name as label from project where id in (?, ?) union select id as id, name as label from project where id = ?  /* tail */) select count(*) from result_for_count"`)
    ...
    test('customize-compound-inline-aggregated-array-via-for-use-as-inline-aggregated-array-value', async () => {
        ...
        const rows = sync(connection.selectFromNoTable()
            .select({ names })
            .executeSelectMany())

        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_group_array(a_1_.result) from (/* compound-head */  select name as result from project where id = ? union select name as result from project where id = ?  /* compound-tail */) as a_1_) as names"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('customize-compound-arm-custom-window-lands-on-un-parenthesized-first-arm', async () => {
        ...
        const arm2 = connection.selectFrom(tProject)
            ...
            .select({ label: tProject.name })
        const result = sync(arm1.union(arm2).orderBy('label').executeSelectMany())
    ...
    test('customize-compound-directly-executed-with-query-hooks-are-a-no-op-boundary', async () => {
        ...
        ctx.mockNext(expected)
        const connection = ctx.conn
        const result = sync(connection.selectFrom(tProject).where(tProject.id.in([1, 2])).select({ id: tProject.id })
        ...
        // the full SQL).
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where id in (?, ?) union select id as id from project where id = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
