...
// tAppUser is referenced only by the commented-out on-constraint tests below;
// the postgres cells where those tests are uncommented use it for real.
void tAppUser
...
describe(ctx.label, () => {
    ...
    test('ignore-if-has-no-value-prunes-the-on-conflict-update-set', async () => {
        // `ignoreIfHasNoValue(...)` chained after `onConflictDoUpdateDynamicSet({...})`
        ...
        // exists, so the conflict fires and the row is updated. MySQL/MariaDB
        // count an `ON DUPLICATE KEY UPDATE` that updates an existing row as
        // 2 affected rows (1 attempted insert + 1 update) — the documented
        // engine semantics, verified against the real container.
        ctx.mockNext(2)
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'ignored' })
                .onConflictDoUpdateDynamicSet({ name: 'Reactivated', archivedAt: null })
    ...
    test('set-if-set-if-value-keeps-staged-slug-when-new-value-missing-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'ignored' })
                .onConflictDoUpdateDynamicSet({ name: 'Reactivated', slug: 'mktg-site-keep' })
    ...
    test('ignore-if-set-drops-a-staged-column-on-conflict', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'ignored' })
                .onConflictDoUpdateDynamicSet({ name: 'Reactivated', slug: 'mktg-site-v2' })
                .ignoreIfSet('slug')
                .executeInsert()
    ...
    */
    ...
    test('one-shot-do-update-set-if-value-emptying-the-on-conflict-update-set-degrades-to-conflict-noop', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.insertInto(tProject)
                .values({ organizationId: 1, slug: 'mktg-site', name: 'ignored' })
                .onConflictDoUpdateSetIfValue({ archivedAt: undefined })
                .executeInsert()
    ...
    test('empty-on-conflict-update-set-degrades-to-conflict-noop-preserving-returning-id', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, slug, name) values (?, ?, ?) returning id as id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    */
    ...
    test('empty-on-conflict-update-set-degrades-to-conflict-noop-preserving-returning-object-many', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, slug, name) values (?, ?, ?) returning id as id, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
...
// NOT-APPLICABLE: MariaDB has no ON CONFLICT ON CONSTRAINT
    ...
    */
    ...
    test('empty-on-conflict-update-set-degrade-on-colliding-row-throws-mandatory-value-not-received', async () => {
        ...
        // row COLLIDES (org 1, 'mktg-site' — the seed row) so it is suppressed. The
        // degrade still types the returningLastInsertedId result as a NON-null `number`,
        // but the suppressed row returns no id, so the guard throws
        // MANDATORY_VALUE_NOT_RECEIVED_FROM_DATABASE. The control twin below resolves null
        // instead.
        ctx.mockNext(null)
        await ctx.withRollback(async () => {
            let caught: unknown
    ...
    */
    ...
    // NOT-APPLICABLE: MySQL/MariaDB use the bare `on duplicate key update` form and have no
    //                 ON CONFLICT conflict-target (partial-index) predicate — the
    //                 `onConflictOn(...).where(...)` arbiter is typed only on PostgreSQL/SQLite.
    /*
    ...
    */
    ...
    test('empty-on-conflict-update-set-degrades-preserving-returning-one-column-none-or-one', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, slug, name) values (?, ?, ?) returning id as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('empty-on-conflict-update-set-degrades-preserving-returning-under-shaped', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = await ctx.conn.insertInto(tProject)
                ...
                .onConflictDoUpdateSetIfValue({ arch: undefined })
                .returning({ id: tProject.id })
                .executeInsertNoneOrOne()

            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert ignore into project (organization_id, name, slug) values (?, ?, ?) returning id as id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
