...
describe(ctx.label, () => {
    ...
    test('set-if-not-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?"`)
            expect(trueSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?, project.slug = ?"`)
    ...
    test('ignore-if-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?, project.slug = ?"`)
            expect(trueSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
    ...
    test('ignore-if-has-no-value-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?, project.archived_at = ?"`)
            expect(trueSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, \`name\`) values (?, ?, ?) as _new_ on duplicate key update project.\`name\` = ?"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
