...
describe(ctx.label, () => {
    ...
    test('avg-int-column-fractional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg((priority * 1.0e0)) as avgPriority from issue where project_id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('average-and-average-distinct-over-bigint', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select avg((duration_ms * 1.0e0)) as avg, avg(distinct (duration_ms * 1.0e0)) as avgD from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customint-sum-distinct-and-average-return-branches', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct cost_cents) as sd, avg((cost_cents * 1.0e0)) as av from issue_worklog"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('sum-distinct-average-distinct-over-int', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(distinct priority) as sd, avg(distinct (priority * 1.0e0)) as avd from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
