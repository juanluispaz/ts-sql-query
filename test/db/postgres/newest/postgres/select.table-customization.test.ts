...
describe(ctx.label, () => {
    ...
    test('table-customization: withSqlHint applied to a VIEW emits the hint on the view name', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vProjCustom)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "v".id as id, "v".name as name from /*+ hint */ project_overview as "v" where "v".id = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('table-customization: three-param customization threads three bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as id from (select * from organization where $1 >= 0 and $2 >= 0 and $3 >= 0) as "o" order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('table-customization: four-param customization threads four bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as id from (select * from organization where $1 >= 0 and $2 >= 0 and $3 >= 0 and $4 >= 0) as "o" order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('table-customization: five-param customization threads five bound params', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as id from (select * from organization where $1 >= 0 and $2 >= 0 and $3 >= 0 and $4 >= 0 and $5 >= 0) as "o" order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
