...
describe(ctx.label, () => {
    ...
    test('shaped-dynamic-set-conditional-family-skips-payload-keys-outside-the-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = :0, "body" = :1, status = :2, priority = :3, assignee_id = :4, parent_id = :5, estimated_hours = :6, external_ref = uuid_to_raw(:7) where id = :8"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Base heading",
