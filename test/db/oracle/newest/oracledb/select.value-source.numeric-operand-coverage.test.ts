...
describe(ctx.label, () => {
    ...
    test('value-source-rhs/bigint-min-max-valuewhennull-nullifvalue', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", greatest(view_count, view_count) as "mn", least(view_count, view_count) as "mx", nvl(view_count, view_count) as "wn", nullif(view_count, view_count) as "ni" from issue where id = :0"`)
    ...
    test('value-source-rhs/customint-arithmetic-and-clamps', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cost_cents - cost_cents as "s", cost_cents * cost_cents as "mu", greatest(cost_cents, cost_cents) as "mn", least(cost_cents, cost_cents) as "mx", nvl(cost_cents, cost_cents) as "wn", nullif(cost_cents, cost_cents) as "ni" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-source-rhs/customdouble-arithmetic-and-clamps', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", billed_amount - billed_amount as "s", billed_amount * billed_amount as "mu", greatest(billed_amount, billed_amount) as "mn", least(billed_amount, billed_amount) as "mx", nvl(billed_amount, billed_amount) as "wn", nullif(billed_amount, billed_amount) as "ni" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-source-rhs/customdouble-power', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", power(billed_amount, :0) as "p" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('const-rhs/double-logn-divide-atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", log(:0, cast(priority as float)) as "ln", cast(priority as float) / :1 as "di", atan2(cast(priority as float), :2) as "at" from issue where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            4,
    ...
    test('const-rhs/customdouble-divide-atan2-logn', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", billed_amount / :0 as "di", atan2(billed_amount, :1) as "at", log(:2, billed_amount) as "ln" from issue_worklog where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
            200,
    ...
    test('const-rhs/double-modulo', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", mod(billed_amount, :0) as "mo", mod(cast(issue_id as float), :1) as "mc" from issue_worklog where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            3,
    ...
    test('customdouble-rounding', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", ceil(billed_amount) as "c", floor(billed_amount) as "f", round(billed_amount) as "r" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('projection-modifiers/customint-as-required-in-optional-object', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cost_cents as "c" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('projection-modifiers/customint-only-when-or-null', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", null as "c" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('projection-modifiers/customdouble-ignore-when-as-null-passthrough', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", billed_amount as "b" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-source-rhs/bigint-modulo', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", mod(view_count, :0) as "mo" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3n,
            1,
    ...
    test('value-source-rhs/bigint-value-when-null-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(duration_ms, :0) as "wn" from issue_worklog where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0n,
            1,
    ...
    test('value-source-rhs/customint-add-cross-table-operand', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_worklog.id as "id", issue_worklog.cost_cents + worklog2.cost_cents as "sum" from issue_worklog join issue_worklog worklog2 on worklog2.id = issue_worklog.id where issue_worklog.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-source-rhs/bigint-add-cross-table-operand', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", issue.view_count + issue2.view_count as "sum" from issue join issue issue2 on issue2.id = issue.id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('value-source-rhs/customdouble-add-cross-table-operand', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_worklog.id as "id", issue_worklog.billed_amount + worklog2.billed_amount as "sum" from issue_worklog join issue_worklog worklog2 on worklog2.id = issue_worklog.id where issue_worklog.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('nullable-const/double-value-when-null-and-null-if-value', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(estimated_hours, :0) as "wn", nullif(estimated_hours, :1) as "ni" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
    ...
    test('subquery-operand/int-req-add-scalar-subquery', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", priority + (select max(id) as "result" from project) as "v" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('subquery-operand/bigint-req-add-scalar-subquery', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", view_count + (select max(duration_ms) as "result" from issue_worklog) as "v" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('subquery-operand/double-req-add-scalar-subquery', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(priority as float) + (select cast(max(id) as float) as "result" from project) as "v" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('subquery-operand/double-receiver-operators', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cast(priority as float) - (select cast(max(id) as float) as "result" from project) as "s", cast(priority as float) * (select cast(max(id) as float) as "result" from project) as "mu", cast(priority as float) / (select cast(max(id) as float) as "result" from project) as "di", mod(cast(priority as float), (select cast(max(id) as float) as "result" from project)) as "mo", power(cast(priority as float), (select cast(max(id) as float) as "result" from project)) as "pw", log((select cast(max(id) as float) as "result" from project), cast(priority as float)) as "ln", round(cast(priority as float), :0) as "rn", atan2(cast(priority as float), (select cast(max(id) as float) as "result" from project)) as "at", greatest(cast(priority as float), (select cast(max(id) as float) as "result" from project)) as "mn", least(cast(priority as float), (select cast(max(id) as float) as "result" from project)) as "mx" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('num-tail/int-receiver-double-column-promotes', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", power(issue_id, billed_amount) as "pw", log(billed_amount, issue_id) as "ln", atan2(issue_id, billed_amount) as "at" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('num-tail/int-opt-receiver-unary-and-arithmetic', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", abs(minutes) as "ab", ceil(minutes) as "ce", floor(minutes) as "fl", sign(minutes) as "sg", cast(minutes as float) as "ad", minutes as "abi", minutes - :0 as "s", mod(minutes, :1) as "mo", greatest(minutes, :2) as "mn", least(minutes, :3) as "mx" from issue_worklog where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            4,
    ...
    test('num-tail/bigint-opt-receiver-rounding-and-arithmetic', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", ceil(duration_ms) as "c", floor(duration_ms) as "f", round(duration_ms) as "r", duration_ms - :0 as "s", mod(duration_ms, :1) as "mo", greatest(duration_ms, :2) as "mn", least(duration_ms, :3) as "mx" from issue_worklog where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1n,
            1000000n,
    ...
    test('num-tail/direct-divide-value-projection', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", priority / id as "dv" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
