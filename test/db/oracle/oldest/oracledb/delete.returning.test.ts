...
describe(ctx.label, () => {
    ...
    test('delete-returning-one-column-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where id = :0 returning priority + :1 into :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
                100,
    ...
    test('delete-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where id = :0 returning id, title || :1 into :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
                " [gone]",
    ...
    test('delete-release-draft-returning-one-column-optional-branded', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from release_draft where id = :0 returning stage into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from release_draft where id = :0 returning id, title, stage into :1, :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                {
    ...
    test('delete-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue where project_id = :0 returning "body" into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProjectReview)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = :0 returning review_date, review_time into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProjectRelease)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_release where id = :0 returning released_on, cutoff_time, published_at into :1, :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
    ...
    test('delete-returning-localdatetime-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.deleteFrom(tProject)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project where id = :0 returning archived_at into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                {
    ...
    test('delete-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = await ctx.conn.deleteFrom(tProjectReview)
                ...
                .executeDeleteOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from project_review where id = :0 returning review_time into :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                {
