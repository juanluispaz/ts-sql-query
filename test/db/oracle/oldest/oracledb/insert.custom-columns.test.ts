...
describe(ctx.label, () => {
    ...
    test('insert-into-project-release-marshals-custom-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, signing_key, released_on, cutoff_time, signed_off_at) values (:0, :1, :2, uuid_to_raw(:3), :4, :5, :6)"`)
    ...
    test('insert-worklog-with-minutes-and-nullable-columns-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue_worklog (issue_id, work_date, started_at, minutes, duration_ms, billable, activity) values (:0, :1, :2, :3, :4, :5, :6)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
    ...
    test('insert-worklog-with-minutes-and-billable-set-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue_worklog (issue_id, work_date, minutes, billable, activity) values (:0, :1, :2, :3, :4)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
    ...
    test('insert-from-select-projects-branded-customs-into-project-release', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, signing_key, released_on, cutoff_time, signed_off_at) select project_id as "projectId", :0 as "version", channel as "channel", signing_key as "signingKey", released_on as "releasedOn", cutoff_time as "cutoffTime", signed_off_at as "signedOffAt" from project_release where id = :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "9.9.9",
                1,
    ...
    test('insert-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, released_on, cutoff_time) values (:0, :1, :2, :3, :4) returning channel into :5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "3.0.0",
