...
- **Typecheck only** (closest thing to a lint): prefer `bun run validate:tsgo` (or `npm run validate:tsgo`) — runs `tsgo --noEmit` (TypeScript 7 Go-based compiler, ~6× faster). Fall back to `bun run validate` / `npm run validate` (`tsc --noEmit`) for the authoritative TS 6 check when diagnosing a discrepancy or before pushing to CI. The new `test/` matrix is on its own `tsconfig` — typecheck it with `bun run validate:tests:tsgo` (preferred for dev) or `bun run validate:tests` (authoritative; what CI runs, and what enforces the negative type tests under `test/db/<db>/types.negative/`).
...
4. Run the verifications before pushing. The minimum set:
    - `bun run validate:tsgo` and `bun run validate:tests:tsgo` for the inner edit-typecheck loop (preferred — ~6× faster than `tsc`). Run `bun run validate` and `bun run validate:tests` once before pushing because that's what CI uses and that's the ground truth for the negative type tests under `test/db/<db>/types.negative/`.
