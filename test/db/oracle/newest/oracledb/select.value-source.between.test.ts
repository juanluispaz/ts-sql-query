...
describe(ctx.label, () => {
    ...
    test('between-with-optional-value-source-bound-projected-is-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when priority between assignee_id and "number" then 1 when not priority between assignee_id and "number" then 0 else null end as "b" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('between-with-optional-upper-bound-projected-is-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when priority between "number" and assignee_id then 1 when not priority between "number" and assignee_id then 0 else null end as "b" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('notBetween-with-optional-upper-bound-projected-is-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when priority not between "number" and assignee_id then 1 when not priority not between "number" and assignee_id then 0 else null end as "b" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
