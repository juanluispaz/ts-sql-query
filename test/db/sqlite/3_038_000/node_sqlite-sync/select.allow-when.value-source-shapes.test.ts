...
describe(ctx.label, () => {
    ...
    test('aggregate-sum-on-allowed-column-passes', async () => {
        ...
        const result = sync(query.executeSelectOne())
    ...
    test('is-with-allowed-column-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is parent_id order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('concat-if-value-on-allowed-string-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title || ? as label from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            " [issue]",
            1,
