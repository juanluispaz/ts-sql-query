...
describe(ctx.label, () => {
    ...
    test('order-by-desc-nulls-first', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, assignee_id as assigneeId from issue order by iif(issue.assignee_id is not null, 1, 0), assigneeId desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-asc-nulls-last-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue order by iif(issue.title is null, 1, 0), lower(title) asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-desc-nulls-first-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue order by iif(issue.title is not null, 1, 0), lower(title) desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-value-source-column-desc-nulls-first', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy(tIssue.id, 'asc')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, assignee_id as assigneeId from issue order by iif(issue.assignee_id is not null, 1, 0), issue.assignee_id desc, issue.id asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; assigneeId?: number }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('order-by-value-source-column-asc-nulls-last-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue order by iif(issue.title is null, 1, 0), lower(issue.title) asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-value-source-column-desc-nulls-first-insensitive', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue order by iif(issue.title is not null, 1, 0), lower(issue.title) desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('order-by-raw-fragment-nulls-mode', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by iif(issue.assignee_id is null, 1, 0), issue.assignee_id asc, id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number }>>>()
        expect(result).toEqual(expected)
    })
