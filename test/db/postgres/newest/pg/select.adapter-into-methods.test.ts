...
describe(ctx.label, () => {
    ...
    test('adapter-int-column-into-numeric-add-scales-operand-and-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, score + $1 as bumped from project_review where id = $2"`)
    ...
    test('adapter-provided-pk-column-into-numeric-add-scales-operand-and-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select invoice_no as "invoiceNo", invoice_no + $1 as next from invoice where invoice_no = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
            100,
    ...
    test('adapter-string-column-into-to-lower-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, lower(reviewer_code) as code from project_review where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-starts-with-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "projectId", project_review.reviewer_code as reviewer from project inner join project_review on project_review.project_id = project.id and project_review.reviewer_code like ($1 || '%') where project.id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "R-",
            1,
    ...
    test('custom-boolean-string-column-as-equals-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from project where id = $1 and (published = 't') = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            true,
    ...
    test('custom-boolean-numeric-column-as-is-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, issue_id as "issueId" from issue_worklog where id = $1 and (invoiced = 1) is not distinct from $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            true,
    ...
    test('custom-boolean-numeric-column-equals-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "issueId", issue_worklog.id as "worklogId" from issue inner join issue_worklog on issue_worklog.issue_id = issue.id and (issue_worklog.invoiced = 1) = $1 where issue.id = $2 order by "worklogId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            1,
    ...
    test('custom-boolean-numeric-column-as-is-if-value-receiver-elided', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, issue_id as "issueId" from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('reviewer-code-replace-all-rebrackets-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(reviewer_code, $1, $2) as rp from project_review where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "-",
            "_",
    ...
    test('adapter-string-column-into-substring-to-end-keeps-result-leaf-bracketed', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substr(reviewer_code, $1) as sub from project_review where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('adapter-string-column-into-replace-all-with-value-source-operand-keeps-result-leaf-bracketed', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(reviewer_code, $1, reviewer_code) as rp from project_review where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "R",
            1,
    ...
    test('adapter-string-column-into-starts-with-predicate-in-where', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1 and reviewer_code like ($2 || '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "R-",
    ...
    test('adapter-string-column-into-not-contains-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1 and reviewer_code not like ('%' || $2 || '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "ZZ",
    ...
    test('adapter-string-column-into-equals-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1 and lower(reviewer_code) = lower($2)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "r-7a2",
    ...
    test('adapter-string-column-into-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1 and reviewer_code ilike $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "r%",
    ...
    test('adapter-string-column-into-not-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1 and reviewer_code not ilike $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "zz%",
    ...
    test('equals-insensitive-if-value-elide', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
