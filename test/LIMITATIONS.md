...
LIMITATIONS.md` on the MariaDB cells below 10.5 (`10_004_000` / `10_003_003` /
...
`TODO[LIMITATION]` rather than `NOT-APPLICABLE` because the API is callable on
...
TODO[LIMITATION]: see LIMITATIONS.md` on the MariaDB cells below 10.5
...
label. Callable on `MariaDBConnection`, accepted by 10.5+, so `TODO[LIMITATION]`
not `NOT-APPLICABLE`.
...
`// TODO[LIMITATION]: see LIMITATIONS.md` on any cell whose engine is below MySQL
...
**What this means for tests** — those cases are commented out with
`// TODO[LIMITATION]: see LIMITATIONS.md` on any cell whose engine is below MySQL 8.0.2
...
commented out with `// TODO[LIMITATION]: see LIMITATIONS.md` on any cell whose
...
`TODO[LIMITATION]` (full canonical body preserved) on the MySQL cells whose engine
