...
describe(ctx.label, () => {
    ...
    test('projecting-optional-values-as-nullable-left-join-miss-yields-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, app_user.id as \`assignee.id\`, app_user.full_name as \`assignee.name\` from issue left join app_user on app_user.id = issue.assignee_id where issue.id = ?"`)
    ...
    test('empty-set-scalar-aggregate-is-undefined', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority) as \`s\`, avg(cast(priority as double)) as \`a\`, min(priority) as lo, max(priority) as hi from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('optional-non-empty-array-over-empty-aggregate-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, (select json_arrayagg(json_object('id', a_1_.id, 'name', a_1_.\`name\`)) from (select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id and \`name\` = ? order by id limit 2147483647) as a_1_) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "__no-such-project__",
            1,
    ...
    test('table-computed-and-virtual-columns-read-the-value-back', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select notes as notes, upper(\`channel\`) as versionTag from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-left-join-miss-inner-object-is-absent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('marker', project.slug, 'issue.id', issue.id, 'issue.title', issue.title)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('aggregate-as-array-null-projector-single-leaf-inner-object-is-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('marker', project.slug, 'issue.id', issue.id)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
