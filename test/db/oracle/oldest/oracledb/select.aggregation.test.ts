...
describe(ctx.label, () => {
    ...
    test('aggregate-arithmetic-expression-as-select-column', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority) + :0 as "x" from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-arithmetic-expression-in-order-by', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('projectId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", sum(priority) as "total" from issue group by project_id order by sum(issue.priority) + :0, "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
