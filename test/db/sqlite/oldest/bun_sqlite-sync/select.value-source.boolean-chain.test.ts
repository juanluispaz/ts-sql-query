...
describe(ctx.label, () => {
    ...
    test('if-value-and-or-both-no-value-elide-the-where', async () => {
        ...
        const andRows = sync(ctx.conn.selectFrom(tIssue)
        ...
        const orRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('if-value-or-if-value-single-elides', async () => {
        ...
        const leftRows = sync(ctx.conn.selectFrom(tIssue)
        ...
        const rightRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('is-null-on-a-comparison-between-required-operands', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (title = ?) is null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Redesign navbar",
          ]
    ...
    test('is-not-null-on-a-comparison-between-required-operands', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (title = ?) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Redesign navbar",
          ]
    ...
    test('is-null-on-a-comparison-with-an-optional-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (assignee_id = ?) is null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('is-not-null-on-a-comparison-with-an-optional-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (assignee_id = ?) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
