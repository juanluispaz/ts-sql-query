...
describe(ctx.label, () => {
    ...
    test('values-sibling-referencing-virtual-column-aliased-via-as-requalifies-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(pp)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchD(id, name) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select pp.id as id, pp.name as name, pp.id * 2 as doubled from projectPatchD as pp order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            "three",
    ...
    test('values-sibling-referencing-virtual-column-for-use-in-left-join-as-requalifies-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchD(id, name) as (values ($1::int4, $2::text)) select project.id as pid, pp.id * 2 as doubled from project left join projectPatchD as pp on pp.id = project.id where project.id <= $3 order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
