...
describe(ctx.label, () => {
    ...
    test('update-from-with-set-to-column-only', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = organization.name from organization where project.organization_id = organization.id and organization.[plan] = @0"`)
    ...
    test('update-from-multiple-source-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = @0 from project, organization where issue.project_id = project.id and project.organization_id = organization.id and organization.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "archived",
                99999,
    ...
    test('update-from-cte-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with verified_orgs as (select id as id, name as name from organization where cast(case when verified = 'Y' then 1 else 0 end as bit) = @0) update project set name = verified_orgs.name from verified_orgs where project.organization_id = verified_orgs.id and project.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                true,
                99999,
    ...
    test('update-from-with-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = organization.name output inserted.id as id, inserted.name as newName, inserted.slug as slug from organization where project.id = @0 and project.organization_id = organization.id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-from-returning-scaled-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set score = @0 output inserted.score as score from project where project_review.project_id = project.id and project_review.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                900,
                1,
    ...
    test('update-from-values-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with orgNames as (select * from (values (@0, @1)) as orgNames(id, name)) update project set name = orgNames.name from orgNames where project.organization_id = orgNames.id and project.id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Renamed via values",
    ...
    // NOT-APPLICABLE: SQL Server's OUTPUT clause can only reference inserted/deleted (the target row), not a FROM-joined table's columns.
    /*
