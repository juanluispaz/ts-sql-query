...
describe(ctx.label, () => {
    ...
    test('equals-and-not-equals-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when lower(email) = lower(full_name) then 1 else 0 end as "eq", case when lower(email) <> lower(full_name) then 1 else 0 end as "neq" from app_user where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('like-and-not-like-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when lower(email) like lower(full_name) escape '\\' then 1 else 0 end as "lk", case when lower(email) not like lower(full_name) escape '\\' then 1 else 0 end as "nlk" from app_user where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('affix-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when lower(email) like lower(replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') escape '\\' then 1 else 0 end as "sw", case when lower(email) not like lower(replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') escape '\\' then 1 else 0 end as "nsw", case when lower(email) like lower('%' || replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_')) escape '\\' then 1 else 0 end as "ew", case when lower(email) not like lower('%' || replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_')) escape '\\' then 1 else 0 end as "new", case when lower(email) like lower('%' || replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') escape '\\' then 1 else 0 end as "ct", case when lower(email) not like lower('%' || replace(replace(replace(full_name, '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%') escape '\\' then 1 else 0 end as "nct" from app_user where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
