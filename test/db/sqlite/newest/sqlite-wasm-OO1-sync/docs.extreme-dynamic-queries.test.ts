...
describe(ctx.label, () => {
    ...
    test('docs:extreme-dynamic-queries/dynamic-condition-for', async () => {
        // Project 2 'Internal tools' matches name~'tools'; project 1 slug
        // 'mktg-site' matches slug startsWith 'mktg'. orderBy id → [1, 2].
        const expected = [
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, slug as slug from project where lower(name) like lower('%' || ? || '%') escape '\\' or lower(slug) like lower(? || '%') escape '\\' order by id"`)
    ...
    test('docs:extreme-dynamic-queries/dynamic-pick', async () => {
        // Only `name` was picked (+id always); `slug` is absent. All four
    ...
    test('docs:extreme-dynamic-queries/allow-when-not-picked', async () => {
        ...
        const rows = sync(connection.selectFrom(tIssue)
            .select(pickedFields)
            .orderBy('id')
            .executeSelectMany())
        // doc-end
    ...
    test('docs:extreme-dynamic-queries/dynamic-condition-nested-projection', async () => {
        // Filter keeps issues whose assignee.fullName contains 'Ada' — only
        // Ada Lovelace (user 1), assigned to issue 1.
        const expected = [
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, issue.title as title, app_user.id as "assignee.id", app_user.full_name as "assignee.fullName" from issue left join app_user on app_user.id = issue.assignee_id where lower(app_user.full_name) like lower('%' || ? || '%') escape '\\' order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Ada",
          ]
    ...
    test('docs-extra:extreme-dynamic-queries/dynamic-condition-nested-and-or', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from project where lower(name) like lower('%' || ? || '%') escape '\\' and (slug like (? || '%') escape '\\' or slug like (? || '%') escape '\\') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mark",
            "mktg",
