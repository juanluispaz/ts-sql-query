...
describe(ctx.label, () => {
    ...
    test('connection.true-in-projection', async () => {
        ...
        const row = await conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select convert(bit, 1) as [result]"`)
    ...
    test('connection.false-in-projection', async () => {
        ...
        const row = await conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select convert(bit, 0) as [result]"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('negate-of-boolean-constants-in-projection', async () => {
        ...
        const row = await conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(case when (0=1) then 1 else 0 end as bit) as notTrue, cast(case when (1=1) then 1 else 0 end as bit) as notFalse"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('and-with-true-constant-reduces-away-the-neutral-operand', async () => {
        ...
        const right = await conn.selectFrom(tOrganization)
            ...
            .select({ id: tOrganization.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
