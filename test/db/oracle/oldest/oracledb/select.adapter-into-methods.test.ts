...
describe(ctx.label, () => {
    ...
    test('adapter-string-column-into-starts-with-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "projectId", project_review.reviewer_code as "reviewer" from project inner join project_review on project_review.project_id = project.id and project_review.reviewer_code like (:0 || '%') escape '\\' where project.id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "R-",
            1,
    ...
    test('adapter-string-column-into-substring-and-substr-keep-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(reviewer_code, :0, :1) as "sg", substr(reviewer_code, :2, :3) as "sb" from project_review where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('adapter-string-column-into-length-passes-through-number-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", length(reviewer_code) as "len" from project_review where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-boolean-string-column-as-equals-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name" from project where id = :0 and case when published = 't' then 1 else 0 end = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('custom-boolean-numeric-column-as-is-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", issue_id as "issueId" from issue_worklog where id = :0 and decode(case when invoiced = 1 then 1 else 0 end, :1, 1, 0 ) = 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('custom-boolean-numeric-column-equals-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "issueId", issue_worklog.id as "worklogId" from issue inner join issue_worklog on issue_worklog.issue_id = issue.id and case when issue_worklog.invoiced = 1 then 1 else 0 end = :0 where issue.id = :1 order by "worklogId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('adapter-string-column-into-substring-to-end-keeps-result-leaf-bracketed', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(reviewer_code, :0) as "sub" from project_review where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
