...
describe(ctx.label, () => {
    ...
    // NOT-APPLICABLE: MariaDB has no CONNECT BY (Oracle-only) — `.startWith` / `.connectBy` / `.connectByNoCycle` are typed `never`; the equivalent shape on MariaDB is a recursive CTE — see `cte.recursive-union-variants.test.ts`.
    /*
