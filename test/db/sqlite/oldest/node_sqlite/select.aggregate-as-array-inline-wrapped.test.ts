...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-order-by-asc-nulls-last', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result is null, result asc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result is not null, result desc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc-nulls-last-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result is null, lower(result) asc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-compound-union-rule1-gate-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_group_array(json_object('ref', a_1_.ref, 'assigneeId', a_1_.assigneeId)) from (select body as ref, assignee_id as assigneeId from issue where id = ? union select body as ref, assignee_id as assigneeId from issue where id = ?) as a_1_) as items"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('inline-aggregate-nested-inside-inline-aggregate-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select json_group_array(json_object('pid', id, 'issues', (select json_group_array(json_object('iid', id)) from issue where project_id = project.id))) from project where organization_id = organization.id) as projects from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result asc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result desc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by lower(result)) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by lower(result) desc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result is not null, lower(result) desc) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
