...
async function main() {
    ...
    try {
        ...
        // Not supported by Sqlite included by LoopBack, replaced with the next one
        ...
        const companyName = connection.selectFrom(tCompany).selectOneColumn(tCompany.name).where(tCompany.id.equals(1)).forUseAsInlineQueryValue()
        i = await connection.update(tCustomer)
            .set({
                lastName: tCustomer.lastName.concat(' - ').concat(companyName).valueWhenNull(tCustomer.lastName)
            })
        ...
        const dateValidation = await connection
            ...
            .select({
                ...
                // time: dateValue.getTime(), // Due limitation this value is wrong in when localdate strategy is used
                dateValue: dateValue,
            })
            .executeSelectOne()
        ...
        // Due some reason this fail in LoopBack, seems to be using a old Sqlite version
