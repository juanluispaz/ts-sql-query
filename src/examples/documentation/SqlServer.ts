...
async function main() {
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, first_name as firstName, last_name as lastName from customer where id = @0 order by customer.birthday desc`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as customerId, first_name as customerFirstName, last_name as customerLastName from customer where company_id in (select id as [result] from company where name like ('%' + @0 + '%')) order by customerFirstName asc, customerLastName`)
    ...
    expectedResult.push([])
    expectedQuery.push(`select id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like lower(@0 + '%') or lower(last_name) like lower(@1 + '%') order by firstName, lastName offset @2 rows fetch next @3 rows only`)
    expectedParams.push(`["Smi","Smi",20,10]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push(0)
    expectedQuery.push(`select count(*) from customer where lower(first_name) like lower(@0 + '%') or lower(last_name) like lower(@1 + '%')`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into customer (first_name, last_name, company_id, birthday) output inserted.id values (@0, @1, @2, @3)`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into customer (first_name, last_name, company_id) output inserted.id values (@0, @1, @2)`)
    ...
    result = [2, 3]
    expectedResult.push(result)
    expectedQuery.push(`insert into customer (first_name, last_name, company_id) output inserted.id values (@0, @1, @2), (@3, @4, @5)`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into custom_company (name, is_big) output inserted.id values (@0, case when (@1 = 1) then 'Y' else 'N' end)`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`delete from company output deleted.id as id, deleted.name as name where name = @0`)
    expectedParams.push(`["ACME"]`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`insert into customer (first_name, last_name, company_id) output inserted.id as id, inserted.first_name as firstName, inserted.last_name as lastName values (@0, @1, @2)`)
    expectedParams.push(`["John","Smith",1]`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`update customer set last_name = customer.last_name + @0 + company.name from company where customer.company_id = company.id and lower(company.name) like lower('%' + @1 + '%')`)
    expectedParams.push(`[" - ","ACME"]`)
    expectedType.push(`update`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`delete from customer from company where customer.company_id = company.id and lower(company.name) like lower('%' + @0 + '%')`)
    expectedParams.push(`["ACME"]`)
    expectedType.push(`delete`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`with inner3 as (select id as id, name as name from custom_company) select id as customerId, first_name as customerFirstName, last_name as customerLastName from customer where company_id in (select inner3.id as [result] from company inner join inner3 on company.id = inner3.id where company.name like ('%' + @0 + '%'))`)
    expectedParams.push(`["Cia."]`)
    expectedType.push(`selectManyRows`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name, (select id as id, first_name as firstName, last_name as lastName from customer where company_id = company.id for json path) as customers from company where id = @0`)
    expectedParams.push(`[1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name, (select id as id, first_name as firstName, last_name as lastName from customer where company_id = company.id order by id offset 0 rows for json path) as customers from company where id = @0`)
    expectedParams.push(`[1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name, (select a_1_.id as id, a_1_.firstName as firstName, a_1_.lastName as lastName from (select id as id, first_name as firstName, last_name as lastName from customer where company_id = company.id union select id as id, first_name as firstName, last_name as lastName from customer where company_id = company.id) as a_1_ for json path) as customers from company where id = @0`)
    expectedParams.push(`[1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`with recursive_select_1 as (select parentCompany.id as id, parentCompany.name as name, parentCompany.parent_id as parentId from company as parentCompany where parentCompany.id = @0 union all select parentCompany.id as id, parentCompany.name as name, parentCompany.parent_id as parentId from company as parentCompany join recursive_select_1 on recursive_select_1.parentId = parentCompany.id) select id as id, name as name, parent_id as parentId, (select id as id, name as name, parentId as parentId from recursive_select_1 for json path) as parents from company where id = @1`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into record (id, title) values (@0, @1)`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, title as title from record where id like ('%' + @0 + '%')`)
    ...
    result = { id: '89bf68fc-7002-11ec-90d6-0242ac120003', title: 'My voice memo' }
    expectedResult.push(result)
    expectedQuery.push(`with with_uuit as (select id as id, title as title from record) select id as id, title as title from with_uuit where id like ('%' + @0 + '%')`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select (select id as id, title as title from record where id like ('%' + @0 + '%') for json path) as records`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    result = { records: [{ id: '89bf68fc-7002-11ec-90d6-0242ac120003', title: 'My voice memo' }] }
    expectedResult.push(result)
    expectedQuery.push(`select (select id as id, title as title from record where id like ('%' + @0 + '%') order by id offset 0 rows for json path) as records`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name, coalesce((select count(*) as [result] from customer where company_id = company.id), @0) as numberOfCustomers from company`)
    ...
    expectedResult.push([])
    expectedQuery.push(`select id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like lower(@0 + '%') or lower(last_name) like lower(@1 + '%') group by id order by firstName, lastName offset @2 rows fetch next @3 rows only`)
    expectedParams.push(`["Smi","Smi",20,10]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push(0)
    ...
    expectedResult.push([])
    expectedQuery.push(`select distinct id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like lower(@0 + '%') or lower(last_name) like lower(@1 + '%') order by firstName, lastName offset @2 rows fetch next @3 rows only`)
    expectedParams.push(`["Smi","Smi",20,10]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push(0)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name from company where lower(name) like lower('%' + @0 + '%') and parent_id = @1`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name from company left outer join customer as favouriteCustomer on company.parent_id = favouriteCustomer.id where favouriteCustomer.id in (select parent_id as [result] from company where lower(name) like lower('%' + @0 + '%')) and company.parent_id = @1`)
    ...
    result = undefined
    expectedResult.push(result)
    expectedQuery.push(`exec myOwnprocedure @0`)
    expectedParams.push(`[10]`)
    ...
    result = 20
    expectedResult.push(result)
    expectedQuery.push(`select myOwnFunction(@0)`)
    expectedParams.push(`[10]`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select sign(id) * power(cast(abs(id) as float), 1.0e0 / 3.0e0) as idCubeRoot, log10(id) as idLog10, log(id, @0) as idLogBase2 from customer where id = @1`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id from customer where (first_name collate Latin1_General_BIN2) = @0`)
    ...
    const caseSensitiveMatch = await connection.selectFrom(tCustomer)
        .where(tCustomer.firstName.collate('Latin1_General_BIN2').equals('ADA'))
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, replace(first_name, @0, @1) as cleaned from customer where id = @2`)
