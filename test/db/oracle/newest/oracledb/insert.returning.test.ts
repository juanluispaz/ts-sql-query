...
describe(ctx.label, () => {
    ...
    test('insert-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (:0, :1, :2) returning id, name || :3 into :4, :5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Mobile app",
    ...
    */
    ...
    test('insert-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4) returning "body" into :5; insert into issue (project_id, "number", title, status, priority, "body") values (:6, :7, :8, :9, :10, :11) returning "body" into :12; end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9001,
    ...
    test('insert-returning-one-column-nullable', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (:0, :1, :2) returning archived_at into :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Scalar nullable",
    ...
    test('insert-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (:0, :1, :2) returning id, slug, archived_at into :3, :4, :5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
    ...
    test('insert-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, archived_at) values (:0, :1, :2, :3) returning id, archived_at into :4, :5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Archived demo",
    ...
    test('insert-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.insertInto(tProjectReview)
                ...
                .executeInsertOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_review (project_id, reviewer_code, score, review_date, review_time) values (:0, :1, :2, :3, :4) returning id, review_date, review_time into :5, :6, :7"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "R-Z9",
    ...
    test('insert-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.insertInto(tProjectRelease)
                ...
                .executeInsertOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, released_on, cutoff_time, published_at) values (:0, :1, :2, :3, :4, :5) returning id, released_on, cutoff_time, published_at into :6, :7, :8, :9"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "9.9.9-ret",
    ...
    test('insert-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = await ctx.conn.insertInto(tProjectReview)
                ...
                .executeInsertOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_review (project_id, reviewer_code, score, review_time) values (:0, :1, :2, :3) returning review_time into :4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "R-Z8",
