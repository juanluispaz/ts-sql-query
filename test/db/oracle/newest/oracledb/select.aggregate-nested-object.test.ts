...
describe(ctx.label, () => {
    ...
    test('aggregate-element-with-depth-4-nested-object-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid", json_arrayagg(json_object('iid' value id, 'outer.mid.inner.title' value title, 'outer.mid.inner.num' value "number")) as "issues" from issue where project_id = :0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-element-with-depth-4-nested-object-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid", json_arrayagg(json_object('iid' value id, 'outer.mid.inner.num' value "number", 'outer.mid.inner.body' value "body")) as "issues" from issue where project_id = :0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
