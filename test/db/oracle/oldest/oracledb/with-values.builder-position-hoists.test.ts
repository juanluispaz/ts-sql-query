...
describe(ctx.label, () => {
    ...
    test('projecting-optional-values-as-nullable-over-values-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optNoteSampler(id, note) as (select :0, :1 from dual union all select :2, :3 from dual) select id as "id", note as "note" from optNoteSampler order by "id""`)
    ...
    test('values-as-insert-from-select-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into calendar_year (year_value, year_label) with newYears(yearValue, "label") as (select :0, :1 from dual union all select :2, :3 from dual) select yearValue as "yearValue", "label" as "label" from newYears"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2025,
                "FY2025",
    ...
    // NOT-APPLICABLE: Oracle has no WITH-before-DELETE, so a Values-source
    // DELETE ... USING cannot hoist its CTE (ORA-00928).
    /*
