...
describe(ctx.label, () => {
    ...
    test('on-conflict-on-custom-columns-do-update-with-values-for-insert', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const affected = sync(ctx.conn.insertInto(tProjectRelease)
                .values({
                    projectId:  1,
                    version:    '1.2.0',
                    channel:    'beta',
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project_release (project_id, version, channel, released_on, cutoff_time) values (?, ?, ?, ?, ?) on conflict (project_id, version) do update set channel = excluded.channel"`)
            ...
            if (ctx.realDbEnabled) {
                ...
                const channel = sync(ctx.conn.selectFrom(tProjectRelease)
