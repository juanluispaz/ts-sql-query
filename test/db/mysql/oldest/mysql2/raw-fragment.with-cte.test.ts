...
describe(ctx.label, () => {
    ...
    test('rawfragment-with-cte-bubbles-with-clause-to-outer-select', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(open_issues.projectId) as result from (select project_id as projectId from issue where \`status\` = ?) as open_issues) as "openCount",  id as id from project"`)
    ...
    test('rawfragment-with-multiple-ctes-from-embedded-subqueries', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(open_issues.projectId) as result from (select project_id as projectId from issue where \`status\` = ?) as open_issues) as "openCount", (select count(closed_issues.projectId) as result from (select project_id as projectId from issue where \`status\` = ?) as closed_issues) as "closedCount",  id as id from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    test('rawfragment-with-cte-and-outer-with-coexist', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select count(closed_issues.projectId) as result from (select project_id as projectId from issue where \`status\` = ?) as closed_issues) as "closedCount",  project.id as id, count(open_issues.projectId) as openCount from project inner join (select project_id as projectId from issue where \`status\` = ?) as open_issues on open_issues.projectId = project.id group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "closed",
            "open",
