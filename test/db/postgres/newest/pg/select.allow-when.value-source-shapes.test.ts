...
describe(ctx.label, () => {
    ...
    test('in-n-with-allowed-column-in-the-value-list-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where id in (number, $1) order by id"`)
    ...
    test('is-with-allowed-column-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is not distinct from parent_id order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('concat-if-value-on-allowed-string-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title || $1 as label from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            " [issue]",
            1,
    ...
    test('replace-all-if-value-with-allowed-second-operand-passes', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(title, $1, status) as label from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "hero",
            1,
