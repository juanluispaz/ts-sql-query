...
describe(ctx.label, () => {
    ...
    test('asInt-on-double-uses-round', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast((priority * 1.0e0) / ? as decimal(65, 30))) as signed) as rounded from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('asDouble-chain-roundtrips-int', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) as \`p\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('asDouble-on-nullable-column-keeps-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (assignee_id * 1.0e0) as \`a\` from issue where id in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('asBigint-on-double-chained-into-bigint-op', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) + ? as \`a\`, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) % ? as \`m\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2n,
            2n,
    ...
    test('asBigint-on-double-keeps-bigint-arithmetic-exact', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast((priority * 1.0e0) as decimal(65, 30))) as signed) + ? as big from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            9007199254740993n,
            1,
