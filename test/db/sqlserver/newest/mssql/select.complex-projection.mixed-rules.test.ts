...
describe(ctx.label, () => {
    ...
    test('merged-two-left-join-leaf-plus-const-inside-aggregate-element-demotes-leaf-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .groupBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_arrayagg(json_object('combined':issue.id + app_user.id, 'tag':'rel')) as items from project left join issue on issue.project_id = project.id left join app_user on app_user.id = issue.assignee_id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
