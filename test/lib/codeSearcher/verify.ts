...
// a BARE section flag turns it on at its lightest non-none level (no explicit level needed)
check('bare --ref-brand → summary',              parseArgs(['--search', 'x', '--ref-brand']).sectionOverrides.refBrand === 'summary')
check('bare --chain → strict',                   parseArgs(['--search', 'x', '--chain']).sectionOverrides.chain === 'strict')
check('bare --surface → own',                    parseArgs(['--search', 'x', '--surface']).sectionOverrides.surface === 'own')
check('bare flag does NOT eat the next flag',    (() => { const a = parseArgs(['--search', 'x', '--ref-brand', '--surface', 'all']); return a.sectionOverrides.refBrand === 'summary' && a.sectionOverrides.surface === 'all' && a.error === null })())
check('bare --refs → summary (whole family)',    buildOptions(parseArgs(['--search', 'x', '--refs'])).sections.refBrand === 'summary')
check('explicit bad level still errors',         parseArgs(['--search', 'x', '--ref-brand', 'bogus']).error !== null)
