...
describe(ctx.label, () => {
    ...
    // Not applicable on MariaDB: `.startWith` / `.connectBy` / `.connectByNoCycle` are typed `never` (Oracle-only `START WITH … CONNECT BY` syntax); equivalent shape on MariaDB is a recursive CTE — see `cte.recursive-union-variants.test.ts`.
    /*
