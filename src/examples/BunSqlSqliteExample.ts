...
import { SQL } from 'bun'
import { BunSqlSqliteQueryRunner } from '../queryRunners/BunSqlSqliteQueryRunner.js'
...
async function main() {
    const connection = new DBConnection(new ConsoleLogQueryRunner(new BunSqlSqliteQueryRunner(db)))
    ...
    try {
        ...
        const allProducts = await connection.selectFrom(tProduct)
            .select({
                ...
                name: tProduct.name,
                releaseInfo: {
                    year: tProduct.releaseYear,
                    price: tProduct.releasePrice,
                }
            })
            .executeSelectMany()
