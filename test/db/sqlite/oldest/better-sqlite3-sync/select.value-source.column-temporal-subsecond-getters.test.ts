...
describe(ctx.label, () => {
    ...
    test('column-localdatetime-subsecond-getters-truncate', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tTemporalPrecision)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(strftime('%S', micro_stamp) as integer) as "s", cast(round(strftime('%f', micro_stamp) * 1000) as integer) % 1000 as ms from temporal_precision where id = ?"`)
    ...
    test('column-microstamp-gettime-rounds-up', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tTemporalPrecision)
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select round((julianday(micro_stamp) - 2440587.5) * 86400000.0) as "t" from temporal_precision where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
