...
describe(ctx.label, () => {
    ...
    test('primary-key-factory-projects-each-comparable-kind', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select m_int as id, m_str as strPk, bin_to_uuid(m_uuid) as uuidPk, m_bigint as bigintPk from col_matrix"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
