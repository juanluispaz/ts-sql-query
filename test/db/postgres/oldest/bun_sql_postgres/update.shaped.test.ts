...
describe(ctx.label, () => {
    ...
    test('shaped-allowing-no-where-updates-whole-table', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.updateAllowingNoWhere(tProject)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Bulk renamed",
              ]
    ...
    test('shaped-set-when-maps-renamed-key-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = $1 where id = $2"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = $1, slug = $2 where id = $3"`)
    ...
    test('shaped-allowing-no-where-set-then-extend-shape-executes', async () => {
        ...
        await ctx.withRollback(async () => {
            const affected = await ctx.conn.updateAllowingNoWhere(tOrganization)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update organization set name = $1, plan = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Bulk renamed",
                "bulk-plan",
