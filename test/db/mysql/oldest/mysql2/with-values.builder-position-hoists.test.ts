...
describe(ctx.label, () => {
    ...
    test('projecting-optional-values-as-nullable-over-values-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optNoteSampler(id, note) as (values row(?, ?), row(?, ?)) select id as id, note as note from optNoteSampler order by id"`)
    ...
    */
    ...
    test('values-as-insert-from-select-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into calendar_year (year_value, year_label) with newYears(yearValue, label) as (values row(?, ?), row(?, ?)) select yearValue as yearValue, label as label from newYears"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2025,
                "FY2025",
    ...
    */
    ...
    test('values-as-delete-using-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with countryCodes(\`code\`) as (values row(?)) delete from country using country, countryCodes where country.\`code\` = countryCodes.\`code\`"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "JP",
              ]
