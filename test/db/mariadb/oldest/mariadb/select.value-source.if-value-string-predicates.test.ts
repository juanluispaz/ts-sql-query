...
describe(ctx.label, () => {
    ...
    test('starts-with-if-value-and-ends-with-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title like concat(?, '%') and title like concat('%', ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Migrate",
            "ESM",
    ...
    test('not-starts-with-if-value-and-not-ends-with-if-value-fire', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not like concat(?, '%') and title not like concat('%', ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Update",
            "users",
    ...
    test('not-contains-if-value-fires-and-elides', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not like concat('%', ?, '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "nav",
          ]
