...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-use-empty-array-for-no-value-explicit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, (select json_arrayagg(json_object('id', a_1_.id, 'name', a_1_.\`name\`)) from (select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id order by id limit 2147483647) as a_1_) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_arrayagg(json_object('id', a_1_.id, 'title', a_1_.title)) from (select id as id, title as title from issue where project_id = project.id order by id limit 2147483647) as a_1_) as \`meta.issues\` from project where organization_id = ? order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-aggregate-order-by-desc-nulls-first', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by result is not null, result desc limit 2147483647) as a_1_) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-asc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.result) from (select \`name\` as result from project where organization_id = \`organization\`.id order by lower(result) asc limit 2147483647) as a_1_) as projectNames from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-one-column-keeps-the-branded-element-type', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(\`channel\`) from project_release where project_id = project.id) as channels from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-compound-union-with-customize-query-survives-in-derived-table', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, \`name\` as \`name\`, (select json_arrayagg(json_object('id', a_1_.id, 'name', a_1_.\`name\`)) from (/* agg-head */  select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id and archived_at is null union select id as id, \`name\` as \`name\` from project where organization_id = \`organization\`.id and archived_at is not null  /* agg-tail */) as a_1_) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-of-compound-union-rule2-left-join-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_arrayagg(json_object('name', a_1_.\`name\`)) from (select app_user.full_name as \`name\` from issue left join app_user on app_user.id = issue.assignee_id where issue.id = ? union select app_user.full_name as \`name\` from issue left join app_user on app_user.id = issue.assignee_id where issue.id = ?) as a_1_) as \`names\`"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            3,
    ...
    test('inline-aggregate-of-compound-union-rule1-gate-leaf-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_arrayagg(json_object('ref', a_1_.ref, 'assigneeId', a_1_.assigneeId)) from (select body as ref, assignee_id as assigneeId from issue where id = ? union select body as ref, assignee_id as assigneeId from issue where id = ?) as a_1_) as items"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('inline-aggregate-nested-inside-inline-aggregate-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as orgId, (select json_arrayagg(json_object('pid', id, 'issues', (select json_arrayagg(json_object('iid', id)) from issue where project_id = project.id))) from project where organization_id = \`organization\`.id) as projects from \`organization\` where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-wrapping-inline-object-aggregate-projecting-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, json_arrayagg((select json_arrayagg(json_object('id', id, 'body', body)) from issue where project_id = project.id)) as \`nested\` from project where id = ? group by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
