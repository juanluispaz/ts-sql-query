...
describe(ctx.label, () => {
    ...
    // TODO[BUG]: see BUGS.md — `_rawFragmentTableAlias` ignores
    // `_supportTableAliasWithAs` on Oracle, so the emitted SQL is
    // `... "organization" as "o"`, which Oracle rejects with
    // `ORA-03048`. The mock-only assertion would still pass but
    // would mask the real-DB rejection; commenting out the body
    // keeps the symmetry stub without pretending the SQL is valid.
    // (Block-comment form would need escaping because the hint
    // template contains `*/` — using per-line comments instead.)
    //
