...
describe(ctx.label, () => {
    ...
    test('asInt-on-double-uses-round', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast(cast(priority as double) / ? as decimal(65, 30))) as signed) as rounded from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('asDouble-chain-roundtrips-int', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) as \`p\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('asDouble-on-nullable-column-keeps-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(assignee_id as double) as \`a\` from issue where id in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('asBigint-on-double-chained-into-bigint-op', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) + ? as \`a\`, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) % ? as \`m\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2n,
            2n,
    ...
    test('asBigint-on-double-keeps-bigint-arithmetic-exact', async () => {
        // Same name as the lossless cells (audit symmetry), opposite outcome. mysql2 reads
        // every integer as a JavaScript number unless the connection sets `supportBigNumbers`
        // (this suite does not), so the exact sum 9007199254740995 comes back rounded to
        // 9007199254740996. The marshaller refuses that rounded number rather than return a
        // clean-but-wrong bigint: PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE. The mock is
        // seeded with the same rounded number the real driver returns, so mock and real agree.
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(round(cast(cast(priority as double) as decimal(65, 30))) as signed) + ? as big from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            9007199254740993n,
            1,
