...
async function main() {
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into custom_company (name, is_big) values ($1, case when $2 then 'Y' else 'N' end) returning id`)
    ...
    expectedQuery.push(`select id as id, name as name from company where name ilike ('%' || $1 || '%')`)
    expectedParams.push(`["ACME"]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push([{
        id: 12,
    ...
    expectedQuery.push(`select id as id, first_name as "firstName", last_name as "lastName", birthday as birthday, company_id as "companyId" from customer where company_id in ($1, $2)`)
    ...
    expectedQuery.push(`select id as id, name as name from company where id in ($1)`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectManyRows`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select customer.id as id, customer.first_name as "name.firstName", customer.last_name as "name.lastName", company.id as "company.id", company.name as "company.name" from customer inner join company on company.id = customer.company_id where customer.id = $1`)
    expectedParams.push(`[12]`)
    expectedType.push(`selectManyRows`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select /*+ some hints */ id as id, first_name as "firstName", last_name as "lastName", birthday as birthday from customer where id = $1 order by id, my_id for update`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectOneRow`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`update /*+ some hints */ customer set first_name = $1, last_name = $2 where id = $3 keep plan`)
    expectedParams.push(`["John","Smith",10]`)
    expectedType.push(`update`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`delete /*+ some hints */ from customer where id = $1 keep plan`)
    expectedParams.push(`[10]`)
    expectedType.push(`delete`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert /*+ some hints */ into customer (first_name, last_name, company_id) values ($1, $2, $3) log errors reject limit unlimited`)
    ...
    expectedQuery.push(`select customer.id as "customer.id", customer.first_name as "customer.firstName", customer.last_name as "customer.lastName", customer.birthday as "customer.birthday", company.id as "company.id", company.name as "company.name" from customer inner join company on company.id = customer.company_id where customer.id = $1`)
    expectedParams.push(`[12]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedQuery.push(`select customer.id as "customerId", customer.first_name as "customerFirstName", customer.last_name as "customerLastName", customer.birthday as "customerBirthday", company.id as "companyId", company.name as "companyName" from customer inner join company on company.id = customer.company_id where customer.id = $1`)
    expectedParams.push(`[12]`)
    expectedType.push(`selectOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`with recursive recursive_select_1 as (select id as companyId, parent_id as companyParentId, id as companyName, parent_id as favouriteCustomerId from company where id = $1 union all select company.id as companyId, company.parent_id as companyParentId, company.id as companyName, company.parent_id as favouriteCustomerId from company join recursive_select_1 on recursive_select_1.companyParentId = company.id) select companyId as "companyId", companyParentId as "companyParentId", companyName as "companyName", favouriteCustomerId as "favouriteCustomerId" from recursive_select_1`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectManyRows`)

    expectedResult.push(0)
    expectedQuery.push(`with recursive recursive_select_1 as (select id as companyId, parent_id as companyParentId, id as companyName, parent_id as favouriteCustomerId from company where id = $1 union all select company.id as companyId, company.parent_id as companyParentId, company.id as companyName, company.parent_id as favouriteCustomerId from company join recursive_select_1 on recursive_select_1.companyParentId = company.id) select count(*) from recursive_select_1`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectOneColumnOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as "companyId", parent_id as "companyParentId", id as "companyName", parent_id as "favouriteCustomerId" from company where id = $1`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectManyRows`)
