...
describe(ctx.label, () => {
    ...
    test('returning-old-and-new-single-column-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output deleted.name as oldName, inserted.name as newName where id = @1"`)
    ...
    test('returning-old-and-new-two-columns-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0, slug = @1 output inserted.id as id, deleted.name as oldName, inserted.name as newName, deleted.slug as oldSlug, inserted.slug as newSlug where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Internal apps",
                "apps",
    ...
    test('returning-one-column-old-value-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output deleted.name as [result] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mktg site v3",
                1,
    ...
    test('returning-computed-expression-over-old-value-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output deleted.name + @1 as oldTag, inserted.name as newName where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mktg v4",
                " [was]",
    ...
    test('returning-old-and-new-adapter-column-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set score = @0 output deleted.score as oldScore, inserted.score as newScore where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                900,
                1,
    ...
    test('returning-old-and-new-folded-into-nested-audit-object-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, deleted.name as [audit.old], inserted.name as [audit.new] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mktg nested",
                1,
    ...
    test('returning-old-optional-column-as-nullable-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output deleted.archived_at as oldArchivedAt, inserted.name as newName where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Internal tools v2",
                2,
    ...
    test('allowing-no-where-returning-old-and-new-via-oldValues', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, deleted.name as oldName, inserted.name as newName"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Renamed all",
              ]
