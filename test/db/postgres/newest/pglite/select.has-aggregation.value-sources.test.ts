...
describe(ctx.label, () => {
    ...
    test('has-aggregation/value-when-no-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id > $1 as busy from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('has-aggregation/in-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) in ($1, $2) as busy from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/in-n-with-an-aggregating-list-element-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id in ($1, count(id)) as busy from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/in-n-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id in ($1, $2) as busy from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('has-aggregation/in-with-a-non-aggregating-subquery-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where project_id in (select id as result from project where slug = $1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mktg-site",
          ]
    ...
    test('has-aggregation/value-when-null-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", coalesce(max(assignee_id), $1) as lead from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('has-aggregation/value-when-null-with-an-aggregating-replacement-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where coalesce(assignee_id, (select count(id) as result from project)) = $1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('has-aggregation/value-when-null-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, title as title from issue where coalesce(assignee_id, (select id as result from project where slug = $1)) = $2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "legacy",
            4,
    ...
    test('has-aggregation/is-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", max(assignee_id) is not distinct from $1 as matches from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('has-aggregation/is-with-an-aggregating-operand-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id is not distinct from min(id) as matches from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ projectId: number; matches: boolean }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('has-aggregation/is-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id is not distinct from $1 as matches from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/in-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId" from issue group by project_id having count(id) in ($1, $2) order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/if-value-comparisons-with-neither-receiver-aggregating-report-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId" from issue group by project_id having project_id > $1 and project_id in ($2, $3, $4) order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('has-aggregation/concat-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (max(title) || $1) = $2 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
    ...
    test('has-aggregation/concat-if-value-with-a-missing-value-and-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (max(title)) = $1 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Redesign navbar",
          ]
    ...
    test('has-aggregation/concat-if-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having (title || $1) = $2 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            "Redesign navbar!",
    ...
    test('has-aggregation/between-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", count(id) between $1 and $2 as "inRange" from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            5,
    ...
    test('has-aggregation/between-with-an-aggregating-lower-bound-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id between count(id) and $1 as "inRange" from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
          ]
    ...
    test('has-aggregation/between-with-an-aggregating-upper-bound-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id between $1 and count(id) as "inRange" from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('has-aggregation/between-with-no-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", project_id between $1 and $2 as "inRange" from issue group by project_id order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            5,
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-receiver-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having replace(max(title), $1, $2) = $3 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
            "header",
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-replacement-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having replace(title, $1, max(status)) = $2 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
    ...
    test('has-aggregation/replace-all-if-value-with-an-aggregating-searched-text-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as name from project group by name having replace(name, max(slug), $1) = $2 order by name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "squad",
            "Internal squad",
    ...
    test('has-aggregation/replace-all-if-value-with-neither-operand-aggregating-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue group by title having replace(title, $1, $2) = $3 order by title"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "navbar",
            "header",
