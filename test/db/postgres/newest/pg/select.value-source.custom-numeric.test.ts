...
// DOES marshal (Meters/Ratio/Score -> double, ScoreCount -> int via
// `baseTypeForCustom`), so their `::numeric`-cast results come back as clean
// numbers. The arithmetic-and-sign arms run over the real `cost_cents`
// ('Cents' -> int) and `billed_amount` ('Money' -> double) worklog columns —
// a typed column keeps `col OP col` resolvable on every engine (a two-const
// operand would emit `$1 OP $2`, which postgres rejects).
...
describe(ctx.label, () => {
    ...
    // Money→double), so the results read back as clean numbers.
    ...
    test('custom-numeric/minmax-optional-customdouble-receiver-poisons', async () => {
        ...
        const result = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, case when budget is null then null else greatest(budget, $1) end as mn, case when budget is null then null else least(budget, $2) end as mx from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            100,
