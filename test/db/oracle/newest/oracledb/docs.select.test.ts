...
describe(ctx.label, () => {
    ...
    test('docs:select/joins-and-order-by', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", issue.title as "title", proj.name as "projectName" from issue inner join project proj on issue.project_id = proj.id where issue.status = :0 order by "id""`)
    ...
    test('docs:select/ordering-by-not-returned-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name" from project where id = :0 order by project.archived_at desc nulls last"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:select/subquery-and-dynamic-order-by', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "projectId", name as "projectName", slug as "projectSlug" from project where organization_id in (select id as "result" from "organization" where name like ('%' || :0 || '%') escape '\\') order by "projectName" asc nulls first, "projectSlug""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Acme",
          ]
    ...
    test('docs:select/left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", issue.title as "title", parent.title as "parentTitle" from issue left join issue parent on issue.parent_id = parent.id order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('docs:select/left-join-complex-projections', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", issue.title as "title", parent.id as "parent.id", parent.title as "parent.title" from issue left join issue parent on issue.parent_id = parent.id order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('docs:select/compound-operator', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name", :0 as "kind" from "organization" union all select id as "id", name as "name", :1 as "kind" from project"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "organization",
            "project",
    ...
    test('docs:select/count-all', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select count(*) as "result" from project where organization_id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:select/inline-subquery-as-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", name as "name" from project where organization_id = (select id as "result" from "organization" where name = :0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Acme Corp",
          ]
    ...
    test('docs-extra:select/clauses-order-where-after-groupby', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization_id as "organizationId", count(id) as "projectCount" from project where archived_at is null group by organization_id having count(id) > :0 order by "organizationId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('docs-extra:select/find-by-email', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", email as "email", full_name as "fullName" from app_user where email = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ada@acme.test",
          ]
    ...
    test('docs-extra:select/aliased-table', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "u".id as "id", "u".full_name as "fullName" from app_user "u" where "u".email like (:0 || '%') escape '\\'"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ada@",
          ]
