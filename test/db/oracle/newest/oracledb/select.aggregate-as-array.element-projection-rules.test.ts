...
describe(ctx.label, () => {
    ...
    test('element-top-rule-3-own-table-optional-leaf-default-drops-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid", json_arrayagg(json_object('title' value title, 'body' value "body")) as "issues" from issue where project_id = :0 group by project_id"`)
    ...
    test('element-containing-a-nested-rule-1-required-in-optional-object-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid", json_arrayagg(json_object('title' value title, 'meta.gate' value status, 'meta.assigneeId' value assignee_id)) as "issues" from issue where project_id = :0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-a-left-joined-inner-object-rule-2-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.project_id as "grp", json_arrayagg(json_object('iid' value issue.id, 'proj.id' value project.id, 'proj.name' value project.name, 'proj.archivedAt' value project.archived_at)) as "items" from issue left join project on project.id = issue.project_id where issue.project_id = :0 group by issue.project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-rule-1-object-with-originally-required-left-join-sibling-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.project_id as "grp", json_arrayagg(json_object('iid' value issue.id, 'meta.ownId' value issue.id, 'meta.gate' value issue.status, 'meta.projName' value project.name)) as "items" from issue left join project on project.id = issue.project_id where issue.project_id = :0 group by issue.project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-left-join-all-optional-object-applies-rule-4-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('pid' value project.id, 'opt.body' value issue."body", 'opt.assigneeId' value issue.assignee_id)) as "items" from project left join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-top-rule-1-required-in-optional-object-gate-null-drops-whole-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('ref' value issue."body", 'assigneeId' value issue.assignee_id)) as "items" from project inner join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-containing-sole-optional-inner-object-collapses-on-aggregate-path-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('iid' value issue.id, 'wrapper.inner.body' value issue."body", 'wrapper.inner.assigneeId' value issue.assignee_id)) as "items" from project inner join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('element-top-pure-rule-4-all-optional-element-drops-when-all-null-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('body' value issue."body", 'assigneeId' value issue.assignee_id)) as "items" from project left join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-containing-rule-2-left-join-object-realizes-a-miss-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('pid' value project.id, 'iss.id' value issue.id, 'iss.title' value issue.title, 'iss.body' value issue."body")) as "items" from project left join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('element-top-rule-2-all-left-join-element-drops-on-miss-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.organization_id as "orgId", json_arrayagg(json_object('id' value issue.id, 'title' value issue.title, 'body' value issue."body")) as "items" from project left join issue on issue.project_id = project.id where project.organization_id = :0 group by project.organization_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-own-table-optional-leaf-default-drops-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('title' value title, 'body' value "body")) from issue where project_id = project.id) as "issues" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-rule-1-gate-leaf-at-top-default-keeps-element-drops-gate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('ref' value "body", 'assigneeId' value assignee_id)) from issue where project_id = project.id) as "issues" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-rule-2-left-join-leaf-at-top-default-keeps-element-drops-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('name' value app_user.full_name)) from issue left join app_user on app_user.id = issue.assignee_id where issue.project_id = project.id) as "assignees" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-top-rule-2-multi-left-join-leaves-as-nullable-surface-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "orgId", (select json_arrayagg(json_object('id' value issue.id, 'title' value issue.title, 'body' value issue."body")) from project left join issue on issue.project_id = project.id where project.organization_id = "organization".id) as "items" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-top-rule-4-all-optional-default-keeps-empty-element', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "orgId", (select json_arrayagg(json_object('body' value issue."body", 'assigneeId' value issue.assignee_id)) from project left join issue on issue.project_id = project.id where project.organization_id = "organization".id) as "items" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-nested-rule-1-gate-object-default-drops-nested-on-null-gate', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('iid' value id, 'meta.gate' value "body", 'meta.assigneeId' value assignee_id)) from issue where project_id = project.id) as "items" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-nested-rule-2-left-join-object-default-drops-nested-on-miss', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "orgId", (select json_arrayagg(json_object('pid' value project.id, 'iss.id' value issue.id, 'iss.title' value issue.title, 'iss.body' value issue."body")) from project left join issue on issue.project_id = project.id where project.organization_id = "organization".id) as "items" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('inline-element-sole-optional-inner-object-collapses-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "orgId", (select json_arrayagg(json_object('iid' value issue.id, 'wrapper.inner.body' value issue."body", 'wrapper.inner.assigneeId' value issue.assignee_id)) from project inner join issue on issue.project_id = project.id where project.organization_id = "organization".id) as "items" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-element-own-table-multi-optional-leaf-as-nullable-surfaces-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "pid", (select json_arrayagg(json_object('title' value title, 'body' value "body", 'assigneeId' value assignee_id)) from issue where project_id = project.id) as "items" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
