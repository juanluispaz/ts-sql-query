#!/bin/bash
# Run every documentation/example script across all docker-backed and
# in-process engines. Dispatches between `bun run` and `tsx` based on
# the runtime that invoked the script (`npm_config_user_agent` prefix);
# direct invocation defaults to bun.
#
# Per-runtime divergence is intentional:
#   * node-only: BetterSqlite3*, NodeSqlite* (Node 24+), PrismaSqlite,
#                BunSql* (require bun runtime).
#   * bun-only:  BunSqlite*, BunSqlSqlite*, BunSqlPostgres*,
#                BunSqlMySql*, BunSqlMariaDB*.
#   * common:    everything under documentation/, Sqlite3Example,
#                Sqlite3WasmOO1*, PgLiteExample, the per-driver scripts
#                that share between runtimes.
...
$RUN ./src/examples/PgExample.ts || { docker stop ts-sql-query-postgres; docker rm ts-sql-query-postgres; exit 1; }
$RUN ./src/examples/EncriptedIDPgExample.ts || { docker stop ts-sql-query-postgres; docker rm ts-sql-query-postgres; exit 1; }
$RUN ./src/examples/PostgresExample.ts || { docker stop ts-sql-query-postgres; docker rm ts-sql-query-postgres; exit 1; }
if [ "$runtime" = "bun" ]; then
    $RUN ./src/examples/BunSqlPostgresExample.ts || { docker stop ts-sql-query-postgres; docker rm ts-sql-query-postgres; exit 1; }
fi
$RUN ./src/examples/PrismaPostgresExample.ts || { docker stop ts-sql-query-postgres; docker rm ts-sql-query-postgres; exit 1; }
...
$RUN ./src/examples/MySql2Example.ts || { docker stop ts-sql-query-mysql; docker rm ts-sql-query-mysql; exit 1; }
if [ "$runtime" = "bun" ]; then
    $RUN ./src/examples/BunSqlMySqlExample.ts || { docker stop ts-sql-query-mysql; docker rm ts-sql-query-mysql; exit 1; }
fi
$RUN ./src/examples/PrismaMySqlExample.ts || { docker stop ts-sql-query-mysql; docker rm ts-sql-query-mysql; exit 1; }
...
$RUN ./src/examples/MariaDBExample.ts || { docker stop ts-sql-query-mariadb; docker rm ts-sql-query-mariadb; exit 1; }
$RUN ./src/examples/MariaDBExample-compatibility.ts || { docker stop ts-sql-query-mariadb; docker rm ts-sql-query-mariadb; exit 1; }
if [ "$runtime" = "bun" ]; then
    $RUN ./src/examples/BunSqlMariaDBExample.ts || { docker stop ts-sql-query-mariadb; docker rm ts-sql-query-mariadb; exit 1; }
fi
$RUN ./src/examples/PrismaMariaDBExample.ts || { docker stop ts-sql-query-mariadb; docker rm ts-sql-query-mariadb; exit 1; }
...
$RUN ./src/examples/MssqlTediousExample.ts || { docker stop ts-sql-query-sqlserver; docker rm ts-sql-query-sqlserver; exit 1; }
$RUN ./src/examples/PrismaSqlServerExample.ts || { docker stop ts-sql-query-sqlserver; docker rm ts-sql-query-sqlserver; exit 1; }
...
$RUN ./src/examples/OracleDBExample.ts || { docker stop ts-sql-query-oracle; docker rm ts-sql-query-oracle; exit 1; }
docker stop ts-sql-query-oracle
docker rm ts-sql-query-oracle

echo "All examples ok ($runtime)"
