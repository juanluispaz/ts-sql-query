...
describe(ctx.label, () => {
    ...
    test('set-if-has-value-updates-when-current-has-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = $1, body = $2 where id = $3"`)
    ...
    test('set-if-set-if-value-requires-both-gates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = $1 where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5,
            1,
    ...
    test('set-if-not-set-if-value-skips-empty-and-already-set', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = $1, title = $2 where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "New title",
    ...
    test('set-if-has-value-if-value-requires-both-current-and-incoming', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = $1, body = $2, priority = $3 where id = $4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Triaged",
            null,
    ...
    test('ignore-if-has-value-drops-only-populated-sets', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set body = $1 where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            1,
    ...
    test('set-when-false-is-noop-and-when-true-delegates', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = $1, priority = $2, body = $3 where id = $4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Base",
            7,
