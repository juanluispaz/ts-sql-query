...
describe(ctx.label, () => {
    ...
    test('projecting-optional-values-as-nullable-over-values-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optNoteSampler as (select * from (values (@0, @1), (@2, @3)) as optNoteSampler(id, note)) select id as id, note as note from optNoteSampler order by id"`)
    ...
    test('values-as-insert-from-select-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with newYears as (select * from (values (@0, @1), (@2, @3)) as newYears(yearValue, [label])) insert into calendar_year (year_value, year_label) select yearValue as yearValue, [label] as [label] from newYears"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2025,
                "FY2025",
    ...
    test('values-as-delete-using-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with countryCodes as (select * from (values (@0)) as countryCodes(code)) delete from country from countryCodes where country.code = countryCodes.code"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "JP",
              ]
