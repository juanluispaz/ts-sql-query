...
describe(ctx.label, () => {
    ...
    test('update-returning-one-column-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = @0 output inserted.priority + @1 as [result] where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                100,
    ...
    test('update-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = @0 output inserted.id as id, inserted.title + @1 as tag where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched",
                " [done]",
    ...
    test('update-returning-nested-object-projection', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = @0, priority = @1 output inserted.id as id, inserted.title as [meta.title], inserted.priority as [meta.priority] where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched",
                5,
    ...
    test('update-returning-object-none-or-one', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = @0 output inserted.id as id, inserted.title as title where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Reordered",
                1,
    ...
    test('update-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = @0 output inserted.id as id, inserted.title as [meta.sibling], inserted.stage as [meta.gate] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Nightly build v2",
                2,
    ...
    test('update-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = @0 output inserted.body as [result] where project_id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                1,
    ...
    test('update-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = @0 output inserted.id as id, inserted.archived_at as [meta.archivedAt] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Legacy app v2",
                4,
    ...
    test('update-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.update(tProjectReview)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_date = @0, review_time = @1 output inserted.review_date as reviewDate, inserted.review_time as reviewTime where id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('update-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.update(tProjectRelease)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_release set released_on = @0, cutoff_time = @1, published_at = @2 output inserted.released_on as releasedOn, inserted.cutoff_time as cutoffTime, inserted.published_at as publishedAt where id = @3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('update-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = await ctx.conn.update(tProjectReview)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_time = @0 output inserted.review_time as [result] where id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1970-01-01T13:45:00.000Z,
                1,
              ]
            `)
