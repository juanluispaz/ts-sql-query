...
describe(ctx.label, () => {
    ...
    test('currentDate', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select cast(getdate() as date) as today"`)
    ...
    test('temporal-between-value-source-bounds-projected-is-optional', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when released_on between released_on and released_on then 1 when not released_on between released_on and released_on then 0 else null end as bit) as [b] from project_release where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal-comparison-value-source-column-overload', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when released_on = released_on then 1 else 0 end as bit) as eq, cast(case when released_on <> released_on then 1 else 0 end as bit) as ne, cast(case when released_on = released_on then 1 else 0 end as bit) as [i], cast(case when released_on <> released_on then 1 else 0 end as bit) as ni from project_release where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
