...
describe(ctx.label, () => {
    ...
    test('do-update-set-where-then-and-or-builds-compound-update-predicate', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? where (project.name <> ? and project.archived_at is null) or (project.published = 't') = ?"`)
    ...
    test('on-columns-where-then-and-or-builds-compound-partial-index-predicate', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) where ((published = 't') = ? and archived_at is null) or name = ? do update set name = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('do-update-set-dynamic-where-matches-the-direct-where', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(dynamicSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? where (project.name <> ? and project.archived_at is null) or (project.published = 't') = ?"`)
    ...
    test('on-columns-dynamic-where-matches-the-direct-where', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(dynamicSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) where ((published = 't') = ? and archived_at is null) or name = ? do update set name = ?"`)
            expect(dynamicParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('do-update-set-where-continuation-carries-returning-one-column', async () => {
        ...
        ctx.mockNext(1)
        await ctx.withRollback(async () => {
            const id = sync(ctx.conn.insertInto(tProject)
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? where project.name <> ? returning id as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('do-update-set-where-and-or-chain-then-returning-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) do update set name = ? where (project.name <> ? and project.archived_at is null) or (project.published = 't') = ? returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('multi-row-do-update-set-where-returning-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?), (?, ?, ?) on conflict (organization_id, slug) do update set name = excluded.name where project.archived_at is null returning id as id, name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
    ...
    test('on-columns-arbiter-where-then-do-update-set-where-lands-both-predicates', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?) on conflict (organization_id, slug) where archived_at is null do update set name = ? where project.name <> ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
