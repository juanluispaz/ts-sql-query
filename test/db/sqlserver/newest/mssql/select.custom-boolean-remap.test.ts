...
describe(ctx.label, () => {
    ...
    test('is-not-null-on-verified-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where cast(case when verified = 'Y' then 1 else 0 end as bit) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('is-not-null-on-published-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where cast(case when published = 't' then 1 else 0 end as bit) is not null order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('equals-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where cast(case when verified = 'Y' then 1 else 0 end as bit) = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('equals-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project where cast(case when published = 't' then 1 else 0 end as bit) = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) = @0 then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('is-not-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when cast(case approved when 'A' then 1 when 'R' then 0 else null end as bit) = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('not-in-on-invoiced-custom-boolean-emits-numeric-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) not in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('not-in-on-verified-custom-boolean-emits-string-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where cast(case when verified = 'Y' then 1 else 0 end as bit) not in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('not-in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) not in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            false,
    ...
    test('equals-if-value-negate-on-invoiced-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (cast(case when invoiced = 1 then 1 else 0 end as bit) = @0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
