...
describe(ctx.label, () => {
    ...
    test('bigint-is-is-not', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when duration_ms = @0 then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5400000n,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when duration_ms = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5400000n,
          ]
    ...
    test('double-is-is-not', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const rows = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when estimated_hours = @0 then 1 else 0 end = 1 order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2.5,
              ]
            ...
            const isNot = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when estimated_hours = @0 then 1 else 0 end = 1) order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2.5,
              ]
    ...
    test('uuid-equals-not-equals', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('uuid-is-is-not', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when external_ref = @0 then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when external_ref = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    // NOT-APPLICABLE: SQL Server orders `uniqueidentifier` by a non-lexical field order
    // (last group most significant), so the byte-range bounds 00000000…/50000000… don't
    // bracket issue 1's `0a…` ref the way byte/lexical engines do — not portable here.
    /*
    test('uuid-between-not-between', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between @0 and @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        const notRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not between @0 and @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "00000000-0000-0000-0000-000000000000",
            "50000000-0000-0000-0000-000000000000",
    ...
    */
    ...
    test('uuid-in-array-in-n', async () => {
        ...
        const inRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (@0, @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
        ...
        const inNRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('numeric-custom-boolean-direct-filter-operand', async () => {
        ...
        const eqRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) = @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
        ...
        const inRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cast(case when invoiced = 1 then 1 else 0 end as bit) in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
          ]
    ...
    test('customUuid-is-is-not-and-value-source-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when signing_key = @0 then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when signing_key = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    // NOT-APPLICABLE: SQL Server orders `uniqueidentifier` by a non-lexical field order
    // (last group most significant), so the byte-range bound 50000000… doesn't order
    // against issue 1's `0a…` ref the way byte/lexical engines do — not portable here.
    /*
    ...
    */
    ...
    test('customLocalDateTime-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        const isIf = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when signed_off_at = @0 then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        const isNotIf = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when signed_off_at = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2024-01-14T12:30:00.000Z,
          ]
    ...
    test('uuid-not-equals-if-value-not-in-if-value-fires-and-elides', async () => {
        ...
        const notInIf = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('is-value-source-operand-enum-custom-customUuid', async () => {
        ...
        const customUuidRows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when (signing_key = signing_key) or (signing_key is null and signing_key is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('uuid-ordered-comparison-and-mixed-between-value-source-operand', async () => {
        ...
        const cv = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between @0 and (select external_ref as [result] from issue where id = @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            2,
        ...
        const vc = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between (select external_ref as [result] from issue where id = @0) and @1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "7b3e9d20-2222-4c55-9b66-dddd00009999",
    ...
    test('plain-boolean-is-not-and-is-not-null', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when billable = @0 then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
          ]
    ...
    test('bigint-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (duration_ms = (select duration_ms as [result] from issue_worklog where id = @0)) or (duration_ms is null and (select duration_ms as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (duration_ms = (select duration_ms as [result] from issue_worklog where id = @0)) or (duration_ms is null and (select duration_ms as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('string-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when title = (select title as [result] from issue where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when title = (select title as [result] from issue where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('double-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const is = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (estimated_hours = (select estimated_hours as [result] from issue where id = @0)) or (estimated_hours is null and (select estimated_hours as [result] from issue where id = @1) is null) then 1 else 0 end = 1 order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                1,
            ...
            const isNot = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when (estimated_hours = (select estimated_hours as [result] from issue where id = @0)) or (estimated_hours is null and (select estimated_hours as [result] from issue where id = @1) is null) then 1 else 0 end = 1) order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                1,
    ...
    test('localDate-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when work_date = (select work_date as [result] from issue_worklog where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when work_date = (select work_date as [result] from issue_worklog where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('localTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (started_at = (select started_at as [result] from issue_worklog where id = @0)) or (started_at is null and (select started_at as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (started_at = (select started_at as [result] from issue_worklog where id = @0)) or (started_at is null and (select started_at as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('localDateTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where case when created_at = (select created_at as [result] from organization where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where not (case when created_at = (select created_at as [result] from organization where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customInt-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when cost_cents = (select cost_cents as [result] from issue_worklog where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when cost_cents = (select cost_cents as [result] from issue_worklog where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('customLocalTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when cutoff_time = (select cutoff_time as [result] from project_release where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when cutoff_time = (select cutoff_time as [result] from project_release where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customUuid-not-in-n', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key not in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('customUuid-in-n', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key in (@0) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('bigint-column-operand-comparable-membership', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (duration_ms = duration_ms) or (duration_ms is null and duration_ms is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (duration_ms = duration_ms) or (duration_ms is null and duration_ms is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('localTime-column-operand-comparable-membership', async () => {
        ...
        const isv = await ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (started_at = started_at) or (started_at is null and started_at is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (started_at = started_at) or (started_at is null and started_at is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalDateTime-column-operand-comparable-membership', async () => {
        ...
        const isv = await ctx.conn.selectFrom(tProjectRelease).where(c.is(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when (signed_off_at = signed_off_at) or (signed_off_at is null and signed_off_at is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when (signed_off_at = signed_off_at) or (signed_off_at is null and signed_off_at is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('uuid-column-operand-comparable-membership', async () => {
        ...
        const isv = await ctx.conn.selectFrom(tIssue).where(c.is(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (external_ref = external_ref) or (external_ref is null and external_ref is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = await ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when (external_ref = external_ref) or (external_ref is null and external_ref is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customUuid-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when (signing_key = signing_key) or (signing_key is null and signing_key is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('boolean-column-operand-equality-membership', async () => {
        ...
        const isv = await ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (billable = billable) or (billable is null and billable is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (billable = billable) or (billable is null and billable is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('boolean-value-when-null-nullif-value-column-arg', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, isnull(billable, billable) as [b] from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customComparable-value-when-null-nullif-value-column-arg', async () => {
        ...
        const vwnRows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, isnull(version, version) as [v] from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customUuid-if-value-twins-fires-and-elides', async () => {
        ...
        const notEqualsF = await ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.notEqualsIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key <> @0 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('double-column-operand-comparable-membership', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssue).where(c.is(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (estimated_hours = estimated_hours) or (estimated_hours is null and estimated_hours is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(is).toEqual(all)
        ...
        const isNot = await ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when (estimated_hours = estimated_hours) or (estimated_hours is null and estimated_hours is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('double-value-when-null-nullif-value-column-arg', async () => {
        ...
        const vwnRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, isnull(estimated_hours, estimated_hours) as [h] from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('boolean-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when (billable = (select billable as [result] from issue_worklog where id = @0)) or (billable is null and (select billable as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when (billable = (select billable as [result] from issue_worklog where id = @0)) or (billable is null and (select billable as [result] from issue_worklog where id = @1) is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('uuid-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where case when (external_ref = (select external_ref as [result] from issue where id = @0)) or (external_ref is null and (select external_ref as [result] from issue where id = @1) is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (case when (external_ref = (select external_ref as [result] from issue where id = @0)) or (external_ref is null and (select external_ref as [result] from issue where id = @1) is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customDouble-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where case when billed_amount = (select billed_amount as [result] from issue_worklog where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (case when billed_amount = (select billed_amount as [result] from issue_worklog where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('customComparable-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when version = (select version as [result] from project_release where id = @0) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when version = (select version as [result] from project_release where id = @0) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customLocalDateTime-is-is-not-value-source-operand', async () => {
        ...
        const is = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where case when (signed_off_at = (select signed_off_at as [result] from project_release where id = @0)) or (signed_off_at is null and (select signed_off_at as [result] from project_release where id = @1) is null) then 1 else 0 end = 1 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (case when (signed_off_at = (select signed_off_at as [result] from project_release where id = @0)) or (signed_off_at is null and (select signed_off_at as [result] from project_release where id = @1) is null) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('uuid-in-n-mixed-const-and-value-source', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (@0, (select external_ref as [result] from issue where id = @1)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "7b3e9d20-2222-4c55-9b66-dddd00009999",
            1,
    ...
    test('customUuid-in-n-mixed-const-and-value-source', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key in (@0, (select signing_key as [result] from project_release where id = @1)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            3,
