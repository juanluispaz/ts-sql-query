...
describe(ctx.label, () => {
    ...
    test('has-aggregation/customize-query-custom-window-hook-without-an-aggregate-reports-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  select /* hint */ /* cols */  id as id, name as name from project window w1 as (partition by organization_id) order by project.organization_id /* first-key */, id, project.name /* tiebreak */  /* tail */"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; name: string }>>>()
        expect(rows).toEqual(expected)
    })
    ...
    test('has-aggregation/customize-query-custom-window-hook-with-an-aggregate-reports-aggregation', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"/* head */  select /* hint */ /* cols */  id as id, name as name from project window w1 as (partition by organization_id /* issues=(select count(id) as result from issue) */) order by project.organization_id /* first-key */, id, project.name /* tiebreak */  /* tail */"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ id: number; name: string }>>>()
        expect(rows).toEqual(expected)
    })
