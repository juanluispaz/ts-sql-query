...
describe(ctx.label, () => {
    ...
    test('primary-key-factory-projects-each-comparable-kind', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select m_int as "id", m_str as "strPk", raw_to_uuid(m_uuid) as "uuidPk", m_bigint as "bigintPk" from col_matrix"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('table-virtual-column-factory-adapter-transforms-each-kind', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", m_int as "mInt", m_bigint as "mBigint", m_double as "mDouble", case when (m_bool = 1) then 1 else 0 end as "mBool", raw_to_uuid(m_uuid) as "mUuid", m_date as "mDate", m_time as "mTime", m_datetime as "mDatetime", m_str as "mStr", m_int as "cents", m_double as "money", raw_to_uuid(m_uuid) as "signingKey", m_str as "semver", m_str as "channel", m_date as "releaseDay", m_time as "cutoffClock", m_datetime as "signOff", m_str as "activity" from col_matrix"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof row, {
            id:           number
            mInt:         number
    ...
    test('table-optional-virtual-column-factory-adapter-transforms-each-kind', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", m_int as "mInt", m_bigint as "mBigint", m_double as "mDouble", case when (m_bool = 1) then 1 when not (m_bool = 1) then 0 else null end as "mBool", raw_to_uuid(m_uuid) as "mUuid", m_date as "mDate", m_time as "mTime", m_datetime as "mDatetime", m_str as "mStr", m_int as "cents", m_double as "money", raw_to_uuid(m_uuid) as "signingKey", m_str as "semver", m_str as "channel", m_date as "releaseDay", m_time as "cutoffClock", m_datetime as "signOff", m_str as "activity" from col_matrix"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof row, {
            id:           number
            mInt?:        number
