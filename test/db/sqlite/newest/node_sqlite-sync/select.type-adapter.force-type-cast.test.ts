...
describe(ctx.label, () => {
    ...
    test('force-type-cast-adapter-on-int-column-in-where', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const rows = sync(ctx.conn.selectFrom(tProjectForcedCast)
    ...
    test('force-type-cast-adapter-in-insert-values-position', async () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"insert into project (id, name) values (?, ?)"`)
    ...
    test('force-type-cast-adapter-in-update-set-position', async () => {
        ...
        expect(built.query()).toMatchInlineSnapshot(`"update project set name = ? where id = ?"`)
        expect(built.params()).toMatchInlineSnapshot(`
          [
            "Renamed",
            1,
