...
describe(ctx.label, () => {
    ...
    test('union-all-of-aggregate-as-array-arms-merges-array-typed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), '}'), ','), ']') as projects from organization left join project on project.organization_id = organization.id where organization.id = @0 group by organization.id union all select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), '}'), ','), ']') as projects from organization left join project on project.organization_id = organization.id where organization.id = @1 group by organization.id order by orgId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('union-all-of-aggregate-as-array-arms-projecting-optional-values-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), ', "archivedAt": ', isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), '}'), ','), ']') as projects from organization left join project on project.organization_id = organization.id where organization.id = @0 group by organization.id union all select organization.id as orgId, concat('[', string_agg(concat('{', '"id": ', isnull(convert(nvarchar, project.id), 'null'), ', "name": ', isnull('"' + string_escape(convert(nvarchar(max), project.name), 'json') + '"', 'null'), ', "archivedAt": ', isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), '}'), ','), ']') as projects from organization left join project on project.organization_id = organization.id where organization.id = @1 group by organization.id order by orgId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
