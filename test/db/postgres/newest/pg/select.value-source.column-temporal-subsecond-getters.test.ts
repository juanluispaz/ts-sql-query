...
//
// The file also covers two plain-column getter facets that need a real column:
// getTime() — the sole ROUNDING getter — on the same µs micro_stamp (999600µs
// rolls the epoch-ms UP to the next second, unlike its truncating siblings); and
// the date-part getters on the shiftHourAdapter temporal columns of tReleaseDraft
// (shiftedStamp/shiftedReleaseDay/shiftedCutoff), where the getter extracts a
// fresh unbranded number leaf and DROPS the column's TypeAdapter, reporting the
// RAW stored component rather than the adapter-shifted (+1h) value.
...
describe(ctx.label, () => {
    ...
    test('column-localdatetime-subsecond-getters-truncate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tTemporalPrecision)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select trunc(extract(second from micro_stamp))::integer as "s", trunc(extract(millisecond from micro_stamp))::integer % 1000 as ms from temporal_precision where id = $1"`)
    ...
    test('column-microstamp-gettime-rounds-up', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tTemporalPrecision)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select round(extract(epoch from micro_stamp) * 1000) as "t" from temporal_precision where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    // The shiftHourAdapter temporal columns on tReleaseDraft (shiftedStamp /
    // shiftedReleaseDay / shiftedCutoff — each a columnWithDefaultValue reading its
    // DB DEFAULT through shiftHourAdapter, a required column). A date-part getter
    // extracts a FRESH UNBRANDED number leaf, so the column's TypeAdapter is
    // DROPPED: the getter reports the RAW stored component, never the
    // adapter-shifted (+1h) value. Draft 1 leaves every shifted column at its
    // schema DEFAULT, so the values below come straight from the DEFAULT literal.
    ...
    test('column-shifthour-adapter-datetime-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from shifted_stamp) as "y", extract(month from shifted_stamp) - 1 as mo, extract(day from shifted_stamp) as "d", extract(dow from shifted_stamp) as dow, extract(hour from shifted_stamp) as "h", extract(minute from shifted_stamp) as "m", trunc(extract(second from shifted_stamp))::integer as "s", trunc(extract(millisecond from shifted_stamp))::integer % 1000 as ms, round(extract(epoch from shifted_stamp) * 1000) as "t" from release_draft where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('column-shifthour-adapter-date-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from shifted_release_day) as "y", extract(month from shifted_release_day) - 1 as mo, extract(day from shifted_release_day) as "d", extract(dow from shifted_release_day) as dow from release_draft where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('column-shifthour-adapter-time-getters-drop-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from shifted_cutoff) as "h", extract(minute from shifted_cutoff) as "m", trunc(extract(second from shifted_cutoff))::integer as "s", trunc(extract(millisecond from shifted_cutoff))::integer % 1000 as ms from release_draft where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
