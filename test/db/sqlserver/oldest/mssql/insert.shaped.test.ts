...
describe(ctx.label, () => {
    ...
    test('shaped-values-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) output inserted.id as [result] values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped returning",
    ...
    test('shaped-multi-row-set-for-all', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug, published) values (@0, @1, @2, case when (@3 = 1) then 't' else 'f' end), (@4, @5, @6, case when (@7 = 1) then 't' else 'f' end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped FA A",
    ...
    test('shaped-values-returning-object', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) output inserted.id as id, inserted.name as name values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
    ...
    test('shaped-values-returning-last-inserted-id', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) output inserted.id values (@0, @1, @2)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped last id",
    ...
    test('shaped-on-conflict-do-update-set-where-then-and-or', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, name, slug) values (@0, @1, @2) on conflict (organization_id, slug) do update set name = @3 where (project.name <> @4 and project.archived_at is null) or cast(case when project.published = 't' then 1 else 0 end as bit) = @5"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "ignored",
