...
describe(ctx.label, () => {
    ...
    test('shaped-allowing-no-where-set-when-touches-all-rows', async () => {
        ...
        ctx.mockNext(4)
        await ctx.withRollback(async () => {
            sync(ctx.conn.updateAllowingNoWhere(tProject)
    ...
    test('shaped-allowing-no-where-ignore-if-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = ?, published = case when ? then 't' else 'f' end"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = ?"`)
            expect(trueSql).not.toBe(falseSql)
            expect(affected).toBe(4)
        })
    })
    ...
    test('shaped-allowing-no-where-ignore-if-has-value-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update project set name = ?, archived_at = ?"`)
            expect(trueSql).toMatchInlineSnapshot(`"update project set name = ?"`)
            expect(trueSql).not.toBe(falseSql)
            expect(affected).toBe(4)
        })
