...
describe(ctx.label, () => {
    ...
    test('numeric-operators-column-rhs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select priority - id as sub, cast(priority as real) / id as div, power(priority, id) as pw, log(id, priority) as ln, round(priority, id) as rnd, atan2(priority, id) as at2, min(priority, number) as mx from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('is-and-isnot-over-two-optional-columns-project-required-boolean-leaf', async () => {
        ...
        ctx.mockNext(expected)
        const r = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.id.equals(1))
            .select({
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select assignee_id is parent_id as eqIs, assignee_id is not parent_id as neqIs from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('lessOrEqual-string-column-rhs', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status <= status as le from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('notIn-and-notInN-value-source-elements', async () => {
        ...
        const notInNRows = sync(ctx.conn.selectFrom(tIssue)
