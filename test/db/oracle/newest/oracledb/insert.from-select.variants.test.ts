...
describe(ctx.label, () => {
    ...
    // Not applicable on Oracle: `RETURNING ... INTO` is restricted to
    // single-row inserts on Oracle, so the library does not expose
    // `.returning(...)` on `insertInto(t).from(source)` for OracleDB.
    // See the canonical cell for the full body.
    /*
    test('insert-from-select-returning-full-row', async () => {
        // ... see other cells for the full body — uses `.returning({...})`
        // which is not typed on OracleConnection's from-select insert.
    })
    */
    ...
    // Not applicable on Oracle: no `ON CONFLICT` clause in the dialect.
    // Oracle's equivalent is `MERGE INTO`, exposed through a different
    // API. The library does not type `.onConflictDoNothing` on the
    // from-select insert for OracleConnection. See the canonical cell
    // for the full body.
    /*
    test('insert-from-select-with-on-conflict-do-nothing', async () => {
        // ... see other cells for the full body — uses
        // `.onConflictDoNothing()` which is not typed on Oracle.
    })
    */
})
