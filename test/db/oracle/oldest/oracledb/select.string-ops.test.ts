    ...
    */
    ...
    test('string-predicate-on-optional-receiver-projects-optional-boolean', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when "body" like ('%' || :0 || '%') escape '\\' then 1 when not "body" like ('%' || :1 || '%') escape '\\' then 0 else null end as "hasNeedle" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "token",
            "token",
    ...
    test('string-transforms-on-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select upper("body") as "up", lower("body") as "lo", length("body") as "len", ltrim("body") as "tl", rtrim("body") as "tr" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('substr-family-with-optional-value-source-argument', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(title, assignee_id + 1) as "se", substr(title, assignee_id + 1) as "sse", substr(title, assignee_id + 1, :0) as "s2", substr(title, assignee_id + 1, :1 - assignee_id) as "sg" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
    ...
    test('substr-numeric-start-value-source-count-and-substring-both-value-sources', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(title, :0, priority) as "sc", substr(title, id + 1, priority - id) as "ss" from issue where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('substr-to-end-on-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(version_bracketed, :0) as "sub" from release_overview where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('substr-to-end-on-optional-adapter-view-column', async () => {
        ...
        const row = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", substr(channel_bracketed, :0) as "sub" from release_overview where id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('affix-predicates-with-a-value-source-term', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when title like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) escape '\\' then 1 when not title like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) escape '\\' then 0 else null end as "ct", case when title like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) escape '\\' then 1 when not title like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') end) escape '\\' then 0 else null end as "ew", case when title not like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) escape '\\' then 1 when not title not like (case when replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') is null then null else '%' || replace(replace(replace("body", '\\', '\\\\'), '%', '\\%'), '_', '\\_') || '%' end) escape '\\' then 0 else null end as "nc" from issue where project_id = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-as-string-string-methods', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", length(raw_to_uuid(external_ref)) as "len", upper(raw_to_uuid(external_ref)) as "up", lower(raw_to_uuid(external_ref)) as "lo" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
