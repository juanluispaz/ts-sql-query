...
describe(ctx.label, () => {
    ...
    test('ignore-if-has-value-drops-only-populated-sets', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, assignee_id, status, priority) values (?, ?, ?, ?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                104,
    ...
    test('disallow-if-no-value-narrows-missing-keys-so-a-staged-value-executes', async () => {
        ...
        ctx.mockNext(1)
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.insertInto(tIssue).dynamicSet()
    ...
    test('set-for-all-if-not-set-if-value-skips-a-no-value-incoming', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority, assignee_id) values (?, ?, ?, ?, ?, ?), (?, ?, ?, ?, ?, ?)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9403,
