...
describe(ctx.label, () => {
    ...
    test('marshalling/bigint-column-scalar-read-past-2p53', async () => {
        ...
        // better-sqlite3 hands wide integers back as a rounded JavaScript number (safeIntegers off),
        // so the exact value is already lost; the marshaller refuses it with
        // PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE rather than a clean-but-wrong bigint. The mock
        // seeds the same rounded number the real driver returns, so mock and real agree.
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
