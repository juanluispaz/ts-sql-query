...
describe(ctx.label, () => {
    ...
    test('docs:aggregate-as-object-array/aggregate-as-array-of-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as id, organization.name as name, json_group_array(project.name) as projectNames from organization left join project on project.organization_id = organization.id where organization.id = ? group by organization.id, organization.name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:aggregate-as-object-array/inline-aggregated-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select json_group_array(json_object('id', a_1_.id, 'name', a_1_.name)) from (select id as id, name as name from project where organization_id = organization.id and archived_at is null order by id) as a_1_) as projects from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:aggregate-as-object-array/inline-aggregated-of-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (select json_group_array(a_1_.result) from (select name as result from project where organization_id = organization.id order by result) as a_1_) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:aggregate-as-object-array/aggregate-as-array-distinct', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id, project.name as name, json_group_array(distinct issue.priority) as priorities from project left join issue on issue.project_id = project.id where project.id = ? group by project.id, project.name"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:aggregate-as-object-array/recursive-query-as-inline-array', async () => {
        ...
        const row = sync(connection.selectFrom(tIssue)
            .select({
                id:        tIssue.id,
                title:     tIssue.title,
