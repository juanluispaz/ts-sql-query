...
describe(ctx.label, () => {
    ...
    test('select-from-view-projects-of-org', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, organization_name as organizationName, organization_plan as organizationPlan from project_overview where organization_id = @0 order by id"`)
    ...
    test('select-view-virtual-column-name-upper', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, upper(name) as nameUpper from project_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('select-view-optional-archived-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, archived_at as archivedAt from project_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('select-from-view-aliased-with-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select po.id as id, po.name as name from project_overview as po where po.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-for-use-in-left-join-matches', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.name as orgName, project_overview.name as projectName, upper(project_overview.name) as projectNameUpper from organization left join project_overview on project_overview.id = organization.id where organization.id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-for-use-in-left-join-as-no-match-yields-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.name as orgName, po.name as projectName from organization left join project_overview as po on po.id = @0 where organization.id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            999,
            1,
