...
describe(ctx.label, () => {
    ...
    test('rule-2-left-join-object-mixing-a-const-leaf-dropped-on-join-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as "iss.title", $1::text as "iss.tag" from project left join issue on issue.project_id = project.id where project.id in ($2, $3) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('two-different-left-joins-merged-in-one-leaf-plus-const-survives-partial-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id + app_user.id as "obj.combined", $1::text as "obj.tag" from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where issue.id in ($2, $3) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('merged-two-left-join-leaf-alone-no-const-drops-object-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id + app_user.id as "obj.combined" from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where issue.id in ($1, $2) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('own-optional-sibling-keeps-object-alive-on-left-join-miss-default', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.title as "obj.issTitle", project.archived_at as "obj.projArchived" from project left join issue on issue.project_id = project.id where project.id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
    ...
    test('merged-leaf-first-with-own-required-anchor-survives-partial-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id + app_user.id as "obj.combined", issue.id as "obj.ownId" from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where issue.id in ($1, $2) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('merged-leaf-mixing-main-and-left-join-source-plus-const-survives-partial-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, issue.id + app_user.id as "obj.combined", $1::text as "obj.tag" from issue left join app_user on app_user.id = issue.assignee_id where issue.id in ($2, $3) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('merged-leaf-plus-const-nested-one-level-deeper-survives-partial-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, project.id + app_user.id as "outer.inner.combined", $1::text as "outer.inner.tag" from issue left join project on project.id = issue.project_id left join app_user on app_user.id = issue.assignee_id where issue.id in ($2, $3) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "rel",
            3,
    ...
    test('merged-same-single-left-join-leaf-stays-rule-2-and-drops-on-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, issue.id + issue.number as "obj.m" from project left join issue on issue.project_id = project.id where project.id in ($1, $2) order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            4,
    ...
    test('object-mixing-own-optional-and-left-join-optional-leaves-drops-on-full-miss-default', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, issue.body as "obj.body", project.archived_at as "obj.arch" from issue left join project on project.id = issue.project_id where issue.id in ($1, $2) order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('merge-optional-required-in-optional-object-fragment-gates-rule-1-drop', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, coalesce(issue.title, issue.status, issue.body) as "grp.frag", project.name as "grp.projName" from project left join issue on issue.project_id = project.id where project.id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            4,
          ]
