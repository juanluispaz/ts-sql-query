...
describe(ctx.label, () => {
    ...
    test('shaped-from-references-from-table-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const projects = sync(ctx.conn.selectFrom(tProject)
    ...
    test('shaped-set-when-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.update(tProject)
                ...
                .executeUpdateOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = ?, slug = ? where id = ? returning id as id, name as name, slug as slug"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('shaped-allowing-no-where-set-then-extend-shape-executes', async () => {
        ...
        ctx.mockNext(2)
        await ctx.withRollback(async () => {
            const affected = sync(ctx.conn.updateAllowingNoWhere(tOrganization)
                .shapedAs({ orgName: 'name' })
