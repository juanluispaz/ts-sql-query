    ...
    try {
        ...
        await connection.queryRunner.executeDatabaseSchemaModification(`
            create table boolean_table (
                ...
                opt_bool_b boolean null,
                bool_c char not null check (bool_c = 'T' or bool_c = 'F'),
                bool_d char not null check (bool_d = 'Y' or bool_d = 'N'),
        ...
        const acmeCompanyWithCustomers8 = await connection.selectFrom(tCompany)
            .where(tCompany.id.equals(1)) // Outer reference replaced by value
            .select({
                id: tCompany.id,
                name: tCompany.name,
        ...
        // For some reason MariaDB skip the number 5
        ...
        // Due some reason prisma returns strings instead numbers for the date parts
        ...
        // and on Prisma's MySQL adapter, but Prisma's mariadb adapter rejects the
        // `WITH name(cols) AS (VALUES (...)) UPDATE/DELETE` form with
        // `DriverAdapterError: LengthMismatch`. The emitted SQL and its bound parameters are correct —
        // the param count matches the `?` placeholders exactly (verified) — so this is the adapter
        // mishandling the `VALUES (...)` table constructor (it appears to treat it as a bulk-insert
        // row list), not a library issue. Stays commented for this connector.
