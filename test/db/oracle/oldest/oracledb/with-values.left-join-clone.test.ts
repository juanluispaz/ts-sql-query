...
describe(ctx.label, () => {
    ...
    test('values-for-use-in-left-join-no-arg-qualifies-by-with-name', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (select :0, :1 from dual) select project.id as "pid", projectPatch.name as "newName" from project left join projectPatch on projectPatch.id = project.id where project.id <= :2 order by "pid""`)
    ...
    test('values-with-virtual-column-aliased-via-as-skips-virtual-in-name-walk', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchV(id, name) as (select :0, :1 from dual union all select :2, :3 from dual) select pp.id as "id", pp.name as "name", 'PATCH' as "label" from projectPatchV pp order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
    ...
    test('values-with-virtual-column-for-use-in-left-join-as-skips-virtual', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchV(id, name) as (select :0, :1 from dual) select project.id as "pid", pp.name as "newName", 'PATCH' as "tag" from project left join projectPatchV pp on pp.id = project.id where project.id <= :2 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
    ...
    test('values-sibling-referencing-virtual-column-aliased-via-as-requalifies-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(pp)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchD(id, name) as (select :0, :1 from dual union all select :2, :3 from dual) select pp.id as "id", pp.name as "name", pp.id * 2 as "doubled" from projectPatchD pp order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            "three",
    ...
    test('values-sibling-referencing-virtual-column-for-use-in-left-join-as-requalifies-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('pid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatchD(id, name) as (select :0, :1 from dual) select project.id as "pid", pp.id * 2 as "doubled" from project left join projectPatchD pp on pp.id = project.id where project.id <= :2 order by "pid""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
