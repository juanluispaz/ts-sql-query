    ...
    */
    ...
    test('compound-insensitive-order-by-wrap-with-limit-offset', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .offset(2)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by lower(label) asc limit ? offset ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('compound-value-source-order-by-wrap-with-limit', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .limit(2)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by label, ? limit ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-wrap-execute-select-page', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by lower(label) asc limit ? offset ?"`)
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (select \`name\` as label from project union select title as label from issue) as result_for_count"`)
    ...
    */
    ...
    test('compound-unionAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(1)).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid from issue where id = ? union all select id as iid from issue where id = ? order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-intersect-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid from issue where id in (?, ?) intersect select id as iid from issue where id = ? order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-intersectAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid from issue where id in (?, ?) intersect all select id as iid from issue where id = ? order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-except-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid from issue where id in (?, ?) except select id as iid from issue where id = ? order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-exceptAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as iid from issue where id in (?, ?) except all select id as iid from issue where id = ? order by iid, ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    */
    ...
    test('compound-order-by-raw-fragment-embedding-multiple-value-sources', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by label, 0 + ? + ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-order-by-raw-fragment-embedding-value-source-in-expression', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by label, coalesce(?, 0)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-order-by-value-source-collate-secondary', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .union(ctx.conn.selectFrom(tIssue).select({ label: tIssue.title }))
            .orderBy('label')
            .orderBy(ctx.conn.const('a', 'string').collate('utf8mb4_bin'))
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select \`name\` as label from project union select title as label from issue order by label, ? collate utf8mb4_bin"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
          ]
