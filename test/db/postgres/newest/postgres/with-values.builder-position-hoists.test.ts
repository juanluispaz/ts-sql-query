...
describe(ctx.label, () => {
    ...
    test('projecting-optional-values-as-nullable-over-values-source', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optNoteSampler(id, note) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select id as id, note as note from optNoteSampler order by id"`)
    ...
    test('values-as-insert-from-select-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with newYears(yearValue, label) as (values ($1::int4, $2::text), ($3::int4, $4::text)) insert into calendar_year (year_value, year_label) select yearValue as "yearValue", label as label from newYears"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2025,
                "FY2025",
