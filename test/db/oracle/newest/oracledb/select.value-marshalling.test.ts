...
describe(ctx.label, () => {
    ...
    test('marshalling/bigint-insert-select-roundtrip', async () => {
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority, view_count) values (:0, :1, :2, :3, :4, :5) returning id into :6"`)
    ...
    test('marshalling/double-insert-select-roundtrip', async () => {
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority, estimated_hours) values (:0, :1, :2, :3, :4, :5) returning id into :6"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9003,
    ...
    test('marshalling/uuid-insert-select-roundtrip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority, external_ref) values (:0, :1, :2, :3, :4, uuid_to_raw(:5)) returning id into :6"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9004,
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", raw_to_uuid(external_ref) as "ref" from issue where "number" = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                9004,
              ]
