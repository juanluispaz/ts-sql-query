...
describe(ctx.label, () => {
    ...
    test('modulo-fractional-literal', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(priority as numeric(38, 16)) % cast(@0 as numeric(38, 16)) as [m] from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2.5,
          ]
    ...
    test('atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, atn2(priority, @0) as [a] from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('double-column-receiver-overloaded-arithmetic-threads-optional-and-emits-uncast', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, estimated_hours + @0 as [a], estimated_hours - @1 as [s], estimated_hours * @2 as mu, cast(estimated_hours as numeric(38, 16)) % cast(@3 as numeric(38, 16)) as mo, case when estimated_hours is null then null else iif(estimated_hours > @4, estimated_hours, @5) end as mn, case when estimated_hours is null then null else iif(estimated_hours < @6, estimated_hours, @7) end as mx from issue where id = @8"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1.5,
                1.5,
    ...
    test('ceil-int-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceiling(priority) as [c] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('round-int-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(priority, 0) as [r] from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('logn-int-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, log(priority, @0) as [l] from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('optional-bigint-receiver-rounding-arithmetic-fan-out', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ceiling(duration_ms) as ce, floor(duration_ms) as fl, round(duration_ms, 0) as ro, duration_ms - @0 as [s], duration_ms % @1 as mo, case when duration_ms is null then null else iif(duration_ms > @2, duration_ms, @3) end as mn, case when duration_ms is null then null else iif(duration_ms < @4, duration_ms, @5) end as mx from issue_worklog where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            400000n,
            1000000n,
