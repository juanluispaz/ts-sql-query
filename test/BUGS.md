...
## `returningLastInsertedId` is gated out of the insert on-conflict path on SQL Server / Oracle, while `returning` / `returningOneColumn` are allowed

**Where**: `OnConflictReturningLastInsertedIdType` in
[src/expressions/insert.ts:712-715](../src/expressions/insert.ts#L712-L715) gates its
dialect list to `'noopDB' | 'postgreSql' | 'sqlite' | 'mariaDB'` and excludes
`sqlServer` / `oracle` with the inline comment `not suported yet`. But on the SAME
interface (`CustomizableExecutableSimpleInsertOnConflict`,
[insert.ts:78-82](../src/expressions/insert.ts#L78)), the sibling members
`returning` (`ReturningFnType`,
[insert.ts:684-687](../src/expressions/insert.ts#L684)) and `returningOneColumn`
(`ReturningOneColumnFnType`, [insert.ts:689-692](../src/expressions/insert.ts#L689))
DO list `sqlServer` and `oracle`. `.defaultValues()` returns that interface.
**Reproduction**: `conn.insertInto(t).defaultValues().returningLastInsertedId()` (t with
an autogenerated PK) — `.returningLastInsertedId()` is `never` on SQL Server / Oracle
(`error TS2349: This expression is not callable. Type 'never' has no call signatures`),
yet `conn.insertInto(t).defaultValues().returningOneColumn(t.id).executeInsertOne()`
compiles on the same dialects and emits the identical SQL (SQL Server
`OUTPUT inserted.id …`, Oracle `… returning id into :0`). So the engine and the SQL
path both support returning the id — only the `returningLastInsertedId` convenience is
gated out. The full `returningLastInsertedId` chain compiles on pg/sqlite/mysql/mariadb.
**Current workaround in the suite**:
`test/db/{sqlserver,oracle}/newest/{mssql,oracledb}/insert.default-values.test.ts` — the
`insert default values` test returns the id via
`.returningOneColumn(tDefaultsOnly.id).executeInsertOne()` (same SQL, fully typed, no
`as any`), carrying a `// TODO[BUG]` above the test that points here; switch back to
`.returningLastInsertedId()` once it is wired for these dialects.

---

## A no-`SET` update is type-executable: `update(t).dynamicSet().where(...)` reaches `executeUpdate`

**Where**: `NotExecutableUpdateExpression` and its `.where(...)`
([src/expressions/update.ts:158-216](../src/expressions/update.ts#L158)), reachable
from `UpdateSetExpression.dynamicSet()`
([update.ts:288](../src/expressions/update.ts#L288)). `.where()` returns
`DynamicExecutableUpdateExpression`
([update.ts:281](../src/expressions/update.ts#L281)), which extends the executable
chain (`executeUpdate`) — so a `dynamicSet()` that set no columns still becomes
executable. The `NotExecutable` gate keys on the missing `WHERE`, never on the
presence of a set.
**Reproduction**: `conn.update(t).dynamicSet().where(t.id.equals(1)).executeUpdate()`
type-checks even though no column was ever set — a semantically empty `UPDATE`
(no `SET`). The runtime short-circuits (resolves 0; the one-row path throws
`NO_COLUMN_SETS`), so the type permits building a statement that has no valid SQL
form. (Open question for the fixing agent: whether `dynamicSet()` should keep 0
columns reachable at all, or whether the `NotExecutable` types are needed in their
current shape.)
**Current workaround in the suite**: none — no test is compromised. The
`test/db/*/newest/*/update.execute-variants.test.ts` `execute-update-*-with-no-sets-*`
tests build a no-`SET` update through the typed surface (it compiles today) and assert
the runtime short-circuit; they pass and need no marker. Documented here only, as a
type-design issue to revisit in `src/`.
