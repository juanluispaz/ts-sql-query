...
describe(ctx.label, () => {
    ...
    test('from-model/emits-the-same-sql-as-the-descriptor-form', async () => {
        ...
        expect(a.sql).toMatchInlineSnapshot(`"select id as "id" from issue where (lower(title) like lower(:0 || '%') escape '\\' or priority > :1) and view_count >= :2 and lower(raw_to_uuid(external_ref)) like lower('%' || :3 || '%') escape '\\' order by "id""`)
