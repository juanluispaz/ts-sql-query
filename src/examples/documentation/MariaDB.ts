...
async function main() {
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as id, name as name, (select json_arrayagg(concat(concat(first_name, ?), last_name) order by concat(concat(first_name, ?), last_name)) from customer where company_id = company.id) as customers from company where id = ?")
    expectedParams.push(`[" "," ",1]`)
    expectedType.push(`selectOneRow`)
