...
describe(ctx.label, () => {
    ...
    test('builder-projecting-flag-and-bare-aggregate-project-optionals-independently', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as "pid", project.archived_at as "optArchived", json_arrayagg(json_object('id' value issue.id, 'body' value issue."body")) as "issues" from project inner join issue on issue.project_id = project.id where project.id = :0 group by project.id, project.archived_at"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
