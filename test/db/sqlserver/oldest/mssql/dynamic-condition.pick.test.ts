...
describe(ctx.label, () => {
    ...
    test('pick/aggregate-as-array-field-default', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, concat('[', string_agg(concat('{', '"title": ', '"' + string_escape(convert(nvarchar(max), issue.title), 'json') + '"', ', "body": ', isnull('"' + string_escape(convert(nvarchar(max), issue.body), 'json') + '"', 'null'), '}'), ','), ']') as issues from project inner join issue on issue.project_id = project.id where project.id = @0 group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('pick/dynamic-pick-paths-without-mandatory-arg', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select title as title from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
