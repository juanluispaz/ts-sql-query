...
describe(ctx.label, () => {
    ...
    test('fragment-with-type-date-time-and-enum-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select work_date as "wd", started_at as "st", activity as "act" from issue_worklog where id = :0"`)
    ...
    test('fragment-with-type-double-custom-and-custom-local-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select 1.5 as "avg", channel as "ch", raw_to_uuid(signing_key) as "key", released_on as "rOn", cutoff_time as "cut", signed_off_at as "soff" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-widen-the-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select version as "v", 1.5 as "avg", raw_to_uuid(signing_key) as "key", channel as "ch" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-scalar-arms', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(title) as "maxTitle", avg(priority) as "avgPrio", case when count(*) > 0 then 1 else 0 end as "any", max(view_count) as "maxView" from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('aggregate-fragment-with-type-optional-scalar-arms', async () => {
        ...
        const result = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(priority) as "mi", avg(priority) as "md", max(title) as "ms", case when max(priority) > 5 then 1 when not (max(priority) > 5) then 0 else null end as "mb" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('aggregate-fragment-with-type-enum-and-custom-arms', async () => {
        ...
        const actRow = await c.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(activity) as "maxActivity" from issue_worklog where issue_id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(upper(activity)) as "tagged" from issue_worklog where issue_id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-custom-kind-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select version as "v" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-local-time-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select max(started_at) as "mlt" from issue_worklog where issue_id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-over-worklog', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when (billable = 1) then 1 when not ((billable = 1)) then 0 else null end as "bill", work_date as "wd", started_at as "st", cost_cents as "cost", billed_amount as "amt", activity as "act" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arms-over-release', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select raw_to_uuid(signing_key) as "key", released_on as "rOn", cutoff_time as "cut", signed_off_at as "soff", version as "v" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-optional-arm-plain-local-date-time', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select created_at as "ca" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when (billable = 1) then 1 else 0 end as "req", case when (billable = 1) then 1 when not ((billable = 1)) then 0 else null end as "opt" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-fragment-with-type-boolean-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when count(*) > 0 then 1 else 0 end as "any" from issue"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('aggregate-fragment-with-type-boolean-optional-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await c.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select case when max(priority) > 5 then 1 when not (max(priority) > 5) then 0 else null end as "any" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -1,
          ]
    ...
    test('fragment-with-type-local-date-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select work_date as "wd" from issue_worklog where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('fragment-with-type-custom-local-date-threads-a-trailing-type-adapter', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select released_on as "rOn" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
