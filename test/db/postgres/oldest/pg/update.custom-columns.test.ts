...
describe(ctx.label, () => {
    ...
    test('update-project-release-custom-column-with-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_release set version = $1 where id = $2"`)
    ...
    test('update-release-draft-optional-custom-column-with-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set min_version = min_version where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_release set channel = $1 where id = $2 returning channel as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "beta",
                1,
    ...
    test('update-country-keyed-on-string-provided-primary-key', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update country set name = $1 where code = $2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('delete-country-keyed-on-string-provided-primary-key', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from country where code = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "JP",
              ]
    ...
    test('update-project-review-scales-non-boolean-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set score = $1, reviewer_code = $2 where id = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                92,
                "R-X",
    ...
    test('update-worklog-invoiced-literal-binds-numeric-boolean', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set invoiced = case when $1::bool then 1 else 0 end, cost_cents = $2, billed_amount = $3 where id = $4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                true,
                999,
    ...
    test('write-through custom-kind-then-adapter columns scale/shift the bound param and round-trip on read', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tReleaseDraft)
                ...
                .where(tReleaseDraft.id.equals(1))
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set scaled_cost = $1, shifted_amount = $2 where id = $3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                10,
                -995,
            ...
            const row = await ctx.conn.selectFrom(tReleaseDraft)
                ...
                .executeSelectOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select scaled_cost as cost, shifted_amount as amount from release_draft where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
