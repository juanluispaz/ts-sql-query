...
describe(ctx.label, () => {
    ...
    test('update-project-release-custom-column-with-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const updated = sync(ctx.conn.update(tProjectRelease)
    ...
    test('update-release-draft-optional-custom-column-with-value-source-rhs', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const updated = sync(ctx.conn.update(tReleaseDraft)
    ...
    test('update-project-release-returning-branded-custom-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('beta')
            const channel = sync(ctx.conn.update(tProjectRelease)
                .set({ channel: 'beta' })
                .where(tProjectRelease.id.equals(1))
    ...
    test('update-worklog-returning-adapter-virtual-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext('CODING')
            const tagged = sync(ctx.conn.update(tIssueWorklog)
                .set({ minutes: 95 })
                .where(tIssueWorklog.id.equals(1))
                .returningOneColumn(tIssueWorklog.activityTagged)
                .executeUpdateOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue_worklog set minutes = ? where id = ? returning upper(activity) as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                95,
                1,
    ...
    test('update-country-keyed-on-string-provided-primary-key', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const updated = sync(ctx.conn.update(tCountry)
    ...
    test('delete-country-keyed-on-string-provided-primary-key', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const deleted = sync(ctx.conn.deleteFrom(tCountry)
    ...
    test('update-project-review-scales-non-boolean-adapter-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const updated = sync(ctx.conn.update(tProjectReview)
    ...
    test('update-worklog-invoiced-literal-binds-numeric-boolean', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            const updated = sync(ctx.conn.update(tIssueWorklog)
    ...
    test('write-through value-transforming adapters shift the bound param and round-trip on read', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const count = 8000n
            ctx.mockNext(1)
            const affected = sync(ctx.conn.update(tReleaseDraft)
                .set({ shiftedStamp: stamp, shiftedCount: count })
                .where(tReleaseDraft.id.equals(1))
                .executeUpdate())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set shifted_stamp = ?, shifted_count = ? where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "2025-01-15 11:00:00",
                7000n,
            ...
            const row = sync(ctx.conn.selectFrom(tReleaseDraft)
                .where(tReleaseDraft.id.equals(1))
                .select({ stamp: tReleaseDraft.shiftedStamp, count: tReleaseDraft.shiftedCount })
                .executeSelectOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select shifted_stamp as stamp, shifted_count as count from release_draft where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('write-through custom-kind-then-adapter columns scale/shift the bound param and round-trip on read', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = sync(ctx.conn.update(tReleaseDraft)
                ...
                .where(tReleaseDraft.id.equals(1))
                .executeUpdate())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set scaled_cost = ?, shifted_amount = ? where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                10,
                -995,
            ...
            const row = sync(ctx.conn.selectFrom(tReleaseDraft)
                ...
                .executeSelectOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select scaled_cost as cost, shifted_amount as amount from release_draft where id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
