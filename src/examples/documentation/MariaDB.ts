...
async function main() {
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, first_name as firstName, last_name as lastName from customer where id = ? order by customer.birthday desc`)
    expectedParams.push(`[10]`)
    expectedType.push(`selectOneRow`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into customer (first_name, last_name, company_id) with sourceCustomers as (select first_name as firstName, last_name as lastName, company_id as companyId from customer where company_id = ?) select firstName as firstName, lastName as lastName, companyId as companyId from sourceCustomers`)
    expectedParams.push(`[1]`)
    expectedType.push(`insert`)
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as `result.id`, first_name as `result.firstName`, last_name as `result.lastName`, birthday as `result.birthday`, company_id as `result.companyId`, concat(first_name, ?, last_name) as `result.name`, calculateAge(birthday) as `result.age` from customer where id = ?")
    expectedParams.push(`[" ",9]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as id, name as name, cast((select json_arrayagg(json_object('id', id, 'firstName', first_name, 'lastName', last_name) order by id) from customer where company_id = company.id) as char) as customers from company where id = ?")
    expectedParams.push(`[1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as id, name as name, cast((select json_arrayagg(concat(first_name, ?, last_name) order by concat(first_name, ?, last_name)) from customer where company_id = company.id) as char) as customers from company where id = ?")
    expectedParams.push(`[" "," ",1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as id, name as name, cast((select json_arrayagg(a_1_.result) from (select concat(first_name, ?, last_name) as result from customer where company_id = ? union select concat(first_name, ?, last_name) as result from customer where company_id = ?) as a_1_) as char) as customers from company where id = ?")
    expectedParams.push(`[" ",1," ",1,1]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push("select id as id, name as name, cast((select json_arrayagg(a_1_.result) from (select concat(first_name, ?, last_name) as result from customer where company_id = ? union select concat(first_name, ?, last_name) as result from customer where company_id = ? order by result limit 2147483647) as a_1_) as char) as customers from company where id = ?")
    expectedParams.push(`[" ",1," ",1,1]`)
    expectedType.push(`selectOneRow`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert into record (id, title) values (?, ?)`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, title as title from record where id like concat('%', ?, '%')`)
    ...
    result = { id: '89bf68fc-7002-11ec-90d6-0242ac120003', title: 'My voice memo' }
    expectedResult.push(result)
    expectedQuery.push(`with with_uuit as (select id as id, title as title from record) select id as id, title as title from with_uuit where id like concat('%', ?, '%')`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    expectedResult.push(result)
    expectedQuery.push(`select cast((select json_arrayagg(json_object('id', id, 'title', title)) from record where id like concat('%', ?, '%')) as char) as records`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    result = { records: [{ id: '89bf68fc-7002-11ec-90d6-0242ac120003', title: 'My voice memo' }] }
    expectedResult.push(result)
    expectedQuery.push(`select cast((select json_arrayagg(json_object('id', id, 'title', title) order by id) from record where id like concat('%', ?, '%')) as char) as records`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    result = { records: [{ id: '89bf68fc-7002-11ec-90d6-0242ac120003', title: 'My voice memo' }] }
    expectedResult.push(result)
    expectedQuery.push(`select cast(json_arrayagg(json_object('id', id, 'title', title)) as char) as records from record where id like concat('%', ?, '%')`)
    expectedParams.push(`["7002"]`)
    expectedType.push(`selectOneRow`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`insert ignore into record (id, title) values (?, ?)`)
    expectedParams.push(`["89bf68fc-7002-11ec-90d6-0242ac120003","My voice memo"]`)
    expectedType.push(`insert`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select customer.first_name as firstName, customer.last_name as lastName, customer.birthday as birthday from customer join company on company.id = customer.company_id where company.name = ?`)
    expectedParams.push(`["My company name"]`)
    expectedType.push(`selectManyRows`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`with customerForUpdate(id, firstName, lastName) as (values (?, ?, ?)) update customer, customerForUpdate set customer.first_name = customerForUpdate.firstName, customer.last_name = customerForUpdate.lastName where customer.id = customerForUpdate.id`)
    ...
    result = 1
    expectedResult.push(result)
    expectedQuery.push(`with customerForDelete(firstName, lastName) as (values (?, ?)) delete from customer using customer, customerForDelete where customer.first_name = customerForDelete.firstName and customer.last_name = customerForDelete.lastName`)
    ...
    expectedResult.push([])
    expectedQuery.push(`select id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like concat(lower(?), '%') or lower(last_name) like concat(lower(?), '%') group by id order by firstName, lastName limit ? offset ?`)
    expectedParams.push(`["Smi","Smi",10,20]`)
    expectedType.push(`selectManyRows`)
    expectedResult.push(0)
    expectedQuery.push(`with result_for_count as (select id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like concat(lower(?), '%') or lower(last_name) like concat(lower(?), '%') group by id) select count(*) from result_for_count`)
    expectedParams.push(`["Smi","Smi"]`)
    expectedType.push(`selectOneColumnOneRow`)
    ...
    expectedType.push(`selectManyRows`)
    expectedResult.push(0)
    expectedQuery.push(`with result_for_count as (select distinct id as id, first_name as firstName, last_name as lastName from customer where lower(first_name) like concat(lower(?), '%') or lower(last_name) like concat(lower(?), '%')) select count(*) from result_for_count`)
    expectedParams.push(`["Smi","Smi"]`)
    expectedType.push(`selectOneColumnOneRow`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name from company where lower(name) like concat('%', lower(?), '%') and parent_id = ?`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push("select company.id as id, company.name as name, concat(favouriteCustomer.first_name, ?, favouriteCustomer.last_name) as `favouriteCustomer.name` from company left outer join customer as favouriteCustomer on company.parent_id = favouriteCustomer.id where exists(select id as result from customer where lower(concat(first_name, ?, last_name)) like concat('%', lower(?), '%')) and company.parent_id = ?")
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select company.id as id, company.name as name from company left outer join customer as favouriteCustomer on company.parent_id = favouriteCustomer.id where favouriteCustomer.id in (select parent_id as result from company where lower(name) like concat('%', lower(?), '%')) and company.parent_id = ?`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id, name as name from company where (exists(select id as result from customer where lower(concat(first_name, ?, last_name)) like concat('%', lower(?), '%')) or exists(select id as result from customer where birthday = ?)) and parent_id = ?`)
    ...
    expectedQuery.push(`insert into customer (first_name, last_name, company_id) values (?, ?, ?) returning id as id, first_name as firstName, last_name as lastName, birthday as birthday`)
    expectedParams.push(`["John","Smith",1]`)
    expectedType.push(`insertReturningOneRow`)
    ...
    result = { firstName: 'Ron' }
    expectedResult.push(result)
    expectedQuery.push(`delete from customer where id = ? returning first_name as firstName, birthday as birthday`)
    expectedParams.push(`[1]`)
    expectedType.push(`deleteReturningOneRow`)
    ...
    result = undefined
    expectedResult.push(result)
    expectedQuery.push(`call myOwnprocedure(?)`)
    expectedParams.push(`[10]`)
    ...
    result = 20
    expectedResult.push(result)
    expectedQuery.push(`select myOwnFunction(?)`)
    expectedParams.push(`[10]`)
    ...
    result = []
    expectedResult.push(result)
    expectedQuery.push(`select id as id from customer where (first_name collate utf8mb4_bin) = ?`)
    ...
    const caseSensitiveMatch = await connection.selectFrom(tCustomer)
        .where(tCustomer.firstName.collate('utf8mb4_bin').equals('ADA'))
    ...
    expectedResult.push(result)
    expectedQuery.push(`select id as id, regexp_replace(first_name, ?, ?) as cleaned from customer where id = ?`)
