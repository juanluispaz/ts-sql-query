...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-of-a-dynamic-pick-projection', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, json_group_array(json_object('iid', issue.id, 'title', issue.title)) as issues from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('builder-projecting-flag-and-bare-aggregate-project-optionals-independently', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, project.archived_at as optArchived, json_group_array(json_object('id', issue.id, 'body', issue.body)) as issues from project inner join issue on issue.project_id = project.id where project.id = ? group by project.id, project.archived_at"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
