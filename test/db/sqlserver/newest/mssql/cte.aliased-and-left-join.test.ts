...
describe(ctx.label, () => {
    ...
    test('cte-aliased-via-as-keeps-original-with-name-in-with-clause', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = @0) select oi.id as id, oi.projectId as projectId from open_issues as oi where oi.projectId = @1 order by id"`)
    ...
    test('cte-forUseInLeftJoinAs-uses-alias-in-on-and-projection', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = @0) select project.name as projectName, oi.id as openId from project left join open_issues as oi on oi.projectId = project.id order by projectName"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
