...
describe(ctx.label, () => {
    ...
    test('null-inline-aggregated-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast(null as char) as direct, cast(null as char) as kept, cast(null as char) as dropped, cast(null as char) as ignored, cast(null as char) as keptIgnored from project where id = ?"`)
    ...
    test('null-aggregate-as-array-projection-modifiers-are-idempotent', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, cast(null as char) as kept, cast(null as char) as dropped, cast(null as char) as ignored, cast(null as char) as keptIgnored from project left join issue on issue.project_id = project.id where project.id = ? group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
