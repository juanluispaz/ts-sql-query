...
describe(ctx.label, () => {
    ...
    test('customize-select-plain-inline-value-renders-query-hooks-but-drops-with-query-hooks', async () => {
        ...
        const arrResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", (/* head */  select json_arrayagg(json_object('iid' value id)) from issue where project_id = project.id  /* tail */) as "issues" from project where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
