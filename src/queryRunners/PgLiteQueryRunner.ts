...
function getPgErrorReason(error: PgLiteDatabaseError): TsSqlErrorReason {
...
function getConstraintViolation(error: PgLiteDatabaseError, constraintType?: 'unique' | 'not null' | 'foreign key' | 'check' | 'exclusion'): TsSqlErrorReason {
    return {
...
function getInvalidValueForColumn(error: PgLiteDatabaseError, errorType?: 'out of range' | 'too long' | 'invalid value'): TsSqlErrorReason {
    return {
...
function getObjectNotFound(error: PgLiteDatabaseError, objectType?: 'schema' | 'table' | 'table or view' | 'column' | 'routine' | 'database'): TsSqlErrorReason {
    return {
...
function getObjectAlreadyExists(error: PgLiteDatabaseError, objectType?: 'schema' | 'table' | 'table or view' | 'column' | 'routine' | 'database'): TsSqlErrorReason {
    return {
...
function getObjectName(error: PgLiteDatabaseError): string | undefined {
    return error.table
        || error.column
        || error.constraint
...
function get57014TimeoutType(error: PgLiteDatabaseError): 'statement' | 'cancelled' {
