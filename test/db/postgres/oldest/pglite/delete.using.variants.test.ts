...
describe(ctx.label, () => {
    ...
    test('delete-using-with-extra-filter-on-using', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project, organization where issue.project_id = project.id and project.organization_id = organization.id and organization.id = $1"`)
    ...
    test('delete-using-multiple-source-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project, organization where issue.project_id = project.id and project.organization_id = organization.id and issue.priority = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    test('delete-using-with-returning-none-or-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = $1 returning issue.id as id, issue.title as title"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    test('delete-using-returning-nullable-projected-optional', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = $1 returning issue.id as id, issue.body as body"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
    ...
    test('delete-using-returning-one-column-auxiliary-using-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project where issue.project_id = project.id and issue.id = $1 returning project.slug as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
