...
describe(ctx.label, () => {
    ...
    test('collation-set: order-by-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate utf8mb4_general_ci"`)
    ...
    test('collation-set: order-by-asc-nulls-first-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate utf8mb4_general_ci asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('collation-set: order-by-desc-insensitive', async () => {
        ...
        await collated.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, full_name as fullName from app_user order by app_user.full_name collate utf8mb4_general_ci desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('collation-set: compound-order-by-insensitive', async () => {
        ...
        const result = await collated.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from project union select title as label from issue order by label collate utf8mb4_general_ci asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
