...
describe(ctx.label, () => {
    ...
    test('custom-brand-survives-nullable-and-optionality-modifiers', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(ifnull(signing_key, uuid_to_bin(?))) as \`key\`, released_on as ro from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
