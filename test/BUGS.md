...
## `UpdatableOnInsertConflictRowShapedAs` rejects value sources (defined as the Values form)

**Where**: `src/extras/types.ts:56-57`. The shaped on-conflict **Row** utility type is defined
as `MakeTypeVisible<OnConflictUpdateValues<TABLE, ResolveShape<TABLE, SHAPE>>>` — the
literal-only **Values** form, identical to `UpdatableOnInsertConflictValuesShapedAs` (58-59). A
copy-paste slip: every other `*RowShapedAs` and the non-shaped sibling delegate to a
value-source-accepting `*SetsContent` type — `UpdatableOnInsertConflictRow` (47) →
`OnConflictUpdateSetsContent`; `InsertableRowShapedAs` (52) → `MandatoryInsertSetsContent`;
`UpdatableRowShapedAs` (67) → `UpdateSetsContent`. Correct form:
`MakeTypeVisible<OnConflictUpdateSetsContent<TABLE, AllowsNoTableOrViewRequired<TABLE[typeof
source]>, ResolveShape<TABLE, SHAPE>>>`.

**Reproduction**: on a table + a rename shape, a value source under a shaped key is rejected:
```ts
const myShape = { newEmail: 'email', newFullName: 'fullName' } as const
// ERRORS (should compile — the Row variant must accept value sources):
const x: UpdatableOnInsertConflictRowShapedAs<typeof tAppUser, typeof myShape> = { newFullName: tAppUser.fullName }
// COMPILE (controls): the non-shaped Row + the sibling *RowShapedAs variants accept a value source:
const nonShaped: UpdatableOnInsertConflictRow<typeof tAppUser> = { email: tAppUser.email }
const insShaped: InsertableRowShapedAs<typeof tAppUser, typeof myShape> = { newEmail: tAppUser.email, newFullName: tAppUser.fullName }
```
`docs/advanced/utility-types.md` documents the Row variant as accepting "valid SQL objects that
you can use in the `onConflictDoUpdateSet` sentence, where `UpdatableOnInsertConflictValues` does
not" — the impl contradicts the docs.

**Current workaround in the suite**: `docs.advanced.utility-types.test.ts`
(`docs-extra:utility-types/updatable-on-insert-conflict-row-shaped-as-equals-values`) asserts
`Exact<ConflictRowShaped, ConflictValuesShaped>` — this equality holds ONLY because of the bug
(the "test whose assertType contradicts documented intent = documents a bug" fingerprint). When
fixed, replace it with a value-source probe under a shaped key + a `keyof`-pin against the Values
sibling (mirroring the sibling `insertable-row-shaped-as` / `updatable-row-shaped-as` tests at
lines 231/265); mark it `// TODO[BUG]` until then.

## Nested projection object anchored by a const drops at runtime when its only table-bound leaf merges two left joins

**Where**: the projection rule selection disagrees between the TYPE
(`AllFromSameLeftJoinWithOriginallyRequired`, `src/complexProjections/projectionRules.ts:67`) and
the RUNTIME rule-2 drop-gate (`src/queryBuilders/AbstractQueryBuilder.ts:303` —
`if (alwaysSameRequiredTablesSize && onlyOuterJoin && !originallyRequiredHaveValue)`, fed by the
required-tables size accounting at 244-267). When a nested object's only table-bound leaf **merges
two different left joins via an operator** (a single leaf registering a size-2 required-tables
set), `alwaysSameRequiredTablesSize` stays `true`, so the runtime applies rule-2 (drop the object
on a partial miss) even though a no-table **const** member is present and the TYPE computes rule-3
(object required, anchored by the const).
...
const rows = await conn.selectFrom(tIssue)
    ...
    .select({ iid: tIssue.id, obj: { combined: tProjLeft.id.add(tAssigneeLeft.id), tag: conn.const('rel', 'string') } })
    .executeSelectMany()
// TYPE:    rows[number] is { iid: number; obj: { combined?: number; tag: string } }  — obj is a REQUIRED key.
// RUNTIME: a partial-miss row (obj.combined = null, obj.tag = 'rel') DROPS the whole object → 'obj' in row === false.
//          A both-hit row (obj.combined = 5) keeps obj. So `row.obj.tag` (typed string, always present) is undefined at runtime.
```
Probe-confirmed on the mock (client-side projection): partial-miss `'obj' in row === false`;
both-hit `obj` present with `combined: 5`. The NO-const variant
(`obj: { combined: tProjLeft.id.add(tAssigneeLeft.id) }`) is SOUND — it types `obj?: {
combined?: number }` and the runtime drop agrees; only the const-anchor case diverges.

**Both readings** (maintainer picks): (a) the TYPE is too loose — `obj` should be optional
(`obj?`), making the runtime drop sound; (b) the RUNTIME over-drops — it should keep the object
carrying the const, matching the covered *separate-leaves* precedent
`two-different-left-joins-plus-const-promotes-to-rule-3`
(`select.complex-projection.mixed-rules.test.ts:804`, where two joins as separate leaves + a const
yield rule-3 and the object survives). The precedent leans (b): collapsing the two joins into one
leaf is what makes the runtime misfire.

**Current workaround in the suite**: no test exercises a two-left-join operator merge inside a
nested object (`grep -rhnE "Left\.[a-z]+\.(add|substract|...)\([a-z]*Left\." test/db` → 0 hits).
The regression test (assert the object survives the partial miss carrying the const, `combined`
absent — reading (b)) should carry `// TODO[BUG]` until the divergence is resolved.
