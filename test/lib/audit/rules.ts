...
export const RULE_SEVERITY: Record<string, Severity> = {
    ...
    'commented-test-reason':  'error',   // a commented-out test with no TODO[BUG]/TODO[LIMITATION]/NOT-SUPPORTED/NOT-APPLICABLE reason
    ...
    'skipped-test-reason':    'error',   // `test.skip` / `test.todo` with no TODO[BUG]/TODO[LIMITATION]/NOT-SUPPORTED/NOT-APPLICABLE reason — the .skip twin of commented-test-reason
    'skip-real-db':           'error',   // `test.skipIf(realDbEnabled)` — a mock-only evasion at the registration level
    'misplaced-marker':       'error',   // a TODO[BUG]/TODO[LIMITATION]/NOT-SUPPORTED/NOT-APPLICABLE marker not at a test (file scope, helper, floating)
...
export const RULE_HINT: Record<string, string> = {
    ...
    'commented-test-reason':
        'A commented-out test must state why it is off with one of the four first-class markers: `// TODO[BUG]: <reason>` (a defect in src/ — re-enabled here once fixed; BUGS.md), `// TODO[LIMITATION]: <reason>` (the library does not cover it yet, or the harness cannot drive it — could re-enable here; LIMITATIONS.md), `// NOT-SUPPORTED: <reason>` (the engine / this engine version / its build / the driver cannot run it — the API is callable, the runtime rejects it; permanent here, ENGINE_SUPPORT.md), or `// NOT-APPLICABLE: <reason>` (a deliberate dialect boundary — the type surface does not even expose it here; the test runs in the dialects that support it). Pick by future: a TODO means pending work in THIS repo that could re-enable it HERE; NOT-SUPPORTED and NOT-APPLICABLE are permanent. It still counts for symmetry, so do not delete it — comment it WITH a marker, or re-enable it.',
    'grouped-commented-tests':
        'This `/* … */` comment groups several commented-out tests under a single reason marker, so the individual justifications are lost (one marker "covers" them all). Split it: one commented-out test per comment block, each with its OWN `// TODO[BUG]: <reason>` / `// TODO[LIMITATION]: <reason>` / `// NOT-SUPPORTED: <reason>` / `// NOT-APPLICABLE: <reason>` marker — then `commented-test-reason` enforces a distinct reason on every one. A normal `//`-per-line commented-out test is never flagged; only a block holding two or more tests.',
    'non-public-api':
    ...
    'skipped-test-reason':
        'A `test.skip` / `it.skip` / `describe.skip` / `test.todo` disables a test — like a commented-out test (`commented-test-reason`), it must state why with one of the four markers within 3 lines above: `// TODO[BUG]: <reason>`, `// TODO[LIMITATION]: <reason>`, `// NOT-SUPPORTED: <reason>` (the engine / version / build / driver cannot run it — permanent here, not a TODO), or `// NOT-APPLICABLE: <reason>` (a permanent dialect boundary, not a TODO — the test runs in the dialects that support it). Re-enable it, or mark WITH a marker so the reason is visible.',
    'skip-real-db':
