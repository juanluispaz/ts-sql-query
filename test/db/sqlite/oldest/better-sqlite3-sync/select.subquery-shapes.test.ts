...
describe(ctx.label, () => {
    ...
    test('multi-level-cte-bubble-up-with-join', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(issuesInActive)
    ...
    test('compound-as-derived-table-via-for-use-in-query-as', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(statuses)
            .select({ s: statuses.s })
            .orderBy('s')
            .executeSelectMany())
    ...
    test('compound-as-inline-aggregated-array-value', async () => {
        ...
        const rows = sync(ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select (select json_group_array(a_1_.result) from (select status as result from issue where status = ? union select status as result from issue where status = ?) as a_1_) as statuses"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
