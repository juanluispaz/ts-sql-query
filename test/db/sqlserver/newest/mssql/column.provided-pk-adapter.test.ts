...
describe(ctx.label, () => {
    ...
    test('returning-reads-the-adapter-scaled-provided-primary-key-back-unscaled', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into invoice (invoice_no, total) output inserted.invoice_no as invoiceNo, inserted.total as total values (@0, @1)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                400,
                900,
