...
describe(ctx.label, () => {
    ...
    test('values in select-from', async () => {
        ...
        sync(ctx.conn.selectFrom(patch)
    ...
    test('values in a compound-union arm', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(patch)
            .select({ id: patch.id, name: patch.name })
            .union(
    ...
    test('values-as-intersect-arm', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(ids)
            .select({ id: ids.id })
            .intersect(
    ...
    test('values-source-group-by', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(rows)
    ...
    test('values-self-joined-to-its-own-clone-hoists-the-with-once', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(nodes)
    ...
    test('values-self-joined-via-left-join-clone-hoists-the-with-once', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (values (?, ?, ?), (?, ?, ?)) select treeNode.id as id, anc.name as ancName from treeNode left join treeNode as anc on anc.id = treeNode.parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-self-joined-with-two-aliased-clones-neither-is-the-source', async () => {
        ...
        const a = nodes.as('a')
        const b = nodes.as('b')
        const result = sync(ctx.conn.selectFrom(a)
            .innerJoin(b).on(b.id.equals(a.parentId))
            .select({ id: a.id, ancName: b.name })
            .orderBy('id')
            .executeSelectMany())

        expect(ctx.lastSql).toMatchInlineSnapshot(`"with treeNode(id, parentId, name) as (values (?, ?, ?), (?, ?, ?)) select "a".id as id, "b".name as ancName from treeNode as "a" inner join treeNode as "b" on "b".id = "a".parentId order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-in-both-arms-of-a-compound-hoists-two-distinct-withs', async () => {
        ...
        const result = sync(ctx.conn.selectFrom(idsA).select({ id: idsA.id })
            .unionAll(ctx.conn.selectFrom(idsB).select({ id: idsB.id }))
            .orderBy('id')
            .executeSelectMany())

        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values (?, ?), (?, ?)), idList(id) as (values (?), (?)) select id as id from projectPatch union all select id as id from idList order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-in-a-non-correlated-exists-hoists-the-with', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with idList(id) as (values (?)) select id as id from project where exists(select id as result from idList) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
