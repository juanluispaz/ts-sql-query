...
describe(ctx.label, () => {
    ...
    test('getFullYear', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, extract(year from created_at) as year from organization where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getMonth-getDate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, extract(month from created_at) - 1 as month, extract(day from created_at) as date from organization where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getHours-getMinutes-getSeconds', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, extract(hour from created_at) as "h", extract(minute from created_at) as "m", trunc(extract(second from created_at))::integer as "s" from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getDay-day-of-week', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, extract(dow from created_at) as dow from organization where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getTime-epoch-millis', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, round(extract(epoch from created_at) * 1000) as "t" from organization where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getMilliseconds', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, trunc(extract(millisecond from created_at))::integer % 1000 as ms from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('localDate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from work_date) as "y", extract(month from work_date) - 1 as mo, extract(day from work_date) as "d", extract(dow from work_date) as dow from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('localTime-getters-optional', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from started_at) as "h", extract(minute from started_at) as "m", trunc(extract(second from started_at))::integer as "s", trunc(extract(millisecond from started_at))::integer % 1000 as ms from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-temporal-getters-and-compare', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from released_on) as "y", extract(hour from cutoff_time) as "h", round(extract(epoch from signed_off_at) * 1000) as "t", released_on < $1 as before from project_release where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('optional-localdate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from review_date) as "y", extract(month from review_date) - 1 as mo, extract(day from review_date) as "d", extract(dow from review_date) as dow from project_review where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('required-localtime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from review_time) as "h", extract(minute from review_time) as "m", trunc(extract(second from review_time))::integer as "s", trunc(extract(millisecond from review_time))::integer % 1000 as ms from project_review where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-localtime-remaining-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(minute from cutoff_time) as "m", trunc(extract(second from cutoff_time))::integer as "s", trunc(extract(millisecond from cutoff_time))::integer % 1000 as ms from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-localdatetime-optional-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from signed_off_at) as "y", extract(month from signed_off_at) - 1 as mo, extract(day from signed_off_at) as "d", extract(dow from signed_off_at) as dow, extract(hour from signed_off_at) as "h", extract(minute from signed_off_at) as "m", trunc(extract(second from signed_off_at))::integer as "s", trunc(extract(millisecond from signed_off_at))::integer % 1000 as ms from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal-in-and-in-n', async () => {
        ...
        const inRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date in ($1, $2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal-between-and-not-between', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on between $1 and $2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        const out = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on not between $1 and $2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-01T00:00:00.000Z",
            "2024-02-28T00:00:00.000Z",
    ...
    test('view-custom-localdatetime-optional-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from signed_off_at) as "y", extract(month from signed_off_at) - 1 as mo, extract(day from signed_off_at) as "d", extract(dow from signed_off_at) as dow, extract(hour from signed_off_at) as "h", extract(minute from signed_off_at) as "m", trunc(extract(second from signed_off_at))::integer as "s", trunc(extract(millisecond from signed_off_at))::integer % 1000 as ms, round(extract(epoch from signed_off_at) * 1000) as "t" from release_overview where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-custom-temporal-getters-realize-the-undefined-inhabitant', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, extract(year from target_day) as "y", extract(month from target_day) - 1 as mo, extract(day from target_day) as "d", extract(dow from target_day) as dow, extract(hour from cutoff) as "h", extract(minute from cutoff) as mi, trunc(extract(second from cutoff))::integer as "s", trunc(extract(millisecond from cutoff))::integer % 1000 as ms from release_draft order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            id: number
