...
describe(ctx.label, () => {
    ...
    // ── const/optionalConst(value, 'localTime'|'localDateTime', adapter) ──
    // The temporal marshaller family through the trailing adapter — a distinct
    // `transformValueFromDB` branch (Date, not number/uuid/boolean). The const
    // Date is sent verbatim; the value read back is shifted one hour by the
    // adapter. localTime / localDateTime round-trip a const cleanly on every
    // engine, so the shifted Date is asserted directly.
