...
describe(ctx.label, () => {
    ...
    test('cte-behind-a-required-optional-join-is-emitted-in-with', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select id as id, project_id as projectId from issue where status = $1) select project.id as id, open_issues.id as "openId" from project left join open_issues on open_issues.projectId = project.id order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
