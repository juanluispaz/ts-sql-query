...
describe(ctx.label, () => {
    ...
    test('compound-wrap-execute-select-page', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with result_for_count("label") as (select name as "label" from project union select title as "label" from issue) select count(*) from result_for_count"`)
    ...
    test('compound-arm-parenthesized-by-its-own-customize-before-order-by-items', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('label')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as "label" from project order by project.name asc offset 0 rows) union select title as "label" from issue order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ label: string }>>>()
        expect(result).toEqual(expected)
    })
    ...
    test('compound-execute-select-one-multiple-rows-throws-more-than-one-row', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as "label" from project where id = :0 union select name as "label" from project where id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-unionAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(1)).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as "iid" from issue where id = :0 union all select id as "iid" from issue where id = :1) o_1_ order by "iid", :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-intersect-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as "iid" from issue where id in (:0, :1) intersect select id as "iid" from issue where id = :2) o_1_ order by "iid", :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-intersectAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as "iid" from issue where id in (:0, :1) intersect all select id as "iid" from issue where id = :2) o_1_ order by "iid", :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-except-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as "iid" from issue where id in (:0, :1) minus select id as "iid" from issue where id = :2) o_1_ order by "iid", :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-exceptAll-order-by-raw-fragment-embedding-value-source', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue).where(tIssue.id.in([1, 2])).select({ iid: tIssue.id })
            ...
            .orderBy(ctx.conn.rawFragment`${ctx.conn.const(1, 'int')}`)
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select id as "iid" from issue where id in (:0, :1) minus all select id as "iid" from issue where id = :2) o_1_ order by "iid", :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('compound-order-by-raw-fragment-embedding-value-source-in-expression', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as "label" from project union select title as "label" from issue) o_1_ order by "label", coalesce(:0, 0)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-order-by-value-source-collate-secondary', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select * from (select name as "label" from project union select title as "label" from issue) o_1_ order by "label", :0 collate BINARY"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
          ]
    ...
    test('compound-insensitive-order-by-on-non-string-column-drops-modifier', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "ref" from project union select id as "ref" from issue order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('compound-order-by-multiple-aliases-inline', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('iid')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "pid", id as "iid" from issue where project_id = :0 union select project_id as "pid", id as "iid" from issue where project_id = :1 order by 1, 2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
