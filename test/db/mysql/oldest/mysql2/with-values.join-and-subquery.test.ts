...
describe(ctx.label, () => {
    ...
    */
    ...
    test('view-for-use-in-query-as-hoists-source-above-derived-table', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sub.projectName as projectName, sub.version as version from (select project_name as projectName, version as version from release_overview where project_id = ?) as sub order by version"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
