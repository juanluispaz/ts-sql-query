    ...
    */
    ...
    test('docs-extra:insert/values-for-insert-in-update', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan) values (?, ?) on duplicate key update name = concat(name, ?, value(name)) returning id as id, name as name, plan as plan"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                "pro",
