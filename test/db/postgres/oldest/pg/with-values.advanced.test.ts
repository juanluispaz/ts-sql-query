...
describe(ctx.label, () => {
    ...
    test('values-aliased-via-as-keeps-original-with-name', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select pp.id as id, pp.name as name from projectPatch as pp order by id"`)
    ...
    test('values-for-use-in-left-join-as-emits-left-join', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectPatch(id, name) as (values ($1::int4, $2::text)) select project.id as pid, pp.name as "newName" from project left join projectPatch as pp on pp.id = project.id where project.id <= $3 order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "one",
    ...
    test('values-with-custom-typed-columns-emits-customint-customdouble-casts', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values ($1::int4, $2::float8), ($3::int4, $4)) select issueId as "issueId", amount as amount from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-virtual-column-from-fragment-with-custom-type-emits-inline-fragment', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values ($1::int4, $2::float8), ($3::int4, $4)) select issueId as "issueId", 'open' as state from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-optional-virtual-column-from-fragment-with-custom-type-emits-inline-fragment', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values ($1::int4, $2::float8), ($3::int4, $4)) select issueId as "issueId", null as "billingRef" from issueBilling order by "issueId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-base-type-optional-virtual-column-projects-optional-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with batchRow(id, label) as (values ($1::int4, $2::text), ($3::int4, $4::text)) select id as id, label as label, null as rank, null as note from batchRow order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-branded-column-through-for-use-in-left-join-as-keeps-the-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values ($1::int4, $2::float8), ($3::int4, $4)), issueBillingJoin(issueId, amount) as (values ($5::int4, $6::int4)) select issueBilling.issueId as base, ibj.issueId as joined from issueBilling left join issueBillingJoin as ibj on ibj.issueId = issueBilling.issueId order by base"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-base-type-required-virtual-column-projects-required-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with reqBatch(id) as (values ($1::int4), ($2::int4)) select id as id, 7 as seq, 'BATCH' as tag from reqBatch order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('values-virtual-column-referencing-a-sibling-column-inlines-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(nums)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vdouble(id) as (values ($1::int4), ($2::int4)) select id as id, id * 2 as doubled from vdouble order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
