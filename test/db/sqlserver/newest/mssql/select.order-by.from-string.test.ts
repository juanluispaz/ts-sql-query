...
describe(ctx.label, () => {
    ...
    test('order-by-from-string-asc-nulls-last-and-desc-nulls-first', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority as priority from issue order by iif(issue.priority is null, 1, 0), priority asc, iif(issue.id is not null, 1, 0), id desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; priority: number }>>>()
    })
