...
describe(ctx.label, () => {
    ...
    test('select-from-view-aliased-with-as', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select po.id as "id", po.name as "name" from project_overview po where po.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-for-use-in-left-join-as-no-match-yields-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "organization".name as "orgName", po.name as "projectName" from "organization" left join project_overview po on po.id = :0 where "organization".id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            999,
            1,
