...
describe(ctx.label, () => {
    ...
    test('inner-join-with-where', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", project.name as "projectName" from issue inner join project on project.id = issue.project_id where project.organization_id = :0 and issue.project_id = :1 order by "id""`)
    ...
    test('join-with-alias', async () => {
        ...
        const result = await ctx.conn.selectFrom(parent)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select parent.id as "id", child.title as "child" from issue parent inner join issue child on child.parent_id = parent.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{
            id:    number
            child: string
    ...
    test('left-join-with-dynamic-on-builds-join-condition', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", app_user.full_name as "assigneeName" from issue left join app_user on app_user.id = issue.assignee_id where issue.id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('left-join-with-dynamic-on-or-accumulates-disjunction', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", app_user.full_name as "assigneeName" from issue left join app_user on app_user.id = issue.assignee_id or app_user.id = :0 where issue.id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            1,
    ...
    test('select-cross-join-comma-from-three-tables', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProject)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.name as "projectName", "organization".name as "orgName", issue.title as "issueTitle" from project, "organization", issue where project.organization_id = "organization".id and issue.project_id = project.id and project.id = :0 and issue.id = :1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('plain-join-emits-bare-join-keyword', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id", project.name as "projectName" from issue join project on project.id = issue.project_id where project.organization_id = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-join-kept-when-only-having-references-it', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.project_id as "projectId", count(issue.id) as "total" from issue left join app_user on app_user.id = issue.assignee_id group by issue.project_id having count(app_user.id) > :0 order by "projectId""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('optional-join-discovery-skips-a-join-with-no-on-condition', () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"select issue.id as "id", app_user.full_name as "assigneeName" from issue join project left join app_user on app_user.id = issue.assignee_id where issue.id = :0"`)
    ...
    test('optional-join-kept-when-only-a-customize-query-fragment-references-it', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as "id" from issue left join app_user on app_user.id = issue.assignee_id where issue.id in (:0, :1) order by "id", app_user.full_name asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
