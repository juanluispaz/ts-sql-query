...
describe(ctx.label, () => {
    ...
    test('boolean-and-method-combines-two-predicates', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status = @0 and priority >= @1 order by id"`)
    ...
    test('boolean-and-of-or-parenthesises-inner-or', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (status = @0 or status = @1) and priority < @2 order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "in_progress",
    ...
    test('boolean-or-of-and-parenthesises-inner-and', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where status = @0 or (priority > @1 and priority < @2) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            1,
    ...
    test('boolean-chain-negate-wraps-in-not', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (status = @0 or status = @1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "in_progress",
    ...
    test('if-value-and-boolean-literal-collapses-to-boolean', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority = @0 and (@1 = 1) then 1 else 0 end as bit) as flag from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            true,
    ...
    test('is-null-on-a-comparison-with-an-optional-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (case when assignee_id = @0 then 0 when not (assignee_id = @1) then 0 else 1 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
    ...
    test('is-not-null-on-a-comparison-with-an-optional-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where (case when assignee_id = @0 then 1 when not (assignee_id = @1) then 1 else 0 end = 1) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            2,
