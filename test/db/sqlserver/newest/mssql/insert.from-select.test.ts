...
describe(ctx.label, () => {
    ...
    test('insert-from-select-returning-last-inserted-id-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) output inserted.id select @0 as projectId, number as number, title as title, @1 as status, priority as priority from issue where project_id = @2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                "draft",
