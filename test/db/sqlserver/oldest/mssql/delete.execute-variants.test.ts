...
describe(ctx.label, () => {
    ...
    test('execute-delete-many-with-min-max-in-range-passes', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.id as id, deleted.status as status where project_id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('execute-delete-one-column-many-non-empty-result', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue output deleted.status as [result] where project_id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('delete-query-called-twice-returns-the-cached-sql-without-re-binding-params', () => {
        ...
        expect(first).toMatchInlineSnapshot(`"delete from issue where id in (@0, @1)"`)
    ...
    test('delete-params-before-query-builds-on-demand', () => {
        ...
        expect(sql).toMatchInlineSnapshot(`"delete from issue where id = @0"`)
