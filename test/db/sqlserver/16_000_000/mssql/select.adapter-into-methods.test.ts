...
describe(ctx.label, () => {
    ...
    test('adapter-int-column-into-numeric-add-scales-operand-and-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, score + @0 as bumped from project_review where id = @1"`)
    ...
    test('adapter-provided-pk-column-into-numeric-add-scales-operand-and-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select invoice_no as invoiceNo, invoice_no + @0 as [next] from invoice where invoice_no = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
            100,
    ...
    test('adapter-string-column-into-to-lower-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, lower(reviewer_code) as code from project_review where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-starts-with-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as projectId, project_review.reviewer_code as reviewer from project inner join project_review on project_review.project_id = project.id and project_review.reviewer_code like (@0 + '%') where project.id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "R-",
            1,
    ...
    test('adapter-string-column-into-upper-and-trim-methods-keep-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(reviewer_code) as up, trim(reviewer_code) as tm, ltrim(reviewer_code) as tl, rtrim(reviewer_code) as tr from project_review where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-reverse-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reverse(reviewer_code) as rev from project_review where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-substring-and-substr-keep-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(reviewer_code, @0, @1) as sg, substring(reviewer_code, @2, @3) as sb from project_review where id = @4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('adapter-string-column-into-concat-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code + @0 as cc from project_review where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            1,
    ...
    test('custom-boolean-string-column-as-equals-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from project where id = @0 and cast(case when published = 't' then 1 else 0 end as bit) = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            true,
    ...
    test('custom-boolean-numeric-column-as-is-if-value-receiver', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, issue_id as issueId from issue_worklog where id = @0 and cast(case when invoiced = 1 then 1 else 0 end as bit) = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            true,
    ...
    test('custom-boolean-numeric-column-equals-predicate-in-join-on', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as issueId, issue_worklog.id as worklogId from issue inner join issue_worklog on issue_worklog.issue_id = issue.id and cast(case when issue_worklog.invoiced = 1 then 1 else 0 end as bit) = @0 where issue.id = @1 order by worklogId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            true,
            1,
    ...
    test('custom-boolean-numeric-column-as-is-if-value-receiver-elided', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, issue_id as issueId from issue_worklog where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-substring-to-end-keeps-result-leaf-bracketed', async () => {
        ...
        const row = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(reviewer_code, @0, 2147483647) as sub from project_review where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            1,
    ...
    test('adapter-string-column-into-starts-with-predicate-in-where', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = @0 and reviewer_code like (@1 + '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "R-",
    ...
    test('adapter-string-column-into-not-contains-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = @0 and reviewer_code not like ('%' + @1 + '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "ZZ",
    ...
    test('adapter-string-column-into-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = @0 and lower(reviewer_code) like lower(@1)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "r%",
    ...
    test('adapter-string-column-into-not-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = @0 and lower(reviewer_code) not like lower(@1)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "zz%",
    ...
    // String predicates on an adapter-bearing receiver (reviewer_code): the *IfValue
    // fire/elide twins and the affix-escape variants, all reading through the column's
    // bracket adapter.
    ...
    test('equals-insensitive-if-value-fire', async () => {
    ...
    test('equals-insensitive-if-value-elide', async () => {
    ...
    test('not-equals-insensitive-if-value-fire', async () => {
    ...
    test('not-equals-insensitive-if-value-elide', async () => {
    ...
    test('like-if-value-elide', async () => {
    ...
    test('not-like-if-value-fire', async () => {
    ...
    test('not-like-if-value-elide', async () => {
    ...
    test('like-insensitive-if-value-fire', async () => {
    ...
    test('like-insensitive-if-value-elide', async () => {
    ...
    test('not-like-insensitive-if-value-fire', async () => {
    ...
    test('not-like-insensitive-if-value-elide', async () => {
    ...
    test('starts-with-escape-percent', async () => {
    ...
    test('starts-with-escape-underscore', async () => {
    ...
    test('ends-with-escape-percent', async () => {
    ...
    test('ends-with-escape-underscore', async () => {
    ...
    test('contains-escape-percent', async () => {
    ...
    test('contains-escape-underscore', async () => {
