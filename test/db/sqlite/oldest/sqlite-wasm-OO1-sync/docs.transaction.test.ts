...
describe(ctx.label, () => {
    ...
    test('docs:transaction/commit-on-success', async () => {
        ...
        await ctx.withReseed(async () => {
            ...
            const id = await connection.transaction(async () => {
                return sync(connection.insertInto(tOrganization)
            ...
            expect(ctx.lastNoTransactionSql).toMatchInlineSnapshot(`"insert into organization (name, "plan") values (?, ?)"`)
    ...
    test('docs:transaction/low-level-begin-commit', async () => {
        ...
        await ctx.withReseed(async () => {
            ...
            try {
                sync(connection.insertInto(tOrganization)
