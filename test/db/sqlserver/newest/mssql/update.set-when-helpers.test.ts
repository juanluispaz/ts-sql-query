...
describe(ctx.label, () => {
    ...
    test('set-when-false-is-noop-true-stages-columns', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update issue set status = @0 where id = @1"`)
            expect(trueSql).toMatchInlineSnapshot(`"update issue set status = @0, title = @1 where id = @2"`)
    ...
    test('set-if-not-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update issue set status = @0 where id = @1"`)
            expect(trueSql).toMatchInlineSnapshot(`"update issue set status = @0, body = @1 where id = @2"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
    ...
    test('ignore-if-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update issue set status = @0, body = @1 where id = @2"`)
            expect(trueSql).toMatchInlineSnapshot(`"update issue set status = @0 where id = @1"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
    ...
    test('ignore-if-has-value-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"update issue set status = @0, body = @1 where id = @2"`)
            expect(trueSql).toMatchInlineSnapshot(`"update issue set body = @0 where id = @1"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
