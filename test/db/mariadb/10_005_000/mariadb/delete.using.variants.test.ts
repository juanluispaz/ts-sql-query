    ...
    */
    ...
    test('delete-using-values-source', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"with projectIds(id) as (values (?)) delete from issue using issue, projectIds where issue.project_id = projectIds.id and issue.id = ?"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                99999,
