...
describe(ctx.label, () => {
    ...
    test('insert-nullable-custom-boolean-true-emits-case-when-remap', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue_worklog (issue_id, work_date, activity, approved) values (?, ?, ?, case ? when true then 'A' when false then 'R' else null end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
