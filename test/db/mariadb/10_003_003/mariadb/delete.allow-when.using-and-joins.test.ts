...
describe(ctx.label, () => {
    ...
    test('delete-using-join-all-gates-open-walks-to-end-and-runs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* gated issue.id */  delete /* gated issue.project_id */ from issue using issue, project inner join organization on organization.id = project.organization_id where issue.project_id = project.id and issue.id = ?  /* gated project.slug */"`)
    ...
    test('delete-using-join-all-gates-open-with-allowed-returning-walks-to-end', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using issue, project inner join organization on organization.id = project.organization_id where issue.project_id = project.id and issue.id = ? returning issue.id as id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
