...
describe(ctx.label, () => {
    ...
    test('set-minmax-poison-case-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .where(tIssue.id.equals(1))
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set assignee_id = least(assignee_id, parent_id) where id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('collation-forks-in-a-mutation-set-and-where', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update app_user set full_name = replace(full_name collate BINARY_CI, :0 collate BINARY_CI, :1) collate USING_NLS_COMP where (email collate BINARY) = :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Love",
                "X",
    ...
    test('concat-with-optional-operand-in-update-set', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const affected = await ctx.conn.update(tIssue)
                ...
                .executeUpdate()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set "body" = case when "body" is null then null else title || "body" end where id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
