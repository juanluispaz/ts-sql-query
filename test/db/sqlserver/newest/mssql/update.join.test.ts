...
describe(ctx.label, () => {
    ...
    test('update-from-table-then-inner-join-returning-nested', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name output inserted.id as id, inserted.name as [meta.name], inserted.slug as [meta.slug] from issue inner join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-from-table-then-left-join-returning-nullable-joined-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = issue.title output inserted.id as id, app_user.full_name as assignee from issue left join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                3,
              ]
    ...
    test('update-from-table-then-inner-join-on-and-compound-join-condition', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id and app_user.full_name = @0 where project.id = issue.project_id and issue.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Ada Lovelace",
                1,
    ...
    test('update-from-table-then-inner-join-dynamic-on-or', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id or app_user.full_name = @0 where project.id = issue.project_id and issue.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "never-matches",
                1,
    ...
    */
    ...
    test('update-allowing-no-where-from-table-then-inner-join-touches-all-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-from-table-then-join-on-from-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('update-from-left-join-returning-rule2-object-miss-drops-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = issue.title output inserted.id as pid, app_user.full_name as [obj.x], @0 as [obj.k] from issue left join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = @1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                3,
    ...
    test('update-from-table-then-two-inner-joins-on-from-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id inner join project_review on project_review.project_id = issue.project_id where project.id = issue.project_id and issue.id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
