...
describe(ctx.label, () => {
    ...
    test('ignore-if-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, "body", status, priority) values (:0, :1, :2, :3, :4, :5)"`)
            expect(trueSql).toMatchInlineSnapshot(`"insert into issue (project_id, "number", title, status, priority) values (:0, :1, :2, :3, :4)"`)
            expect(trueSql).not.toBe(falseSql)
        })
    })
    ...
    test('set-for-all-if-not-set-when-dispatches-on-true', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(falseSql).toMatchInlineSnapshot(`"begin insert into issue (project_id, "number", title, "body", status, priority) values (:0, :1, :2, :3, :4, :5); insert into issue (project_id, "number", title, status, priority) values (:6, :7, :8, :9, :10); end;"`)
            expect(trueSql).toMatchInlineSnapshot(`"begin insert into issue (project_id, "number", title, "body", status, priority) values (:0, :1, :2, :3, :4, :5); insert into issue (project_id, "number", title, status, priority, "body") values (:6, :7, :8, :9, :10, :11); end;"`)
        })
    })
