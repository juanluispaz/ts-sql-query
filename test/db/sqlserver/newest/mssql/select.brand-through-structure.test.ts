...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-of-one-column-preserves-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, json_arrayagg(version null on null) as versions, json_arrayagg(channel null on null) as channels from project_release where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-preserves-enum-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, json_arrayagg(activity null on null) as activities from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
