...
describe(ctx.label, () => {
    ...
    test('inline-aggregate-of-distinct-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select distinct status as [result] from issue where project_id = project.id) as a_1_) as statuses from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-order-by-asc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select name as [result] from project where organization_id = organization.id order by [result] asc offset 0 rows) as a_1_) as projectNames from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select name as [result] from project where organization_id = organization.id order by [result] desc offset 0 rows) as a_1_) as projectNames from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select name as [result] from project where organization_id = organization.id order by lower(name) offset 0 rows) as a_1_) as projectNames from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('inline-aggregate-order-by-desc-insensitive', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.[result] null on null) from (select name as [result] from project where organization_id = organization.id order by lower(name) desc offset 0 rows) as a_1_) as projectNames from organization where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
