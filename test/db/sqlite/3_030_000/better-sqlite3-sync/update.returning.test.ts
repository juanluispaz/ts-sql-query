...
import { tIssue, tOrganization, tProject, tProjectRelease, tProjectReview, tReleaseDraft, type ReleaseStage } from '../../domain/connection.js'
...
describe(ctx.label, () => {
    ...
    test('update-returning-one-row', async () => {
        ...
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.update(tIssue)
    ...
    test('update-returning-one-column', async () => {
        ...
        await ctx.withRollback(async () => {
            const newName = sync(ctx.conn.update(tOrganization)
    ...
    test('update-returning-many', async () => {
        ...
        await ctx.withRollback(async () => {
            const rows = sync(ctx.conn.update(tIssue)
                .set({ priority: 9 })
    ...
    test('update-returning-projecting-optional-values-as-nullable', async () => {
        ...
        await ctx.withRollback(async () => {
            const updated = sync(ctx.conn.update(tIssue)
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set status = ? where id = ? returning id as id, body as body"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "in_progress",
                3,
    ...
    test('update-returning-one-column-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            const bumped = sync(ctx.conn.update(tIssue)
                .set({ priority: 5 })
                .where(tIssue.id.equals(1))
                .returningOneColumn(tIssue.priority.add(100))
                .executeUpdateOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = ? where id = ? returning priority + ? as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                1,
    ...
    test('update-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = ? where id = ? returning id as id, title || ? as tag"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched",
                1,
    ...
    test('update-returning-nested-object-projection', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = ?, priority = ? where id = ? returning id as id, title as "meta.title", priority as "meta.priority""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched",
                5,
    ...
    test('update-release-draft-returning-one-column-optional-branded', async () => {
        ...
        await ctx.withRollback(async () => {
            const stage = sync(ctx.conn.update(tReleaseDraft)
                .set({ title: 'Alpha cut v2' })
                .where(tReleaseDraft.id.equals(1))
                .returningOneColumn(tReleaseDraft.stage)
                .executeUpdateOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = ? where id = ? returning stage as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Alpha cut v2",
                1,
    ...
    test('update-release-draft-returning-object-optional-branded-default', async () => {
        ...
        ctx.mockNext(expected)
        await ctx.withRollback(async () => {
            const row = sync(ctx.conn.update(tReleaseDraft)
                .set({ title: 'Alpha cut v2' })
                .where(tReleaseDraft.id.equals(1))
                .returning({ id: tReleaseDraft.id, stage: tReleaseDraft.stage })
                .executeUpdateOne())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = ? where id = ? returning id as id, stage as stage"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Alpha cut v2",
                1,
    ...
    test('update-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = ? where id = ? returning id as id, title as "meta.sibling", stage as "meta.gate""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Nightly build v2",
                2,
    ...
    test('update-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const bodies = sync(ctx.conn.update(tIssue)
                .set({ priority: 5 })
                .where(tIssue.projectId.equals(1))
                .returningOneColumn(tIssue.body)
                .executeUpdateMany())

            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = ? where project_id = ? returning body as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                1,
    ...
    test('update-returning-nested-object-optional-leaf-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const present = sync(ctx.conn.update(tProject)
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = ? where id = ? returning id as id, archived_at as "meta.archivedAt""`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Legacy app v2",
                4,
            ...
            const absent = sync(ctx.conn.update(tProject)
    ...
    test('update-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = sync(ctx.conn.update(tProjectReview)
                ...
                .executeUpdateOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_date = ?, review_time = ? where id = ? returning review_date as reviewDate, review_time as reviewTime"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "2026-09-09",
                "07:05:00",
    ...
    test('update-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = sync(ctx.conn.update(tProjectRelease)
                ...
                .executeUpdateOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_release set released_on = ?, cutoff_time = ?, published_at = ? where id = ? returning released_on as releasedOn, cutoff_time as cutoffTime, published_at as publishedAt"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "2026-04-04",
                "22:10:00",
    ...
    test('update-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = sync(ctx.conn.update(tProjectReview)
                .set({ reviewTime })
                .where(tProjectReview.id.equals(1))
                .returningOneColumn(tProjectReview.reviewTime)
                .executeUpdateOne())
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_time = ? where id = ? returning review_time as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "13:45:00",
                1,
