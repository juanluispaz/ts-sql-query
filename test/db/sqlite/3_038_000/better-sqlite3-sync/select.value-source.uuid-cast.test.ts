...
describe(ctx.label, () => {
    ...
    test('uuid-asString-on-const', async () => {
        ...
        const result = sync(connection.selectFromNoTable()
    ...
    test('uuid-asString-on-column', async () => {
        ctx.mockNext(REF1)
        const ref = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('uuid-asString-concat', async () => {
        ...
        const ref = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select external_ref || external_ref as result from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-asString-stringConcat', async () => {
        ...
        const refs = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.projectId.equals(1))
            .selectOneColumn(ctx.conn.stringConcat(tIssue.externalRef.asString(), ','))
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select group_concat(external_ref, ?) as result from issue where project_id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            ",",
            1,
    ...
    // NOT-APPLICABLE: SQLite has no string_agg DISTINCT with separator, so
    // the two-argument stringConcatDistinct is not typed on SqliteConnection.
    /*
    ...
    */
    ...
    test('uuid-asString-startsWithInsensitive', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where lower(external_ref) like lower(? || '%') escape '\\' order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0A8F",
          ]
    ...
    test('uuid-asString-notStartsWithInsensitive', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where lower(external_ref) not like lower(? || '%') escape '\\' order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ffff",
          ]
    ...
    test('uuid-asString-through-cte', async () => {
        ...
        const ref = sync(ctx.conn.selectFrom(inner)
            .selectOneColumn(inner.ref.concat('-x'))
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with w_uuid as (select external_ref as ref from issue where id = ?) select ref || ? as result from w_uuid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "-x",
    ...
    test('customuuid-asString-on-optional-column', async () => {
        ...
        const key = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('uuid-receiver-modifier-trio', async () => {
        ...
        const row = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, external_ref as req, null as own, null as ign from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-uuid-receiver-modifier-trio', async () => {
        ...
        const row = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, signing_key as req, null as own, null as ign from project_release where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-asString-reverse', async () => {
        ...
        const ref = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select reverse(external_ref) as result from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('uuid-asString-trimLeft', async () => {
        ...
        const ref = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select ltrim(external_ref) as result from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-asString-trimRight', async () => {
        ...
        const ref = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select rtrim(external_ref) as result from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-asString-substr-negative-start', async () => {
        ...
        const ref = sync(ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectOne())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select substr(substr(external_ref, ?), 1, ?) as result from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            -8,
            4,
