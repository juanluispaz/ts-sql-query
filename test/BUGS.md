...
## Single-row `returningLastInsertedId().executeInsert()` never throws on a null id — dead guard from a method-vs-field typo

**Where**: `src/queryBuilders/InsertQueryBuilder.ts:102`, inside
`executeInsert()`'s single-row `returningLastInsertedId` branch
(the `!multiple && !this.__from` arm, lines 81–116). The guard reads
`if (!this.onConflictDoNothing) { … }` but `onConflictDoNothing`
**without** the `__` prefix is the *method* (declared at :1676), not the
*field* `this.__onConflictDoNothing` (declared at :41, set at :1678/:1775).
Every sibling reads the `__` field; this line reads the method, which is
always a truthy function reference, so `!this.onConflictDoNothing` is a
constant `false` and the guard body (lines 103–108) is **dead code**.

**Reproduction**: the guard is meant to enforce the non-null type contract
of a *plain* (non-do-nothing) `returningLastInsertedId()` — its result type
is `AutogeneratedPrimaryKeyColumnsTypesOf<TABLE>` (no `| null`), and
`AbstractConnection.transformValueFromDB` maps both `null` and `undefined`
→ `null`, so a DB/driver that returns no id must throw
`MANDATORY_VALUE_NOT_RECEIVED_FROM_DATABASE`. Because the guard is dead, it
resolves `null` instead. Coordinator runtime probe (mock) on
`postgres/newest/pg`:

```
ctx.mockNext(null)   // plain, no onConflictDoNothing
await conn.insertInto(tOrganization).values({ name: 'X', plan: 'free' })
    .returningLastInsertedId().executeInsert()
// → RESOLVES null   (type is non-null `number` ⇒ unsound; should THROW MANDATORY_VALUE_NOT_RECEIVED_FROM_DATABASE)

ctx.mockNext(undefined)  // same chain
// → RESOLVES null   (should THROW)

ctx.mockNext(null)   // .onConflictDoNothing() CONTROL — type is `number | null`
await conn.insertInto(tOrganization).values({ name: 'X', plan: 'free' })
    .onConflictDoNothing().returningLastInsertedId().executeInsert()
// → RESOLVES null   (legitimate — this arm is the one the guard should let through)
```

The plain arm and the `onConflictDoNothing` arm are **runtime-indistinguishable**
(both resolve `null`) even though their result types differ — which is exactly
what the guard is supposed to differentiate. The multi-row / from-select branches
(:117–166) have their own always-on `MANDATORY_VALUE_NOT_RECEIVED_FROM_DATABASE`
guard (per-row, `rowIndex`) and are unaffected; only the single-row
`returningLastInsertedId` path is broken. `executeUpdate`/`executeDelete` builders
were scanned and have no equivalent typo.

**Fix**: `this.onConflictDoNothing` → `this.__onConflictDoNothing` at :102.

**Current workaround in the suite**: none — the negative-path test that would
assert the throw is *blocked by the bug* (it would fail on the plain arm today),
so this entry is the marker. Once fixed, add to a `postgres/newest/pg`-style cell
(RETURNING-gated, like the existing `errors.insert-guards.test.ts` /
`insert.execute-variants.test.ts` mock negative-path tests): a `mockNext(null)`
plain `returningLastInsertedId().executeInsert()` asserting
`MANDATORY_VALUE_NOT_RECEIVED_FROM_DATABASE`, plus the `onConflictDoNothing()`
control asserting it resolves `null` (the value arm the existing
`on-conflict-do-nothing-returning-last-inserted-id-is-nullable` test in
`insert.on-conflict.test.ts:161` never realizes — it mocks a non-null id).
