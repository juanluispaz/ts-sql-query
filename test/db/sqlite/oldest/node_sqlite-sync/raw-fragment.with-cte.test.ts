...
describe(ctx.label, () => {
    ...
    test('rawfragment-with-cte-bubbles-with-clause-to-outer-select', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select project_id as projectId from issue where status = ?) select (select count(projectId) as result from open_issues) as "openCount",  id as id from project"`)
    ...
    test('rawfragment-with-multiple-ctes-from-embedded-subqueries', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select project_id as projectId from issue where status = ?), closed_issues as (select project_id as projectId from issue where status = ?) select (select count(projectId) as result from open_issues) as "openCount", (select count(projectId) as result from closed_issues) as "closedCount",  id as id from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
    ...
    test('rawfragment-with-cte-and-outer-with-coexist', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with open_issues as (select project_id as projectId from issue where status = ?), closed_issues as (select project_id as projectId from issue where status = ?) select (select count(projectId) as result from closed_issues) as "closedCount",  project.id as id, count(open_issues.projectId) as openCount from project inner join open_issues on open_issues.projectId = project.id group by project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
            "closed",
