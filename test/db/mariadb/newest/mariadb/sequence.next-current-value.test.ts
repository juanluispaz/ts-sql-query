...
describe(ctx.label, () => {
    ...
    test('sequence-current-value-in-select', async () => {
        await ctx.withCommit(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select lastval(issue_id_seq) as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('sequence-bigint-current-value-emission', async () => {
        await ctx.withCommit(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select lastval(audit_tag_seq) as result"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
