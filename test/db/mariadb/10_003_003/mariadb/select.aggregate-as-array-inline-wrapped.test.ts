...
describe(ctx.label, () => {
    ...
    */
    ...
    test('inline-aggregate-use-empty-array-for-no-value-explicit', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, cast((select json_arrayagg(json_object('id', id, 'name', name) order by id) from project where organization_id = organization.id) as char) as projects from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, cast((select json_arrayagg(json_object('id', id, 'title', title) order by id) from issue where project_id = project.id) as char) as \`meta.issues\` from project where organization_id = ? order by pid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    */
    ...
    test('inline-aggregate-order-by-asc-nulls-last', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((select json_arrayagg(name order by name is null, name asc) from project where organization_id = organization.id) as char) as projectNames from organization where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB does not allow outer references inside a compound subquery FROM, so forUseAsInlineAggregatedArrayValue is typed never on a compound inline aggregate
    /*
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB types `forUseAsInlineAggregatedArrayValue` as `never` for a `distinct` inline aggregate (`distinct` is a wrap trigger like `group by` / `having` / `compound`) — MariaDB allows no outer references in an inner FROM (no LATERAL), so the correlated derived-table shape is forbidden by design. A MariaDB user can never build it; casting past the guard only emits SQL MariaDB rejects at execution. Body kept (canonical shape, mariadb SQL) for cross-cell diff parity per the symmetry rule.
    /*
    test('inline-aggregate-of-distinct-one-column', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(a_1_.result) from (select distinct status as result from issue where project_id = project.id) as a_1_) as statuses from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-of-distinct-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(json_object('status', a_1_.status, 'priority', a_1_.priority)) from (select distinct status as status, priority as priority from issue where project_id = project.id) as a_1_) as issueKinds from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB types `forUseAsInlineAggregatedArrayValue` as `never` for an inline aggregate carrying `group by` — MariaDB allows no outer references in an inner FROM (no LATERAL), so this correlated-inline derived-table shape is forbidden by design. A MariaDB user can never build it. Runs on the dialects that allow the correlated-inline wrap.
    /*
    ...
    */
    ...
    // NOT-APPLICABLE: same MariaDB boundary as the group-by case — a `having` inline aggregate correlates the outer column into an inner FROM, which MariaDB does not support (no LATERAL); `forUseAsInlineAggregatedArrayValue` types as `never` here by design.
    /*
    ...
    */
    ...
    // NOT-APPLICABLE: MariaDB types `forUseAsInlineAggregatedArrayValue` as `never` for a `distinct` inline aggregate (`distinct` is a wrap trigger like `group by` / `having` / `compound`) — MariaDB allows no outer references in an inner FROM (no LATERAL), so the correlated derived-table shape is forbidden by design.
    /*
    test('inline-aggregate-of-distinct-object-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select json_arrayagg(json_object('status', a_1_.status, 'body', a_1_.\`body\`)) from (select distinct status as status, \`body\` as \`body\` from issue where project_id = project.id) as a_1_) as kinds from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('inline-aggregate-of-compound-union-projecting-optionals-as-nullable', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as pid, (select json_arrayagg(json_object('title', a_1_.title, 'body', a_1_.\`body\`)) from (select title as title, \`body\` as \`body\` from issue where project_id = project.id and id = ? union select title as title, \`body\` as \`body\` from issue where project_id = project.id and id = ?) as a_1_) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
...
void tAppUser; void tProjectRelease
