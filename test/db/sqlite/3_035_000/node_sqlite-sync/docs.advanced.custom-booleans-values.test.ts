...
describe(ctx.label, () => {
    ...
    test('docs:custom-booleans-values/insert-maps-true-false-to-Y-N', async () => {
        await ctx.withRollback(async () => {
            ...
            // doc-start
            const id = sync(ctx.conn.insertInto(tOrganization).set({
                    name:     'Demo Corp',
                    plan:     'free',
                    verified: true,
                })
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, "plan", verified) values (?, ?, case when ? then 'Y' else 'N' end) returning id"`)
    ...
    test('docs:custom-booleans-values/select-maps-Y-N-to-true-false', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, (verified = 'Y') as verified from organization where verified = 'Y'"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
