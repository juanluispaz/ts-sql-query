...
describe(ctx.label, () => {
    ...
    test('aggregate-as-array-of-one-column-preserves-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as "projectId", json_arrayagg(version) as "versions", json_arrayagg(channel) as "channels" from project_release where project_id = :0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-as-array-of-one-column-preserves-enum-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as "issueId", json_arrayagg(activity) as "activities" from issue_worklog where issue_id = :0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('compound-union-carries-the-seed-brand-onto-the-result', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select channel as "channel" from project_release where project_id = :0 union select channel as "channel" from project_release where project_id = :1 order by 1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
