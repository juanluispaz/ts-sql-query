...
describe(ctx.label, () => {
    ...
    // Not applicable on SQL Server: this dialect has no INSERT ... ON CONFLICT / ON DUPLICATE KEY UPDATE (SQL Server uses MERGE).
    /*
    ...
    */
    ...
    test('fragment-from-joined-table-registers-required-column-in-update-from-old-values', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = project.name + @0 + organization.name output inserted.id as id, deleted.name as oldName, inserted.name as newName, (deleted.name || ' (was) vs ' || organization.name) as stamp from organization where project.id = @1 and project.organization_id = organization.id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                " / ",
                1,
