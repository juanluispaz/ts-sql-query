...
describe(ctx.label, () => {
    ...
    test('union-execute-select-page-with-extras-data-skips-data-query', async () => {
        ...
        expect(ctx.history.length).toBe(1)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with result_for_count("label") as (select title as "label" from issue union all select name as "label" from project order by 1) select count(*) from result_for_count"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
            data:  Array<{ label: string }>
            count: number
