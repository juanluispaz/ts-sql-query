...
describe(ctx.label, () => {
    ...
    test('aggregate-of-boolean-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, concat('[', string_agg(isnull(convert(nvarchar, cast(case project.published when 't' then 1 when 'f' then 0 else null end as bit)), 'null'), ','), ']') as published from issue left join project on project.id = issue.project_id where issue.id = @0 group by issue.id"`)
    ...
    test('aggregate-of-local-date-time-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg('"' + convert(nvarchar, created_at, 127) + '"', ','), ']') as createdAts from issue where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-local-date-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, concat('[', string_agg('"' + convert(nvarchar, work_date, 127) + '"', ','), ']') as dates from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-optional-local-date-time-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, concat('[', string_agg(isnull('"' + convert(nvarchar, project.archived_at, 127) + '"', 'null'), ','), ']') as archivedAts from issue left join project on project.id = issue.project_id where issue.id = @0 group by issue.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-bigint-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg('"' + convert(nvarchar, view_count) + '"', ','), ']') as views from issue where project_id = @0 group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-optional-double-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg(isnull(convert(nvarchar, estimated_hours, 3), 'null'), ','), ']') as hours from issue where project_id = @0 group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-optional-uuid-column-as-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg(isnull('"' + convert(nvarchar(36), external_ref) + '"', 'null'), ','), ']') as refs from issue where project_id = @0 group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-object-with-bigint-uuid-and-double', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg(concat('{', '"views": ', '"' + convert(nvarchar, view_count) + '"', ', "externalRef": ', isnull('"' + convert(nvarchar(36), external_ref) + '"', 'null'), ', "estimatedHours": ', isnull(convert(nvarchar, estimated_hours, 3), 'null'), '}'), ','), ']') as issues from issue where project_id = @0 group by project_id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-object-with-boolean-and-local-date-time', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, concat('[', string_agg(concat('{', '"published": ', isnull(convert(nvarchar, cast(case project.published when 't' then 1 when 'f' then 0 else null end as bit)), 'null'), ', "createdAt": ', isnull('"' + convert(nvarchar, project.created_at, 127) + '"', 'null'), '}'), ','), ']') as projects from issue left join project on project.id = issue.project_id where issue.id = @0 group by issue.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-object-with-adapter-transformed-columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg(concat('{', '"score": ', convert(nvarchar, score), ', "reviewer": ', '"' + string_escape(convert(nvarchar(max), reviewer_code), 'json') + '"', '}'), ','), ']') as reviews from project_review where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-one-column-optional-branded-adapter-strips-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project_id as projectId, concat('[', string_agg(isnull('"' + convert(nvarchar, optional_release_ordinal) + '"', 'null'), ','), ']') as ords from release_overview where project_id = @0 group by project_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-double-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull(convert(nvarchar, a_1_.[result], 3), 'null'), ','), ']') from (select estimated_hours as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as hours from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-bigint-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result]) + '"', ','), ']') from (select view_count as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as views from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-uuid-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull('"' + convert(nvarchar(36), a_1_.[result]) + '"', 'null'), ','), ']') from (select external_ref as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as refs from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-time-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result], 127) + '"', ','), ']') from (select created_at as [result] from issue where project_id = project.id order by [result] offset 0 rows) as a_1_) as stamps from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-boolean-column-as-wrapped-array', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull(convert(nvarchar, a_1_.[result]), 'null'), ','), ']') from (select billable as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as flags from issue where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('aggregate-of-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result], 127) + '"', ','), ']') from (select work_date as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as dates from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result], 3) + '"', ','), ']') from (select billed_amount as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as amounts from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-double-column-as-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue_id as issueId, concat('[', string_agg('"' + convert(nvarchar, billed_amount, 3) + '"', ','), ']') as amounts from issue_worklog where issue_id = @0 group by issue_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull('"' + convert(nvarchar, a_1_.[result], 127) + '"', 'null'), ','), ']') from (select started_at as [result] from issue_worklog where issue_id = issue.id order by [result] offset 0 rows) as a_1_) as clocks from issue where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-uuid-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull('"' + convert(nvarchar(36), a_1_.[result]) + '"', 'null'), ','), ']') from (select signing_key as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as keys from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result], 127) + '"', ','), ']') from (select released_on as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as days from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + convert(nvarchar, a_1_.[result], 127) + '"', ','), ']') from (select cutoff_time as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as cutoffs from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-custom-local-date-time-column-as-wrapped-array', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg(isnull('"' + convert(nvarchar, a_1_.[result], 127) + '"', 'null'), ','), ']') from (select signed_off_at as [result] from project_release where project_id = project.id order by [result] offset 0 rows) as a_1_) as signoffs from project where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('aggregate-of-wrapped-object-with-uuid-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select external_ref as [ref], count(id) as cnt from issue where project_id = project.id group by external_ref for json path) as refs from project where id = @0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
