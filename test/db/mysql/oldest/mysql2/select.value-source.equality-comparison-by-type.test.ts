...
describe(ctx.label, () => {
    ...
    test('bigint-is-is-not', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (duration_ms <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            5400000n,
          ]
    ...
    test('double-is-is-not', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const isNot = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (estimated_hours <=> ?) order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2.5,
              ]
    ...
    test('customInt-is-is-not', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (cost_cents <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            400,
          ]
    ...
    test('customDouble-is-is-not', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billed_amount <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            50,
          ]
    ...
    test('uuid-equals-not-equals', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref = uuid_to_bin(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('uuid-is-is-not', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (external_ref <=> uuid_to_bin(?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('uuid-between-not-between', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between uuid_to_bin(?) and uuid_to_bin(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        const notRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not between uuid_to_bin(?) and uuid_to_bin(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "00000000-0000-0000-0000-000000000000",
            "50000000-0000-0000-0000-000000000000",
    ...
    test('uuid-in-array-in-n', async () => {
        ...
        const inRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (uuid_to_bin(?), uuid_to_bin(?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
        ...
        const inNRows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (uuid_to_bin(?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('customUuid-is-is-not-and-value-source-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key <=> uuid_to_bin(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (signing_key <=> uuid_to_bin(?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('localDateTime-is-is-not', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from \`organization\` where not (created_at <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2023-06-15T08:00:00.000Z,
          ]
    ...
    test('customLocalDateTime-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        const isNotIf = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (signed_off_at <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2024-01-14T12:30:00.000Z,
          ]
    ...
    test('uuid-not-equals-if-value-not-in-if-value-fires-and-elides', async () => {
        ...
        const notInIf = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not in (uuid_to_bin(?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('string-is-is-not-const', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (title <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Migrate to ESM",
          ]
    ...
    test('uuid-ordered-comparison-and-mixed-between-value-source-operand', async () => {
        ...
        const cv = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between uuid_to_bin(?) and (select external_ref as result from issue where id = ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            2,
        ...
        const vc = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref between (select external_ref as result from issue where id = ?) and uuid_to_bin(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "7b3e9d20-2222-4c55-9b66-dddd00009999",
    ...
    test('bigint-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (duration_ms <=> (select duration_ms as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('string-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (title <=> (select title as result from issue where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('double-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const isNot = await ctx.conn.selectFrom(tIssue)
                ...
                .orderBy('id')
                .executeSelectMany()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (estimated_hours <=> (select estimated_hours as result from issue where id = ?)) order by id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('localDate-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (work_date <=> (select work_date as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('localTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (started_at <=> (select started_at as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('localDateTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from \`organization\` where not (created_at <=> (select created_at as result from \`organization\` where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customInt-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (cost_cents <=> (select cost_cents as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('customLocalTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (cutoff_time <=> (select cutoff_time as result from project_release where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('bigint-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (duration_ms <=> duration_ms) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customInt-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (cost_cents <=> cost_cents) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customDouble-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billed_amount <=> billed_amount) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('localDate-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (work_date <=> work_date) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('localTime-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (started_at <=> started_at) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customComparable-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (version <=> version) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalTime-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (cutoff_time <=> cutoff_time) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalDate-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (released_on <=> released_on) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalDateTime-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (signed_off_at <=> signed_off_at) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('localDateTime-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tOrganization).where(c.isNot(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from \`organization\` where not (created_at <=> created_at) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('uuid-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (external_ref <=> external_ref) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('string-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (title <=> title) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customUuid-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (signing_key <=> signing_key) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('custom-column-operand-equality-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (\`channel\` <=> \`channel\`) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('enum-column-operand-equality-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (activity <=> activity) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('boolean-column-operand-equality-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billable <=> billable) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('uuid-nullif-value-column-arg', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(nullif(external_ref, external_ref)) as ref from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('localDate-if-value-twins-fires-and-elides', async () => {
        ...
        const isNotF = await ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.isNotIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (work_date <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2024-03-05T00:00:00.000Z,
          ]
    ...
    test('localTime-if-value-twins-fires-and-elides', async () => {
        ...
        const isNotF = await ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.isNotIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (started_at <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "10:30:00",
          ]
    ...
    test('enum-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        const isNotF = await ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.activity.isNotIfValue('coding'))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (activity <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "coding",
          ]
    ...
    test('customLocalTime-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        const isF = await ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isIfValue(ct))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time <=> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
          ]
        ...
        const isNotF = await ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isNotIfValue(ct))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (cutoff_time <=> ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
          ]
    ...
    test('double-column-operand-comparable-membership', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (estimated_hours <=> estimated_hours) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customUuid-nullif-value-column-arg', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id').executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, bin_to_uuid(nullif(signing_key, signing_key)) as \`k\` from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('boolean-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billable <=> (select billable as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where not (external_ref <=> (select external_ref as result from issue where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customDouble-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where not (billed_amount <=> (select billed_amount as result from issue_worklog where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('customComparable-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (version <=> (select version as result from project_release where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('customLocalDateTime-is-is-not-value-source-operand', async () => {
        ...
        const isNot = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where not (signed_off_at <=> (select signed_off_at as result from project_release where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('uuid-in-n-mixed-const-and-value-source', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (uuid_to_bin(?), (select external_ref as result from issue where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "7b3e9d20-2222-4c55-9b66-dddd00009999",
            1,
    ...
    test('customUuid-in-n-mixed-const-and-value-source', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key in (uuid_to_bin(?), (select signing_key as result from project_release where id = ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
            3,
