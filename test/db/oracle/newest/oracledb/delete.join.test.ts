...
describe(ctx.label, () => {
    ...
    test('delete-using-table-then-inner-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project inner join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and project.id = :0"`)
    ...
    test('delete-using-table-then-inner-join-returning-nested', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project inner join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and project.id = :0 returning issue.id, issue.title, issue.priority into :1, :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                {
    ...
    test('delete-using-table-then-left-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project left join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and project.id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-outer-join-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project left outer join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and project.id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
              ]
    ...
    test('delete-using-table-then-left-join-returning-nullable-joined-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project left join "organization" on "organization".id = project.organization_id where issue.project_id = project.id and project.id = :0 returning issue.id, "organization".name into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                2,
                {
    ...
    test('delete-using-table-then-left-join-returning-left-join-optional-column-as-nullable-surfaces-present-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project left join project_release on project_release.project_id = project.id where issue.project_id = project.id and issue.id = :0 returning issue.id, project_release.version into :1, :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                4,
                {
    ...
    test('delete-using-table-then-inner-join-on-and-compound-join-condition', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project inner join "organization" on "organization".id = project.organization_id and "organization".name = :0 where issue.project_id = project.id and project.id = :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Acme Corp",
                2,
    ...
    test('delete-using-table-then-inner-join-dynamic-on-or', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project inner join "organization" on "organization".id = project.organization_id or "organization".name = :0 where issue.project_id = project.id and project.id = :1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "never-matches",
                2,
    ...
    */
    ...
    test('delete-using-table-then-two-inner-joins-on-using-tables', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"delete from issue using project inner join "organization" on "organization".id = project.organization_id inner join project_release on project_release.project_id = project.id where issue.project_id = project.id and issue.id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                99999,
              ]
