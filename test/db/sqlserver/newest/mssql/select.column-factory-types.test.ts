...
describe(ctx.label, () => {
    ...
    test('custom-comparable-comparison-surface', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when version <= @0 then 1 else 0 end as bit) as le, cast(case when version between @1 and @2 then 1 else 0 end as bit) as bt, cast(case when version in (@3, @4) then 1 else 0 end as bit) as isin, cast(case when version <> @5 then 1 else 0 end as bit) as ne from project_release where id = @6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "1.3.0",
            "0.9.0",
    ...
    test('custom-equality-only-non-equals-surface', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when channel in (@0, @1) then 1 else 0 end as bit) as isin, cast(case when channel not in (@2) then 1 else 0 end as bit) as notin, cast(case when channel <> @3 then 1 else 0 end as bit) as ne from project_release where id = @4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "stable",
            "beta",
    ...
    test('const-sourced-custom-comparable-operator', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when @0 < version then 1 else 0 end as bit) as lt from project_release where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0.5.0",
            1,
