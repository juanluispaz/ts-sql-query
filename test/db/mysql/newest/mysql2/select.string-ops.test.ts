...
describe(ctx.label, () => {
    ...
    test('concat', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, concat(email, ?, full_name) as label from app_user where id = ?"`)
    ...
    test('concat-chain-with-conditional-flattens', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select concat(email, ?, ?, ?, ?) as label from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            " (",
            "verified",
    ...
    test('length', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, char_length(full_name) as len from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('length-non-ascii', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select char_length(?) as accented, char_length(?) as cjk"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "café",
            "日本",
    ...
    test('length-and-substr-keep-trailing-spaces', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select char_length(?) as len, substr(?, ?) as whole, substr(?, ?) as tail"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Draft  ",
            "Draft  ",
    ...
    test('negative-index-follows-javascript', async () => {
        ...
        const result = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select right(?, ?) as lastChar, right(?, ?) as fromEnd, right(?, ?) as fromEndFar, left(right(?, ?), ?) as fromEndCount, left(right(?, ?), ?) as fromEndFarCount, substr(?, ?) as clamped, substr(?, ?, ?) as clampedEnd, substr(?, ?, ?) as swapped"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            1,
    ...
    test('negative-count-clamps-to-empty-string', async () => {
        ...
        const result = await conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select substr(?, ?, ?) as fromStart, left(right(?, ?), ?) as fromEnd"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "abcdef",
            3,
    ...
    test('string-predicate-on-optional-receiver-projects-optional-boolean', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, body like concat('%', ?, '%') as hasNeedle from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "token",
            2,
    ...
    test('string-transforms-on-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select upper(body) as up, lower(body) as lo, char_length(body) as len, ltrim(body) as tl, rtrim(body) as tr from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('string-reverse-on-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select reverse(body) as rev from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('concat-with-optional-value-source-argument-and-optional-receiver', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, concat(title, body) as \`j\`, concat(body, ?) as \`r\` from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            2,
    ...
    test('uuid-as-string-string-methods', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, char_length(bin_to_uuid(external_ref)) as len, upper(bin_to_uuid(external_ref)) as up, lower(bin_to_uuid(external_ref)) as lo from issue where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
