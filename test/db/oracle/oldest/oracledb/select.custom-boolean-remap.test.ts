    ...
    */
    ...
    test('combinator: verified custom boolean column as the and receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where (verified = 'Y') and "plan" = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "pro",
          ]
    ...
    test('combinator: verified custom boolean column as the or receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where (verified = 'Y') or id = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('combinator: invoiced numeric custom boolean column as the and receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (invoiced = 1) and id > :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('combinator: approved nullable custom boolean as the and receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (approved = 'A') and id > :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('combinator: approved nullable custom boolean as the or receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (approved = 'A') or id = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('combinator: invoiced numeric custom boolean as the or receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (invoiced = 1) or id > :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('combinator: verified custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where (verified = 'Y') and (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('combinator: verified custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where (verified = 'Y') or (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('combinator: published custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where (published = 't') and (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('combinator: published custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where (published = 't') or (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('combinator: approved nullable custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (approved = 'A') and (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('combinator: approved nullable custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (approved = 'A') or (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('combinator: invoiced numeric custom boolean and literal true', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (invoiced = 1) and (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('combinator: invoiced numeric custom boolean or literal false', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where (invoiced = 1) or (:0 = 1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('equals-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where case when verified = 'Y' then 1 else 0 end = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('equals-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where case approved when 'A' then 1 when 'R' then 0 else null end = :0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where decode(case when published = 't' then 1 else 0 end, :0, 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where decode(case when verified = 'Y' then 1 else 0 end, :0, 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where decode(case approved when 'A' then 1 when 'R' then 0 else null end, :0, 1, 0 ) = 1 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-not-if-value-on-published-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where decode(case when published = 't' then 1 else 0 end, :0, 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-not-if-value-on-verified-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where decode(case when verified = 'Y' then 1 else 0 end, :0, 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-not-if-value-on-approved-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where decode(case approved when 'A' then 1 when 'R' then 0 else null end, :0, 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('is-not-if-value-on-invoiced-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where decode(case when invoiced = 1 then 1 else 0 end, :0, 1, 0 ) = 0 order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('paren-and-with-or-right-operand-remaps-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where id > :0 and ((verified = 'Y') or "plan" = :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            "pro",
    ...
    test('negate-and-with-remapping-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where not ((verified = 'Y') and "plan" = :0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "pro",
          ]
    ...
    test('not-in-on-invoiced-custom-boolean-emits-numeric-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where case when invoiced = 1 then 1 else 0 end not in (:0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
          ]
    ...
    test('not-in-on-verified-custom-boolean-emits-string-remap', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from "organization" where case when verified = 'Y' then 1 else 0 end not in (:0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where case when invoiced = 1 then 1 else 0 end in (:0, :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            0,
    ...
    test('not-in-n-variadic-on-invoiced-custom-boolean', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where case when invoiced = 1 then 1 else 0 end not in (:0, :1) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            0,
    ...
    test('or-of-and-from-published-custom-boolean-receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where (published = 't') or (id > :0 and archived_at is not null) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('and-of-or-from-published-custom-boolean-receiver', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from project where (published = 't') and (id < :0 or archived_at is not null) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('equals-if-value-negate-on-invoiced-custom-boolean-fires-and-elides', async () => {
        ...
        const firedRows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id" from issue_worklog where not (case when invoiced = 1 then 1 else 0 end = :0) order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
