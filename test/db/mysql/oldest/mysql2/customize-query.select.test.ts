...
describe(ctx.label, () => {
    ...
    */
    ...
    test('customize-select-before-with-query-and-after-with-query-wrap-cte', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as id, open_issues.id as issueId from project inner join (select id as id, project_id as projectId from issue where \`status\` = ?) as open_issues on open_issues.projectId = project.id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "open",
          ]
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"select open_issues.id as id, open_issues.projectId as projectId from (select /* hint */ /* cols */  id as id, project_id as projectId from issue where id in (?, ?) window w1 as (partition by project_id) order by project_id asc, id, priority asc) as open_issues order by id"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             1,
//             2,
...
//     })
    ...
    test('customize-select-plain-order-by-limit-execute-select-page-places-hooks-in-count-wrap', async () => {
        ...
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"/* head */  select id as id, \`name\` as \`name\` from project where id <= ? order by id limit ?  /* tail */"`)
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (/* head */  select id as id, \`name\` as \`name\` from project where id <= ?  /* tail */) as result_for_count"`)
...
//           ]
//         `)
//         expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (select /* hint */ /* cols */  id as id, \`name\` as \`name\` from project where id <= ? window w1 as (partition by organization_id)) as result_for_count"`)
//         expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
//           [
//             3,
//           ]
...
//     })
    ...
    test('customize-select-distinct-page-hooks-ride-count-wrap', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (/* head */  select distinct id as id from project where id <= ?  /* tail */) as result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            3,
          ]
    ...
    test('customize-select-group-by-page-hooks-ride-count-wrap', async () => {
        ...
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"select count(*) from (/* head */  select \`status\` as \`status\`, count(id) as total from issue group by \`status\`  /* tail */) as result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof page, {
...
//             .executeSelectMany()
//
//         expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as result from issue where id = ? union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) select id as id, cast((select json_arrayagg(a_2_.result) from (select /* hint */ /* cols */  result as result from recursive_select_1 window w1 as (partition by result) order by result limit 2147483647) as a_2_) as char) as tree from project where id = ?"`)
//         expect(ctx.lastParams).toMatchInlineSnapshot(`
//           [
//             1,
//             1,
...
//     })
    ...
    test('customize-recursive-one-column-ordering-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as result from issue where id = ? union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) select id as id, cast((select json_arrayagg(a_2_.result) from (select result as result from recursive_select_1 order by result limit 2147483647) as a_2_) as char) as tree from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    */
    ...
    test('customize-recursive-execute-select-page-count-strips-order-by-hooks', async () => {
        ...
        expect(ctx.history.length).toBe(2)
        expect(ctx.history[0]!.sql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as id, parent_id as parentId from issue where project_id = ? union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId) select id as id, parentId as parentId from recursive_select_1 order by id asc, parentId asc"`)
        expect(ctx.history[0]!.params).toMatchInlineSnapshot(`
          [
            1,
          ]
        `)
        expect(ctx.history[1]!.sql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as (select id as id, parent_id as parentId from issue where project_id = ? union all select issue.id as id, issue.parent_id as parentId from issue join recursive_select_1 on issue.id = recursive_select_1.parentId), result_for_count as (select id as id, parentId as parentId from recursive_select_1) select count(*) from result_for_count"`)
        expect(ctx.history[1]!.params).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    */
    ...
    test('customize-select-plain-inline-value-renders-query-hooks-but-drops-with-query-hooks', async () => {
        ...
        const arrResult = await connection.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast((/* head */  select json_arrayagg(json_object('iid', id)) from issue where project_id = project.id  /* tail */) as char) as issues from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
