...
describe(ctx.label, () => {
    ...
    test('bigint-between-not-between', async () => {
        ...
        const notRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('double-equals-not-equals', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 2.5 }).where(tIssue.id.equals(1)).executeUpdate())
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 7.5 }).where(tIssue.id.equals(2)).executeUpdate())
            ...
            ctx.mockNext(expectedNe)
            const ne = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('double-is-is-not', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expectedIsNot)
            const isNot = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('double-between-not-between', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expectedNot)
            const notRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('double-in-array-in-n', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expectedIn)
            const inRows = sync(ctx.conn.selectFrom(tIssue)
            ...
            const expectedInN = [{ id: 1 }]
            ctx.mockNext(expectedInN)
            const inNRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('double-in-subquery-not-in-subquery', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expectedNotIn)
            const notInRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('customUuid-in-subquery-not-in-subquery', async () => {
        ...
        const expectedIn = [{ id: 1 }]
        ctx.mockNext(expectedIn)
        const inRows = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const expectedNotIn = [{ id: 3 }]
        ctx.mockNext(expectedNotIn)
        const notInRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customComparable-is-is-not', async () => {
        ...
        const expectedIsNot = [{ id: 2 }, { id: 3 }]
        ctx.mockNext(expectedIsNot)
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('numeric-custom-boolean-direct-filter-operand', async () => {
        ...
        const eqRows = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedTrue)
        const isRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('customUuid-is-is-not-and-value-source-operand', async () => {
        ...
        const eq = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('string-ordered-comparison-and-between', async () => {
        ...
        const lt = sync(ctx.conn.selectFrom(tIssue)
        ...
        const gt = sync(ctx.conn.selectFrom(tIssue)
        ...
        const le = sync(ctx.conn.selectFrom(tIssue)
        ...
        const ge = sync(ctx.conn.selectFrom(tIssue)
        ...
        const between = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('customComparable-greater-or-equal-and-not-between', async () => {
        ...
        ctx.mockNext(expectedGe)
        const ge = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedNot)
        const notRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('localDateTime-equals-not-equals', async () => {
        ...
        const expectedNe = [{ id: 2 }]
        ctx.mockNext(expectedNe)
        const ne = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('localDateTime-is-is-not', async () => {
        ...
        ctx.mockNext(expectedIsNot)
        const isNot = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('localDateTime-in-array-in-n', async () => {
        ...
        const expectedIn = [{ id: 1 }, { id: 2 }]
        ctx.mockNext(expectedIn)
        const inRows = sync(ctx.conn.selectFrom(tOrganization)
        ...
        const expectedInN = [{ id: 1 }]
        ctx.mockNext(expectedInN)
        const inNRows = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('localDateTime-between-not-between', async () => {
        ...
        const expectedNot = [{ id: 2 }]
        ctx.mockNext(expectedNot)
        const notRows = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('required-customLocalDateTime-equalable-comparable-surface', async () => {
        ...
        const is = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedBetween)
        const between = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('required-localTime-equalable-comparable-surface', async () => {
        ...
        const expectedEq = [{ id: 1 }]
        ctx.mockNext(expectedEq)
        const eq = sync(ctx.conn.selectFrom(tProjectReview)
        ...
        const expectedIs = [{ id: 1 }]
        ctx.mockNext(expectedIs)
        const is = sync(ctx.conn.selectFrom(tProjectReview)
        ...
        ctx.mockNext(expectedBetween)
        const between = sync(ctx.conn.selectFrom(tProjectReview)
        ...
        ctx.mockNext(expectedGe)
        const ge = sync(ctx.conn.selectFrom(tProjectReview)
        ...
        const expectedIn = [{ id: 1 }]
        ctx.mockNext(expectedIn)
        const inRows = sync(ctx.conn.selectFrom(tProjectReview)
    ...
    test('optional-bigint-is-null-is-not-null', async () => {
        ...
        const nullRows = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const notNullRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('optional-double-is-null-is-not-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            ctx.mockNext(expectedNull)
            const nullRows = sync(ctx.conn.selectFrom(tIssue)
            ...
            ctx.mockNext(expectedNotNull)
            const notNullRows = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('optional-customUuid-is-null-is-not-null', async () => {
        ...
        const expectedNull = [{ id: 2 }]
        ctx.mockNext(expectedNull)
        const nullRows = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const expectedNotNull = [{ id: 1 }, { id: 3 }]
        ctx.mockNext(expectedNotNull)
        const notNullRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('optional-customInt-is-null-is-not-null', async () => {
        ...
        const expectedNull = [{ id: 2 }]
        ctx.mockNext(expectedNull)
        const nullRows = sync(ctx.conn.selectFrom(vReleaseOverview)
        ...
        const expectedNotNull = [{ id: 1 }, { id: 3 }]
        ctx.mockNext(expectedNotNull)
        const notNullRows = sync(ctx.conn.selectFrom(vReleaseOverview)
    ...
    test('bigint-between-mixed-overloads-and-column-operand', async () => {
        ...
        const cv = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedBoth)
        const vc = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedBoth)
        const vv = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedGt)
        const gt = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('customLocalDateTime-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        const expectedIs = [{ id: 1 }]
        ctx.mockNext(expectedIs)
        const isIf = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedIs)
        const isDirect = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const expectedIsNot = [{ id: 2 }, { id: 3 }]
        ctx.mockNext(expectedIsNot)
        const isNotIf = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedIsNot)
        const isNotDirect = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const isElide = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedAll)
        const isNotElide = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('uuid-not-equals-if-value-not-in-if-value-fires-and-elides', async () => {
        ...
        const expectedNe = [{ id: 2 }]
        ctx.mockNext(expectedNe)
        const neIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedNe)
        const neDirect = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.externalRef.notEquals(ref1))
            .select({ id: tIssue.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        ctx.mockNext(expectedNe)
        const notInIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedNe)
        const notInDirect = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedAll)
        const neElide = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedAll)
        const notInElide = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('bigint-ordered-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext(expectedLt)
        const ltIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedLt)
        const ltDirect = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedGe = [{ id: 1 }]
        ctx.mockNext(expectedGe)
        const geIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedGe)
        const geDirect = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedAll = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(expectedAll)
        const ltElide = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedAll)
        const geElide = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('bigint-ordered-const-single-bounds', async () => {
        ...
        const expectedLt = [{ id: 3 }]
        ctx.mockNext(expectedLt)
        const lt = sync(ctx.conn.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.durationMs.lessThan(bound))
            .select({ id: tIssueWorklog.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedGe = [{ id: 1 }]
        ctx.mockNext(expectedGe)
        const ge = sync(ctx.conn.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.durationMs.greaterOrEqual(bound))
            .select({ id: tIssueWorklog.id })
            .orderBy('id')
            .executeSelectMany())
    ...
    test('string-is-is-not-const', async () => {
        ...
        ctx.mockNext(expectedIs)
        const is = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('customLocalDateTime-equals-not-equals-value-source-operand', async () => {
        ...
        const expectedNe = [{ id: 3 }]
        ctx.mockNext(expectedNe)
        const ne = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('is-value-source-operand-enum-custom-customUuid', async () => {
        ...
        const enumRows = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const customRows = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedReleases)
        const customUuidRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customInt-ordered-comparison-and-mixed-between-value-source-operand', async () => {
        ...
        ctx.mockNext(expectedLe)
        const le = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('uuid-ordered-comparison-and-mixed-between-value-source-operand', async () => {
        ...
        // between(TYPE, VALUE): const lower, value-source upper.
        ctx.mockNext(expectedBetween)
        const cv = sync(ctx.conn.selectFrom(tIssue)
        ...
        // between(VALUE, TYPE): value-source lower, const upper.
        ctx.mockNext(expectedBetween)
        const vc = sync(ctx.conn.selectFrom(tIssue)
        ...
        // between(VALUE, VALUE): both bounds value-sources.
        ctx.mockNext(expectedBetween)
        const vv = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('customComparable-ordered-comparison-value-source-operand', async () => {
        ...
        const expectedLt = [{ id: 3 }]
        ctx.mockNext(expectedLt)
        const lt = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customLocalDateTime-ordered-comparison-value-source-operand', async () => {
        ...
        const expectedGt = [{ id: 3 }]
        ctx.mockNext(expectedGt)
        const gt = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('customInt-customDouble-if-value-fires-and-elides', async () => {
        ...
        const costIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedCost)
        const costDirect = sync(ctx.conn.selectFrom(tIssueWorklog)
            .where(tIssueWorklog.costCents.equals(400))
            .select({ id: tIssueWorklog.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedAll = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(expectedAll)
        const costElide = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const billedIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedAll)
        const billedElide = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('numeric-custom-equals-value-source-operand', async () => {
        ...
        const expectedCost = [{ id: 3 }]
        ctx.mockNext(expectedCost)
        const costRows = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedBilled = [{ id: 2 }]
        ctx.mockNext(expectedBilled)
        const billedRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('customUuid-ordered-comparison-value-source-operand', async () => {
        ...
        ctx.mockNext(expectedBetween)
        const bt = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const nb = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(expectedLe)
        const le = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('enum-not-equals-if-value-not-in-if-value-fires-and-elides', async () => {
        ...
        const expected = [{ id: 2 }, { id: 3 }]
        ctx.mockNext(expected)
        const neIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expected)
        const notInIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(all)
        const neElide = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(all)
        const notInElide = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('custom-in-n-not-in-n', async () => {
        ...
        const expectedIn = [{ id: 1 }]
        ctx.mockNext(expectedIn)
        const inN = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const expectedNotIn = [{ id: 1 }, { id: 2 }]
        ctx.mockNext(expectedNotIn)
        const notInN = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('custom-equals-if-value-not-in-if-value-fires-and-elides', async () => {
        ...
        const expectedEq = [{ id: 1 }]
        ctx.mockNext(expectedEq)
        const eqIf = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const expectedNotIn = [{ id: 1 }, { id: 2 }]
        ctx.mockNext(expectedNotIn)
        const notInIf = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        const all = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(all)
        const eqElide = sync(ctx.conn.selectFrom(tProjectRelease)
        ...
        ctx.mockNext(all)
        const notInElide = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('plain-boolean-is-not-and-is-not-null', async () => {
        ...
        const expectedNotNull = [{ id: 1 }, { id: 2 }]
        ctx.mockNext(expectedNotNull)
        const notNull = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('boolean-not-in-array-in-n-not-in-n', async () => {
        ...
        const notInNRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('localDateTime-in-subquery-not-in-subquery', async () => {
        ...
        const expectedNotIn = [{ id: 2 }]
        ctx.mockNext(expectedNotIn)
        const notInRows = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('bigint-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const expectedEq = [{ id: 1 }]
        ctx.mockNext(expectedEq)
        const eq = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedIs = [{ id: 1 }]
        ctx.mockNext(expectedIs)
        const is = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('string-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        ctx.mockNext(expectedEq)
        const eq = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('localDateTime-equals-not-equals-is-is-not-value-source-operand', async () => {
        ...
        const expectedEq = [{ id: 1 }]
        ctx.mockNext(expectedEq)
        const eq = sync(ctx.conn.selectFrom(tOrganization)
        ...
        const expectedIs = [{ id: 1 }]
        ctx.mockNext(expectedIs)
        const is = sync(ctx.conn.selectFrom(tOrganization)
    ...
    test('bigint-ordered-if-value-le-gt-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext(expectedLe)
        const leIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedLe)
        const leDirect = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedGt = [{ id: 1 }]
        ctx.mockNext(expectedGt)
        const gtIf = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedGt)
        const gtDirect = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        const expectedAll = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(expectedAll)
        const leElide = sync(ctx.conn.selectFrom(tIssueWorklog)
        ...
        ctx.mockNext(expectedAll)
        const gtElide = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('string-ordered-if-value-twins-fires-and-elides', async () => {
        ...
        const expectedLt = [{ id: 4 }]
        ctx.mockNext(expectedLt)
        const ltIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedLt)
        const ltDirect = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.title.lessThan(mid))
            .select({ id: tIssue.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedGt = [{ id: 1 }, { id: 2 }]
        ctx.mockNext(expectedGt)
        const gtIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedGt)
        const gtDirect = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.title.greaterThan(mid))
            .select({ id: tIssue.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedLe = [{ id: 3 }, { id: 4 }]
        ctx.mockNext(expectedLe)
        const leIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedLe)
        const leDirect = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.title.lessOrEqual(mid))
            .select({ id: tIssue.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedGe = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(expectedGe)
        const geIf = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedGe)
        const geDirect = sync(ctx.conn.selectFrom(tIssue)
            .where(tIssue.title.greaterOrEqual(mid))
            .select({ id: tIssue.id })
            .orderBy('id')
            .executeSelectMany())
        ...
        const expectedAll = [{ id: 1 }, { id: 2 }, { id: 3 }, { id: 4 }]
        ctx.mockNext(expectedAll)
        const ltElide = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedAll)
        const gtElide = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedAll)
        const leElide = sync(ctx.conn.selectFrom(tIssue)
        ...
        ctx.mockNext(expectedAll)
        const geElide = sync(ctx.conn.selectFrom(tIssue)
    ...
    test('double-ordered-if-value-twins-fires-and-elides', async () => {
        ...
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 2.0 }).where(tIssue.id.equals(1)).executeUpdate())
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 4.0 }).where(tIssue.id.equals(2)).executeUpdate())
            ctx.mockNext(1)
            sync(ctx.conn.update(tIssue).set({ estimatedHours: 6.0 }).where(tIssue.id.equals(3)).executeUpdate())
    ...
    test('customUuid-not-in-n', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('customUuid-in-n', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProjectRelease)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "0a8f9c1e-1111-4222-8333-444455556666",
          ]
    ...
    test('localTime-in-n', async () => {
        ...
        const expected = [{ id: 1 }]
        ctx.mockNext(expected)
        const rows = sync(ctx.conn.selectFrom(tProjectReview)
    ...
    test('bigint-column-operand-comparable-membership', async () => {
        ...
        const eq = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.equals(c))
        ...
        ctx.mockNext(none)
        const ne = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.notEquals(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms <> duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(all)
        const is = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms is duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(is).toEqual(all)

        ctx.mockNext(none)
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms is not duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)

        ctx.mockNext(none)
        const lt = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.lessThan(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms < duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(none)
        const gt = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.greaterThan(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms > duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(nonNull)
        const le = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.lessOrEqual(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms <= duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(nonNull)
        const ge = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.greaterOrEqual(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms >= duration_ms order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(nonNull)
        const bt = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.between(c, c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const nbt = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.notBetween(c, c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nonNull)
        const inC = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.in(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const notInC = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.notIn(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nonNull)
        const inN = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.inN(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms in (duration_ms) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(none)
        const notInN = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.notInN(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms not in (duration_ms) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('customInt-column-operand-comparable-membership', async () => {
        ...
        const is = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents is cost_cents order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(is).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents is not cost_cents order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customDouble-column-operand-comparable-membership', async () => {
        ...
        ctx.mockNext(all)
        const isv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount is billed_amount order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount is not billed_amount order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
        ...
        ctx.mockNext(nn)
        const inNv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.inN(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount in (billed_amount) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(none)
        const notInNv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.notInN(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount not in (billed_amount) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('localDate-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date is work_date order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date is not work_date order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('localTime-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at is started_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at is not started_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customComparable-column-operand-comparable-membership', async () => {
        ...
        ctx.mockNext(nn)
        const eq = sync(ctx.conn.selectFrom(tProjectRelease).where(c.equals(c))
        ...
        ctx.mockNext(none)
        const ne = sync(ctx.conn.selectFrom(tProjectRelease).where(c.notEquals(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version <> version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ne).toEqual(none)
        ...
        ctx.mockNext(all)
        const isv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.is(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version is version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)

        ctx.mockNext(none)
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version is not version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)

        ctx.mockNext(none)
        const lt = sync(ctx.conn.selectFrom(tProjectRelease).where(c.lessThan(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version < version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lt).toEqual(none)
        ...
        ctx.mockNext(none)
        const gt = sync(ctx.conn.selectFrom(tProjectRelease).where(c.greaterThan(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version > version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(gt).toEqual(none)
        ...
        ctx.mockNext(nn)
        const le = sync(ctx.conn.selectFrom(tProjectRelease).where(c.lessOrEqual(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version <= version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(le).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const ge = sync(ctx.conn.selectFrom(tProjectRelease).where(c.greaterOrEqual(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version >= version order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ge).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const bt = sync(ctx.conn.selectFrom(tProjectRelease).where(c.between(c, c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const nbt = sync(ctx.conn.selectFrom(tProjectRelease).where(c.notBetween(c, c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inC = sync(ctx.conn.selectFrom(tProjectRelease).where(c.in(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const notInC = sync(ctx.conn.selectFrom(tProjectRelease).where(c.notIn(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inNv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.inN(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version in (version) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inNv).toEqual(nn)
        ...
        ctx.mockNext(none)
        const notInNv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.notInN(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version not in (version) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInNv).toEqual(none)
    ...
    test('customLocalTime-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.is(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time is cutoff_time order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time is not cutoff_time order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalDate-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.is(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on is released_on order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where released_on is not released_on order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customLocalDateTime-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tProjectRelease).where(c.is(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at is signed_off_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at is not signed_off_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('localDateTime-column-operand-comparable-membership', async () => {
        ...
        ctx.mockNext(nn)
        const eq = sync(ctx.conn.selectFrom(tOrganization).where(c.equals(c))
        ...
        ctx.mockNext(none)
        const ne = sync(ctx.conn.selectFrom(tOrganization).where(c.notEquals(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at <> created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ne).toEqual(none)
        ...
        ctx.mockNext(all)
        const isv = sync(ctx.conn.selectFrom(tOrganization).where(c.is(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at is created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)

        ctx.mockNext(none)
        const isNot = sync(ctx.conn.selectFrom(tOrganization).where(c.isNot(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at is not created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)

        ctx.mockNext(none)
        const lt = sync(ctx.conn.selectFrom(tOrganization).where(c.lessThan(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at < created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lt).toEqual(none)
        ...
        ctx.mockNext(none)
        const gt = sync(ctx.conn.selectFrom(tOrganization).where(c.greaterThan(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at > created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(gt).toEqual(none)
        ...
        ctx.mockNext(nn)
        const le = sync(ctx.conn.selectFrom(tOrganization).where(c.lessOrEqual(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at <= created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(le).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const ge = sync(ctx.conn.selectFrom(tOrganization).where(c.greaterOrEqual(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at >= created_at order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ge).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const bt = sync(ctx.conn.selectFrom(tOrganization).where(c.between(c, c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const nbt = sync(ctx.conn.selectFrom(tOrganization).where(c.notBetween(c, c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inC = sync(ctx.conn.selectFrom(tOrganization).where(c.in(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const notInC = sync(ctx.conn.selectFrom(tOrganization).where(c.notIn(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inNv = sync(ctx.conn.selectFrom(tOrganization).where(c.inN(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at in (created_at) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inNv).toEqual(nn)
        ...
        ctx.mockNext(none)
        const notInNv = sync(ctx.conn.selectFrom(tOrganization).where(c.notInN(c))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at not in (created_at) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInNv).toEqual(none)
    ...
    test('uuid-column-operand-comparable-membership', async () => {
        ...
        ctx.mockNext(nn)
        const eq = sync(ctx.conn.selectFrom(tIssue).where(c.equals(c))
        ...
        ctx.mockNext(none)
        const ne = sync(ctx.conn.selectFrom(tIssue).where(c.notEquals(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref <> external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ne).toEqual(none)
        ...
        ctx.mockNext(all)
        const isv = sync(ctx.conn.selectFrom(tIssue).where(c.is(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref is external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)

        ctx.mockNext(none)
        const isNot = sync(ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref is not external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)

        ctx.mockNext(none)
        const lt = sync(ctx.conn.selectFrom(tIssue).where(c.lessThan(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref < external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lt).toEqual(none)
        ...
        ctx.mockNext(none)
        const gt = sync(ctx.conn.selectFrom(tIssue).where(c.greaterThan(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref > external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(gt).toEqual(none)
        ...
        ctx.mockNext(nn)
        const le = sync(ctx.conn.selectFrom(tIssue).where(c.lessOrEqual(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref <= external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(le).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const ge = sync(ctx.conn.selectFrom(tIssue).where(c.greaterOrEqual(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref >= external_ref order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(ge).toEqual(nn)
        ...
        ctx.mockNext(nn)
        const bt = sync(ctx.conn.selectFrom(tIssue).where(c.between(c, c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const nbt = sync(ctx.conn.selectFrom(tIssue).where(c.notBetween(c, c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inC = sync(ctx.conn.selectFrom(tIssue).where(c.in(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(none)
        const notInC = sync(ctx.conn.selectFrom(tIssue).where(c.notIn(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(nn)
        const inNv = sync(ctx.conn.selectFrom(tIssue).where(c.inN(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (external_ref) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inNv).toEqual(nn)
        ...
        ctx.mockNext(none)
        const notInNv = sync(ctx.conn.selectFrom(tIssue).where(c.notInN(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not in (external_ref) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInNv).toEqual(none)
    ...
    test('string-column-operand-comparable-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tIssue).where(c.is(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title is title order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title is not title order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('customUuid-column-operand-comparable-membership', async () => {
        ...
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key is not signing_key order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('custom-column-operand-equality-membership', async () => {
        ...
        const isNot = sync(ctx.conn.selectFrom(tProjectRelease).where(c.isNot(c))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where channel is not channel order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('enum-column-operand-equality-membership', async () => {
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where activity is not activity order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('boolean-column-operand-equality-membership', async () => {
        ...
        const isv = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.is(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable is billable order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isv).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssueWorklog).where(c.isNot(c))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable is not billable order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
    ...
    test('int-column-operand-less-than-less-or-equal', async () => {
        ...
        const lt = [{ id: 2 }, { id: 4 }]
        ctx.mockNext(lt)
        const ltRows = sync(ctx.conn.selectFrom(tIssue).where(tIssue.priority.lessThan(tIssue.id))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(le)
        const leRows = sync(ctx.conn.selectFrom(tIssue).where(tIssue.priority.lessOrEqual(tIssue.id))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where priority <= id order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('boolean-value-when-null-nullif-value-column-arg', async () => {
        ...
        ctx.mockNext(vwn)
        const vwnRows = sync(ctx.conn.selectFrom(tIssueWorklog)
            .select({ id: tIssueWorklog.id, b: tIssueWorklog.billable.valueWhenNull(tIssueWorklog.billable) })
            .orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ifnull(billable, billable) as "b" from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(niv)
        const nivRows = sync(ctx.conn.selectFrom(tIssueWorklog)
    ...
    test('customComparable-value-when-null-nullif-value-column-arg', async () => {
        ...
        ctx.mockNext(vwn)
        const vwnRows = sync(ctx.conn.selectFrom(tProjectRelease)
            .select({ id: tProjectRelease.id, v: tProjectRelease.version.valueWhenNull(tProjectRelease.version) })
            .orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ifnull(version, version) as "v" from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        const niv = [{ id: 1 }, { id: 2 }, { id: 3 }]
        ctx.mockNext(niv)
        const nivRows = sync(ctx.conn.selectFrom(tProjectRelease)
    ...
    test('bigint-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 3 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.notEqualsIfValue(5400000n))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.notEqualsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.isIfValue(5400000n))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.isNotIfValue(5400000n))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([{ id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.notInIfValue([5400000n]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where duration_ms not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.durationMs.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('double-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([])
        const equalsF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.equalsIfValue(5))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.equalsIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([])
        const notEqualsF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.notEqualsIfValue(5))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.notEqualsIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([])
        const isF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.isIfValue(5))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.isIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        const isNotF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.isNotIfValue(5))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.isNotIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([])
        const inF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.inIfValue([5]))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.inIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([])
        const notInF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.notInIfValue([5]))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.estimatedHours.notInIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('string-is-if-value-in-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.isIfValue(s))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.isIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        const isNotF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.isNotIfValue(s))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.isNotIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.inIfValue([s]))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.inIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
    ...
    test('boolean-is-not-if-value-in-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.isNotIfValue(true))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.inIfValue([true]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.inIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.notInIfValue([true]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('uuid-if-value-ordered-is-in-fires-and-elides', async () => {
        ...
        ctx.mockNext([])
        const lessThanF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.lessThanIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.lessThanIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([{ id: 2 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.greaterThanIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.greaterThanIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.lessOrEqualIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.lessOrEqualIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.greaterOrEqualIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.greaterOrEqualIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.isIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.isIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }, { id: 4 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.isNotIfValue(ref))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.isNotIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.inIfValue([ref]))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssue).where(tIssue.externalRef.inIfValue(undefined))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
    ...
    test('localDate-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 2 }])
        const equalsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.equalsIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-03-05",
          ]
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.equalsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(equalsE).toEqual(all)
        ...
        const notEqualsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.notEqualsIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.notEqualsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.isIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-03-05",
          ]
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.isNotIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-03-05",
          ]
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.lessThanIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.lessThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.greaterThanIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.greaterThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.lessOrEqualIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.lessOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.greaterOrEqualIfValue(d))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.greaterOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const inF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.inIfValue([d]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.inIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.notInIfValue([d]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where work_date not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.workDate.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('localTime-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 3 }])
        const equalsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.equalsIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "10:30:00",
          ]
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.equalsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(equalsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.notEqualsIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.notEqualsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.isIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "10:30:00",
          ]
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.isNotIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "10:30:00",
          ]
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.lessThanIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.lessThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.greaterThanIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.greaterThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.lessOrEqualIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.lessOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.greaterOrEqualIfValue(t))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.greaterOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const inF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.inIfValue([t]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "10:30:00",
          ]
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.inIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.notInIfValue([t]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('localDateTime-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const equalsF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.equalsIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.equalsIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext([{ id: 2 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.notEqualsIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.notEqualsIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.isIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.isIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const isNotF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.isNotIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.isNotIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([])
        const lessThanF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.lessThanIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.lessThanIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.greaterThanIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.greaterThanIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.lessOrEqualIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.lessOrEqualIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.greaterOrEqualIfValue(ts))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.greaterOrEqualIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.inIfValue([ts]))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.inIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const notInF = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.notInIfValue([ts]))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization where created_at not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2023-06-15 08:00:00",
          ]
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tOrganization).where(tOrganization.createdAt.notInIfValue(undefined))
            .select({ id: tOrganization.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('customInt-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 3 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.notEqualsIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.notEqualsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.isIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.isNotIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([])
        const lessThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.lessThanIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.lessThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.greaterThanIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.greaterThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.lessOrEqualIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.lessOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.greaterOrEqualIfValue(100))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.greaterOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }])
        const inF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.inIfValue([100]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.inIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.notInIfValue([100]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where cost_cents not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.costCents.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('customDouble-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 2 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.notEqualsIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.notEqualsIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.isIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.isNotIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const lessThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.lessThanIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.lessThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([])
        const greaterThanF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.greaterThanIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.greaterThanIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 2 }, { id: 3 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.lessOrEqualIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.lessOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.greaterOrEqualIfValue(200))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.greaterOrEqualIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const inF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.inIfValue([200]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.inIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }])
        const notInF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.notInIfValue([200]))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billed_amount not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billedAmount.notInIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('customComparable-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const equalsF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.equalsIfValue(sv))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.equalsIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(equalsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.notEqualsIfValue(sv))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.notEqualsIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.isIfValue(sv))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.isIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.isNotIfValue(sv))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where version is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.isNotIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.notInIfValue([sv]))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.version.notInIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('custom-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.channel.isIfValue('stable'))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where channel is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.channel.isIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.channel.isNotIfValue('stable'))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where channel is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.channel.isNotIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
    ...
    test('enum-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.activity.isIfValue('coding'))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.activity.isIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.activity.isNotIfValue('coding'))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.activity.isNotIfValue(undefined))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
    ...
    test('customUuid-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 3 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.notEqualsIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.notEqualsIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.isIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.isIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.isNotIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.isNotIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
        ...
        ctx.mockNext([])
        const lessThanF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.lessThanIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.lessThanIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.greaterThanIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.greaterThanIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.lessOrEqualIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.lessOrEqualIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.greaterOrEqualIfValue(key))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.greaterOrEqualIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.inIfValue([key]))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.inIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.notInIfValue([key]))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signing_key not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signingKey.notInIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('customLocalDate-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.isIfValue(rd))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.isIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.isNotIfValue(rd))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.releasedOn.isNotIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
    ...
    test('customLocalTime-is-if-value-is-not-if-value-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const isF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isIfValue(ct))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
          ]
        ...
        ctx.mockNext(all)
        const isE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isE).toEqual(all)
        ...
        ctx.mockNext([{ id: 2 }, { id: 3 }])
        const isNotF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isNotIfValue(ct))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where cutoff_time is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "17:00:00",
          ]
        ...
        ctx.mockNext(all)
        const isNotE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.cutoffTime.isNotIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNotE).toEqual(all)
    ...
    test('customLocalDateTime-if-value-twins-fires-and-elides', async () => {
        ...
        ctx.mockNext([{ id: 1 }])
        const equalsF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.equalsIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at = ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const equalsE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.equalsIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(equalsE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const notEqualsF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.notEqualsIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at <> ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notEqualsE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.notEqualsIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notEqualsE).toEqual(all)
        ...
        ctx.mockNext([])
        const lessThanF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.lessThanIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at < ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessThanE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.lessThanIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const greaterThanF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.greaterThanIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at > ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterThanE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.greaterThanIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterThanE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const lessOrEqualF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.lessOrEqualIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at <= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const lessOrEqualE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.lessOrEqualIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(lessOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }, { id: 3 }])
        const greaterOrEqualF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.greaterOrEqualIfValue(st))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at >= ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const greaterOrEqualE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.greaterOrEqualIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(greaterOrEqualE).toEqual(all)
        ...
        ctx.mockNext([{ id: 1 }])
        const inF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.inIfValue([st]))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "2024-01-14 12:30:00",
          ]
        ...
        ctx.mockNext(all)
        const inE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.inIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(inE).toEqual(all)
        ...
        ctx.mockNext([{ id: 3 }])
        const notInF = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.notInIfValue([st]))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release where signed_off_at not in (?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
        ...
        ctx.mockNext(all)
        const notInE = sync(ctx.conn.selectFrom(tProjectRelease).where(tProjectRelease.signedOffAt.notInIfValue(undefined))
            .select({ id: tProjectRelease.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from project_release order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInE).toEqual(all)
    ...
    test('string-in-subquery-not-in-subquery', async () => {
        ...
        ctx.mockNext(inRes)
        const inRows = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.in(titlesSub))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title in (select title as result from issue where id in (?, ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
        ...
        ctx.mockNext(notRes)
        const notRows = sync(ctx.conn.selectFrom(tIssue).where(tIssue.title.notIn(titlesSub))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where title not in (select title as result from issue where id in (?, ?)) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            2,
    ...
    test('boolean-in-subquery-not-in-subquery', async () => {
        ...
        const inRes = [{ id: 1 }]
        ctx.mockNext(inRes)
        const inRows = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.in(billableSub))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        ...
        const notRes = [{ id: 2 }]
        ctx.mockNext(notRes)
        const notRows = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.billable.notIn(billableSub))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where billable not in (select billable as result from issue_worklog where id = ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('localTime-in-n-multi-const', async () => {
        ...
        const expected = [{ id: 1 }, { id: 3 }]
        ctx.mockNext(expected)
        const rows = sync(ctx.conn.selectFrom(tIssueWorklog).where(tIssueWorklog.startedAt.inN(t1, t3))
            .select({ id: tIssueWorklog.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue_worklog where started_at in (?, ?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('customComparable-optional-is-not-null-is-null', async () => {
        ...
        const nn = sync(ctx.conn.selectFrom(tReleaseDraft).where(tReleaseDraft.minVersion.isNotNull())
        ...
        const inl = sync(ctx.conn.selectFrom(tReleaseDraft).where(tReleaseDraft.minVersion.isNull())
            .select({ id: tReleaseDraft.id }).orderBy('id').executeSelectMany())
    ...
    test('double-column-operand-comparable-membership', async () => {
        ...
        ctx.mockNext(all)
        const is = sync(ctx.conn.selectFrom(tIssue).where(c.is(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours is estimated_hours order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(is).toEqual(all)
        ...
        const isNot = sync(ctx.conn.selectFrom(tIssue).where(c.isNot(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours is not estimated_hours order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(isNot).toEqual(none)
        ...
        ctx.mockNext(none)
        const inN = sync(ctx.conn.selectFrom(tIssue).where(c.inN(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours in (estimated_hours) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        ...
        ctx.mockNext(none)
        const notInN = sync(ctx.conn.selectFrom(tIssue).where(c.notInN(c))
            .select({ id: tIssue.id }).orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where estimated_hours not in (estimated_hours) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        expect(notInN).toEqual(none)
    ...
    test('double-value-when-null-nullif-value-column-arg', async () => {
        ...
        const vwnRows = sync(ctx.conn.selectFrom(tIssue)
            .select({ id: tIssue.id, h: tIssue.estimatedHours.valueWhenNull(tIssue.estimatedHours) })
            .orderBy('id').executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ifnull(estimated_hours, estimated_hours) as "h" from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof vwnRows, Array<{ id: number; h?: number }>>>()
        expect(vwnRows).toEqual(allDropped)

        ctx.mockNext(allDropped)
        const nivRows = sync(ctx.conn.selectFrom(tIssue)
