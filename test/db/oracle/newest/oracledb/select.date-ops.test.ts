...
describe(ctx.label, () => {
    ...
    test('currentDate', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select trunc(current_timestamp) as "today" from dual"`)
    ...
    test('currentDateTime', async () => {
        ...
        const rows = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select current_timestamp as "now" from dual"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('getFullYear', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", extract(year from created_at) as "year" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getMonth-getDate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", extract(month from created_at) - 1 as "month", extract(day from created_at) as "date" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getHours-getMinutes-getSeconds', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", extract(hour from created_at) as "h", extract(minute from created_at) as "m", trunc(extract(second from created_at)) as "s" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getDay-day-of-week', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", mod(trunc(created_at) - trunc(created_at, 'IW') + 1, 7) as "dow" from "organization" where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('getMilliseconds', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", to_number(to_char(created_at, 'FF3')) as "ms" from issue where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-localdate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from review_date) as "y", extract(month from review_date) - 1 as "mo", extract(day from review_date) as "d", mod(trunc(review_date) - trunc(review_date, 'IW') + 1, 7) as "dow" from project_review where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('required-localtime-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(hour from review_time) as "h", extract(minute from review_time) as "m", trunc(extract(second from review_time)) as "s", to_number(to_char(review_time, 'FF3')) as "ms" from project_review where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('custom-localtime-remaining-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(minute from cutoff_time) as "m", trunc(extract(second from cutoff_time)) as "s", to_number(to_char(cutoff_time, 'FF3')) as "ms" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal-between-value-source-bounds-projected-is-optional', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when released_on between released_on and released_on then 1 when not released_on between released_on and released_on then 0 else null end as "b" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal-comparison-value-source-column-overload', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when released_on = released_on then 1 else 0 end as "eq", case when released_on <> released_on then 1 else 0 end as "ne", case when decode(released_on, released_on, 1, 0 ) = 1 then 1 else 0 end as "i", case when decode(released_on, released_on, 1, 0 ) = 0 then 1 else 0 end as "ni" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('temporal-value-when-null-with-optional-value-source-default', async () => {
        ...
        const result = await ctx.conn.selectFrom(tProjectRelease)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(signed_off_at, signed_off_at) as "soff" from project_release where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('view-custom-localdate-getters', async () => {
        ...
        const rows = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select extract(year from released_on) as "y", extract(month from released_on) - 1 as "mo", extract(day from released_on) as "d", mod(trunc(released_on) - trunc(released_on, 'IW') + 1, 7) as "dow" from release_overview where id = :0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('optional-custom-temporal-getters-realize-the-undefined-inhabitant', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", extract(year from target_day) as "y", extract(month from target_day) - 1 as "mo", extract(day from target_day) as "d", mod(trunc(target_day) - trunc(target_day, 'IW') + 1, 7) as "dow", extract(hour from cutoff) as "h", extract(minute from cutoff) as "mi", trunc(extract(second from cutoff)) as "s", to_number(to_char(cutoff, 'FF3')) as "ms" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            id: number
