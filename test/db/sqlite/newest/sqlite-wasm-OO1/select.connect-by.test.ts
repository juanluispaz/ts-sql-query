...
describe(ctx.label, () => {
    ...
    // NOT-APPLICABLE: `START WITH … CONNECT BY` is hierarchical-query syntax not supported on SQLite; `.startWith` / `.connectBy` / `.connectByNoCycle` are typed `never` here. The equivalent shape on SQLite is a recursive CTE.
    /*
