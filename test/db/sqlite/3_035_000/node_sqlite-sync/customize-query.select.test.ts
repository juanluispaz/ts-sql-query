...
describe(ctx.label, () => {
    ...
    test('customize-select-projection-only-hooks-survive-as-cte', async () => {
        ...
        const result = sync(connection.selectFrom(openIssues)
    ...
    test('customize-select-distinct-page-hooks-ride-count-wrap', async () => {
        ...
        ctx.mockNext(3)
        const connection = ctx.conn
        const page = sync(connection.selectDistinctFrom(tProject)
            .where(tProject.id.lessOrEqual(3))
            .select({ id: tProject.id })
            .orderBy('id')
            .limit(2)
    ...
    test('customize-recursive-select-hooks-render-in-inline-aggregated-array-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with recursive recursive_select_1 as /* warmup */ (select id as result from issue where id = ? union all select issue.id as result from issue join recursive_select_1 on issue.parent_id = recursive_select_1.result) /* end-of-with */ select id as id, (/* head */  select /* hint */ /* cols */  json_group_array(result) from recursive_select_1  /* tail */) as tree from project where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('customize-select-plain-inline-value-renders-query-hooks-but-drops-with-query-hooks', async () => {
        ...
        const scalar = connection.subSelectUsing(tProject)
            ...
            .forUseAsInlineQueryValue()
        const scalarResult = sync(connection.selectFrom(tProject)
            .where(tProject.id.equals(1))
        ...
        const arr = connection.subSelectUsing(tProject)
            ...
            .customizeQuery(hooks())
            .forUseAsInlineAggregatedArrayValue()
        const arrResult = sync(connection.selectFrom(tProject)
            .where(tProject.id.equals(1))
