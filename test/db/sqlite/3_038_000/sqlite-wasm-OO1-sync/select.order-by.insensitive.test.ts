...
describe(ctx.label, () => {
    ...
    test('collation-empty: order-by-from-string-insensitive', async () => {
        ...
        ctx.mockNext([])
        sync(noWrapper.selectFrom(tAppUser)
            .select({ id: tAppUser.id, fullName: tAppUser.fullName })
    ...
    test('collation-set: compound-order-by-insensitive', async () => {
        ...
        const result = await collated.selectFrom(tProject)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select name as label from project union select title as label from issue order by label collate NOCASE asc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
