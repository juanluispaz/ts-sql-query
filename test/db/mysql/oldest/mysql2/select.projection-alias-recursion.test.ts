...
describe(ctx.label, () => {
    ...
    test('as-alias-recurses-through-a-required-nested-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select pc.pid as pid, pc.\`info.id\` as \`info.id\`, pc.\`info.name\` as \`info.name\` from (select id as pid, id as \`info.id\`, \`name\` as \`info.name\` from project where archived_at is null) as pc order by pid"`)
    ...
    test('for-use-in-left-join-as-recurses-through-the-nested-object-and-makes-it-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, pc.\`info.id\` as \`proj.id\`, pc.\`info.name\` as \`proj.name\` from issue left join (select id as pid, id as \`info.id\`, \`name\` as \`info.name\` from project) as pc on pc.pid = issue.project_id order by iid"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{
            iid:   number
