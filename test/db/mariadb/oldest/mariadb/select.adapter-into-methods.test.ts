...
describe(ctx.label, () => {
    ...
    test('adapter-string-column-into-reverse-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reverse(reviewer_code) as rev from project_review where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-concat-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, concat(reviewer_code, ?) as cc from project_review where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            1,
    ...
    test('adapter-string-column-into-length-passes-through-number-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, char_length(reviewer_code) as len from project_review where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('adapter-string-column-into-starts-with-predicate-in-where', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and reviewer_code like concat(?, '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "R-",
    ...
    test('adapter-string-column-into-not-contains-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and reviewer_code not like concat('%', ?, '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "ZZ",
    ...
    test('adapter-string-column-into-like-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and reviewer_code like ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "R%",
    ...
    test('adapter-string-column-into-not-like-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and reviewer_code not like ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "ZZ%",
    ...
    test('adapter-string-column-into-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and lower(reviewer_code) like lower(?)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "r%",
    ...
    test('adapter-string-column-into-not-like-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and lower(reviewer_code) not like lower(?)"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "zz%",
    ...
    test('adapter-string-column-into-contains-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and lower(reviewer_code) like concat('%', lower(?), '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "r-7a",
    ...
    test('adapter-string-column-into-not-contains-insensitive-predicate', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ? and lower(reviewer_code) not like concat('%', lower(?), '%')"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "zzz",
    ...
    test('equals-insensitive-if-value-elide', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProjectReview)
            ...
            .select({ id: tProjectReview.id, reviewer: tProjectReview.reviewerCode })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reviewer_code as reviewer from project_review where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
