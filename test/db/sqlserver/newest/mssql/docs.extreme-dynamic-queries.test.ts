...
describe(ctx.label, () => {
    ...
    test('docs:extreme-dynamic-queries/dynamic-condition-for', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name, slug as slug from project where lower(name) like lower('%' + @0 + '%') or lower(slug) like lower(@1 + '%') order by id"`)
    ...
    test('docs:extreme-dynamic-queries/dynamic-condition-nested-projection', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as id, issue.title as title, app_user.id as [assignee.id], app_user.full_name as [assignee.fullName] from issue left join app_user on app_user.id = issue.assignee_id where lower(app_user.full_name) like lower('%' + @0 + '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Ada",
          ]
    ...
    test('docs-extra:extreme-dynamic-queries/dynamic-condition-nested-and-or', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from project where lower(name) like lower('%' + @0 + '%') and (slug like (@1 + '%') or slug like (@2 + '%')) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "mark",
            "mktg",
