...
describe(ctx.label, () => {
    ...
    test('multi-row-bare-on-conflict-do-update-set-with-inserted-row-ref', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into project (organization_id, slug, name) values (?, ?, ?), (?, ?, ?) on conflict do update set name = excluded.name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "mktg-site",
            ...
            if (ctx.realDbEnabled) {
                ...
                const updated = sync(ctx.conn.selectFrom(tProject)
