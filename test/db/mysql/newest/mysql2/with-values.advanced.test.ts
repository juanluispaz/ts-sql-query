...
describe(ctx.label, () => {
    ...
    test('values-base-type-optional-virtual-column-projects-optional-and-is-not-a-tuple-member', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with batchRow(id, label) as (values row(?, ?), row(?, ?)) select id as id, label as label, null as \`rank\`, null as note from batchRow order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            "a",
    ...
    test('values-branded-column-through-for-use-in-left-join-as-keeps-the-brand', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with issueBilling(issueId, amount) as (values row(?, ?), row(?, ?)), issueBillingJoin(issueId, amount) as (values row(?, ?)) select issueBilling.issueId as base, ibj.issueId as joined from issueBilling left join issueBillingJoin as ibj on ibj.issueId = issueBilling.issueId order by base"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            101,
            19.99,
    ...
    test('values-virtual-column-referencing-a-sibling-column-inlines-the-reference', async () => {
        ...
        const rows = await ctx.conn.selectFrom(nums)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with vdouble(id) as (values row(?), row(?)) select id as id, id * 2 as doubled from vdouble order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            3,
            5,
