...
describe(ctx.label, () => {
    ...
    test('values-tuple-cast-per-numeric-and-boolean-kind', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(v)
    ...
    test('values-tuple-optional-custom-kind-column-with-type-adapter-shifts-and-passes-null', async () => {
        ...
        const present = sync(ctx.conn.selectFrom(v1)
        ...
        const none = sync(ctx.conn.selectFrom(v2)
    ...
    test('values-tuple-required-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with exprSampler(id) as (values (? + (2))) select id as id from exprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
    ...
    test('values-tuple-optional-cell-is-a-no-table-value-source-expression', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(v)
            .select({ id: v.id })
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"with optExprSampler(id) as (values (? + (2))) select id as id from optExprSampler"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            40,
          ]
