...
describe(ctx.label, () => {
    ...
    test('marshalling/bigint-insert-select-roundtrip', async () => {
        await ctx.withRollback(async () => {
            ...
            const newId = sync(ctx.conn.insertInto(tIssue)
                .values({
                    projectId: 1,
                    number:    9002,
    ...
    test('marshalling/double-insert-select-roundtrip', async () => {
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority, estimated_hours) values (?, ?, ?, ?, ?, ?) returning id"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                9003,
    ...
    test('marshalling/bigint-column-scalar-read-past-2p53', async () => {
        ...
        // node:sqlite refuses to return a wide integer that would not fit a JavaScript number
        // (readBigInts off) — the driver throws and that error is normalized to the same
        // PRECISION_LOST_RECEIVING_VALUE_FROM_DATABASE. The mock seeds the rounded number a rounding
        // driver would return (also precision-lost), so mock and real agree on the reason.
        await ctx.withRollback(async () => {
            ctx.mockNext(1)
