...
describe(ctx.label, () => {
    ...
    test('if-value-and-boolean-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority = @0 and title like ('%' + @1 + '%') then 1 else 0 end as bit) as flag from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "hero",
    ...
    test('boolean-and-if-value-present-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority = @0 and status = @1 then 1 else 0 end as bit) as flag from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "closed",
    ...
    test('boolean-and-if-value-no-value-elides-to-just-the-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority = @0 then 1 else 0 end as bit) as flag from issue where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('boolean-or-if-value-present-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when priority = @0 or status = @1 then 1 else 0 end as bit) as flag from issue where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            "open",
    ...
    test('boolean-or-merging-optional-operand-projected-as-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, cast(case when activity = @0 or (billable = 1) then 1 when not (activity = @1 or (billable = 1)) then 0 else null end as bit) as flag from issue_worklog where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "coding",
            "coding",
