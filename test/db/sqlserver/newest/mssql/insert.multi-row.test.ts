...
describe(ctx.label, () => {
    ...
    test('multi-row-values-with-uniform-shape', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan]) values (@0, @1), (@2, @3)"`)
    ...
    test('multi-row-values-with-some-rows-omitting-optional-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, [plan], verified) values (@0, @1, case when (@2 = 1) then 'Y' else 'N' end), (@3, @4, case when (@5 = 1) then 'Y' else 'N' end), (@6, @7, case when (@8 = 1) then 'Y' else 'N' end)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Mixed-A",
                "free",
    ...
    */
    ...
    // NOT-APPLICABLE: SQL Server has no INSERT…ON CONFLICT (uses MERGE); the bare `onConflictDoUpdateSet` is not typed on SqlServerConnection.
    /*
