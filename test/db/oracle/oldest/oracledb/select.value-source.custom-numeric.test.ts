...
describe(ctx.label, () => {
    ...
    test('custom-numeric/bigint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", view_count + :0 as "a", view_count - :1 as "s", mod(view_count, :2) as "m", greatest(view_count, :3) as "mn", least(view_count, :4) as "mx", abs(view_count) as "ab", sign(view_count) as "sg" from issue where id = :5"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1n,
            1n,
    ...
    test('custom-numeric/customdouble-roundn-with-number-value-source-precision', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when round(:0, priority) > :1 then 1 else 0 end as "big" from issue where id = :2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            7,
    ...
    test('custom-numeric/customint-arithmetic-and-sign', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", cost_cents + :0 as "a", cost_cents - :1 as "s", cost_cents * :2 as "mu", mod(cost_cents, cost_cents) as "mo", greatest(cost_cents, :3) as "mn", least(cost_cents, :4) as "mx", sign(cost_cents) as "sg" from issue_worklog where id = :5"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('custom-numeric/value-when-null-and-null-if-value-on-bigint-customint-customdouble', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(duration_ms, :0) as "dwn", nullif(duration_ms, :1) as "dni", nvl(cost_cents, :2) as "cwn", nullif(cost_cents, :3) as "cni", nvl(billed_amount, :4) as "bwn", nullif(billed_amount, :5) as "bni" from issue_worklog where id = :6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0n,
            0n,
    ...
    test('custom-numeric/customdouble-roundn-with-customdouble-value-source-precision', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", case when round(:0, :1) > :2 then 1 else 0 end as "big" from issue where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customint-rounding', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", ceil(:0) as "c", floor(:1) as "f", round(:2) as "r" from issue where id = :3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            6,
    ...
    test('custom-numeric/brand-survival-customint-minmax', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", greatest(round(:0), :1) as "mn", least(round(:2), :3) as "mx" from issue where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            10,
    ...
    test('custom-numeric/brand-survival-customint-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", round(:0) + :1 as "a", round(:2) - :3 as "s", round(:4) * :5 as "mu", mod(round(:6), :7) as "mo" from issue where id = :8"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            4,
    ...
    test('custom-numeric/brand-survival-customint-nullhandling', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", nvl(:0, :1) as "vwn", nullif(:2, :3) as "niv" from issue where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            6,
            0,
    ...
    test('custom-numeric/brand-survival-customdouble-power-logn-atan2', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", power(:0, :1) as "p", log(:2, :3) as "lg", atan2(:4, :5) as "at2" from issue where id = :6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-arithmetic', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", round(:0) + :1 as "a", round(:2) - :3 as "s", round(:4) * :5 as "mu" from issue where id = :6"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/brand-survival-customdouble-divide-modulo', async () => {
        ...
        const result = await ctx.conn.selectFrom(tIssue)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", :0 / :1 as "di", mod(:2, :3) as "mo" from issue where id = :4"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            8,
            2,
    ...
    test('custom-numeric/minmax-optional-customdouble-receiver-poisons', async () => {
        ...
        const result = await ctx.conn.selectFrom(tReleaseDraft)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", greatest(budget, :0) as "mn", least(budget, :1) as "mx" from release_draft order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            100,
            100,
    ...
    test('custom-numeric/minmax-operand-optional-customint-keeps-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", greatest(release_ordinal, optional_release_ordinal) as "mn", least(release_ordinal, optional_release_ordinal) as "mx" from release_overview order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
    test('custom-numeric/minmax-both-optional-customint-keeps-brand', async () => {
        ...
        const result = await ctx.conn.selectFrom(vReleaseOverview)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as "id", greatest(optional_release_ordinal, optional_release_ordinal) as "mn", least(optional_release_ordinal, optional_release_ordinal) as "mx" from release_overview order by "id""`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof result, Array<{ id: number; mn?: ReleaseTag; mx?: ReleaseTag }>>>()
        expect(result).toEqual(expected)
    })
