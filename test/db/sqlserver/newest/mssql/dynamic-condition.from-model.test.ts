...
describe(ctx.label, () => {
    ...
    test('from-model/emits-the-same-sql-as-the-descriptor-form', async () => {
        ...
        expect(a.sql).toMatchInlineSnapshot(`"select id as id from issue where (lower(title) like lower(@0 + '%') or priority > @1) and view_count >= @2 and external_ref like ('%' + @3 + '%') order by id"`)
