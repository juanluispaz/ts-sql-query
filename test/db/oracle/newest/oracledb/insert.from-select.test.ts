...
// path through `_buildInsertFromSelect`. Oracle and MySQL don't allow
// `.returning(...)` on the from-select form (typed as `never`), so this
// cell asserts the affected-row count instead.
