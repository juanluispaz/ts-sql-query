...
describe(ctx.label, () => {
    ...
    test('insert-with-explicit-default-keyword', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan, created_at) values ($1, $2, default)"`)
    ...
    test('update-set-to-default-keyword', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set created_at = default where id = $1"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
    ...
    test('multi-row-insert-with-default-in-some-rows', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan, created_at) values ($1, $2, default), ($3, $4, $5)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "MultiA",
                "free",
    ...
    test('default-on-custom-boolean-column', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into organization (name, plan, verified) values ($1, $2, default)"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "CustomBoolDefault",
                "free",
