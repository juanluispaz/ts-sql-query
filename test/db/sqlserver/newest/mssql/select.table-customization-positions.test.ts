...
describe(ctx.label, () => {
    ...
    test('table-customization: wrapper as left-join target widens columns', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.name as projectName, organization.name as orgName from project left join organization on organization.id = project.organization_id where project.organization_id = @0 order by projectName"`)
