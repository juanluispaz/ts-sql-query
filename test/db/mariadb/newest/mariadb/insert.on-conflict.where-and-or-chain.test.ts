...
describe(ctx.label, () => {
    ...
    // Not applicable on MariaDB: `.onConflictOn(...)` is not typed on `MariaDBConnection` (MariaDB's `ON DUPLICATE KEY UPDATE` grammar takes no column list and no WHERE clause); see `test/db/mariadb/types.negative/insert.test.ts`.
    /*
