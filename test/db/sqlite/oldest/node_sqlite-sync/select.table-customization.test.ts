...
describe(ctx.label, () => {
    ...
    test('table-customization: unaliased wrapper emits table name + empty alias slot', async () => {
        ...
        ctx.mockNext([])
        sync(ctx.conn.selectFrom(tOrgCustom)
    ...
    test('table-customization: parameterized customization threads a runtime param into the raw fragment', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tOrgFiltered)
    ...
    test('table-customization: parameterized customization as an INNER JOIN target threads its bound param', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tProject)
            ...
            .orderBy('projectId')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as projectId, "o".name as orgName from project inner join (select * from organization where ? >= 0) as "o" on "o".id = project.organization_id where project.organization_id = ? order by projectId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            1,
    ...
    test('table-customization: withSqlHint applied to a VIEW emits the hint on the view name', async () => {
        ...
        ctx.mockNext(expected)
        const rows = sync(ctx.conn.selectFrom(vProjCustom)
    ...
    test('table-customization: two-param customization threads two bound params', async () => {
        ...
        const rows = sync(ctx.conn.selectFrom(tOrgFiltered)
            ...
            .orderBy('id')
            .executeSelectMany())
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select "o".id as id from (select * from organization where ? >= 0 and ? >= 0) as "o" order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            0,
