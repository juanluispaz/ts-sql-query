    ...
    test('shaped-multi-row-ignore-if-has-value', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await ctx.conn.insertInto(tProject)
                ...
                .ignoreIfHasValue('archived')
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into project (organization_id, name, slug) values (:0, :1, :2); insert into project (organization_id, name, slug, archived_at) values (:3, :4, :5, :6); end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped IHV A",
    ...
    test('shaped-multi-row-ignore-if-has-no-value', async () => {
        ...
        await ctx.withRollback(async () => {
            const inserted = await ctx.conn.insertInto(tProject)
                ...
                .ignoreIfHasNoValue('archived')
                .executeInsert()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"begin insert into project (organization_id, name, slug, archived_at) values (:0, :1, :2, :3); insert into project (organization_id, name, slug) values (:4, :5, :6); end;"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
                "Shaped IHNV A",
