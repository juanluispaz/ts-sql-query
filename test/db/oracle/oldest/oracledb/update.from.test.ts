    ...
    */
    ...
    test('update-from-minmax-poison-case-qualifies-operands', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set issue.assignee_id = least(issue.assignee_id, issue.parent_id) from project where issue.project_id = project.id and issue.id = :0"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
