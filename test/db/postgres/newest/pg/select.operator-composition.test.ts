...
describe(ctx.label, () => {
    ...
    test('aggregate-over-composed-expression', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select sum(priority + $1) as "s", count(coalesce(assignee_id, $2)) as "c", max(priority + $3) as "m" from issue"`)
    ...
    test('merge-optional-originally-required-with-required-in-optional-object-through-operator', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, organization.id + project.organization_id as combined from project left join organization on organization.id = project.organization_id where project.id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('two-modifiers-as-optional-then-as-required-in-optional-object', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority as "p" from issue where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('two-modifiers-value-when-null-then-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(coalesce(priority, $1), $2) as "q" from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            0,
            2,
    ...
    test('operator-on-originally-required-left-join-column-projected-as-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select project.id as pid, organization.id + $1 as "orgPlus" from project left join organization on organization.id = project.organization_id where project.id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            1,
    ...
    test('if-value-and-boolean-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority = $1 and title like ('%' || $2 || '%') as flag from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "hero",
    ...
    test('boolean-and-merging-optional-operand-projected-as-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, activity = $1 and billable as flag from issue_worklog where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "coding",
            1,
    ...
    test('if-value-or-boolean-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority = $1 or title like ('%' || $2 || '%') as flag from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "hero",
    ...
    test('boolean-and-if-value-present-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority = $1 and status = $2 as flag from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            "closed",
    ...
    test('boolean-and-if-value-no-value-elides-to-just-the-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority = $1 as flag from issue where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            1,
    ...
    test('boolean-or-if-value-present-collapses-to-projectable-boolean', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, priority = $1 or status = $2 as flag from issue where id = $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            99,
            "open",
    ...
    test('boolean-or-merging-optional-operand-projected-as-optional', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, activity = $1 or billable as flag from issue_worklog where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "coding",
            1,
    ...
    test('operator-on-originally-required-left-join-column-projected-as-nullable-leaf-on-miss', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select issue.id as iid, app_user.id + $1 as "assigneePlus" from issue left join app_user on app_user.id = issue.assignee_id where issue.id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
