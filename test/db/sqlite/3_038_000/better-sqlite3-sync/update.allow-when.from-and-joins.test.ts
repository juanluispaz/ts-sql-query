...
describe(ctx.label, () => {
    ...
    test('update-from-join-all-gates-open-walks-to-end-and-runs', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"/* gated issue.id */  update /* gated issue.project_id */ project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = ?  /* gated project.slug */"`)
    ...
    test('update-from-join-all-gates-open-with-allowed-returning-walks-to-end', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project set name = app_user.full_name from issue inner join app_user on app_user.id = issue.assignee_id where project.id = issue.project_id and issue.id = ? returning project.id as id, project.name as name"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                1,
              ]
