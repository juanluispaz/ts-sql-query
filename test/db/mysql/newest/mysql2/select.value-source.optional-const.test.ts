...
describe(ctx.label, () => {
    ...
    test('optional-const/uuid-enum-custom-comparable', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select bin_to_uuid(uuid_to_bin(?)) as \`u\`, ? as \`e\`, ? as \`c\`, ? as cc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('optional-const/custom-numeric-and-uuid', async () => {
        ...
        const row = await ctx.conn.selectFromNoTable()
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select ? as ci, ? as cd, bin_to_uuid(uuid_to_bin(?)) as cu"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            7,
            1.5,
