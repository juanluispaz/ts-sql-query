...
describe(ctx.label, () => {
    ...
    test('like-insensitive', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) like lower(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%@ACME.TEST",
          ]
    ...
    test('not-like-insensitive', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) not like lower(?) order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "%@ACME.TEST",
          ]
    ...
    test('not-starts-with-insensitive', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from app_user where lower(email) not like concat(lower(?), '%') order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "ADA",
          ]
    ...
    test('equals-and-not-equals-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select lower(email) = lower(full_name) as eq, lower(email) <> lower(full_name) as neq from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('like-and-not-like-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select lower(email) like lower(full_name) as lk, lower(email) not like lower(full_name) as nlk from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('affix-insensitive-column-rhs', async () => {
        ...
        const result = await ctx.conn.selectFrom(tAppUser)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select lower(email) like concat(lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')), '%') as sw, lower(email) not like concat(lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')), '%') as nsw, lower(email) like concat('%', lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'))) as ew, lower(email) not like concat('%', lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_'))) as \`new\`, lower(email) like concat('%', lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')), '%') as ct, lower(email) not like concat('%', lower(replace(replace(replace(full_name, '\\\\', '\\\\\\\\\\\\\\\\'), '%', '\\\\%'), '_', '\\\\_')), '%') as nct from app_user where id = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
