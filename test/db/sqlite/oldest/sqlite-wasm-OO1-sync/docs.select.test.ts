...
describe(ctx.label, () => {
    ...
    test('docs:select/joins-and-order-by', async () => {
        ...
        const openIssues = sync(connection.selectFrom(tIssue)
    ...
    test('docs:select/ordering-by-not-returned-column', async () => {
        ...
        // doc-start
        const project = sync(connection.selectFrom(tProject)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, name as name from project where id = ? order by project.archived_at desc"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('docs:select/subquery-and-dynamic-order-by', async () => {
        ...
        const projectsInAcme = sync(connection.selectFrom(tProject)
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as projectId, name as projectName, slug as projectSlug from project where organization_id in (select id as result from organization where name like ('%' || ? || '%') escape '\\') order by projectName asc, projectSlug"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "Acme",
          ]
    ...
    test('docs:select/aggregate-and-group-by', async () => {
        ...
        // doc-start
        const projectCountPerOrganization = sync(connection.selectFrom(tOrganization)
    ...
    test('docs:select/left-join', async () => {
        ...
        const issuesWithParentTitle = sync(connection.selectFrom(tIssue)
    ...
    test('docs:select/left-join-complex-projections', async () => {
        ...
        const issuesWithParent = sync(connection.selectFrom(tIssue)
            .leftJoin(parent).on(tIssue.parentId.equals(parent.id))
            .select({
                id:    tIssue.id,
                title: tIssue.title,
    ...
    test('docs:select/compound-operator', async () => {
        ...
        // doc-start
        const allNamed = sync(connection.selectFrom(tOrganization)
            .select({
                id:   tOrganization.id,
    ...
    test('docs:select/with-clause', async () => {
        ...
        const acmeOrganizations = sync(connection.selectFrom(projectCountPerOrgWith)
    ...
    test('docs:select/count-all', async () => {
        ...
        const numberOfProjects = sync(connection.selectFrom(tProject)
    ...
    test('docs:select/inline-subquery-as-value', async () => {
        ...
        const acmeProjects = sync(connection.selectFrom(tProject)
    ...
    test('docs:select/inline-subquery-referencing-outer', async () => {
        ...
        const organizationsWithProjectCount = sync(connection.selectFrom(tOrganization)
            .select({
                id:           tOrganization.id,
                name:         tOrganization.name,
    ...
    test('docs-extra:select/find-by-email', async () => {
        ...
        const user = sync(connection.selectFrom(tAppUser)
    ...
    test('docs-extra:select/aliased-table', async () => {
        ...
        const onlyAda = sync(connection.selectFrom(u)
