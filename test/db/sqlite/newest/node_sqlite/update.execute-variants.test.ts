...
describe(ctx.label, () => {
    ...
    test('execute-update-none-or-one-with-no-sets-resolves-null', async () => {
        ...
        // `executeUpdateNoneOrOne` is a RETURNING executor, so it is reached
        // through `returningOneColumn`; the empty-set short-circuit fires
        // before the projection matters, so null still comes back without
