...
describe(ctx.label, () => {
    ...
    test('collation: affix operators with a uuid receiver', async () => {
        ...
        await collated.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where convert(nvarchar(36), external_ref) like (@0 + '%') collate Latin1_General_CI_AS"`)
        ...
        await empty.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref like ('%' + @0 + '%')"`)
    })
    ...
    test('collation: negated affix operators with a uuid receiver', async () => {
        ...
        await collated.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where convert(nvarchar(36), external_ref) not like (@0 + '%') collate Latin1_General_CI_AS"`)
        ...
        await empty.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where external_ref not like ('%' + @0 + '%')"`)
    })
    ...
    test('collation: aggregate-array-order-by-insensitive', async () => {
        ...
        await empty.selectFrom(tOrganization)
            ...
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (select concat('[', string_agg('"' + string_escape(convert(nvarchar(max), a_1_.[result]), 'json') + '"', ','), ']') from (select name as [result] from project where organization_id = organization.id order by [result] asc offset 0 rows) as a_1_) as [names] from organization where id = @0"`)
    })
})
