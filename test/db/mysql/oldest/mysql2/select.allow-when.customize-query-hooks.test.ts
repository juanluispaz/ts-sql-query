...
describe(ctx.label, () => {
    ...
    // NOT-APPLICABLE: MySQL 5.7 has no common table expressions, so `forUseInQueryAs(...)` inlines the query as a subquery and the with-query hooks are never rendered — the introspection verdict still reports the query disallowed, but nothing throws.
