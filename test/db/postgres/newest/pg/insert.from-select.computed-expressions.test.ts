...
describe(ctx.label, () => {
    ...
    test('insert-from-select-arithmetic-string-and-column-ops', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) select id as "projectId", id + $1 as number, name || $2 as title, $3::text as status, id + organization_id as priority from project where id = $4"`)
    ...
    test('insert-from-select-multiply-substring-and-least', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"insert into issue (project_id, number, title, status, priority) select id as "projectId", (id * $1) + $2 as number, substr(name, $3, $4) as title, $5::text as status, least(id, $6) as priority from project where id = $7"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                100,
                2,
