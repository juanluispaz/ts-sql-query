...
describe(ctx.label, () => {
    ...
    test('is-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is ?"`)
    ...
    test('is-not-with-null', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is not ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-with-column-comparison', async () => {
        ...
        await ctx.conn.selectFrom(tIssue)
            ...
            .select({ id: tIssue.id })
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where assignee_id is parent_id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    })
    ...
    test('is-with-null-const-receiver-and-column-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where ? is body order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-not-with-null-const-receiver-and-column-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where ? is not body order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
          ]
    ...
    test('is-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where ? is ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            "See ADR-014",
    ...
    test('is-not-with-null-const-receiver-and-literal-operand', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssue)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id from issue where ? is not ? order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            null,
            "See ADR-014",
    ...
    test('is-between-two-columns-sharing-a-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('userId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as organizationId, app_user.id as userId from organization inner join app_user on app_user.verified is organization.verified order by organizationId, userId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
    ...
    test('is-not-between-two-columns-sharing-a-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('userId')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select organization.id as organizationId, app_user.id as userId from organization inner join app_user on app_user.verified is not organization.verified order by organizationId, userId"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`[]`)
        assertType<Exact<typeof rows, Array<{ organizationId: number, userId: number }>>>()
        expect(rows).toEqual(expected)
    })
