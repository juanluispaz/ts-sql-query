...
describe(ctx.label, () => {
    ...
    test('version-bracketed-into-to-lower-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, lower(version_bracketed) as lc from release_overview where id = @0"`)
    ...
    test('version-bracketed-into-to-upper-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(version_bracketed) as uc from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('version-bracketed-into-trim-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, trim(version_bracketed) as tm from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('version-bracketed-into-trim-left-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ltrim(version_bracketed) as tl from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('version-bracketed-into-trim-right-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, rtrim(version_bracketed) as tr from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('version-bracketed-into-reverse-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reverse(version_bracketed) as rev from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('version-bracketed-into-substring-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(version_bracketed, @0, @1) as sg from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('version-bracketed-into-substr-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(version_bracketed, @0, @1) as sb from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('version-bracketed-into-concat-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, version_bracketed + @0 as cc from release_overview where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "!",
            2,
    ...
    test('version-bracketed-into-replace-all-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(version_bracketed collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT as rp from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            ".",
            "_",
    ...
    test('channel-bracketed-into-to-lower-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, lower(channel_bracketed) as lc from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-to-upper-case-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, upper(channel_bracketed) as uc from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-trim-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, trim(channel_bracketed) as tm from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-trim-left-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, ltrim(channel_bracketed) as tl from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-trim-right-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, rtrim(channel_bracketed) as tr from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-reverse-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, reverse(channel_bracketed) as rev from release_overview where id = @0"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('channel-bracketed-into-substring-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(channel_bracketed, @0, @1) as sg from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('channel-bracketed-into-substr-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, substring(channel_bracketed, @0, @1) as sb from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
            3,
    ...
    test('channel-bracketed-into-replace-all-keeps-result-leaf-bracketed', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(channel_bracketed collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, @1) collate DATABASE_DEFAULT as rp from release_overview where id = @2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "a",
            "A",
    ...
    test('project-name-replace-all-with-bracketed-find-string-inherits-the-operands-adapter', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, replace(project_name collate Latin1_General_BIN2, @0 collate Latin1_General_BIN2, version_bracketed) collate DATABASE_DEFAULT as rp from release_overview where id = @1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "site",
            1,
