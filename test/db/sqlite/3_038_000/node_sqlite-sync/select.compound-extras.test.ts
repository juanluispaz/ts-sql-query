...
describe(ctx.label, () => {
    ...
    */
    ...
    test('minus-routes-through-the-dialect-alias', async () => {
        ...
        const small = ctx.conn.selectFrom(tIssue)
            ...
            .select({ status: tIssue.status })
        const result = sync(all.minus(small).executeSelectMany())
    ...
    */
    ...
    test('compounded-interface-except-after-union', async () => {
        ...
        const result = sync(a.union(b).except(c).executeSelectMany())
        assertType<Exact<typeof result, Array<{ status: string }>>>()
    ...
    */
    ...
    test('compounded-interface-intersect-after-union', async () => {
        ...
        const result = sync(a.union(b).intersect(c).executeSelectMany())
        assertType<Exact<typeof result, Array<{ status: string }>>>()
    ...
    // NOT-APPLICABLE: SQLite has no `INTERSECT ALL` — `intersectAll` is `never`
    // on the compounded interface too (compile-time frontier, paired with this
    // dialect's `types.negative` suite).
    /*
    test('compounded-interface-intersect-all-after-union', async () => {
        ...
        assertType<Exact<typeof result, Array<{ status: string }>>>()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as status from issue where id <= $1 union select status as status from issue where id = $2 intersect all select status as status from issue where id <= $3"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
            4,
    ...
    */
    ...
    // NOT-APPLICABLE: SQLite has no `EXCEPT ALL` — `exceptAll` is `never` on the
    // compounded interface too (compile-time frontier, paired with this
    // dialect's `types.negative` suite).
    /*
    ...
    */
    ...
    test('compounded-interface-minus-after-union', async () => {
        ...
        const b = ctx.conn.selectFrom(tIssue).where(tIssue.id.equals(4)).select({ status: tIssue.status })
        const c = ctx.conn.selectFrom(tIssue).where(tIssue.status.equals('open')).select({ status: tIssue.status })
        const result = sync(a.union(b).minus(c).executeSelectMany())
        assertType<Exact<typeof result, Array<{ status: string }>>>()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select status as status from issue where id <= ? union select status as status from issue where id = ? except select status as status from issue where status = ?"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
