...
describe(ctx.label, () => {
    ...
    test('valueWhenNull-int-receiver-widens-to-a-fractional-double-fallback', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce(assignee_id, priority::float / $1) as "w" from issue order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2,
          ]
    ...
    test('temporal/local-date-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce(work_date, $1) as "d", coalesce(work_date, work_date) as d2 from issue_worklog where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
    ...
    test('temporal/local-date-time-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce(created_at, $1) as "d", coalesce(created_at, created_at) as d2 from organization where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            1,
    ...
    test('temporal/custom-local-date-value-when-null', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce(released_on, $1) as "d", coalesce(released_on, released_on) as d2 from project_release where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            2,
    ...
    test('temporal/local-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(started_at, $1) as "t", nullif(started_at, started_at) as t2 from issue_worklog where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "08:00:00",
            1,
    ...
    test('temporal/custom-local-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(cutoff_time, $1) as "t", nullif(cutoff_time, cutoff_time) as t2 from project_release where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            "16:00:00",
            1,
    ...
    test('temporal/custom-local-date-time-null-if-value', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, nullif(signed_off_at, $1) as "x", nullif(signed_off_at, signed_off_at) as x2 from project_release where id = $2"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            2099-01-01T00:00:00.000Z,
            1,
    ...
    test('modifier-trio-on-enum-activity-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, activity as req, null as own, null as ign from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-channel-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, channel as req, null as own, null as ign from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-comparable-version-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, version as req, null as own, null as ign from project_release where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-custom-boolean-leaf', async () => {
        ...
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (published = 't') as req, null::bool as own, null::bool as ign from project where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('null-modifiers-on-verified-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tOrganization)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce((verified = 'Y'), $1) as wn, nullif((verified = 'Y'), $2) as ni from organization order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
            false,
    ...
    test('null-modifiers-on-published-required-custom-boolean-string-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tProject)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce((published = 't'), $1) as wn, nullif((published = 't'), $2) as ni from project order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
            false,
    ...
    test('null-modifiers-on-invoiced-required-numeric-custom-boolean-adapter', async () => {
        ...
        const rows = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .orderBy('id')
            .executeSelectMany()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, coalesce((invoiced = 1), $1) as wn, nullif((invoiced = 1), $2) as ni from issue_worklog order by id"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            false,
            false,
    ...
    test('modifier-trio-on-verified-required-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tOrganization)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (verified = 'Y') as req, null::bool as own, null::bool as ign from organization where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-approved-nullable-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (approved = 'A') as req, null::bool as own, null::bool as ign from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
    ...
    test('modifier-trio-on-invoiced-required-numeric-custom-boolean-leaf', async () => {
        ...
        const row = await ctx.conn.selectFrom(tIssueWorklog)
            ...
            .executeSelectOne()
        expect(ctx.lastSql).toMatchInlineSnapshot(`"select id as id, (invoiced = 1) as req, null::bool as own, null::bool as ign from issue_worklog where id = $1"`)
        expect(ctx.lastParams).toMatchInlineSnapshot(`
          [
            1,
          ]
