...
describe(ctx.label, () => {
    ...
    // Not applicable on PostgreSQL: `.startWith` / `.connectBy` / `.connectByNoCycle` are typed `never` (Oracle-only `START WITH … CONNECT BY` syntax); equivalent shape on PostgreSQL is a recursive CTE — see `cte.recursive-union-variants.test.ts`.
    /*
