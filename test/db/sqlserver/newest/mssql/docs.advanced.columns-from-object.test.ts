...
describe(ctx.label, () => {
    ...
    test('extract-writable-columns', async () => {
        ...
        // doc-end
        // TODO[BUG] inconsistency between type and runtime:
        // `this.column(name, type)` returns `Column` which the type
        // system considers a `WritableDBColumn`, but the runtime
        // `__writable` flag is only set for optionalColumn /
        // columnWithDefaultValue / autogeneratedPrimaryKey. So this
        // helper currently omits regular required columns (title,
        // status, priority, projectId, number).
        const k = Object.keys(cols).sort()
        // The columns that DO appear today (until the lib fixes the
        // inconsistency).
        expect(k).toContain('id')
        expect(k).toContain('body')
        expect(k).toContain('assigneeId')
        expect(k).toContain('parentId')
        expect(k).toContain('createdAt')
        expect(k).toContain('updatedAt')
    })
