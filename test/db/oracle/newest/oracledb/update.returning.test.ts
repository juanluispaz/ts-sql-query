...
describe(ctx.label, () => {
    ...
    test('update-returning-one-column-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = :0 where id = :1 returning priority + :2 into :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                1,
    ...
    test('update-returning-object-computed-expression', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set title = :0 where id = :1 returning id, title || :2 into :3, :4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Patched",
                1,
    ...
    test('update-release-draft-returning-one-column-optional-branded', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = :0 where id = :1 returning stage into :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Alpha cut v2",
                1,
    ...
    test('update-release-draft-returning-object-optional-branded-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = :0 where id = :1 returning id, stage into :2, :3"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Alpha cut v2",
                1,
    ...
    test('update-returning-rule1-gate-null-drops-object-default', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update release_draft set title = :0 where id = :1 returning id, title, stage into :2, :3, :4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                "Nightly build v2",
                2,
    ...
    test('update-returning-one-column-many-coerces-undefined-to-null', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update issue set priority = :0 where project_id = :1 returning "body" into :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
                5,
                1,
    ...
    test('update-returning-temporal-plain-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.update(tProjectReview)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_date = :0, review_time = :1 where id = :2 returning review_date, review_time into :3, :4"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('update-returning-temporal-custom-kinds-round-trip', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const ret = await ctx.conn.update(tProjectRelease)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_release set released_on = :0, cutoff_time = :1, published_at = :2 where id = :3 returning released_on, cutoff_time, published_at into :4, :5, :6"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
    ...
    test('update-returning-one-column-temporal-real-value', async () => {
        ...
        await ctx.withRollback(async () => {
            ...
            const v = await ctx.conn.update(tProjectReview)
                ...
                .executeUpdateOne()
            expect(ctx.lastSql).toMatchInlineSnapshot(`"update project_review set review_time = :0 where id = :1 returning review_time into :2"`)
            expect(ctx.lastParams).toMatchInlineSnapshot(`
              [
